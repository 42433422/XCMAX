...
with patch("app.services.conversation_service.AIConversation", return_value=conv):
    ...
    print("inside patch conv is:", id(conv))
    print("conv.__dict__ keys:", [k for k in conv.__dict__.keys() if not k.startswith("_")])
    print("conv.intent:", repr(conv.intent))

# Try setting attr manually
conv2 = MagicMock()
conv2.x = 1
print("conv2.x:", conv2.x)
