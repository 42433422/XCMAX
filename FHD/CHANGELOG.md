...
- 三产品线统一控制面（消灭「版本/完成度靠手抄」造成的状态漂移）：新增声明式输入 `config/product_lines.yaml`（只有「哪条线包含哪些路径/能力域/发布 SSOT」，不含任何数字）与生成器 `scripts/dev/product_lines_status.py`，把三条产品线的版本、发布状态、能力计数与平台 Release Gate 结论全部改为从各自权威源现算，产出 `docs/PRODUCT_LINES_STATUS.md`（人读）与 `metrics/product_lines_status.generated.json`（机器可读）；7 篇活文档 25 处硬编码版本改为指向 `VERSION.md`，`PROJECT_STATE.md` 的手填打分换成「状态数字来源（禁止自评）」表；`ssot.yaml` 新增 blocking 域 `product-lines`（生成物漂移即 CI 判红）；顺带修复能力中心把 Markdown 表格分隔行当成数据行、把 `"----": "------"` 写进官网公开数据（`data/capabilities.json` 与 `catalog.html`）的缺陷。
