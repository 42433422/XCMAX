...
### 安卓超级员工局域网直连修复（2026-07-09 · v10 线内迭代）

- Flutter：`local_base_url` 的 17500 改写为手机可达 5011；派工前从云端刷新桌面局域网地址
- 桌面中继：poll 上报当前 host/port，避免复用身份后局域网地址过期

### 安卓超级员工进度卡去掉「中断」（2026-07-09 · v10 线内迭代）

- Flutter：中继进度卡不再显示气泡内「中断」；取消仍用底部「停止」

### 安卓超级员工云端中继派工对齐（2026-07-09 · v10 线内迭代）

- Flutter：与 Android 一致，账号下 `status=paired` 即可走云端中继；不再用 last_seen≤5min 误报「没有在线的电脑执行端」
