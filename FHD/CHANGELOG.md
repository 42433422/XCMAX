...
- **Windows 故障注入脚本首次实跑暴露三处缺陷全修复（run 34642734307 实证）**：① 健康判定死磕 `status=healthy`，但无 LLM 凭据环境 `/api/health` 因 neuro 降级返回 `degraded`，后端明明已在 17500 监听却四场景全误判 FAIL——现与验收脚本口径一致（200 响应即算启动成功，degraded 状态与原因记入证据）；② 失败时无任何应用日志可查、根因无法定位——新增失败取证（端口/进程状态、后端日志尾部、数据根清单随证据落盘）；③ 进程清理只杀不等待子进程树、场景间残留孤儿监听导致级联失败，且汇总统计的总减法在 pwsh 下直接报「Argument types do not match」——现改 taskkill 树杀 + 按端口归属兜底清场，统计改为直接按结果分类计数。
