...
## 1.0.0.1（待稳定发布）— 桌面端可靠性与发布链收敛

- **版本 SSOT**：Windows/macOS 企业桌面包、下载目录、更新元数据和运行时产品版本统一为 `1.0.0.1`；npm/Electron/Apple 工具链保持合法三段映射 `1.0.0`。
- **发布门禁**：只允许通过统一 Release Desktop 链路上架，必须完成 Windows Authenticode、macOS Developer ID 签名/公证、最终 SHA 嵌入、双平台安装启动和公开下载校验。
- **功能验收**：稳定版以已登录账户的核心业务、Mod 装载、桌面运行状态、日志增长和更新元数据实测为准；外部模型或基础设施不可用时必须明确降级，不能伪报可用。
