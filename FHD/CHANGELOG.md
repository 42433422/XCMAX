...
- **跨业务误路由修复**：说「客户列表」「本月销售汇总」「产品 A100 入库 50 件」「产品 A100 出库 50 件」「给星光报价」「库存有多少」「这个月花了多少钱」「记一笔收入5000」「销售订单列表」「销售报表」「创建发货单」「导出客户报表」「供应商列表」「采购订单有哪些」「开一张采购单」「今天的发货记录」「查一下应收账款」「做个利润表」「这个月毛利多少」「这个月销售额多少」「A100 还有多少库存」「物料列表」「系统设置」「哪些产品快没货了」「本月订单有哪些」「查一下销售单」「本月支出明细」「上个月的账」「采购汇总」时，各自走客户、报表、库存、销售、财务、发货、采购、供应商、物料、设置的正确工具，不再一律当成查产品或乱写数据库；信息不全时先反问补齐，高风险操作仍需你确认后才执行。业务任务基准从 5/22 提升到 43/43 全对（含出库端到端写库校验、客户列表导出 Excel、应收账龄、采购/供应商/发货记录、物料/设置/低库存预警路由）。
