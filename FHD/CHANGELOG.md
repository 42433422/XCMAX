...
- **macOS 发布交付 SSOT 落地**：新增 [MACOS_RELEASE_SSOT.md](docs/MACOS_RELEASE_SSOT.md) 作为 macOS 发版唯一事实来源——版本/产物/SHA256/构建环境/下载与更新地址/测试机与 12 项 Release Gate（构建、安装、安全项、首启、登录、Mod 与 AI 员工、业务、更新发现/安装、数据保留、更新后业务、重启复验）状态一次登记，无真机证据不得标绿，CI 通过不等于验收通过；每次发版复用同一份并回填 Gate 状态。
