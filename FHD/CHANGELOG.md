...
- **客户产品独立关联表**：增加关联模型及可回退迁移，保留旧产品单位；关系写入与旧数据回填继续实施。

- **产品关联要求保留**：注册产品创建工具收到客户名称形式的 unit_name 时明确返回尚不支持关联，避免丢弃关联要求后创建普通产品。

- **打印机查询意图**：聊天和规则规划器识别打印机列表、当前打印机查询，通过注册能力读取配置并返回明确失败信息。

- **工作流否定指令**：与聊天路由共用否定动作识别，在模型规划之前返回不执行结果，正反混合指令要求重新说明操作。

- **普通聊天指令分流**：否定操作返回澄清提示，打印标签不再被通用打印关键词分流为发货。

- **事件总线模块拆分**：独立维护处理器重试、身份传递及失败记账逻辑，恢复架构文件规模门禁。

- **审批内容快照**：创建审批时深复制产品明细、待恢复计划与运行上下文，避免原计划后续修改改变已提交内容。

- **发货明细自然语言澄清**：可用“编号9803，规格12，一共3桶”补齐明细，保留已选客户并进入审批；缺失数量和否定回答不继续。

- **财务事件申请人一致性**：将载荷申请人作为一致性约束交给审批服务，拒绝与宿主发布身份不符的事件审批。

- **本地事件身份传递**：入队时保存宿主账号与租户，消费及线程池处理时恢复；重复发布对象使用独立队列条目，远程载荷不授予本地执行身份。

- **财务事件审批**：审批提交要求持久化成功；失败发布审批失败事件，避免仅在内存创建却报告成功。

- **门控引擎审批**：要求审批持久化成功后才能继续策略判断，存储失败时停止并清除临时请求。

- **审批接口持久化**：审批创建接口只有数据库持久化成功才返回成功，失败时清除内存中的临时请求。

- **审批申请人身份**：从可信登录或执行上下文解析申请人，拒绝缺少身份或运行参数与身份冲突的审批持久化请求。

- **默认高风险审批**：未命中自定义规则时，审批服务根据可信动作注册表要求高风险动作审批，避免只标风险却不进入审批；模型节点风险不能降级该判断。

- **发货澄清与审批**：补齐产品明细时校验标识、正整数桶数和有限正规格；生成动作明确要求交互审批，防止澄清完成后直接执行。

- **发货记录回执**：在会话关闭前读取持久化记录编号，修复文档生成后返回记录编号时的脱离会话异常。

- **发货单工作流**：明确生成请求接入订单解析；信息缺失时先澄清，AI 规划禁用默认桶数和规格。当前变更覆盖规划入口，实际文档与打印仍需验收。

- **AI 入库规划**：明确入库请求保留数量和单位，解析产品后询问缺失仓库；单位不一致时继续询问带库存单位的换算数量，再提交审批；入库执行校验仓库、库位归属及单位一致性，不同库位库存独立累计。

- **入库数量校验**：库存服务和 AI 入库入口拒绝零、负数、非有限值及布尔值，避免类型转换后误增库存。

- **销售导出编排与并行身份**：支持查询本月销售后导出其结果；查询输出缺失时拒绝生成文件。并行节点继承账号与租户上下文，各线程独立隔离。

- **报表导出回执**：工作流导出返回账号私有下载附件，避免将二进制内容直接放入聊天结果；缺少可信账号或租户身份时不生成文件。

- **看板与库存查询**：明确的经营看板请求进入报表接口；指定产品库存查询按名称或型号精确筛选，保留租户隔离。当前库存和看板不再追问接口不支持的日期范围。

- **销售规划与确认**：明确报价可解析当前租户客户和产品；月度销售汇总包含月末全天。候选重名或否定回答不再误确认，报价失败不自动重试。

- **产品新增澄清**：信息不足时询问产品字段，不再附带客户创建操作；补齐后仍按原审批流程执行。

- **报价与客户查询**：报价前拒绝缺失或无效的数量、单价；补齐客户列表及名称查询表达，避免误查产品。

- **AI 更新状态回读**：明确区分尚未观察、检查中、无更新、下载完成、错误和跳过检查；返回已下载版本信息，安装请求仍需重启后确认结果。

- **AI 桌面原生操作**：支持读取安装身份与更新状态、设置并回读开机启动，以及提交更新检查/下载/安装请求；写操作沿用审批，更新后仍需核对实际运行版本。

- **Mod 激活失败隔离**：待重启包校验或写入失败会记录具体失败阶段，不再中断其他独立模块的批量启动。

- **AI 附件错误反馈**：导出模块重载后，过期、损坏或无权使用的附件仍准确报告输入无效，避免被误归类为导出失败。

- **Mod 安装写入恢复**：归档先于代码替换；回执写入失败时恢复原目录，重启切换失败保留旧代码和待生效包，支持重新尝试。

- **Mod 初始化结果**：初始化异常、缺失函数或无法满足的参数会记录为加载失败，不再误报已加载，其依赖模块也不会继续启动。

- **Mod 启动顺序**：批量启动先加载依赖，避免因名称排序跳过可用模块；循环依赖及其下游记录失败，独立模块继续启动，保留版本和权益检查。

- **Mod 包预检**：指定包编号和版本后验证真实签名包，并检查当前账号可用的依赖版本；错包和缺失依赖不再返回固定可安装结果，预检不执行安装。

- **Mod 依赖版本校验**：加载前检查已加载依赖的实际版本，正确区分四段版本；版本不足或约束格式不支持时阻止启动，避免把依赖存在误判为兼容。

- **产品名称解析**：可按名称或型号查询当前租户和模块的真实产品；唯一精确匹配返回编号，重名与近似匹配返回候选并要求确认，避免默认选错产品。

- **标签模块权益**：标签生成、下载、确认和提交每次重新检查当前模块权益，权益撤销后不能继续访问或提交已有任务。

- **标签预览续接**：工作流任务可直接打开已生成标签，下载 PDF、核对并确认打印；账号或模块变化时清除旧预览与确认，提交结果未知时暂停重试。

- **单标签工作流**：使用明确产品和模板生成标签预览；缺少纸张或模板配置、生成失败时显示具体反馈，切换账号或模块后不显示旧结果。仅型号和数量的旧请求需补齐配置。

- **PDF 标签入口**：支持按明确的产品、模板、份数和纸张尺寸生成预览，返回下载和确认入口，确认后才提交打印。

- **标签任务模块隔离**：生成的标签文件和打印确认绑定来源模块，切换模块后不能访问或提交；缺少来源记录的旧任务保留文件并提示重新生成。

- **后台 Mod 任务作用域**：任务保存服务端确认的模块与会话记录，执行时重新检查账号、租户和模块权益。任务账本与队列固定使用主机库；访问旧 Mod 账本时复制历史并保留原库，待执行任务先暂停确认，原执行结果未知的任务禁止自动重试。
- **周期任务限时限次自动执行**：任务中心可查看具体动作和参数，设置授权截止时间及任务次数上限。授权内任务自动入队；次数耗尽、授权撤销、计划暂停或账号停用时，未开始的任务退回审批。重新授权可恢复待审批任务，已开始的操作仍从工作区查看和控制。
- **AI 周期计划**：支持固定间隔和指定时区的每日计划，保存到宿主数据库；任务中心可查询、暂停、恢复和取消。触发生成独立待审批任务，避免未完成任务堆积；停机错过多次只补一次，触发失败暂停并保留重试位置。周期授权自动执行仍在完善。
- **AI 一次性定时任务**：模型和任务接口可指定带时区的执行时间，审批后进入原有任务队列，到期才领取；暂停恢复保留时间，取消后不执行，任务面板显示最早执行时间。周期调度仍在完善。
- **AI 软件控制完善**：支持查询动作参数和当前页面目录，补齐下拉选择、勾选和按键操作；执行前检查参数与页面状态，HTTP 错误不会再误报成功，控制快照隐藏私密字段，AIOPEN 输入追踪隐藏输入正文。
- **AI 任务与记忆账号隔离**：后台业务工具恢复任务所属租户；创建、审批和恢复任务使用服务端身份，账号切换中止旧窗口指令。记忆新增查询与汇总，读写绑定当前账号；导航被拦截、输入值被拒绝时返回实际结果。
- **AI 员工执行上下文**：员工线程保留任务账号、租户与 Mod 上下文；调用参数不能替换任务账号，上游协作使用宿主传入的身份和会话。被风险门或产品权限阻止的定时任务不再计为成功。
