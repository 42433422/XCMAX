...
- **跨业务误路由修复**：说「客户列表」「本月销售汇总」「产品 A100 入库 50 件」「产品 A100 出库 50 件」「给星光报价」「库存有多少」「这个月花了多少钱」「记一笔收入5000」「销售订单列表」「销售报表」「创建发货单」「导出客户报表」时，各自走客户、报表、库存、销售、财务、发货的正确工具，不再一律当成查产品或乱写数据库；信息不全时先反问补齐，高风险操作仍需你确认后才执行。业务任务基准从 5/22 提升到 28/28 全对（含出库端到端写库校验、客户列表导出 Excel）。
