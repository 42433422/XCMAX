...
def cmd_export_fhd_shell(ns: argparse.Namespace) -> int:
    ...
    if ns.output:
        ...
        out = project_root().parent / "app" / "shell" / "fhd_shell_mods.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp = sub.add_parser(
        "export-fhd-shell",
        parents=[lib_arg],
        help="从库生成 FHD app/shell/fhd_shell_mods.json（供 GET /api/mods 读取）",
    )
    sp.set_defaults(func=cmd_export_fhd_shell)
    sp.add_argument(
        "-o",
        "--output",
        help="输出 JSON 路径（默认: <FHD>/app/shell/fhd_shell_mods.json，即 MODstore 上级仓库）",
    )
    sp.add_argument(
        "--dry-run",
        action="store_true",
