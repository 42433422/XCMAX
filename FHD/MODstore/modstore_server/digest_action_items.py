...
_SECTION_RE = re.compile(r"(?ms)^##\s*\[(?P<eid>[^\]]+)\]\s*(?P<label>[^\n·]*)[^\n]*\n(?P<body>.*?)(?=^##\s*\[|\Z)")
...
def _parse_markdown(markdown: str, *, kind: str) -> List[Dict[str, Any]]:
    ...
    for m in _SECTION_RE.finditer(text):
        ...
        for im in _ITEM_RE.finditer(body):
            ...
            low = item_text.lower()
            if item_text.startswith(("职责", "员工包版本", "建议 scope", "scope")) or low.startswith(("scope", "pack_version")):
                continue
            if _is_noise_sub_item(item_text, indent):
                continue
            pr = _infer_priority(item_text, kind=kind, section_priority=section_priority, indent=indent)
...
def parse_and_store_action_items(
    *,
    ...
    for kind, md, lk in ((KIND_PATCH, patches_markdown, "patches"), (KIND_UPDATE, updates_markdown, "updates")):
    ...
    logger.info("action_items stored day=%s patch=%s update=%s", day, inserted.get("patch"), inserted.get("update"))
...
def sync_dispatched_for_work_units(record_id: int, units: Sequence[Any]) -> Dict[str, Any]:
    ...
    matched: List[int] = []
    for u in units:
        lk = str(getattr(u, "list_kind", None) or (u.get("list_kind") if isinstance(u, dict) else "") or "")
        kind = KIND_PATCH if lk == "patches" else KIND_UPDATE
        eid = str(getattr(u, "employee_id", None) or (u.get("employee_id") if isinstance(u, dict) else "") or "")
        brief = str(getattr(u, "task_brief", None) or (u.get("task_brief") if isinstance(u, dict) else "") or "")
        if not brief:
            continue
...
def sync_merged_on_deploy(*, record_id: Optional[int] = None, day: Optional[str] = None) -> Dict[str, Any]:
