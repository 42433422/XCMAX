...
def _load_digest_execute_context(record_id: int) -> Optional[Dict[str, Any]]:
    ...
    with sf() as session:
        ...
        return {
            ...
            "base_version": str(vibe_meta.get("base_version") or vibe_meta.get("version") or "").strip(),
            "md_map": {
    ...
    *,
    ...
    if line == DISPATCH_PW:
        ...
        filtered = [
            u
            for u in filtered
            if u.pipeline_step in ("P1", "P8") or u.list_kind == "updates"
        ]
        ...
        filtered = [
            u
            for u in filtered
            if u.list_kind == "patches" and u.pipeline_step == "P2"
        ]
    else:
...
def execute_digest_line_work_units(
    ...
    *,
    ...
    try:
        ...
        try:
            conc = max(1, min(int(max_concurrency or os.environ.get("MODSTORE_DAILY_VIBE_EXECUTE_CONCURRENCY", "2")), 8))
        except ValueError:
            conc = 2
        ...
        logger.exception("execute_digest_line_work_units failed record_id=%s line=%s", record_id, line)
        err_payload = {
            "ok": False,
            "error": str(exc),
            "dispatch_line": line,
