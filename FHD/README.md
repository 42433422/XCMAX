# XCAGI v10.0 - 独立宿主 + 平台 MOD 变身垂直系统
...
**XCAGI v10.0** 通过 OCR、混合意图识别与大模型工作流，把「上传 Excel / 对话指令 → 解析 → 业务动作（出货、打印、库存等）」连成可运维的闭环，并新增 Windows / macOS 桌面交付形态。

> 🚀 **v10.0 / 10.0.0 当前版本**: `Electron 桌面壳 + FastAPI 本地子进程 + Vue 3 前端复用`，同时保留 Docker / Nginx web 版并行交付；Mod 商店 / 员工商店 / Token 认证钱包继续作为两种形态共用的商业化能力；跨行业 UI 适配 + Mod 制作端行业选择。
...
> 本节的**版本号表、产品定位、相关文档索引、发版前自检命令**与 [`VERSION.md`](VERSION.md) 对齐；完整叙事与按日期的发布说明仍以 [`CHANGELOG.md`](CHANGELOG.md) 为准（当前 GA 章节：**v10.0.0 (2026-05-21) — 跨行业适配**）。
...
发版打安装包时脚本使用的 **`-Version` / 参数版本字符串** 须与上表 **10.0.0** 一致（示例见下文 CHANGELOG 摘要中的命令块）。

### 当前定位（v10.0）

**跨平台企业 AI 员工桌面平台** — Windows/macOS 桌面版 + Web 版并行交付，保留 Neuro-DDD + FastAPI + Mod 生态 + Token 认证钱包 + 跨行业 UI 适配。

### v10.0.0 发布摘要（详见 CHANGELOG）
...
**XCAGI v10.0 — 企业 AI 员工平台（Neuro-DDD + FastAPI + Mod 生态 + 可选桌面版 + 跨行业适配）**
