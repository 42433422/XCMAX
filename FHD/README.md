...
**XCAGI 稳定版**通过 OCR、混合意图识别与大模型工作流，把「上传 Excel / 对话指令 → 解析 → 业务动作（出货、打印、库存等）」连成可运维的闭环，并提供 Windows / macOS 桌面交付形态。

> 🚀 **当前稳定版本**（数字唯一来源：[`VERSION.md`](VERSION.md)）：`Electron 桌面壳 + FastAPI 本地子进程 + Vue 3 前端复用`，同时保留 Docker / Nginx Web 版并行交付；npm/Electron/Flutter/Apple 工具链锚点使用等价三段版本 `1.0.0`。
...
> 本节与 [`VERSION.md`](VERSION.md) 对齐；完整叙事与按日期的历史发布说明仍以 [`CHANGELOG.md`](CHANGELOG.md) 为准。

### 版本号（必须同步的锚点）

| 组件 | 版本 | 文件 |
|------|------|------|
| **稳定产品版本** | 见 [`VERSION.md`](VERSION.md)（唯一数字来源） | 发布目录、下载清单、Python/FastAPI、Android、Windows 文件版本 |
...
发版脚本的 **`-Version` / 版本参数**必须传入 [`VERSION.md`](VERSION.md) 中的产品版本；脚本会自动派生工具链版本 `1.0.0`。

### 当前定位（稳定版）

**跨平台企业 AI 员工桌面平台** — Windows/macOS 桌面版 + Web 版并行交付，保留 Neuro-DDD + FastAPI + Mod 生态 + Token 认证钱包 + 跨行业 UI 适配。

### 历史 v10.0.0 发布摘要（详见 CHANGELOG）

以下内容对应 `CHANGELOG.md` **v10.0.0 (2026-05-21) — 跨行业适配** 一节，此处仅作索引；细节、命令与注意事项以 CHANGELOG 全文为准。
...
```bash
# Windows 桌面安装包
powershell -ExecutionPolicy Bypass -File scripts/package/build-installer.ps1 -ProductSku enterprise

# macOS 桌面安装包（版本省略时取脚本默认值 = VERSION.md 产品版本）
bash scripts/package/build-installer.sh enterprise
```
...
| **Git 标签** | 稳定版标签 **`FHD/v<VERSION.md 产品版本>`** 与产品版本对齐（发版时打在根仓 `main`）。 |
...
| 指标       | v3.0  | v4.0  | v5.0              | v6.0                         | v7.0                                      | v10.0                                      |
| -------- | ----- | ----- | ----------------- | ---------------------------- | ----------------------------------------- | ----------------------------------------- |
...
| 组件    | v1.0 | v2.0 | v3.0     | v4.0       | v5.0            | v6.0                  | v7.0                         | **v10.0**                         |
| ----- | ---- | ---- | -------- | ---------- | --------------- | --------------------- | -------------------------------- | -------------------------------- |
...
**XCAGI 稳定版 — 企业 AI 员工平台（Neuro-DDD + FastAPI + Mod 生态 + 桌面版 + 跨行业适配）**
