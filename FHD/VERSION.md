...
| 口径 | 版本 | 用途 |
|------|------|------|
| **XCAGI 稳定产品版本** | `1.0.0.5` | 对外版本、发布目录、下载清单、Windows 文件/产品版本、Python/FastAPI、Android `versionName` |
| **工具链兼容版本** | `1.0.0` | npm/Electron、Dart/Flutter、iOS/macOS `MARKETING_VERSION` |

四段产品版本 `1.0.0.5` 是稳定上线版本的唯一对外口径。npm SemVer、Electron、Dart pub 和 Apple 市场版本只接受三段版本，因此这些锚点使用等价映射 `1.0.0`；它们不代表另一个产品版本。

## 🔗 必须同步的锚点

| 组件 | 版本 | 文件 |
|------|------|------|
| **Python 包（根）** | `1.0.0.5` | `pyproject.toml` |
| **Python 包（XCAGI 子树）** | `1.0.0.5` | `XCAGI/pyproject.toml` |
| **FastAPI 应用** | `1.0.0.5` | `app/fastapi_app/factory.py` |
| **Mod 依赖校验基线** | `1.0.0.5` | `app/infrastructure/mods/manifest.py` |
| **Android versionName** | `1.0.0.5` | Flutter Android Runner Gradle 配置 |
...
> 独立子工程保留自己的包版本；其产品下载路径和发布清单仍必须引用稳定产品版本 `1.0.0.5`。
...
*最后更新：2026-09-17（稳定产品版本 1.0.0.5；工具链映射 1.0.0）*
