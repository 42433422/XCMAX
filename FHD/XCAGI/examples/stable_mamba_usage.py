...
from app.neuro_domains.stable_mamba import AdaptiveMambaLayer, StableMambaLayer
...
def demo_basic_usage():
    ...
    print("   ✅ 创建成功：d_model=768, d_state=16")
...
def demo_long_sequence():
    ...
    for seq_len in sequence_lengths:
        ...
        print(f"   {seq_len:5d} tokens - {status} - 最大范数：{stats['max_state_norm']:.2f}")
...
def demo_training_mode():
    ...
    # 创建带 dropout 的层
    layer = StableMambaLayer(d_model=768, d_state=16, use_dropout=True, dropout_rate=0.1)
...
def demo_performance_comparison():
    ...
    # 不同长度的性能测试
    test_configs = [("短序列", 128), ("中等序列", 512), ("长序列", 2048), ("超长序列", 8192)]
...
def demo_integration_with_service():
    ...
    # 创建配置
    config = ModelConfig(
        model_type=ModelType.MAMBA_BERT_HYBRID, hidden_size=768, num_layers=12, hybrid_ratio=0.5
    )
