...
def generate_markdown(schema: Dict[str, Any]) -> str:
    ...
    for path, path_item in paths.items():
        for method, operation in path_item.items():
            if method in ["get", "post", "put", "patch", "delete"]:
                ...
                if tag not in tags:
                    tags[tag] = []
                tags[tag].append({"path": path, "method": method.upper(), "operation": operation})
    ...
    for tag in sorted(tags.keys()):
        ...
        for item in sorted(tags[tag], key=lambda x: x["path"]):
            ...
            lines.append(
                f"  - [{item['method']} {item['path']}](#{item['method'].lower()}-{item['path'].replace('/', '-').replace('_', '-')}) - {summary}"
            )
        ...
        for item in sorted(tags[tag], key=lambda x: x["path"]):
            ...
            if parameters:
                ...
                for param in parameters:
                    ...
                    lines.append(
                        f"| {name} | {in_param} | {param_type} | {required} | {description} |"
                    )
                ...
                if "application/json" in content:
                    ...
                    lines.append(
                        generate_json_example(
                            schema_ref, schema.get("components", {}).get("schemas", {})
                        )
                    )
                    lines.append("```")
                    lines.append("")
