...
def _facade():
    return importlib.import_module("taiyangniao_attendance.mapper")
...
def _detail_side_month_link_column_map(ws) -> dict[str, int]:
    ...
    """
    crit_rows = (1, max(1, _facade().DETAIL_HEADER_ROWS - 1), _facade().DETAIL_HEADER_ROWS)
    out: dict[str, int] = {}
        ...
        if isinstance(ce0, str) and ce0.startswith("="):
            ...
            def _night(tpl: str) -> str:
                ...
                return _facade().re.sub(
                    "COLUMN\\(([B-D])\\d+\\)", lambda m: f"COLUMN({m.group(1)}{n_mark})", out
                )
...
def write_detail_sheet(
    workbook, employees: dict[str, _facade().EmployeeMonthTemplateData], *, month_label: str
    ...
    for br in block_base_rows:
        _facade()._normalize_block_chinese_numerals(
            ws, br, _facade().DETAIL_SUM_COL_START, _facade().DETAIL_TEMPLATE_SUMMARY_BEGIN_COL - 1
        )
    matched = 0
    unmatched: list[str] = []
