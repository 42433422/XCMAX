  ...
  <div class="page-view" id="view-business-docking">
    <div class="page-content">
      <div class="page-header">
        <h2>数据对接中心</h2>
        <p class="muted">上传业务 Excel，自动解析字段映射并入库（产品 / 客户 / 业务单据）</p>
      </div>
      ...
      <div class="dock-card">
        ...
        <div class="dock-actions">
          <label class="excel-file-picker">
            ...
            <span class="excel-file-picker-button">选择文件</span>
            <span class="excel-file-picker-name">{{ fileDisplayName || '未选择任何文件' }}</span>
          </label>
          <button
            type="button"
            class="btn btn-sm btn-primary dock-extract-btn"
      ...
      <div class="dock-results-stack" data-tutorial-anchor="business-dock-results">
        ...
        <div v-if="extractResult" class="result-layout" :style="dockPaneStyle">
          <div class="field-panel">
            ...
            <PaneResizeHandle
              v-if="isDockPaneResizable"
              orientation="vertical"
              label="调整字段列表宽度"
              @resize-start="onDockPaneResizeStart"
              @reset="resetDockPaneWidth"
            />
          </div>
...
const DOCK_LAYOUT_MQ = '(max-width: 960px)'
const isDockPaneResizable = ref(true)
let dockPaneViewportMedia = null
...
const {
  paneStyle: dockPaneStyle,
  startResize: onDockPaneResizeStart,
  resetSize: resetDockPaneWidth,
  stopResize: stopDockPaneResize,
} = useResizablePane({
  paneKey: 'business-docking.fields-panel',
  cssVarName: '--dock-field-panel-width',
  orientation: 'vertical',
  defaultSize: 280,
  minSize: 220,
  maxSize: 420,
  enabled: () => isDockPaneResizable.value,
})

function onDockPaneViewportChange(event) {
  isDockPaneResizable.value = !event.matches
  if (!isDockPaneResizable.value) {
    stopDockPaneResize()
  }
}
...
async function runExtractGrid(file, preferredSheetName = '', opts = {}) {
  ...
  if (!file) {
    if (showAlert) await appAlert('请先选择 Excel 文件')
...
function goTemplatePreview() {
  ...
  if (scope) {
    pushErpPage(router, { path: '/template-preview', query: { scope } })
    return
  }
...
onMounted(() => {
  dockPaneViewportMedia = window.matchMedia(DOCK_LAYOUT_MQ)
  onDockPaneViewportChange(dockPaneViewportMedia)
  if (typeof dockPaneViewportMedia.addEventListener === 'function') {
    dockPaneViewportMedia.addEventListener('change', onDockPaneViewportChange)
  } else if (typeof dockPaneViewportMedia.addListener === 'function') {
    dockPaneViewportMedia.addListener(onDockPaneViewportChange)
  }
})

onBeforeUnmount(() => {
  stopDockPaneResize()
  if (!dockPaneViewportMedia) return
  if (typeof dockPaneViewportMedia.removeEventListener === 'function') {
    dockPaneViewportMedia.removeEventListener('change', onDockPaneViewportChange)
  } else if (typeof dockPaneViewportMedia.removeListener === 'function') {
    dockPaneViewportMedia.removeListener(onDockPaneViewportChange)
  }
})
</script>
...
.excel-file-picker:focus-within {
  border-color: #42a5f5;
  box-shadow: 0 0 0 2px rgba(66, 165, 245, 0.18);
}
...
.field-panel {
  position: relative;
  flex: 0 0 var(--dock-field-panel-width);
  width: var(--dock-field-panel-width);
}
