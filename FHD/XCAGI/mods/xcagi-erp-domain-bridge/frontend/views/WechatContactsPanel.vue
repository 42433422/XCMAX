  ...
  <div class="wechat-contacts-panel" data-tutorial-id="wechat-contacts-panel">
    <div class="card" data-tutorial-id="wechat-search-star">
      ...
      <div class="form-group wechat-contacts-panel-toolbar">
        ...
        <button type="button" class="btn btn-secondary" @click="goBindEnterprise">绑定企业客户</button>
        <input
          type="text"
    ...
    <div class="card" data-tutorial-id="wechat-starred-list">
      ...
      <div>
        ...
        <div v-else-if="filteredContacts.length === 0" class="empty-state wechat-contacts-panel-empty">
          <p>暂无星标群聊。可先搜索并添加星标，或在「用户 Mod 管理」为企业绑定群聊。</p>
          <button type="button" class="btn btn-secondary btn-sm" @click="goBindEnterprise">去 Mod 管理绑定</button>
        </div>
        <div v-else class="wechat-contact-list-form">
          <div v-for="contact in filteredContacts" :key="contact.id" class="wechat-contact-form-item">
            ...
            <div class="wechat-contact-actions">
              ...
              <button class="btn btn-danger" @click="deleteContact(contact.id)">取消星标</button>
            </div>
          </div>
        </div>
      </div>
...
async function syncGroupMessages() {
  ...
  try {
    let msg = '';
    try {
      const data = await wechatGroupBridgeApi.syncGroups();
      const synced = data?.synced ?? data?.synced_groups ?? 0;
      const failed = data?.failed ?? 0;
      msg =
        data?.message ||
        `已同步 ${synced} 个群聊${failed ? `（失败 ${failed}）` : ''}`;
    } catch {
      const data = await wechatApi.refreshMessagesCache();
      if (!data?.success) throw new Error(data?.message || '同步失败');
      msg = data.message || '聊天记录已同步到应用库';
    }
    await appAlert(msg);
    await loadContacts();
  } catch (e: any) {
    await appAlert(`同步失败: ${e?.message || '未知错误'}`);
  } finally {
    syncingGroups.value = false;
  }
}

function goBindEnterprise() {
...
async function addToStar(item: any) {
  try {
    const data = await wechatApi.addStarredContact({
      ...
      remark: item.remark || '',
      wechat_id: item.username || item.wechat_id || '',
...
async function deleteContact(contactId: number) {
  if (!(await appConfirm('确定要取消该联系人的星标吗？'))) return;
  try {
    const data = await wechatApi.deleteStarredContact(contactId);
    if (!data?.success) throw new Error(data?.message || '取消星标失败');
    await loadContacts();
  } catch (e: any) {
    await appAlert(`取消星标失败: ${e?.message || '未知错误'}`);
  }
}
