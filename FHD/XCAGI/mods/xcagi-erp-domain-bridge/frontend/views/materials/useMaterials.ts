...
export function useMaterials() {
  ...
  const materialForm = ref<Record<string, any>>({ id: null, code: '', name: '', category: '', spec: '', unit: '个', quantity: 0, price: 0, supplier: '', min_stock: 0 })

  const showWarehouseModalFlag = ref(false)
  const isEditWarehouse = ref(false)
  const warehouseForm = ref<Record<string, any>>({ id: null, code: '', name: '', type: '', address: '', manager: '' })
  const selectedWarehouse = ref<any>(null)

  const showLocationModalFlag = ref(false)
  const isEditLocation = ref(false)
  const locationForm = ref<Record<string, any>>({ id: null, warehouse_id: null, code: '', name: '', max_capacity: null })

  const showInModalFlag = ref(false)
  const showOutModalFlag = ref(false)
  const inoutForm = ref<Record<string, any>>({ product_id: '', warehouse_id: '', quantity: 1, batch_no: '', remark: '' })

  const showSupplierModalFlag = ref(false)
  const isEditSupplier = ref(false)
  const supplierForm = ref<Record<string, any>>({ id: null, code: '', name: '', contact_person: '', contact_phone: '', rating: 3 })
  ...
  const loadMaterials = async () => {
    try {
      ...
      const res: any = await get('/api/materials', params)
      if (res.success) {
  ...
  const loadWarehouses = async () => {
    try {
      const res: any = await get('/api/inventory/warehouses')
      if (res.success) warehouses.value = res.data || []
    } catch (e) { console.error(e) }
  }

  const loadLocations = async (warehouseId: any) => {
    try {
      const res: any = await get('/api/inventory/locations', { warehouse_id: warehouseId })
  ...
  const loadTransactions = async () => {
    try {
      const res: any = await get('/api/inventory/transactions', { per_page: 100 })
  ...
  const saveMaterial = async () => {
    ...
    try {
      const res: any = isEditMaterial.value
        ? await api.put(`/api/materials/${materialForm.value.id}`, materialForm.value)
  ...
  const deleteMaterial = async (item: any) => {
    ...
    try {
      const res: any = await api.delete(`/api/materials/${item.id}`)
  ...
  const saveWarehouse = async () => {
    ...
    try {
      const res: any = isEditWarehouse.value
        ? await api.put(`/api/inventory/warehouses/${warehouseForm.value.id}`, warehouseForm.value)
  ...
  const saveLocation = async () => {
    ...
    try {
      const res: any = isEditLocation.value
        ? await api.put(`/api/inventory/locations/${locationForm.value.id}`, locationForm.value)
  ...
  const doIn = async () => {
    ...
    try {
      const res: any = await post('/api/inventory/in', inoutForm.value)
  ...
  const doOut = async () => {
    ...
    try {
      const res: any = await post('/api/inventory/out', inoutForm.value)
  ...
  const saveSupplier = async () => {
    ...
    try {
      const res: any = isEditSupplier.value
        ? await api.put(`/api/purchase/suppliers/${supplierForm.value.id}`, supplierForm.value)
  ...
  const exportData = async () => {
    try {
      ...
      const message = e && typeof e === 'object' ? String((e as any).message || '') : ''
