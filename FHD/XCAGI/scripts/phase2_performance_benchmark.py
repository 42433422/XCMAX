...
class Phase2PerformanceRunner:
    ...
    def __init__(self):
        ...
        self.report_file = (
            Path("test_reports")
            / f"phase2_performance_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def run_performance_test(self, command, description, timeout=600):
        ...
        try:
            ...
            return {"success": False, "duration": duration, "error": "性能测试超时"}
                ...
                if result["success"] and "performance_metrics" in result:
                    ...
                    for metric_name, target_info in performance_targets.items():
                        if metric_name in metrics:
                            ...
                            print(
                                f"{target_info['description']}: {actual_value}{target_info['unit']} vs 目标{target_value}{target_info['unit']} ({status})"
                            )
    ...
    def print_performance_summary(self, report, improvement_analysis):
        ...
        for metric_key, analysis in improvement_analysis.items():
            print(
                f"   {analysis['description']}: {analysis['actual']} vs 目标{analysis['target']} ({analysis['status']})"
            )
        print()

        # 总体评估
        performance_targets_met = sum(
            1 for a in improvement_analysis.values() if "✅" in a["status"]
        )
        ...
        if total_targets > 0:
            ...
            print(
                f"性能目标达成率: {target_success_rate:.1f}% ({performance_targets_met}/{total_targets})"
            )

        overall_success = summary["success_rate"] > 80 and (
            total_targets == 0 or target_success_rate > 70
        )
