...
function sendMessage() {
    ...
    // 如果当前处于“客户产品列表库”的多轮确认阶段，优先走 sendChatMessage
    ...
    var _xcagiChat = window.__XCAGI_CHAT__;
    if (_xcagiChat && typeof _xcagiChat.send === 'function') {
        _xcagiChat.send(message);
        if (input) input.value = '';
        return;
    }
...
function sendChatMessage(message, options = {}) {
    ...
    // pending import：当文件上传识别到“客户产品列表库”后，用户回复是/否/改成... 将触发导入
    try {
        const storageKey = 'xcagiPendingUnitProductsImport';
        let pending = window && window.__xcagiPendingUnitProductsImport;
        ...
        if (pending && pending.saved_name && pending.stage) {
            ...
            if (stage === 'confirm_filename') {
                const yesMatch = /^(是|确认|确定|导入|开始导入|执行|添加客户|添加购买单位)/.test(msgTrim);
                ...
                if (noMatch) {
                    ...
                    if (candidates.length > 0) {
                        const list = candidates
                            ...
                            .map((u, i) => `${i + 1})${u}`)
                            .join('  ');
                        addAi('好的。我从该库里提取到一些“客户”候选：' + list + '。回复序号或直接输入名称即可导入。');
                    } else {
                        addAi('我从该库里没有提取到客户候选。请你直接回复“改成 <名称>”。');
                    }
                    return;
                }
                if (yesMatch) {
                    ...
                    if (candidates.length > 0) {
                        const list = candidates
                            ...
                            .map((u, i) => `${i + 1})${u}`)
                            .join('  ');
                        addAi('好的。但我没法从文件名猜到客户名称。候选如下：' + list + '。回复序号或直接输入名称即可导入。');
                    } else {
                        addAi('我从该库里没有提取到客户候选。请你直接回复“改成 <名称>”。');
                    }
                    return;
            ...
            if (stage === 'choose_candidate') {
                ...
                addAi('请回复候选序号（如 1/2/3）或直接输入客户名称。');
                return;
            }
        }
    } catch (e) {
...
if (!(window.__XCAGI_CHAT__ && window.__XCAGI_CHAT__.ownsAutoAction) && !window.__VUE_HANDLE_AUTO_ACTION__) {
...
function ensureCustomersExportEntry() {
    ...
    entry.innerHTML = `
        ...
        <span class="entry-text">下载客户XLSX</span>
    `;
...
function getFilenameFromDisposition(disposition, fallback = '客户列表.xlsx') {
...
async function downloadCustomersXlsxFile(url) {
    ...
    try {
        ...
        setTimeout(() => {
            if (textEl) textEl.textContent = oldText || '下载客户XLSX';
...
function renderUserListPanel(customers, userMessage = '') {
    ...
    if (!panel) return;
    if (header) {
        header.innerHTML = '<span>用户名单（可编辑）</span><button class="btn-icon btn-excel" id="panelExportExcelBtn" title="导出客户Excel"><svg class="excel-icon-svg" viewBox="0 0 24 24" width="20" height="20"><rect width="24" height="24" rx="3" fill="#217346"/><path stroke="#fff" stroke-width="2.2" stroke-linecap="round" fill="none" d="M7 7l10 10M17 7L7 17"/></svg></button>';
