...
function buildExcelNextStepPrompt(suggestedUseCustomersImport) {
    ...
    return [
        ...
        ...options,
        '',
        suggestedUseCustomersImport ? '💡 若为客户列表，回复「导入客户」或「上传客户管理」并重新选择该文件即可自动导入。' : '当前模板能力：发货单模板、出货记录模板（后续可继续扩展）。'
    ].join('\n');
}
...
function openImportWindow(purpose = 'general') {
    ...
    if (titleEl) {
        if (currentImportPurpose === 'customers_import') {
            titleEl.textContent = '上传客户 Excel 更新客户';
    ...
    if (hintEl) {
        if (currentImportPurpose === 'customers_import') {
            hintEl.textContent = '请上传客户列表 .xlsx（需含「单位名称」列），将校验格式并更新联系人/电话/地址';
        } else if (currentImportPurpose === 'product_import') {
...
async function processGenericFiles(files, progressBar, progressText, progressPercent, status, uploadSessionId) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        if (currentImportPurpose === 'customers_import') {
            if (!/\.xlsx$/i.test(file.name || '')) {
                appendUploadMessage(`❌ ${file.name}：客户导入仅支持 .xlsx，且表头需含「单位名称」列。`);
            } else {
                try {
                    ...
                    if (importData.success) {
                        ...
                        let tip = `✅ 客户已更新：${importData.message}，列表已自动刷新`;
                        if (skipped > 0) {
        ...
        try {
            ...
            if (result && result.success) {
                ...
                if (result.suggested_use === 'unit_products_db' && result.saved_name) {
                    ...
                    messageParts.push(
                        guess
                            ? `下一步：请到聊天里回复“是”以导入客户「${guess}」的产品；也可回复“否 / 改成 <名称>”。`
                            : `下一步：请到聊天里回复“是 / 否 / 改成 <名称>”以完成客户 + 产品导入。`
                    );
                }
    ...
    if (wasCustomersImportOnly) {
        status.textContent = `客户导入完成（${files.length} 个文件）`;
