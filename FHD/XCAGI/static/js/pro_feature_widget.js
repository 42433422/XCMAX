...
class ProFeatureWidget {
        ...
        try {
            ...
            if (listEl) {
                ...
                customers.forEach((c, index) => {
                    ...
                    const name = this._safeText(c.customer_name || c.unit_name || c.name || `单位${index + 1}`);
                    ...
                    <div class="customer-row" data-action-row="wechat">
                        ...
                        <div class="inline-detail">
                            <div class="customer-card" style="padding: 8px 12px;">
                                <span style="color: #7f8c8d; font-size: 12px;">管理微信联系人列表（与客户类似）</span>
                            </div>
                        </div>
                    </div>
    ...
    renderProductQueryPanel() {
        ...
        main.innerHTML = `
            <div class="product-query-header">
                <div class="product-query-company-name" id="proProductCompanyName">请选择客户</div>
            ...
            <div class="product-query-list" id="proProductList">
                <div class="product-query-empty">先选择右侧客户</div>
            </div>
        `;
    ...
    async loadProductQueryCompanies(config = {}) {
        ...
        if (companyNameEl) companyNameEl.textContent = '请选择客户';
        if (productListEl) productListEl.innerHTML = '<div class="product-query-empty">先选择右侧客户</div>';
        ...
        try {
            ...
            this._productCompanies = customers.map((item, index) => ({
                id: Number(item.id || 0),
                name: String(item.customer_name || item.unit_name || item.name || `单位${index + 1}`)
            })).filter(item => item.id > 0 || item.name);

            if (!this._productCompanies.length) {
                if (cloud) cloud.innerHTML = '<div class="product-query-empty">暂无可用客户</div>';
                return;
            }
