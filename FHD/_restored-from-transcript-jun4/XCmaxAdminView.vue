<template>
  <div class="xcmax-admin-view" id="view-xcmax-admin">
    <header class="xcmax-admin-topbar">
      <div class="xcmax-admin-brand">
        <img
          class="xcmax-admin-logo"
        ...
        />
        <div>
          <h1 class="xcmax-admin-title">XCMAX 服务器后台</h1>
          <p class="xcmax-admin-subtitle">平台管理员 · 运维控制台</p>
        </div>
      </div>
      <button
        type="button"
...
async function logout() {
  ...
  try {
    ...
    await router.replace({ name: 'login', query: { account: 'admin' } })
  } catch (e) {
...
onActivated(async () => {
  if (activeTab.value === 'overview' && !overviewBootstrapped.value) {
    await refreshAll()
