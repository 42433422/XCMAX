...
async function reviewGovernanceAudit() {
  ...
  try {
    ...
    governanceReviewError.value = String(err?.message || err?.detail || err || '治理审计复核失败')
  } finally {
...
const evidence = computed<any>(() => asRecord(raw.value?.evidence))
const memory = computed<any>(() => asRecord(raw.value?.memory))
const policy = computed<any>(() => asRecord(raw.value?.policy))
const gate = computed<any>(() => asRecord(raw.value?.current_gate))
const cron = computed<any>(() => asRecord(raw.value?.cron))
const activeGates = computed<any>(() => {
...
const runtimeContract = computed<any>(() => asRecord(asRecord(raw.value).contract))
const runtimeContractValidation = computed<any>(() => asRecord(asRecord(raw.value).contract_validation))
...
const runtimeSurfaceReadiness = computed<any>(() =>
...
const runtimeSurfaceIncidentSummary = computed<any>(() =>
  asRecord(runtimeContractValidation.value.surface_incident_summary),
)
const runtimeContractStatus = computed<any>(() => {
...
const runtimeContractPrimaryRoute = computed<any>(() =>
...
const runtimeSurfaceIncident = computed<any>(() => runtimeSurfaceIncidents.value[0] || {})
...
const kbSummary = computed<any>(() => asRecord(raw.value?.kb_summary))
const evolutionMetrics = computed<any>(() => asRecord(raw.value?.evolution_metrics_summary))
const rosterAlignment = computed<any>(() => asRecord(raw.value?.roster_alignment))
const currentGovernanceGate = computed<any>(() => asRecord(raw.value?.governance_gate))
const uiBridge = computed<any>(() => asRecord(raw.value?.ui_bridge))
const governanceAudit = computed<any>(() => asRecord(raw.value?.governance_audit))
const governanceAuditSummary = computed<any>(() => asRecord(governanceAudit.value.summary))
const governanceAuditLast = computed<any>(() => asRecord(governanceAudit.value.last))
...
const employeeSpaceBridge = computed<any>(() => asRecord(uiBridge.value.employee_space))
const dutyRosterBridge = computed<any>(() => asRecord(uiBridge.value.duty_roster_graph))
const uiBridgeGovernanceAction = computed<any>(() => asRecord(uiBridge.value.governance_action))
...
const latestComplete = computed<any>(() => asRecord(evidence.value.latest_complete))
const latestSkip = computed<any>(() => asRecord(evidence.value.latest_skip))
const lastRun = computed<any>(() => {
...
const decision = computed<any>(() => asRecord(memory.value.last_policy_decision))
const mergeDecision = computed<any>(() => {
...
const proactiveSignals = computed<any>(() => asRecord(gate.value.proactive_signals))
...
function proactiveCandidateTitle(item: Record<string, any>): string {
  return firstText(
    item.title,
    item.summary,
    item.reason,
...
function proactiveCandidateMeta(item: Record<string, any>): string {
  return [
...
const rosterGate = computed<any>(() => asRecord(rosterAlignment.value.gate))
const rosterRemediation = computed<any>(() => asRecord(rosterAlignment.value.remediation))
...
.selp {
  --selp-accent: #60a5fa;
  --selp-bg: rgba(15, 23, 42, 0.72);
  --selp-border: rgba(148, 163, 184, 0.22);
  --selp-text: #e5eefc;
  --selp-muted: #94a3b8;
  display: flex;
  ...
  border-radius: 16px;
  background:
    radial-gradient(circle at 18% 0%, rgba(96, 165, 250, 0.20), transparent 34%),
    linear-gradient(135deg, rgba(15, 23, 42, 0.96), rgba(2, 6, 23, 0.90));
  color: var(--selp-text);
  overflow: auto;
}

.selp--running { --selp-accent: #60a5fa; }
.selp--ok { --selp-accent: #22c55e; }
.selp--warn { --selp-accent: #f59e0b; }
.selp--bad { --selp-accent: #ef4444; }
.selp--idle { --selp-accent: #94a3b8; }
...
.selp-state-dot {
  ...
  box-shadow: 0 0 0 5px color-mix(in srgb, var(--selp-accent) 18%, transparent);
}

.selp-refresh {
  border: 1px solid color-mix(in srgb, var(--selp-accent) 32%, rgba(148, 163, 184, 0.35));
  border-radius: 999px;
...
.selp-proactive {
  ...
  border-radius: 12px;
  background:
    radial-gradient(circle at 12% 0%, rgba(129, 140, 248, 0.16), transparent 36%),
    rgba(15, 23, 42, 0.58);
}
...
.selp-metrics {
  ...
  border-radius: 12px;
  background:
    radial-gradient(circle at 12% 0%, rgba(34, 197, 94, 0.14), transparent 36%),
    rgba(15, 23, 42, 0.58);
}
...
.selp-metrics-card--ok { background: rgba(34, 197, 94, 0.10); }
.selp-metrics-card--warn { background: rgba(245, 158, 11, 0.10); }
.selp-metrics-card--bad { background: rgba(239, 68, 68, 0.12); }
...
.selp-stage::after {
  ...
  height: 70px;
  border-radius: 999px;
  background: color-mix(in srgb, var(--stage-color, #64748b) 18%, transparent);
}

.selp-stage--ok { --stage-color: #22c55e; }
.selp-stage--warn { --stage-color: #f59e0b; }
.selp-stage--bad { --stage-color: #ef4444; }
.selp-stage--running { --stage-color: #60a5fa; }
.selp-stage--idle { --stage-color: #94a3b8; }
...
.selp-stage-dot {
  ...
  box-shadow: 0 0 10px color-mix(in srgb, var(--stage-color, #94a3b8) 58%, transparent);
}
...
.selp-timeline-index {
  ...
  height: 22px;
  border-radius: 999px;
  background: color-mix(in srgb, var(--selp-accent) 15%, rgba(15, 23, 42, 0.90));
  color: var(--selp-accent);
  font-size: 11px;
  font-weight: 900;
}
...
.selp-team-chip {
  ...
  padding: 7px 9px;
  border-radius: 10px;
  background: color-mix(in srgb, var(--selp-accent) 13%, rgba(15, 23, 42, 0.90));
  color: #f8fafc;
}
...
.selp-ui-bridge {
  ...
  background:
    ...
    linear-gradient(135deg, rgba(15, 23, 42, 0.74), rgba(2, 6, 23, 0.62));
}

.selp-ui-bridge--run {
  border-color: rgba(20, 184, 166, 0.36);
  background: linear-gradient(135deg, rgba(20, 184, 166, 0.14), rgba(15, 23, 42, 0.68));
}
...
.selp-ui-bridge--bad {
  border-color: rgba(239, 68, 68, 0.32);
  background: linear-gradient(135deg, rgba(239, 68, 68, 0.14), rgba(15, 23, 42, 0.68));
}
...
.selp-roster-card--run { background: rgba(20, 184, 166, 0.12); }
.selp-roster-card--ok { background: rgba(34, 197, 94, 0.10); }
.selp-roster-card--warn { background: rgba(245, 158, 11, 0.10); }
.selp-roster-card--bad { background: rgba(239, 68, 68, 0.12); }
