/**
 * DutyRosterGraphPanel 顶部工具栏（标题 / 统计 / 视图切换 / 更多操作）。
 *
...
import { ALL_PLANNED_IDS } from './dutyRosterConstants'
...
import type { DutyGraphBuild } from './useDutyGraphBuild'
...
const emit = defineEmits<{
  (e: 'update:showStatsDetail', v: Deref<DutyRosterState['showStatsDetail']>): void
  (e: 'update:viewMode', v: Deref<DutyRosterState['viewMode']>): void
  (e: 'update:showMoreActions', v: Deref<DutyRosterState['showMoreActions']>): void
  (e: 'update:autoRefresh', v: Deref<DutyRosterState['autoRefresh']>): void
  (e: 'close'): void
}>()

const props = defineProps<{
  showStatsDetail: Deref<DutyRosterState['showStatsDetail']>
  viewMode: Deref<DutyRosterState['viewMode']>
  showMoreActions: Deref<DutyRosterState['showMoreActions']>
  autoRefresh: Deref<DutyRosterState['autoRefresh']>
  open: boolean
  isPage: boolean
  goBackFromPage: () => void
  load: Deref<DutyData['load']>
  employees: Deref<DutyRosterState['employees']>
  loading: Deref<DutyRosterState['loading']>
  loadingP2: Deref<DutyRosterState['loadingP2']>
  capLoading: Deref<DutyRosterState['capLoading']>
  countdown: Deref<DutyRosterState['countdown']>
  llmStatusFailed: Deref<DutyRosterState['llmStatusFailed']>
  ...
  stats: Deref<DutyGraphBuild['stats']>
  loopRuntimeStatus: Deref<DutyLoopCore['loopRuntimeStatus']>
  loopRuntimeContractStatus: Deref<DutyLoopCore['loopRuntimeContractStatus']>
  loopStatusLabel: Deref<DutyLoopGovernance['loopStatusLabel']>
  loopOpenRunCount: Deref<DutyLoopCore['loopOpenRunCount']>
  loopParticipantIdSet: Deref<DutyLoopCore['loopParticipantIdSet']>
  loopParticipantIds: Deref<DutyLoopCore['loopParticipantIds']>
  employeeSpaceLocation: Deref<DutyLoopCore['employeeSpaceLocation']>
  openNoKeyPanel: Deref<DutyPanels['openNoKeyPanel']>
  togglePanel: Deref<DutyPanels['togglePanel']>
}>()
