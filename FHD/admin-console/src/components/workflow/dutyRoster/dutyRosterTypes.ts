/**
 * DutyRosterGraphPanel 类型定义（由原文机械切分，内容保持不变）。
 */
...
export type ViewMode = 'hub' | 'department' | 'legacy-area' | 'client' | 'loop'
export type GraphViewMode = Exclude<ViewMode, 'loop'>
...
/** 头部统计条对象（原 stats computed 的形状，供子组件 props 复用）。 */
export type DutyStats = {
  total: number; catalogOk: number; v1Only: number; healthy: number; depEdges: number
  llmActive: number; llmNoKey: number; execReady: number; highRisk: number
}
