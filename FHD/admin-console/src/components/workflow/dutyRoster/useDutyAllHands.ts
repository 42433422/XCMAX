...
import type { AllHandsEmployeeRow, AllHandsReport, AllHandsProgress, AllHandsSessionSnapshot, MeetingMinutesBlock, MeetingMinutesEmailMeta } from './dutyRosterTypes'
...
import { AREA_COLORS, stripEmbeddedReasoningTrace } from './dutyRosterConstants'
export function useDutyAllHands(s: DutyRosterState, panels: DutyPanels, ctx: { focusEmployee: (id: string) => void }) {
  ...
  const { closeOtherPanels, togglePanel, openNoKeyPanel, isDetailOpen, toggleDetail, openGapPanel } = panels
...
function parseAllHandsReportFromArtifact(artifact: Record<string, unknown> | null | undefined): AllHandsReport | null {
  ...
  if (!Array.isArray((report as { employees?: unknown }).employees)) return null
  return report
}
...
let   allHandsPollTimer = 0
  ...
  return {
    ...
    applyAllHandsProgress,
    copyAllHandsMeetingMinutes,
    downloadAllHandsMeetingMinutes,
    stopAllHandsPolling,
    pollAllHandsSession,
    runAllHands,
    askAllHandsQuestion,
    toggleAllHandsRow,
    requestPlainLang,
    focusAllHandsEmployee,
    publishFollowUpToButler,
    allHandsAreaPalette,
  }
}

export type DutyAllHands = ReturnType<typeof useDutyAllHands>
