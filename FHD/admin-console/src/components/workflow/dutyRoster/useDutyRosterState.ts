/**
 * DutyRosterGraphPanel 共享状态（全部 ref + 在岗员工 computed）。
 *
 * 由原文机械切分而来（行为保持不变）。
 */
...
import type {
  EmpRow, HealthSt, ExecRow, HealthLv, ViewMode, LlmProviderSt, EmpLlmCfg, EmpCapability, RunNodeStatus, LlmActLv,
  NoKeyResponse, AllHandsReport, AllHandsProgress, MeetingMinutesBlock, MeetingMinutesEmailMeta,
  DutyGraphRun,
} from './dutyRosterTypes'
import type { EmployeeCapabilityView } from '@host/domain/butlerEmployeeProfile'
import type { ClientWorkshop } from '@host/domain/clientWorkshops'
...
import { parseViewModeFromQuery, isDutyGraphMember, ALL_AREAS, AREA_COLORS, isVirtualEmployee } from './dutyRosterConstants'

export function useDutyRosterState(route: RouteLocationNormalizedLoaded) {
...
const loopRuntimeStatus = ref<Record<string, unknown> | null>(null)
...
export type DutyRosterState = ReturnType<typeof useDutyRosterState>
