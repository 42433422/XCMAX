<template>
  <div class="approval-hub-view" id="view-autonomy-approval-hub">
    <div class="page-header">
      <div>
        <h2>自治审批中心</h2>
        <p>待办审批 · 审计日志 · 动作详情（每 30s 自动刷新）</p>
      </div>
      <div class="header-actions">
        <span class="pill">待办 {{ pending.length }}</span>
    ...
    <div class="hub-grid">
      <section class="panel">
        ...
        <div v-if="!pending.length" class="empty">暂无待审批动作</div>
      ...
      <section class="panel">
        ...
        <ul v-else class="audit-list">
          <li v-for="(row, idx) in auditItems" :key="row.id || idx" class="audit-item">
            <div class="audit-top">
...
async function refreshAll() {
  ...
  try {
    ...
    const raw = (auditRes as { items?: AuditRow[] })?.items
