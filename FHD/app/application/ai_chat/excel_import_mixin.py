...
class AIChatExcelImportMixin:
    ...
    def _default_purchase_unit_for_import(
        ...
        if isinstance(preview_data, dict):
            ...
            if hint:
                return hint
        grid_hint = _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._customer_hint_from_preview_grid(preview_data)
        if grid_hint:
            return grid_hint
        fp = _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._resolve_excel_path_for_import(excel_analysis, preview_data)
        sheet = _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._resolve_sheet_name_for_reimport(
        ...
        return _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._guess_default_purchase_unit(excel_analysis)
    ...
    def _try_structured_reload_records(
        ...
        force_hdr = _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._resolve_force_header_row_1based(
        ...
        try:
            ...
            for row in rows:
                if isinstance(row, dict):
                    out.append(
                        {
                            k: _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._sanitize_import_scalar(v)
    ...
    def _infer_excel_column_roles_with_llm(self, records: list[dict[str, Any]]) -> dict[str, str]:
        ...
        try:
            ...
            resp = _facade_attr("httpx", httpx).post(
                api_url,
    ...
    def _resolve_unit_price_column(
        ...
        before, after, generic = _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._price_column_buckets(keyset)
    ...
    def _extract_excel_import_records(
        ...
        *,
        ...
        records = [
            (
                {k: _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._sanitize_import_scalar(v) for k, v in r.items()}
        ...
        header_roles = _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._header_hint_column_roles(
        ...
        keys = [str(k).strip() for k in records[0].keys() if str(k).strip()]
        merged_intent = _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._merge_user_intent_for_price_resolution(
        ...
        price_col, price_err = _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._resolve_unit_price_column(
        ...
        if unit_key:
            ...
            if _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._packaging_or_measure_ratio(col_vals) >= 0.45:
                unit_key = ""
        ...
        for row in records:
            ...
            if (
                ...
                or (
                    default_unit
                    and unit_name
                    and _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._excel_cell_looks_like_product_measure_unit(
                        unit_name
                    )
                )
            ):
    ...
    def _looks_like_smart_workflow_intent(text: str, context: dict[str, Any] | None = None) -> bool:
        ...
        if not t:
            return False
        if _facade_attr("AIChatApplicationService", AIChatExcelImportMixin)._looks_like_explicit_workflow_tool_intent(t):
            return True
