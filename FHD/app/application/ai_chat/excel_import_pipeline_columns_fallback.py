...
"""Excel 导入管线：表头词条与列统计的回退式角色启发（叶子模块）。"""
...
class ExcelImportColumnFallbackMixin:
    @staticmethod
    ...
    def _header_hint_column_roles(keys: list[str]) -> dict[str, str]:
        ...
        for key in keys:
            ...
            normalized = re.sub("[\\s:_：\\-_/（）()]+", "", text).lower()
            if not normalized:
                continue
            if normalized in {
                "客户",
    ...
    def _fallback_excel_product_name_column(
        self, records: list[dict[str, Any]], reserved: set[str]
    ) -> str:
        """
        ...
        if not records or not isinstance(records[0], dict):
            return ""
        skip_re = re.compile("(序|序号|行号|单号|单据|^id$|^no\\.?$)", re.I)
        best_col = ""
        best_score = -1.0
