...
"""Excel 导入管线：列语义角色推断（规则评分 + LLM 兜底）与单价列仲裁（叶子模块）。"""
...
class ExcelImportColumnInferMixin:
    ...
    def _price_column_buckets(keys: list[str]) -> tuple[list[str], list[str], list[str]]:
        ...
        for raw in keys:
            ...
            if not cn or "数量" in cn or "计量" in cn or ("件数" in cn):
                continue
            if not re.search("(单价|价格|报价|含税价|含税单价|金额)", cn):
                continue
            if re.search("(调价\\s*前|调价前|调整前|原价)", cn):
                before.append(cn)
            elif re.search("(调价\\s*后|调价后|折后|执行价|现用)", cn):
                after.append(cn)
            else:
                generic.append(cn)
    ...
    def _merge_user_intent_for_price_resolution(
        user_message: str, request_context: dict[str, Any] | None
    ) -> str:
        """
    ...
    def _resolve_unit_price_column(
        keys: list[str], current: str, user_message: str, overrides: dict[str, Any] | None
    ) -> tuple[str, str | None]:
        """
        结合列名与用户话术确定入库单价列。
        ...
        before, after, generic = ExcelImportColumnInferMixin._price_column_buckets(keyset)
