...
"""Excel 导入管线：记录提取、角色合成去重入库与可执行意图短路（叶子模块）。"""
...
from app.domain.services.conversation.chat_tool_intent import looks_like_explicit_workflow_tool_intent
from app.utils.operational_errors import RECOVERABLE_ERRORS

logger = logging.getLogger(__name__)


class ExcelImportExtractMixin:
    ...
    def _looks_like_smart_workflow_intent(text: str, context: dict[str, Any] | None = None) -> bool:
        ...
        if not t:
            return False
        if ExcelImportExtractMixin._looks_like_explicit_workflow_tool_intent(t):
            return True
