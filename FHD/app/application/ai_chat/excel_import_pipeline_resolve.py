...
"""Excel 导入管线：路径/表头/工作表解析与默认采购单位推断（叶子模块）。"""
...
class ExcelImportResolveMixin:
    ...
    def _guess_default_purchase_unit(excel_analysis: dict[str, Any]) -> str:
        ...
        if not stem:
            return ""
        stem = re.sub("\\d{2,4}年?$", "", stem).strip()
    ...
    def _try_structured_reload_records(
        ...
        try:
            ...
            return None
        sheet = ExcelImportResolveMixin._resolve_sheet_name_for_reimport(
            excel_analysis, preview_data, request_context
        )
        force_hdr = ExcelImportResolveMixin._resolve_force_header_row_1based(
        ...
        try:
            ...
            for row in rows:
                if isinstance(row, dict):
                    out.append(
                        {
                            k: ExcelImportResolveMixin._sanitize_import_scalar(v)
