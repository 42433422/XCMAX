...
class _AIChatApplicationServicePart03Mixin:
    @staticmethod
    def _iter_agentic_artifact_payloads(
    ...
    def _dispatch_workflow_tool(
        self, tool_id: str, action: str, params: dict[str, _facade().Any]
    ...
    def _run_workflow_with_state_updates(
        ...
        *,
        ...
        resume: bool = False,
        **kwargs: _facade().Any,
    ) -> tuple[_facade().Any, list[dict[str, _facade().Any]]]:
        ...
        if resume and checkpointer is not None:
            ...
            if latest is not None:
                ...
                run_result = self.workflow_engine.run(
                    ...
                    max_retries=max_retries,
                    state_event_callback=record_state_event,
                    checkpointer=checkpointer,
                    **kwargs,
                )
        else:
    ...
    def _open_clarification_gate(
        ...
        *,
        ...
        return {
            ...
            "message": "需要澄清",
            "response": question,
            "data": {"text": question, "action": "clarification_required", "data": clarify_inner},
        }

    def _continue_after_clarification(
        self, user_id: str, pending: dict[str, _facade().Any], message: str
    ) -> dict[str, _facade().Any] | None:
        ...
        if confirmed is None and target.tool_id == "business_db" and (not candidates):
            ...
            resolved = prepare_business_db_write_target(
                entity, str(target.params.get("operation") or ""), payload
            )
    ...
    def _handle_confirmation_flow(
        self, user_id: str, message: str, file_context: dict[str, _facade().Any] | None
    ) -> None:
        """处理确认流程"""
        if not file_context:
            return
        ...
        if saved_name and suggested_use == "unit_products_db" and unit_name:
            self.ai_service.set_pending_confirmation(
                user_id,
                {
                    ...
                    "params": {"saved_name": saved_name, "unit_name": unit_name},
    ...
    def _build_response(
        self, ai_result: dict[str, _facade().Any], source: str | None, original_message: str = ""
    ) -> dict[str, _facade().Any]:
        """构建响应数据"""
        response_data = {
            "success": True,
    ...
    def _handle_tool_call(
        ...
        ai_result: dict[str, _facade().Any],
