...
职责：
- Excel 向量上下文注入（语义检索）
- Excel 导入列角色推断（规则 + LLM 兜底）
- Excel 导入记录抽取、去重、单价列歧义消解
- Excel 导入相关的客户/单位/型号兜底猜测

以 Mixin 形式提供给 ``AIChatApplicationService`` 组合继承，方法内的 ``self`` 在实际运行时
指向组合后的 ``AIChatApplicationService`` 实例（依赖其 ``__init__`` 提供的 ``ai_service`` 等属性），
...
class AIChatExcelContextService:
    """
    AI 聊天 Excel 上下文子服务（Mixin）

    编排 Excel 向量检索、列角色推断与导入记录抽取，供 ``AIChatApplicationService`` 组合使用。
    """
        ...
        for row in records:
            ...
            if (
                ...
                or (
                    default_unit
                    and unit_name
                    and self._excel_cell_looks_like_product_measure_unit(
                        unit_name
                    )
                )
            ):
