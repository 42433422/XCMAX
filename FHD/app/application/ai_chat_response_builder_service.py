...
职责：
- 构建统一的聊天响应格式（``_build_response`` / ``_build_fallback_response``）
- 即时工具执行（products / customers / shipments / shipment_generate），
  分别覆盖专业模式（pro）与普通模式（normal）两套派发路径

以 Mixin 形式提供给 ``AIChatApplicationService`` 组合继承，方法内的 ``self`` 在实际运行时
指向组合后的 ``AIChatApplicationService`` 实例，因此这里的方法签名与实现与拆分前保持一致，
不改变任何行为。
"""
...
class AIChatResponseBuilderService:
    """
    AI 聊天响应构建子服务（Mixin）

    编排统一响应格式构建与即时工具执行，供 ``AIChatApplicationService`` 组合使用。
    """
