...
职责：
- 判定是否应进入工作流规划（显式工具意图 / 智能意图 / 待确认工作流）
- 「规则导入捷径」：excel_analysis + 导入关键词 → 直接生成导入计划并写库（可由前端/环境变量关闭）
- 动态工作流规划、风险评估、Agent 运行编排（AgentOrchestrator / AgenticLoop 两条执行路径）
- 工作流确认/取消应答处理与 AgentRun 追踪
- 工作流/AgentRun 结果格式化为统一聊天响应

以 Mixin 形式提供给 ``AIChatApplicationService`` 组合继承，方法内的 ``self`` 在实际运行时
指向组合后的 ``AIChatApplicationService`` 实例（依赖其 ``__init__`` 提供的 ``workflow_planner`` /
``risk_gate`` / ``workflow_engine`` / ``approval_service`` / ``_pending_workflows`` 等属性，以及
``AIChatExcelContextService`` / ``AIChatResponseBuilderService`` 混入的方法），因此这里的方法签名
与实现与拆分前保持一致，不改变任何行为。

注意：``self.approval_service`` 是 ``app.application.workflow.get_approval_service()`` 返回的
审批域服务（用于动态工作流高风险节点的审批请求），与 ``AIChatApprovalService``（聊天确认流程）
是两个不同的概念。
"""
...
from app.application.ai_chat_approval_service import _enrich_confirmation_inner
...
class AIChatWorkflowService:
    """
    AI 聊天动态工作流子服务（Mixin）

    编排规则导入捷径、动态工作流规划与执行、AgentRun 结果格式化，供
    ``AIChatApplicationService`` 组合使用。
    """
