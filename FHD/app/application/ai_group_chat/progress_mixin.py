...
class AiGroupChatProgressMixin:
    def get_messages(
    ...
    def _latest_progress_row(rows: list[dict[str, Any]], task_id: str) -> dict[str, Any] | None:
        for row in reversed(rows):
            if _facade_attr("AiGroupChatService", AiGroupChatProgressMixin)._report_relay_task_id(row) == task_id:
                return row
        return None
    ...
    def _should_append_progress(*, last: dict[str, Any] | None, status: str) -> bool:
        ...
        if last_status and last_status != status:
            return True
        last_at = _facade_attr("AiGroupChatService", AiGroupChatProgressMixin)._parse_created_at(str(last.get("created_at") or ""))
        if last_at is None:
            return True
