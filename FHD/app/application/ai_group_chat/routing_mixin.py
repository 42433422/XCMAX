...
class AiGroupChatRoutingMixin:
    ...
    def _heuristic_dispatch_targets(
        ...
        by_id = {str(m.get("employee_id") or ""): m for m in candidates}
        text = (task or "").lower()
        if _facade_attr("AiGroupChatService", AiGroupChatRoutingMixin)._dispatch_difficulty(task) == "simple":
            preferred = _facade_attr("AiGroupChatService", AiGroupChatRoutingMixin)._preferred_single_dispatch_target(candidates, task)
