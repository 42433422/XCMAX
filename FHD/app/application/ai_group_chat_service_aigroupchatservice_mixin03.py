...
class _AiGroupChatServicePart03Mixin:
    ...
    @staticmethod
    def _format_work_report_message(member: dict[str, _facade().Any], report: dict[str, _facade().Any]) -> str:
        ...
        status = {'queued': '已接单', 'accepted': '已接单', 'assigned': '已接单', 'running': '执行中', 'in_progress': '执行中', 'completed': '完成', 'done': '完成', 'failed': '失败', 'blocked': '阻塞'}.get(raw_status, '完成' if ok else '失败')
    ...
    def _relay_report_message(self, *, user_id: int, group_id: str, task_id: str) -> dict[str, _facade().Any] | None:
    ...
    def _append_work_acceptance_if_ready(self, *, user_id: int, group_id: str, work_order_id: str) -> dict[str, _facade().Any] | None:
        if not work_order_id:
            return None
        rows = [row for row in self._read_messages() if int(row.get('user_id') or 0) == int(user_id) and str(row.get('group_id') or '') == str(group_id) and (str(row.get('work_order_id') or '') == str(work_order_id))]
        if not rows:
            return None
        ...
        initial_reports = [row for row in rows if str(row.get('kind') or '') == 'work_report' and self._report_relay_task_id(row)]
        ...
        row = self._message_row(user_id=user_id, group_id=group_id, role='ai', sender_id=_facade()._XIAOC_ASSISTANT_ID, sender_name='小C助理', sender_avatar='', body=self._format_work_acceptance_message(work_order=work_order, final_reports=ordered_finals, ok_count=ok_count, total=len(ordered_finals), all_ok=all_ok), kind='work_acceptance', status=acceptance_status, work_order_id=work_order_id, payload={'work_order_id': work_order_id, 'status': acceptance_status, 'total': len(ordered_finals), 'completed': ok_count, 'task_ids': expected_task_ids, 'branch_context': str((dict(work_order['payload']) if isinstance(work_order.get('payload'), dict) else {}).get('branch_context') or '')})
        self._append_messages([row])
        return self._public_message(row)

    @classmethod
    def _format_work_acceptance_message(cls, *, work_order: dict[str, _facade().Any], final_reports: list[dict[str, _facade().Any]], ok_count: int, total: int, all_ok: bool) -> str:
        ...
        for row in final_reports:
            ...
            report: dict[str, _facade().Any] = dict(raw_report) if isinstance(raw_report, dict) else {}
            name = str((row or {}).get('sender_name') or report.get('employee_name') or '负责人')
            status = cls._effective_report_status(row)
            focus = str(report.get('assignment_focus') or '').strip()
            summary = cls._chat_friendly_summary(str(report.get('summary') or row.get('body') or ''), limit=_facade().CHAT_ACCEPTANCE_SUMMARY_CHARS, include_detail_note=False)
    ...
    def _public_status_label(status: str) -> str:
        return {'completed': '完成', 'done': '完成', 'failed': '失败', 'blocked': '阻塞', 'cancelled': '已取消'}.get(str(status or '').strip().lower(), str(status or '已回报'))
    ...
    def _relay_task_report(self, *, task: dict[str, _facade().Any], member: dict[str, _facade().Any]) -> dict[str, _facade().Any]:
        raw_payload = task.get('payload')
        payload: dict[str, _facade().Any] = dict(raw_payload) if isinstance(raw_payload, dict) else {}
        raw_context = payload.get('context')
        context: dict[str, _facade().Any] = dict(raw_context) if isinstance(raw_context, dict) else {}
        ...
        missing_evidence = self._completed_report_lacks_required_evidence(task_text, summary, result)
        ...
        if not isinstance(context, dict):
            context = {}
        return {'work_order_id': str(context.get('work_order_id') or ''), 'employee_id': str(context.get('employee_id') or member.get('employee_id') or ''), 'employee_name': str(member.get('name') or member.get('employee_id') or ''), 'task': str(payload.get('message') or ''), 'original_task': str(context.get('original_task') or ''), 'assignment_focus': str(context.get('assignment_focus') or ''), 'branch_context': str(context.get('branch') or payload.get('branch') or ''), 'status': 'completed' if success and status == 'done' else effective_status, 'success': success, 'summary': summary, 'risk': '回报只有调研/方案或缺少改动文件、命令、测试、构建、安装证据，不能自动验收。' if missing_evidence else self._relay_result_risk(result=result, success=success, task_id=str(task.get('task_id') or ''), dispatcher=dispatcher), 'raw': {'task_id': str(task.get('task_id') or ''), 'relay_id': str(task.get('relay_id') or ''), 'kind': str(task.get('kind') or ''), 'dispatcher': dispatcher, 'dispatch_status': dispatch_status, 'evidence_required': missing_evidence}}

    @classmethod
    def _relay_result_summary(cls, result: dict[str, _facade().Any], status: str, task_id: str) -> str:
        for value in (result.get('summary'), result.get('message'), result.get('output'), result.get('report'), result.get('reply'), result.get('error')):
    ...
    @staticmethod
    def _chat_friendly_summary(value: str, limit: int=_facade().CHAT_REPORT_SUMMARY_CHARS, *, include_detail_note: bool=True) -> str:
    ...
    def _seed_department_groups(self, user_id: int) -> list[dict[str, _facade().Any]]:
        ...
        if not pairs:
            pairs = list(_facade()._FALLBACK_ENTERPRISE_DEPARTMENTS if self._mode == 'enterprise' else _facade()._FALLBACK_DEPARTMENTS)
        ...
        try:
            for emp in self._employee_loader() or [] if self._mode == 'admin' else []:
                ...
                if not dk:
                    continue
                members_by_dept.setdefault(dk, []).append({'employee_id': str(emp.get('employee_id') or ''), 'mod_id': str(emp.get('mod_id') or ''), 'name': str(emp.get('name') or emp.get('employee_id') or '')[:60], 'avatar': str(emp.get('avatar') or ''), 'summary': str(emp.get('summary') or '')[:280]})
        except _facade().RECOVERABLE_ERRORS:
            members_by_dept = {}
        seeded: list[dict[str, _facade().Any]] = []
        for (key, label) in pairs:
            members = _facade()._with_required_group_members(members_by_dept.get(key, []))
            roster_ids = sorted({_facade()._member_employee_id(member) for member in members_by_dept.get(key, []) if _facade()._member_employee_id(member)})
            seeded.append({'id': f'dept:{key}', 'user_id': int(user_id), 'name': label, 'department_key': key, 'members': members, 'members_seeded': bool(roster_ids), 'members_seeded_employee_ids': roster_ids, 'is_pinned': False, 'is_hidden': False, 'is_followed': True, 'unread_count': 0, 'created_at': _facade()._utc_now()})
        for g in seeded:
