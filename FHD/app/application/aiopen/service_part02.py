...
def openclaw_chat_proxy(message: str) -> tuple[dict[str, _facade().Any], int]:
    ...
    req = _facade().urllib.request.Request(target_url, data=payload, headers={'Content-Type': 'application/json'}, method='POST')
    try:
        ...
        return ({'success': False, 'target': target_url, 'status_code': err.code, 'message': body or str(err)}, 502)
