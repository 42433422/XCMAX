...
from app.mod_sdk.workspace import resolve_safe_workspace_relpath  # noqa: F401 - compatibility hook
...
_PERSONNEL_EXPORT_TARGET_RE = re.compile(
    r"员工(?:名单|列表|表格|数据|档案)?|人员(?:名单|列表|表格|数据|档案)?|"
    r"职员(?:名单|列表|表格|数据|档案)?|工号(?:名单|列表|表格|数据)?",
    re.IGNORECASE,
)
_PERSONNEL_EXPORT_VERB_RE = re.compile(
    r"导出|下载|整理成|生成(?:一|个|份|张)?(?:表|名单|列表|文件)", re.IGNORECASE
)
...
_EMPLOYEE_NUMBER_PATTERNS = (
    re.compile(r"(?:工号|员工号)[\s:：#-]*([A-Za-z0-9_-]+)", re.IGNORECASE),
...
def classify_business_chat_intent(message: str) -> BusinessChatIntent | None:
    ...
    # Exporting a personnel roster is a protected data action.  Intercept it
    # before the generic employee-agent planner; otherwise phrases such as
    # “导出全部员工名单” can be misrouted to employee.list and expose internal
    # workflow diagnostics without ever creating a file.
    if (
        has_personnel
        and _PERSONNEL_EXPORT_TARGET_RE.search(text)
        and _PERSONNEL_EXPORT_VERB_RE.search(text)
    ):
        return BusinessChatIntent("personnel_export", "personnel")
    ...
    strong_record_question = (
        any(
            cue in text
            for cue in ("有没有", "是否", "是不是", "几点", "谁出勤", "谁打卡", "多少次", "几次")
        )
        or ("今天" in text and any(term in text for term in record_terms))
        or ("吗" in text and any(term in text for term in record_terms))
    )
...
def try_handle_business_chat_action(
    ...
    *,
    ...
    from app.application.chat_business_safety_actions import (
        _handle_attendance_export,
        _handle_attendance_print,
        _handle_leave_write,
        _handle_personnel_export,
    )
    from app.application.chat_business_safety_read import (
        _handle_attendance_read,
        _handle_personnel_read,
    )
    ...
    if intent.operation == "personnel_export":
        return _handle_personnel_export(message, intent, actor=actor)
