...
def _default_storage_path() -> _facade().Path:
    configured = (
        _facade().os.environ.get("DATASET_RAG_STORE_PATH")
        or _facade().os.environ.get("XCAGI_DATASET_RAG_STORE_PATH")
        or ""
    ).strip()
    if configured:
...
def _build_dataset_vector_index_backend(
    *,
    ...
    storage_path: _facade().Path,
    vector_index_path: str | _facade().Path | None,
) -> _facade().DatasetVectorIndexBackend | None:
    configured = (
        backend_name
        if backend_name is not None
        else _facade().os.environ.get("DATASET_RAG_VECTOR_BACKEND")
        or _facade().os.environ.get("XCAGI_DATASET_RAG_VECTOR_BACKEND")
        or "sqlite"
    )
    ...
    if name in {"sqlite", "sqlite_vector"}:
        path = (
            _facade().Path(vector_index_path).expanduser().resolve()
            if vector_index_path is not None
            else _facade().default_dataset_vector_index_path(storage_path)
        )
        return _facade().DatasetVectorSQLiteIndex(path)
    if name in {"pgvector", "postgres", "postgresql"}:
        database_url = (
            _facade().os.environ.get("DATASET_RAG_PGVECTOR_DATABASE_URL")
            or _facade().os.environ.get("XCAGI_DATASET_RAG_PGVECTOR_DATABASE_URL")
            or _facade().os.environ.get("PGVECTOR_DATABASE_URL")
            or _facade().os.environ.get("DATABASE_URL")
            or ""
        ).strip()
        dimension_raw = (
            _facade().os.environ.get("DATASET_RAG_PGVECTOR_DIMENSION")
            or _facade().os.environ.get("XCAGI_DATASET_RAG_PGVECTOR_DIMENSION")
            or "256"
        )
        try:
...
def _empty_rebuild_queue_summary(
    max_concurrent_jobs: int, *, worker_enabled: bool
...
def _coerce_access_context(
    value: _facade().DatasetAccessContext | dict[str, _facade().Any] | None,
) -> _facade().DatasetAccessContext | None:
    if value is None:
        return None
    ...
    if isinstance(permissions_value, str):
        ...
        permissions = frozenset()
    return _facade().DatasetAccessContext(
        actor_id=str(value.get("actor_id") or value.get("user_id") or ""),
        tenant_id=_facade()._clean_key(str(value.get("tenant_id") or ""), default="")
...
def _has_dataset_permission(
    context: _facade().DatasetAccessContext | None, permission: str
) -> bool:
    if context is None:
        return True
...
def _dataset_permission_denied(
    *,
    ...
    permission: str,
    message: str,
    context: _facade().DatasetAccessContext | None,
) -> dict[str, _facade().Any]:
    return {
        "success": False,
...
def _ensure_dataset_permission(
    context: _facade().DatasetAccessContext | None, permission: str, *, dataset_id: str
) -> dict[str, _facade().Any] | None:
    if _facade()._has_dataset_permission(context, permission):
        return None
    return _facade()._dataset_permission_denied(
        dataset_id=dataset_id,
...
def _ensure_tenant_allowed(
    ...
    *,
    ...
    if (
        context is None
        or context.is_admin
        or _facade().DATASET_ADMIN_PERMISSION in context.permissions
    ):
        return None
    ...
    if not actor_tenant:
        return _facade()._dataset_permission_denied(
            dataset_id=dataset_id,
            permission=_facade().DATASET_READ_PERMISSION,
    ...
    if not target_tenant:
        return _facade()._dataset_permission_denied(
            dataset_id=dataset_id,
            permission=_facade().DATASET_ADMIN_PERMISSION,
...
def _resolve_tenant_for_access(
    access_context: _facade().DatasetAccessContext | dict[str, _facade().Any] | None,
    ...
    *,
    ...
    denied = _facade()._ensure_dataset_permission(
        context, required_permission, dataset_id=dataset_id
    )
    if denied is not None:
        return ("", denied)
    requested = (
        _facade()._clean_key(str(requested_tenant_id or ""), default="")
    ...
    if context is None:
        ...
        return (
            _facade()._clean_key(str(default_without_context), default=default_without_context)
    ...
    if not actor_tenant:
        return (
            "",
            _facade()._dataset_permission_denied(
                dataset_id=dataset_id,
...
def _stable_document_id(
    dataset_id: str, tenant_id: str, source: str, version: int, text: str
) -> str:
    digest = (
        _facade()
        .hashlib.sha256(f"{dataset_id}\x00{tenant_id}\x00{source}\x00{version}\x00{text}".encode())
        .hexdigest()
    )
...
def _document_from_dict(data: dict[str, _facade().Any]) -> _facade().DatasetDocument:
    ...
    return _facade().DatasetDocument(
        document_id=str(data.get("document_id") or ""),
        source=str(data.get("source") or ""),
        parser=str(data.get("parser") or ""),
        text_length=int(data.get("text_length") or 0),
        chunk_count=int(data.get("chunk_count") or 0),
        tenant_id=_facade()._clean_key(
            str(data.get("tenant_id") or metadata.get("tenant_id") or "default"), default="default"
        ),
        version=version,
        version_label=str(
...
def _rebuild_job_from_dict(data: dict[str, _facade().Any]) -> _facade().DatasetRebuildJob:
    ...
    return _facade().DatasetRebuildJob(
...
def _chunk_to_dict(
    chunk: _facade().RetrievedChunk, *, public: bool = False
) -> dict[str, _facade().Any]:
