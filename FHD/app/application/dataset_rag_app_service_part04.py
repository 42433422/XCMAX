...
def _build_knowledge_graph_payload(
    *,
    ...
    ordered_documents = sorted(
        documents, key=lambda item: (item.source.casefold(), -int(item.version), item.document_id)
    )[:40]
    ...
    selected_chunks = _facade()._select_graph_chunks(
        graph_chunks, limit=max(1, limit - len(ordered_documents) - 20)
    )
    root_id = f"persy:{dataset_id}"
    nodes: list[dict[str, _facade().Any]] = [
        {
            "id": root_id,
            "label": "Persy",
            "type": "core",
            "summary": f"{len(ordered_documents)} 个来源，{len(graph_chunks)} 条知识切片"
    ...
    for doc in ordered_documents:
        ...
        nodes.append(
            {
                "id": node_id,
                "label": _facade()._graph_source_label(doc.source),
                "type": "source",
                ...
                "metadata": {
                    ...
                    **_facade()._public_graph_metadata(doc.metadata),
                },
            }
        )
        edges.append(
    ...
    for chunk in selected_chunks:
        ...
        nodes.append(
            {
                "id": node_id,
                "label": _facade()._graph_knowledge_label(chunk.text),
                "type": "knowledge",
                "summary": _facade()._graph_excerpt(chunk.text, 420),
                "source": chunk.source,
    ...
    return {
        ...
        "generated_at": _facade()._utc_now_iso(),
    }


def _select_graph_chunks(
    chunks: list[_facade().RetrievedChunk], *, limit: int
...
def _extract_graph_topics(
    chunk_rows: list[tuple[str, _facade().RetrievedChunk]], *, limit: int
    ...
    return sorted(
        topics.values(), key=lambda item: (-int(item["count"]), str(item["label"]).casefold())
    )[:limit]
...
def _graph_topic_candidates(chunk: _facade().RetrievedChunk) -> list[str]:
    ...
    for match in _facade().re.finditer(
        "(?:^|[，。；：、\\s])([A-Za-z][A-Za-z0-9_.-]{2,30}|[\\u4e00-\\u9fff]{2,16})(?=是|为|包括|支持|需要|必须|负责|属于|采用|禁止|应当|不得)",
        str(chunk.text or ""),
    ):
        values.append(match.group(1))
    english_counts = _facade().Counter(
        token.casefold()
        for token in _facade().re.findall("[A-Za-z][A-Za-z0-9_-]{2,30}", str(chunk.text or ""))
    )
...
def _embedding_metadata(
    embedder: _facade().Callable[[str], list[float]] | None, text: str
) -> dict[str, _facade().Any]:
    if embedder is None:
        return {}
    try:
...
def _filter_chunks(
    ...
    *,
    ...
    if metadata_filter:
        selected = [
            chunk for chunk in selected if _facade()._metadata_matches(chunk, metadata_filter)
        ]
    ...
    if version_text.lower() == "latest":
        ...
        for chunk in selected:
            ...
            latest_by_scope[scope] = max(
                latest_by_scope.get(scope, 0), int(metadata.get("document_version") or 1)
            )
        return [
            chunk
            for chunk in selected
...
def _metadata_matches(
    chunk: _facade().RetrievedChunk, metadata_filter: dict[str, _facade().Any]
) -> bool:
    metadata = dict(chunk.metadata or {})
...
def _rerank_chunks(
    query: str, chunks: list[_facade().RetrievedChunk], *, top_k: int
) -> list[_facade().RetrievedChunk]:
