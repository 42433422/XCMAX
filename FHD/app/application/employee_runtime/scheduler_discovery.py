...
def _configured_jobs() -> dict[str, EmployeeCronJob]:
    ...
    auto_enabled = automation_enabled and _facade()._truthy(os.environ.get('MODSTORE_EMPLOYEE_AUTO_CRON_ENABLED'), default=True)
    daily_enabled = automation_enabled and _facade()._truthy(os.environ.get('MODSTORE_DAILY_ORCHESTRATOR_ENABLED'), default=True)
    ...
    task = str(os.environ.get('MODSTORE_DAILY_ORCHESTRATOR_TASK') or '每日定时：在独立分支上做最小修复，并把写操作提交到审批队列。')
    job = _facade().EmployeeCronJob(job_id='daily-orchestrator', employee_id='daily-orchestrator', task=task, schedule='daily', hour=hour, minute=minute, timezone=str(tz.key), enabled=auto_enabled and daily_enabled, max_retries=max_retries, source='env')
    if job.enabled:
...
def _job_from_manifest(employee_id: str, manifest: dict[str, Any], tz: ZoneInfo) -> EmployeeCronJob | None:
    ...
    depends_on = [str(x).strip() for x in depends_on_raw if str(x).strip()] if isinstance(depends_on_raw, list) else []
    now = datetime.now(tz)
    job = _facade().EmployeeCronJob(job_id=employee_id, employee_id=employee_id, task=task, schedule='daily', hour=hour, minute=minute, timezone=str(tz.key), enabled=True, max_retries=max_retries, source='manifest', depends_on=depends_on)
    job.next_run_at = _facade()._next_daily_run(now, hour, minute)
...
def _discover_manifest_jobs() -> dict[str, EmployeeCronJob]:
    ...
    for child in root.iterdir():
        ...
        if employee_id == 'daily-orchestrator':
            continue
        per_emp_flag = os.environ.get(f"MODSTORE_EMPLOYEE_CRON_{employee_id.upper().replace('-', '_')}_ENABLED")
