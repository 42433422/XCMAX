...
def _execute_job_task(job: EmployeeCronJob, *, task: str | None, input_data: dict[str, Any] | None, user_id: int, workspace_root: str | None, session_id: str | None, source: str) -> tuple[bool, dict[str, Any], str]:
    ...
    try:
        ...
        result = execute_employee_task_local(job.employee_id, task or job.task, payload, user_id=user_id, workspace_root=workspace_root, session_id=session_id)
...
def _apply_job_outcome(job: EmployeeCronJob, *, ok: bool, error: str, duration_ms: float, finished: datetime) -> None:
    ...
    if ok:
        ...
        if _facade()._is_non_retryable_configuration_error(job.last_error):
            ...
            logger.info('employee cron job retry scheduled job_id=%s retry=%d/%d backoff=%.0fs', job.job_id, job.retry_count, job.max_retries, backoff)
        else:
...
def run_employee_cron_job(job_id: str, *, task: str | None=None, input_data: dict[str, Any] | None=None, user_id: int=0, workspace_root: str | None=None, session_id: str | None=None, source: str='manual') -> dict[str, Any]:
    ...
    with _facade()._lock:
        ...
        if job.running:
            return {'success': False, 'error': f'employee cron job already running: {jid}', 'job': job.to_dict()}
        job.running = True
    ...
    (ok, result, error) = _facade()._execute_job_task(job, task=task, input_data=input_data, user_id=user_id, workspace_root=workspace_root, session_id=session_id, source=source)
    finished = datetime.now(UTC)
    duration_ms = round((finished - started).total_seconds() * 1000, 1)
    with _facade()._lock:
        job = _facade()._jobs[jid]
        _facade()._apply_job_outcome(job, ok=ok, error=error, duration_ms=duration_ms, finished=finished)
