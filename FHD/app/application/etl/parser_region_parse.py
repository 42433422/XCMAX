...
def parse_customer_product_regions(
    path: _facade().Path, *, max_rows: int, target_type: str = "customer_products"
) -> _facade().ParsedDataset | None:
    ...
    try:
        for worksheet in workbook.worksheets:
            recent: _facade().deque[tuple[int, tuple[_facade().Any, ...]]] = _facade().deque(
                maxlen=5
            )
            ...
            for row_number, raw_values in enumerate(worksheet.iter_rows(values_only=True), start=1):
                ...
                if candidate is not None:
                    ...
                    probe = {
                        ...
                        "context_rows": [
                            {
                                "row": number,
                                "text": _facade()._joined_row(
                                    row, max_col=int(candidate["last_col"])
                                ),
                            }
                            for (number, row) in context_rows[-5:]
                            if _facade()._joined_row(row, max_col=int(candidate["last_col"]))
                        ],
                    ...
                    if role == "delivery_note" and meta.get("customer_name"):
                        active = {
                            ...
                            "source_by_col": _facade()._unique_source_headers(
                ...
                if active is not None:
                    ...
                    name = _facade().clean_cell_text(
                        _facade()._value_at(values, mapping.get("name"))
                    )
                    model = _facade().clean_cell_text(
                        _facade()._value_at(values, mapping.get("model_number"))
                    )
                    if (name or model) and _facade()._has_measure(values, mapping):
                        ...
                        if _facade()._NON_PRODUCT_RE.fullmatch(business_name):
                            ...
                            projected_input: dict[str, _facade().Any] = {
                            ...
                            source_values = _facade().project_delivery_region(
                                projected_input, target_type=target_type, meta=active["meta"]
                            )
                            ...
                            parsed_rows.append(
                                _facade().ParsedRow(
                                    ...
                                    provenance={
                                        ...
                                        "source_date": _facade()._normalized_order_date(
        ...
        sheet_domain_hints = {
            worksheet.title: _facade()._sheet_domain_hint(worksheet)
            for worksheet in workbook.worksheets
        }
        ...
        for worksheet in workbook.worksheets:
            ...
            if remaining <= 0:
                break
            companion_rows = _facade().parse_shipment_history_rows(
                worksheet, canonical_by_alias=canonical_by_alias, max_rows=remaining
            )
            if not companion_rows:
                companion_rows = _facade().parse_structured_shipment_history_rows(
                    worksheet, canonical_by_alias=canonical_by_alias, max_rows=remaining
                )
            if not companion_rows:
                companion_rows = _facade().parse_quote_rows(
                    worksheet, canonical_by_alias=canonical_by_alias, max_rows=remaining
                )
            if companion_rows:
                    ...
                    future_dated_source_rows.append(
                        {
                            "sheet": row.sheet,
                            "row": row.row_number,
                            "source_date": _facade()._companion_source_date(row),
                        }
                    )
                    continue
                key = _facade().product_match_key(row.values)
    ...
    if not parsed_rows:
        return None
    if target_type == "customer_products":
        identity_issues = _facade().source_model_ambiguity_issues(
            [row.values for row in parsed_rows], unit_field="customer_name"
        )
        if identity_issues:
            ...
            for index, issues in identity_issues.items():
                ...
                ambiguous_keys.add(
                    _facade().product_name_key(row.values, unit_field="customer_name")
                )
    ...
    warnings: list[dict[str, _facade().Any]] = [
        {
            "code": "ETL_MULTI_REGION_WORKBOOK_PLANNED",
            "message": f"已从混合工作簿识别 {len(selected)} 个{target_label}业务区块，排除 {len(excluded)} 个其他业务区块。",
        ...
        warnings.append(
            {
                ...
                "message": f"已隔离 {len(future_dated_source_rows)} 条日期晚于当前日期的历史/报价记录，它们不会被当作最新客户产品事实。",
        ...
        warnings.append(
            {
                ...
                "message": f"发现 {same_date_source_conflicts} 组同日跨表产品事实冲突，已保留为错误行，需人工确认。",
        ...
        warnings.append(
            {
                ...
                "message": f"已从出货历史增补 {history_product_count} 个客户产品候选，仅用于客户产品预演，不会作为新的发货记录写入。",
        ...
        warnings.append(
            {
                ...
                "message": f"发现 {model_identity_ambiguity_count} 组同客户同产品同时有型号和无型号的数据，已标为错误，需补全型号或拆分后重新预演。",
        ...
        warnings.append(
            {
                ...
                "message": f"同一客户同一产品存在 {companion_stale_records_skipped} 条较早或同日旧记录，已按来源日期优先保留最新有效数据。",
        ...
        warnings.append(
            {
                ...
                "message": f"已在附表发现 {companion_candidate_count} 个客户产品候选。它们不会写入当前发货记录预演；可新建客户及产品预演后确认。",
                "count": companion_candidate_count,
            }
        )
    sheet_plan = _facade()._build_sheet_plan(
    ...
    return _facade().ParsedDataset(
        headers=all_headers,
        rows=parsed_rows,
        source_features={
            "structure_detection": "deterministic_regions_v1",
            **_facade().region_source_features(
                target_type=target_type, regions=regions, rows=len(parsed_rows)
            ),
            "regions": regions,
            ...
            "headers": all_headers,
            "llm_structure": {**llm.public_metadata(), "suggestion_count": len(llm_by_region)},
        },
        warnings=warnings,
    )
