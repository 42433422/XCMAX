...
class ExecutionServiceMixin:
    def execute(
        ...
        *,
        ...
        document_understanding = source_features.get("document_understanding")
        if (
            isinstance(document_understanding, dict)
            and document_understanding.get("requires_confirmation")
            and not draft.get("document_confirmed")
        ):
            raise EtlConflict(
                "ETL_DOCUMENT_CONFIRMATION_REQUIRED",
                "请先核对单据类型、单据头和明细范围，再确认执行",
            )
