...
class MaterialApplicationService:
    ...
    def create_material(self, data: dict[str, Any] | None = None, **kwargs) -> dict[str, Any]:
        if data is None:
            data = kwargs
        else:
            data = {**data, **kwargs}
        ...
        if not material_name:
            return {"success": False, "message": "原材料名称不能为空"}
        ...
        if result.get("success"):
            self._log_action(
                "create_material", material_id=result.get("data", {}).get("id"), data=data
            )

        return result

    def update_material(
        self, material_id: int, data: dict[str, Any] | None = None, **kwargs
    ) -> dict[str, Any]:
    ...
    def get_material_statistics(self, category: str | None = None) -> dict[str, Any]:
        result = self._repository.find_all(category=category, page=1, per_page=1)
    ...
    def export_to_excel(
        ...
        return self._repository.export_to_excel(
            search=search, category=category, template_id=template_id
        )
...
instrument_application_service_class(MaterialApplicationService)

_material_app_service: MaterialApplicationService | None = None
...
def init_material_application_service(
    ...
    """初始化原材料应用服务 (用于依赖注入)"""
    global _material_app_service
    _material_app_service = MaterialApplicationService(repository=repository)
    return _material_app_service
