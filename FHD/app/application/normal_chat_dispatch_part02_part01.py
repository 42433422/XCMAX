...
def build_inventory_alert_response_dict(
...
def build_inventory_count_response_dict(
    route_result: dict[str, _facade().Any],
) -> dict[str, _facade().Any] | None:
    ...
    return {
        "success": True,
        "response": "库存盘点需先确认：请提供产品（型号/名称）、仓库及实盘数量，例如「盘点 A001 主仓 实盘 120」。系统会核对账面数量并显示差异，确认后再执行调整。",
        "data": {
...
def build_mrp_production_response_dict(
    route_result: dict[str, _facade().Any],
) -> dict[str, _facade().Any] | None:
    ...
    try:
        ...
        if not orders:
            ...
            lines = [
                f"- {o.get('order_no', '')} {o.get('product_name', '')} ×{o.get('quantity', 0)}（{o.get('status', '')}）"
                for o in orders[:10]
            ]
            msg = f"共 {total} 条生产工单：\n" + "\n".join(lines)
            if total > 10:
...
def build_aging_report_response_dict(
    route_result: dict[str, _facade().Any],
) -> dict[str, _facade().Any] | None:
    ...
    if not party_id:
        return {
            "success": True,
            "response": f"账龄分析（{account_type}）需要指定客户/供应商。请提供客户或供应商名称/ID，例如「查看 XX 客户的应收账龄」，我会按账期分组汇总未结余额。",
    ...
    try:
        ...
        lines = [
            f"- {b.get('bucket', '')}：￥{_facade().format_money(_facade().safe_float(b.get('amount')))}"
            for b in buckets
        ]
        msg = (
...
def build_label_print_response_dict(
    route_result: dict[str, _facade().Any],
) -> dict[str, _facade().Any] | None:
    """标签打印槽位响应。"""
    ...
    try:
        ...
        result = get_print_application_service().print_single_label(
            product_name=model_number, model_number=model_number, quantity=quantity
        )
...
def build_materials_query_response_dict(
    route_result: dict[str, _facade().Any],
) -> dict[str, _facade().Any] | None:
    ...
    try:
        ...
        result = get_material_application_service().get_all_materials(
            search=keyword or None, category=None, page=1, per_page=20
        )
        if isinstance(result, dict) and result.get("success") is False:
            return {
                "success": False,
        ...
        if not items:
            ...
            lines = [
                f"- {m.get('name', '')} 库存 {m.get('quantity', 0)} {m.get('unit', '')}（{m.get('material_code', '')}）"
                for m in items[:10]
            ]
...
def build_purchase_query_response_dict(
    ...
    try:
        ...
        if "供应商" in text or "供应商" in str(
            ...
            keyword = (
                _facade().re.sub("(?:供应商|进货|采购|有哪些|哪些|一下|查询|查)", "", text).strip()
            )
        ...
        if not orders:
            ...
            lines = [
                f"- {o.get('order_no', '')} {o.get('supplier_name', '')} ￥{_facade().format_money(_facade().safe_float(o.get('total_amount')))}"
                for o in orders[:10]
            ]
            msg = f"共 {total} 条采购订单：\n" + "\n".join(lines)
            if total > 10:
