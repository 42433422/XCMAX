...
金额比较全部使用 ``Decimal`` 避免浮点误差；凭证与分配在调用方事务内执笔，
失败时显式回滚（与 W1-04 ``create_journal_entry(db=...)`` 一致）。
"""
...
def payment(
    *,
    ...
    with get_db() as db:
        order = db.query(SalesOrder).filter(SalesOrder.id == int(sales_order_id)).first()
        if order is None:
            return {"success": False, "message": f"销售订单不存在: sales_order_id={sales_order_id}"}

        receivable = _to_decimal(order.total_amount)
        existing = _existing_allocations(db, sales_order_id)
        ...
        entry_result = create_journal_entry(
            ...
            db=db,  # 调用方事务内执笔（W1-04 一致）：凭证与分配同事务提交/回滚
        )
    ...
    *,
    ...
    with get_db() as db:
        ...
        if alloc.status == RECEIVABLE_STATUS_REFUNDED:
            return {"success": False, "message": "该收款分配已退款/冲销，不能重复冲销"}
        ...
        entry_result = create_journal_entry(
            ...
            db=db,  # 调用方事务内执笔（W1-04 一致）：反向凭证与分配更新同事务提交/回滚
        )
        if not entry_result.get("success"):
            db.rollback()
            return entry_result
