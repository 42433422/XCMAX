...
class _SalesAppServicePart02Mixin:
    ...
    def _closed_loop_resolve_customer(
        self, db, order: dict[str, _facade().Any], tenant_id: int
    ) -> dict[str, _facade().Any]:
        cid = order.get("customer_id")
        if cid is not None:
            cid_int = self._closed_loop_coerce_id(cid, "resolve_customer", "customer_id")
            customer = (
                db.query(_facade().Customer)
                .filter(_facade().Customer.id == cid_int, _facade().Customer.tenant_id == tenant_id)
                .first()
            )
            if customer is None:
        ...
        matches = (
            db.query(_facade().Customer)
            .filter(
                _facade().Customer.customer_name == cname, _facade().Customer.tenant_id == tenant_id
            )
            .count()
        )
        if matches == 1:
            customer = (
                db.query(_facade().Customer)
                .filter(
                    _facade().Customer.customer_name == cname,
                    _facade().Customer.tenant_id == tenant_id,
                )
                .first()
            )
            return {"id": customer.id, "name": customer.customer_name}
        ...
        if len(purchase_units) != 1:
            raise _facade().ClosedLoopExecutionError(
                "resolve_customer", f"采购单位匹配数为 {len(purchase_units)}（应为恰好 1）: {cname}"
            )
        purchase_unit = purchase_units[0]
        customer = _facade().Customer(
        ...
        for idx, item in enumerate(order["items"]):
            ...
            if pid is not None:
                ...
                product = (
                    ...
                    .filter(
                        _facade().Product.id == pid_int, _facade().Product.tenant_id == tenant_id
                    )
                    .first()
                )
                if product is None:
                ...
                matches = (
                    db.query(_facade().Product)
                    .filter(
                        _facade().Product.name == pname, _facade().Product.tenant_id == tenant_id
                    )
                    .count()
                )
                if matches != 1:
                    raise _facade().ClosedLoopExecutionError(
                        "resolve_product", f"产品名匹配数为 {matches}（应为恰好 1）: {pname}"
                    )
                product = (
                    db.query(_facade().Product)
                    .filter(
            ...
            if product.unit and product.unit != unit:
                raise _facade().ClosedLoopExecutionError(
                    "resolve_product", f"产品单位不匹配: 产品={product.unit} vs 载荷={unit}"
                )
            resolved.append(
                {
                    "product_id": product.id,
                    "unit": unit,
                    "quantity": _facade().Decimal(str(item["quantity"])),
                    "unit_price": _facade().Decimal(str(item["unit_price"])),
    ...
    def _closed_loop_resolve_warehouse(
        self, db, fulfillment: dict[str, _facade().Any], tenant_id: int
    ) -> dict[str, _facade().Any]:
        ...
        if wid is not None:
            ...
            wh = (
                ...
                .filter(
                    _facade().Warehouse.id == wid_int, _facade().Warehouse.tenant_id == tenant_id
                )
                .first()
            )
            if wh is None:
        ...
        wh = (
            db.query(_facade().Warehouse)
            .filter(
                _facade().Warehouse.status == "active", _facade().Warehouse.tenant_id == tenant_id
            )
            .order_by(_facade().Warehouse.id.asc())
            .first()
        )
        if wh is None:
    ...
    def _closed_loop_resolve_inventory_source(
        self, db, *, product_id: int, warehouse_id: int, quantity: _facade().Decimal, tenant_id: int
    ) -> dict[str, _facade().Any]:
        ...
        ledgers = (
            ...
            .filter(
                ...
                _facade().InventoryLedger.tenant_id == tenant_id,
                _facade().InventoryLedger.available_quantity >= quantity,
            )
            .order_by(_facade().InventoryLedger.id.asc())
            .limit(2)
            .all()
        )
        if len(ledgers) != 1:
