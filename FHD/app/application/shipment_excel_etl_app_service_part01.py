...
def _resolve_profile(
    profile: _facade().ShipmentEtlProfile | None = None, profile_id: str | None = None
) -> _facade().ShipmentEtlProfile:
...
def _profiles_for_parse(
    profile: _facade().ShipmentEtlProfile | None = None, profile_id: str | None = None
) -> list[_facade().ShipmentEtlProfile]:
...
def _pick_best_profile_for_sheet(
    ws, profiles: list[_facade().ShipmentEtlProfile]
) -> tuple[_facade().ShipmentEtlProfile, int, int, str]:
...
def _kb_resolve_layout(ws) -> tuple[int | None, dict[str, int], str]:
    ...
    for row in range(1, min(20, max_row) + 1):
        ...
        if len(headers) < 2:
            continue
        fp = _facade().sheet_layout_fingerprint(
            sheet_title=str(ws.title or ""), header_cells=headers
        )
...
def _remember_sheet_layout(
    ...
    *,
    ...
    try:
        _facade().get_excel_etl_kb().remember(
            _facade().TemplateMemory(
                fingerprint=fp,
...
def _infer_columns_from_samples(ws, header_row: int, mapping: dict[str, int]) -> dict[str, int]:
    ...
    for col in range(1, max_col + 1):
        ...
        if nums >= max(1, len(samples) // 2 + 1):
            kind = "number"
        elif _facade().re.search("[A-Za-z0-9\\-_/]{2,}", joined) and (
            not _facade().re.search("[\\u4e00-\\u9fff]{2,}", joined)
        ):
            kind = "code"
    ...
    _take(
        "order_number",
        lambda c, s, k: any(_facade().re.search("[A-Za-z].*\\d|\\d.*[A-Za-z]", x) for x in s),
    )
    return out


def _classify_sheet_role(
    ws, profile: _facade().ShipmentEtlProfile, *, d_score: int, l_score: int
) -> str:
...
def _parse_buyer_meta(ws, header_row: int, profile: _facade().ShipmentEtlProfile) -> dict[str, str]:
    ...
    if meta["unit_name"]:
        ...
        meta["unit_name"] = (
            _facade()
            .re.split(
                "(?i)\\s{2,}|\\s+(?:Incoterms|Payment|Tel|Phone|地址|电话)\\b",
                meta["unit_name"],
                maxsplit=1,
            )[0]
            .strip()
    ...
    if meta["unit_name"] and (
        _facade()._unit_name_looks_truncated(meta["unit_name"])
        or unit_name_is_weak(meta["unit_name"])
    ):
        meta["unit_name"] = ""
    return meta
