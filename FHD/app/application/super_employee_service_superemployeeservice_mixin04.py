...
class _SuperEmployeeServicePart04Mixin:
    ...
    def _compose_direct_chat_reply(
        self, text: str, context: dict[str, _facade().Any]
    ) -> tuple[str, str]:
        ...
        return (
            f"{self._p.display_tool} CLI 暂时没有返回内容，请确认本机 {self._p.display_tool} 已登录后重试。",
    ...
    def _upsert_direct_reply_messages(
        ...
        request_ids_with_reply = {
            ...
            and (
                ...
                or (
                    ...
                    and str(item.get("kind") or "") != _facade().DISPATCHER_MESSAGE_KIND
                )
            )
        }
        changed = False
    ...
    def _refresh_dispatcher_row(
        self, row: dict[str, _facade().Any], task: dict[str, _facade().Any]
    ) -> bool:
    ...
    def _upsert_result_messages(
        ...
        *,
        user_id: int,
        dispatch_row: dict[str, _facade().Any],
        task: dict[str, _facade().Any],
        ...
        for subtask in self._task_subtasks(task):
            ...
            rows.append(
                self._message_row(
                    ...
                    role="assistant",
                    body=body,
                    created_at=str(subtask.get("completed_at") or _facade()._utc_now()),
    ...
    def _task_subtasks(self, task: dict[str, _facade().Any]) -> list[dict[str, _facade().Any]]:
        subtasks = _facade()._coerce_list(task.get("subTasks")) or _facade()._coerce_list(
            task.get("subtasks")
        )
        return [item for item in subtasks if isinstance(item, dict)]

    def _result_body(
        self, task: dict[str, _facade().Any], subtask: dict[str, _facade().Any]
    ) -> str:
        logs = [
            str(log.get("content") or "").strip()
            for log in _facade()._coerce_list(subtask.get("logs"))
