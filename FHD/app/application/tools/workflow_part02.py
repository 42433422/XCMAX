...
def _resolve_new_tool_dispatch(name: str) -> _facade().Any:
    ...
    if _facade()._NEW_TOOL_DISPATCH_CACHE is None:
        try:
            from app.application.tools.customer_crud_tools import delete_customer, list_customers, update_customer
            from app.application.tools.rbac_tools import assign_role, create_role, delete_role, list_roles, update_role
            from app.application.tools.report_config_tools import configure_report, list_report_configs
            from app.application.tools.shipment_crud_tools import delete_order, list_orders, update_order
            _facade()._NEW_TOOL_DISPATCH_CACHE = {'delete_order': delete_order, 'update_order': update_order, 'list_orders': list_orders, 'update_customer': update_customer, 'delete_customer': delete_customer, 'list_customers': list_customers, 'configure_report': configure_report, 'list_report_configs': list_report_configs, 'create_role': create_role, 'update_role': update_role, 'delete_role': delete_role, 'assign_role': assign_role, 'list_roles': list_roles}
        except _facade().RECOVERABLE_ERRORS:
            _facade().logger.debug('new tool dispatch init failed', exc_info=True)
            _facade()._NEW_TOOL_DISPATCH_CACHE = {}
    return _facade()._NEW_TOOL_DISPATCH_CACHE.get(name) if _facade()._NEW_TOOL_DISPATCH_CACHE else None

def _handle_import_excel_to_database(args: dict[str, _facade().Any], workspace_root: str | None=None, db_write_token: str | None=None) -> str:
    """处理 Excel 导入数据库请求。"""
    _ = db_write_token
    try:
        ...
        if required_token and workspace_root is None:
            if not provided_token:
                return _facade().json.dumps({'success': False, 'requires_token': True, 'token_name': 'DB_WRITE_TOKEN'}, ensure_ascii=False)
            if provided_token != required_token:
                return _facade().json.dumps({'success': False, 'error': 'invalid_token'}, ensure_ascii=False)
        p = _facade().resolve_safe_excel_path(workspace_root or str(_facade().Path.cwd()), file_path)
        if not p.exists():
            return _facade().json.dumps({'success': False, 'error': 'file not found'}, ensure_ascii=False)
        ...
        if not unit_name:
            ...
            if not unit_name:
                if excel_analysis_ctx:
                    unit_name = str(excel_analysis_ctx.get('customer_hint') or (excel_analysis_ctx.get('preview_data') or {}).get('customer_hint') or '').strip()
            if not unit_name:
                try:
            ...
            if not unit_name:
                try:
                    ...
                    unit_name = str(_extract_customer_hint_from_excel(str(p), sheet_n if sheet_n else None) or '').strip()
                except _facade().RECOVERABLE_ERRORS:
                    _facade().logger.debug('suppressed exception', exc_info=True)
        ...
        if header_1b is None and excel_analysis_ctx:
            try:
                ...
                header_1b = detected_excel_header_row_1based(excel_analysis_ctx, preferred_sheet_name=str(sheet_n or '').strip() or None)
            except _facade().RECOVERABLE_ERRORS:
                header_1b = None
        try:
            df = _facade()._read_excel_dataframe(p, sheet_name=sheet_n, header_row_1based=header_1b)
        except _facade().RECOVERABLE_ERRORS as e:
            return _facade().json.dumps({'success': False, 'error': f'read_excel_failed: {e}', 'sheet_name': sheet_n, 'header_row': header_1b}, ensure_ascii=False)
        if df.empty:
            return _facade().json.dumps({'success': False, 'error': 'Excel file is empty'}, ensure_ascii=False)
        ...
        if last_data_i is not None and last_data_i >= 1:
            ...
            if n_keep < 1:
                return _facade().json.dumps({'success': False, 'error': 'invalid_last_data_row', 'message': 'last_data_row_1based 必须大于 header_row（或表头在第 1 行时大于 1）', 'header_row': hdr_eff, 'last_data_row_1based': last_data_i}, ensure_ascii=False)
            df = df.iloc[:n_keep]
        columns = list(df.columns.astype(str))
        row_count = len(df)
        read_meta = {'sheet_name': sheet_n, 'header_row': header_1b, 'last_data_row_applied': last_data_i}
        if import_type == 'products':
            return _facade().cast('str', _facade()._import_products_preview_or_execute(df, columns, unit_name, confirm, row_count, read_meta=read_meta, price_column_hint=price_column_hint))
        elif import_type == 'customers':
            return _facade().cast('str', _facade()._import_customers_preview_or_execute(df, columns, confirm, row_count))
        elif import_type == 'orders':
            return _facade().cast('str', _facade()._import_orders_preview_or_execute(df, columns, unit_name, confirm, row_count))
        else:
            return _facade().json.dumps({'success': True, 'preview': True, 'import_type': import_type, 'columns': columns, 'row_count': row_count, 'sample_data': _facade().json.loads(df.head(5).replace({float('nan'): None}).to_json(orient='records')), 'message': '未实现该类型的自动导入，请先导出为模板格式再导入。'}, ensure_ascii=False)
    except _facade().RECOVERABLE_ERRORS as e:
        return _facade().json.dumps({'success': False, 'error': str(e)}, ensure_ascii=False)

def _infer_product_field_mapping(columns: list[str], *, price_column_hint: str | None=None) -> dict[str, str]:
    """按列名推断产品字段映射。"""
    ...
    if 'price' not in mapping:
        price_order = [('调价前', 'price'), ('调价后', 'price'), ('现价', 'price'), ('单价', 'price'), ('价格', 'price'), ('price', 'price')]
