...
def _import_products_preview_or_execute(df, columns, unit_name, confirm, row_count, *, read_meta: dict[str, _facade().Any] | None=None, price_column_hint: str | None=None):
    field_mapping = _facade()._infer_product_field_mapping(columns, price_column_hint=price_column_hint)
    ...
    for (_, row) in df.iterrows():
        ...
        model_val = _facade()._excel_cell_as_clean_str(row.get(field_mapping['model_number'], '')) if 'model_number' in field_mapping else ''
        ...
        try:
            ...
            qty_i = 0
        record = {'model_number': model_val or None if 'model_number' in field_mapping else None, 'name': name_val or None if name_col else None, 'specification': spec_val or None, 'price': _facade()._excel_cell_as_float(row.get(field_mapping.get('price', ''), 0), 0.0) if 'price' in field_mapping else 0.0, 'unit': detected_unit or unit_name or '件', 'quantity': qty_i if 'quantity' in field_mapping else 0, 'description': _facade()._excel_cell_as_clean_str(row.get(field_mapping['description'], '')) or None if 'description' in field_mapping else None, 'brand': _facade()._excel_cell_as_clean_str(row.get(field_mapping['brand'], '')) or None if 'brand' in field_mapping else None, 'category': _facade()._excel_cell_as_clean_str(row.get(field_mapping['category'], '')) or None if 'category' in field_mapping else None}
        if record['name']:
            records.append(record)
    if not confirm:
        payload = {'success': True, 'preview': True, 'import_type': 'products', 'detected_unit': detected_unit, 'field_mapping': field_mapping, 'row_count': len(records), 'skipped_clause_like_rows': skipped_clause_like, 'sample_data': records[:5], 'message': f"检测到 {len(records)} 条产品记录，绑定客户: {detected_unit or unit_name or '未指定'}。" + (f' 已跳过疑似表尾条款行 {skipped_clause_like} 条。' if skipped_clause_like else '') + '当前为预览模式，传 confirm=true 或去掉 preview_only 可直接导入。'}
        if read_meta:
    ...
    try:
        ...
        if detected_unit or unit_name:
            ...
            if not find_purchase_unit(unit_name=target_unit):
                ...
                customer_service.create({'customer_name': target_unit, 'contact_person': None, 'contact_phone': None, 'contact_address': None})
        ...
        msg = str(result.get('message') if isinstance(result, dict) and result.get('message') else f'成功导入 {imported} 条产品')
        return _facade().json.dumps({'success': True, 'preview': False, 'imported': imported, 'failed': failed, 'skipped_clause_like_rows': skipped_clause_like, 'message': msg + (f'（另跳过疑似条款/表尾行 {skipped_clause_like} 条）' if skipped_clause_like else '')}, ensure_ascii=False)
    except _facade().RECOVERABLE_ERRORS as e:
        return _facade().json.dumps({'success': False, 'error': f'导入失败: {str(e)}'}, ensure_ascii=False)
...
def _import_customers_preview_or_execute(df, columns, confirm, row_count):
    ...
    for (_, row) in df.iterrows():
        ...
        if record.get('customer_name'):
            records.append(record)
    if not confirm:
        return _facade().json.dumps({'success': True, 'preview': True, 'import_type': 'customers', 'row_count': len(records), 'sample_data': records[:5], 'message': f'检测到 {len(records)} 条客户记录。当前为预览模式，传 confirm=true 或去掉 preview_only 可直接导入。'}, ensure_ascii=False)
    try:
        ...
        for record in records:
            ...
            if result.get('success'):
                ...
                failed += 1
        return _facade().json.dumps({'success': True, 'preview': False, 'imported': imported, 'failed': failed, 'message': f'成功导入 {imported} 条客户，失败 {failed} 条'}, ensure_ascii=False)
    except _facade().RECOVERABLE_ERRORS as e:
        return _facade().json.dumps({'success': False, 'error': f'导入失败: {str(e)}'}, ensure_ascii=False)

def _import_orders_preview_or_execute(df, columns, unit_name, confirm, row_count):
    """从 Excel 导入出货记录（订单）。"""
    sample_data = _facade().json.loads(df.head(5).replace({float('nan'): None}).to_json(orient='records'))
    ...
    if not confirm:
        return _facade().json.dumps({'success': True, 'preview': True, 'import_type': 'orders', 'columns': columns, 'column_mapping': col_map, 'row_count': row_count, 'sample_data': sample_data, 'message': f'检测到 {row_count} 条出货记录，确认导入请设置 confirm=true。'}, ensure_ascii=False)
    try:
        ...
        for (_, row) in df.iterrows():
            try:
                ...
                failed += 1
        return _facade().json.dumps({'success': True, 'preview': False, 'imported': imported, 'failed': failed, 'message': f'成功导入 {imported} 条出货记录，失败 {failed} 条'}, ensure_ascii=False)
    except _facade().RECOVERABLE_ERRORS as e:
        return _facade().json.dumps({'success': False, 'error': f'订单导入失败: {str(e)}'}, ensure_ascii=False)
