...
class _WorkflowEnginePart02Mixin:
    ...
    def _append_node_trace(
        runtime_context: dict[str, _facade().Any], result: _facade().NodeExecutionResult
    ) -> None:
        ...
        while retries <= effective_max_retries:
            ...
            try:
                ...
                output = self._dispatch(
                    tool_id=node.tool_id, action=node.action, params=merged_params
                )
                ...
                _facade().logger.warning(
                    "执行节点失败 node=%s err=%s", node.node_id, err, exc_info=True
                )
            retries += 1
        finished_at = _facade().datetime.now(_facade().UTC).isoformat()
        return _facade().NodeExecutionResult(
    ...
    def _run_clarify_node(
        self, node: _facade().WorkflowNode, runtime_context: dict[str, _facade().Any]
    ) -> _facade().NodeExecutionResult:
        """反问澄清节点执行：不调用业务工具。
