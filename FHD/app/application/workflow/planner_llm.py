...
class LLMWorkflowPlanner:
    def __init__(self) -> None:
        from app.application.workflow.planner import get_ai_conversation_service
    ...
    def plan(
        ...
        registry_for_plan = planner_mod._filter_tool_registry_for_profile(tool_registry, profile)
    ...
    def _critic_repair_with_llm(
        ...
        try:
            ...
            response = planner_mod._get_planner_http_client().post(
                api_url,
    ...
    def _fallback_plan(
        ...
        if (
            not nodes
            and planner_mod._looks_like_business_db_write(message, lower)
            and "business_db" in tool_registry
        ):
            node = planner_mod._extract_business_db_write_node(message)
            if node is not None:
        ...
        if not nodes and (
            ...
            if "business_db" in tool_registry:
                ...
                entity = planner_mod._infer_business_db_entity(message)
                keyword = planner_mod._extract_business_db_read_keyword(message, entity)
                nodes.append(
                    WorkflowNode(
