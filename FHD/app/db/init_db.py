...
def init_wechat_tasks_table(db_path: str | None = None) -> None:
    ...
    桌面 SQLite 不执行完整 Alembic 链；三张表必须作为一个外键集合补齐，
    否则联系人只读接口会在首次安装的空库上直接 500。
    """
    ...
    with sqlite_conn(db_path) as conn:
        ...
        cur.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_wechat_contacts_active_starred
            ON wechat_contacts (is_active, is_starred)
            """
        )

        cur.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_wechat_contact_context_contact
