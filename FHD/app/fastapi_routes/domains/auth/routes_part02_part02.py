...
async def auth_qr_status(qr_id: str = _facade().Query(""), poll_secret: str = _facade().Query("")):
    ...
    if not rec:
        return _facade().JSONResponse(
            _facade().error_envelope(_facade().QR_NOT_FOUND, "二维码无效"), status_code=404
        )
...
def auth_profile_patch(
    ...
    if not kwargs:
        return _facade().JSONResponse(
            _facade().error_envelope(_facade().INVALID_INPUT, "无有效字段"), status_code=400
        )
    result = get_user_app_service().update_user(user.id, **kwargs)
    if not result.get("success"):
        return _facade().JSONResponse(
            _facade().error_envelope(_facade().UPDATE_FAILED, result.get("message", "更新失败")),
            status_code=400,
        )
    ...
    with get_db() as db:
        ...
        if row is None:
            return _facade().JSONResponse(
                _facade().error_envelope(_facade().NOT_FOUND, "用户不存在"), status_code=404
            )
...
async def auth_profile_avatar_upload(
    ...
    if file is None or not file.filename:
        return _facade().JSONResponse(
            _facade().error_envelope(_facade().INVALID_INPUT, "请选择图片文件"), status_code=400
        )
    ...
    try:
        ...
        return _facade().JSONResponse(
            _facade().error_envelope(_facade().INVALID_FILE, str(exc)), status_code=400
        )
    except OSError as exc:
        _facade().logger.exception("avatar save failed user_id=%s", user.id)
        return _facade().JSONResponse(
            _facade().error_envelope(_facade().SAVE_FAILED, f"头像保存失败：{exc}"), status_code=500
        )
    from app.db.models.user import User
    from app.db.session import get_db
...
def auth_profile_avatar_get(user=_facade().Depends(_facade().get_logged_in_user)):
    ...
    if path is None:
        return _facade().JSONResponse(
            status_code=404, content={"success": False, "message": "未设置头像"}
        )
...
def auth_logout(request: _facade().Request):
    ...
    if not sid:
        return _facade().JSONResponse(
            _facade().error_envelope(_facade().NO_SESSION, "无有效会话"), status_code=400
        )
...
def auth_password_change(
    ...
    if not old_password or not new_password:
        return _facade().JSONResponse(
            _facade().error_envelope(_facade().INVALID_INPUT, "请填写完整信息"), status_code=400
        )
...
def users_list(
    include_inactive: str = _facade().Query(default="false"),
    _user=_facade().Depends(_facade()._require_admin),
):
...
def users_create(
    ...
    if len(password) < 6:
        return _facade().JSONResponse(
            _facade().error_envelope(_facade().WEAK_PASSWORD, "密码至少6个字符"), status_code=400
        )
    role = body.get("role", "viewer")
    if role not in ["viewer", "operator", "admin"]:
        return _facade().JSONResponse(
            _facade().error_envelope(_facade().INVALID_ROLE, "无效的角色"), status_code=400
        )
    user_service = get_user_app_service()
    ...
    if not result["success"]:
        return _facade().JSONResponse(
            _facade().error_envelope(
                _facade().CREATE_FAILED,
                str(result.get("error") or result.get("message") or "创建失败"),
            ),
            status_code=400,
        )
    return _facade().JSONResponse(
        {"success": True, "data": {"user": result["user"]}}, status_code=201
    )
...
def users_update(
    ...
    if not result["success"]:
        return _facade().JSONResponse(
            _facade().error_envelope(
                _facade().UPDATE_FAILED,
                str(result.get("error") or result.get("message") or "更新失败"),
            ),
            status_code=400,
        )
