...
def _xcagi_guarded_planner_stream_events(body: XcagiCompatChatBody, *, runtime_context: dict[str, Any] | None, workspace_root: str, client: Any):
    event_queue: queue.Queue[Any] = queue.Queue()
    done_marker = object()

    def _worker() -> None:
        try:
            for ev in _facade().chat_stream_sse_events(body.message, runtime_context=runtime_context or None, system_prompt=body.system_prompt, workspace_root=workspace_root, db_write_token=body.db_write_token, client=client):
                event_queue.put(ev)
    ...
    while True:
        ...
        if elapsed >= total_timeout:
            raise TimeoutError(f'流式对话总超时（>{int(total_timeout)} 秒）。请稍后重试，或缩短问题范围。')
        ...
        try:
            ...
            if not first_event_seen:
                raise TimeoutError(f'流式对话首包超时（>{int(first_token_timeout)} 秒）。模型服务暂未返回首个分片，请稍后重试。')
...
def strip_planner_stream_markers(merged: str) -> tuple[str, str | None]:
    ...
    cleaned = re.sub('\\[(?:正在调用工具:[^\\]\\n]+|工具已返回[^\\]\\n]*|工具未成功[^\\]\\n]*|需要授权:[^\\]\\n]+|请提供令牌:[^\\]\\n]+)\\]', '', text)
    cleaned = re.sub('\\s{2,}', ' ', cleaned).strip()
    return (cleaned, thinking)

async def _xcagi_planner_stream_bytes_async(request: Request, body: XcagiCompatChatBody, *, ai_tier: str):
...
def _xcagi_planner_stream_bytes(request: Request, body: XcagiCompatChatBody, *, ai_tier: str):
    ...
    try:
        ...
        for ev in _facade()._xcagi_guarded_planner_stream_events(body, runtime_context=runtime_context, workspace_root=workspace_root, client=llm_client):
            et = ev.get('type')
            if et == 'error':
        ...
        yield _facade()._sse_event_line({'type': 'error', 'message': exc.detail if isinstance(exc.detail, str) else str(exc.detail), 'status_code': exc.status_code})
