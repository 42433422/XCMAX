...
def admin_ai_groups_create(
    ...
    try:
        group = (
            _facade()
            .AiGroupChatService()
            .create_group(user_id=_facade()._uid(user), name=str(body.get("name") or ""))
        )
...
def admin_ai_group_messages(
    ...
    try:
        messages = (
            _facade()
            .AiGroupChatService()
            .get_messages(user_id=_facade()._uid(user), group_id=group_id, limit=limit)
        )
...
def admin_ai_group_remove_member(
    ...
    try:
        group = (
            _facade()
            .AiGroupChatService()
            .remove_member(user_id=_facade()._uid(user), group_id=group_id, employee_id=employee_id)
        )
        return {"success": True, "group": group}
    except ValueError as exc:
        return _facade().JSONResponse({"success": False, "message": str(exc)}, status_code=400)
