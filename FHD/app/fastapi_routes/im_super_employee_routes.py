...
def claude_super_employee_invoke(
    ...
    try:
        ...
        result = (
            ...
            .invoke(user_id=uid, message=text, context=context)
        )
        return {"success": True, **result}
    except ValueError as exc:
        return _facade().JSONResponse({"success": False, "message": str(exc)}, status_code=400)
...
def admin_factory_employees(request: Request, user: CurrentUser = Depends(require_identified_user)):
    ...
    try:
        ...
        items = [
            ...
            for meta in _facade().assistant_ssot.factory_employees().values()
        ]
        return {"success": True, "employees": items}
    except _facade().RECOVERABLE_ERRORS:
        _facade().logger.exception("admin_factory_employees")
        return _facade().JSONResponse(
            {"success": False, "message": "加载工厂员工失败"}, status_code=500
        )
    finally:
        db.close()
...
def trae_super_employee_invoke(
    ...
    try:
        ...
        context = _facade().factory_context(workspace_id=workspace_id, base=context)
        result = (
            _facade().TraeSuperEmployeeService().invoke(user_id=uid, message=text, context=context)
        )
        return {"success": True, **result}
    except ValueError as exc:
        return _facade().JSONResponse({"success": False, "message": str(exc)}, status_code=400)
