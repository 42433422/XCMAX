...
def _market_base_url() -> str:
    return (
        (_facade().os.environ.get("XCAGI_MARKET_BASE_URL") or "http://127.0.0.1:8765")
        .strip()
        .rstrip("/")
    )
...
def bind_market_auth_to_session(
    request: _facade().Request, market_result: dict[str, _facade().Any]
) -> tuple[str, str]:
    ...
    if token:
        _facade().save_session_market_token(
            _facade().session_id_from_request(request), token, refresh or None
        )
    return (token, refresh)


def save_session_market_token(
    session_id: str, token: str, refresh_token: str | None = None
) -> None:
...
def session_market_refresh_token(session_id: str) -> str:
    ...
    try:
        ...
        _facade().logger.exception(
            "session_market_refresh_token: DB read failed for session_id=%s", sid
        )
    return ""
...
async def _authorization_from_request_resolved(
    ...
    if not session_tok and (sid or user_id is not None):
        session_tok = _facade()._normalize_bearer_token(
...
def _transport_error_message(exc: Exception) -> tuple[str, int]:
    ...
    if isinstance(exc, httpx.ReadTimeout):
        return (
            f"连接修茈市场超时（{label}）。请检查网络或增大 XCAGI_MARKET_HTTP_TIMEOUT；当前 XCAGI_MARKET_BASE_URL={base}",
            503,
        )
    return (
