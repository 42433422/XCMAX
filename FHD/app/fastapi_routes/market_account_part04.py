...
def _checkout_sign_body_from_request(body: dict[str, _facade().Any]) -> dict[str, _facade().Any]:
    ...
    if wallet_recharge is True or str(wallet_recharge).strip().lower() in {'1', 'true', 'yes', 'on'}:
...
async def _resolve_market_authorization_for_checkout(request: _facade().Request, body: dict[str, _facade().Any]) -> tuple[str, dict[str, _facade().Any] | None]:
    return (await _facade()._authorization_from_request_resolved(request, body), None)

@_facade().router.post('/payment/direct-checkout')
async def market_payment_direct_checkout(request: _facade().Request, body: dict[str, _facade().Any]=_facade().Body(default_factory=dict)):
    ...
    signed = await _facade()._proxy_json('POST', '/api/payment/sign-checkout', json_body=sign_body, authorization=authorization, return_error_payload=True)
    if isinstance(signed, dict) and signed.get('__proxy_error__'):
        return _facade().JSONResponse({'success': False, 'message': _facade()._error_message(signed.get('payload'), int(signed.get('status_code') or 502))}, status_code=int(signed.get('status_code') or 502))
    checkout_body = {**body}
    if isinstance(signed, dict):
        checkout_body.update(signed)
    payload = await _facade()._proxy_json('POST', '/api/payment/checkout', json_body=checkout_body, authorization=authorization, return_error_payload=True)
...
async def market_payment_orders(request: _facade().Request, status: str | None=None, limit: int=50, offset: int=0):
    path = f'/api/payment/orders?limit={int(limit)}&offset={int(offset)}'
    if status:
        path += f'&status={status.strip()}'
    payload = await _facade()._proxy_json('GET', path, authorization=_facade()._market_auth_from_request(request), return_error_payload=True)
    if isinstance(payload, dict) and payload.get('__proxy_error__'):
        return _facade().JSONResponse({'success': False, 'message': _facade()._error_message(payload.get('payload'), int(payload.get('status_code') or 502))}, status_code=int(payload.get('status_code') or 502))
...
async def market_payment_query(request: _facade().Request, out_trade_no: str):
    payload = await _facade()._proxy_json('GET', f'/api/payment/query/{out_trade_no}', authorization=_facade()._market_auth_from_request(request), return_error_payload=True)
    if isinstance(payload, dict) and payload.get('__proxy_error__'):
        return _facade().JSONResponse({'success': False, 'message': _facade()._error_message(payload.get('payload'), int(payload.get('status_code') or 502))}, status_code=int(payload.get('status_code') or 502))
    return {'success': True, 'data': payload}
...
async def market_wallet_overview(request: _facade().Request):
    ...
    if not authorization:
        return _facade().JSONResponse({'success': False, 'message': '尚未绑定市场账号；请重新登录软件以自动同步'}, status_code=401)
    payload = await _facade()._proxy_json('GET', '/api/wallet/overview', authorization=authorization, return_error_payload=True)
    if isinstance(payload, dict) and payload.get('__proxy_error__'):
        return _facade().JSONResponse({'success': False, 'message': _facade()._error_message(payload.get('payload'), int(payload.get('status_code') or 502))}, status_code=int(payload.get('status_code') or 502))
    return {'success': True, 'data': payload}
...
async def market_status():
    ...
    return {'success': reachable, 'data': {'market_base_url': _facade()._market_base_url(), 'reachable': reachable, 'raw': payload.get('payload') if isinstance(payload, dict) and payload.get('__proxy_error__') else payload}}

@_facade().router.post('/dev-create-account')
async def market_dev_create_account(body: dict[str, _facade().Any]=_facade().Body(default_factory=dict)):
    ...
    if len(password) < 6:
        return _facade().JSONResponse({'success': False, 'message': 'password 至少 6 位'}, status_code=400)
    ...
    if isinstance(payload, dict) and payload.get('__proxy_error__'):
        ...
        if status_code == 409 or '存在' in _facade()._error_message(raw_error, status_code):
            ...
            return _facade().JSONResponse({'success': False, 'message': _facade()._error_message(raw_error, status_code), 'data': raw_error}, status_code=status_code)
    token = _facade()._token_from_auth_response(payload)
    if not token:
        return _facade().JSONResponse({'success': False, 'message': '账号创建成功但未返回 token', 'data': payload}, status_code=502)
    overview = await _facade()._proxy_json('GET', '/api/account/bootstrap', authorization=token, return_error_payload=True)
    return {'success': True, 'data': {'market_base_url': _facade()._market_base_url(), 'username': username, 'email': email, 'password': password, 'token': token, 'overview_ok': not (isinstance(overview, dict) and overview.get('__proxy_error__')), 'overview': overview.get('payload') if isinstance(overview, dict) and overview.get('__proxy_error__') else overview}}
