"""Shared mobile extension helpers (split from mobile_api_extensions).

Re-exported from ``mobile_api_extensions`` for tests and ``mext`` patch compatibility.
Cross-module calls use ``mext`` so ``patch("...mobile_api_extensions.X")`` still works.
"""
...
def _ai_circle_employee_profiles() -> dict[str, dict[str, str]]:
    ...
    for mod in mext._mobile_mod_items():
...
# ── 中继用户解析（使用 mext.RECOVERABLE_ERRORS，需留在主模块以支持测试 patch） ──
...
def _resolve_mobile_relay_user(user: Any, *, prefer_admin: bool = False) -> dict[str, Any]:
    ...
    if uid > 0 and (not prefer_admin or role in {"admin", "super_admin", "owner"}):
        return mext._mobile_user_public_dict(user)
    ...
    try:
        with get_db() as db:
            ...
            if row is None:
                ...
                db.add(row)
                db.flush()
            public = mext._mobile_user_public_dict(row)
        ...
        if prefer_admin:
            return mext._relay_admin_fallback_user()
        raise
...
def _register_desktop_relay_for_pairing(host: str, port: int) -> dict[str, Any] | None:
    ...
    if enabled in {"0", "false", "off", "no"}:
        return None
    if not mext._host_is_private_or_loopback(host):
        return None
    try:
        ...
        for m in get_mod_manager().list_all_mods() or []:
            if isinstance(m, dict):
                ...
                item = {
                    ...
                    "workflow_employees": mext._enrich_workflow_employees(
                        mid,
                        employees,
                        market_profiles,
            ...
            if mid:
                items.append(item)
        mext._upsert_admin_duty_mod_item(
            items,
            market_profiles,
            market_connected=market_connected,
        )
...
def _admin_roster_area_labels() -> dict[str, str]:
    ...
    for _area_key, area in areas.items():
        if not isinstance(area, dict):
            continue
        label = mext._compact_text(area.get("label"))
...
def _admin_employee_manifest(employee_id: str) -> dict[str, Any]:
    ...
    if not eid:
        return {}
    manifest = Path(__file__).resolve().parents[3] / "mods" / "_employees" / eid / "manifest.json"
    try:
...
def _admin_duty_records_from_roster() -> list[dict[str, Any]]:
    registry = mext._load_admin_duty_records()
    roster_ids = mext._admin_roster_ids_by_department_order()
    if not roster_ids:
        return registry
    ...
    area_labels = mext._admin_roster_area_labels()
    records: list[dict[str, Any]] = []
    for eid in roster_ids:
        raw = dict(registry_by_id.get(eid) or {})
        manifest = mext._admin_employee_manifest(eid)
        employee_meta = (
...
def _admin_employee_items(
    ...
    *,
    ...
    items: list[dict[str, Any]] = []
    for raw in mext._admin_duty_records_from_roster():
        employee_id = str(raw.get("id") or raw.get("pkg_id") or "").strip()
        if not employee_id:
            continue
        name = mext._compact_text(raw.get("name") or employee_id)
        area = mext._compact_text(raw.get("yuangon_area") or raw.get("industry"))
        item = {
            "id": employee_id,
            "name": name,
            "label": name,
            "title": name,
            "panel_title": name,
            "description": mext._compact_text(raw.get("description")),
            "panel_summary": mext._compact_text(raw.get("description")),
            "version": str(raw.get("version") or "").strip(),
            "industry": mext._compact_text(raw.get("industry")),
            "yuangon_area": area,
            "employee_scope": mext._compact_text(raw.get("employee_scope") or "duty"),
            "employee_source": mext._compact_text(raw.get("employee_source") or "duty_roster"),
            ...
            "stored_filename": mext._compact_text(raw.get("stored_filename")),
            "file_size": raw.get("file_size") or 0,
        }
        profile = None
        if market_profiles:
            for key in mext._admin_employee_match_keys(raw, employee_id, name):
                profile = market_profiles.get(key)
                if profile:
                    break
        mext._apply_market_profile(item, profile, market_connected=market_connected)
        if im_summary:
...
def _upsert_admin_duty_mod_item(
    ...
    *,
    ...
    duty_mod = mext._admin_duty_mod_item(market_profiles, market_connected=market_connected)
    if not duty_mod:
        return
...
# ── 客服持久化（使用 mext.OPERATIONAL_ERRORS，须留在主模块） ──
...
def _persist_mobile_cs_request(
    ...
    *,
    ...
    username = mext._safe_user_text(user, "username")
    extra = {
        "message_id": message_id,
        "mobile_user_id": mext._safe_user_id(user),
        "username": username,
        "ai_reply": reply,
        "backend": backend,
    ...
    try:
        with get_db() as db:
            ...
            row = ServiceRequest(
                source_instance_id=mext._mobile_cs_source_id(user),
                source_instance_name=mext._mobile_cs_source_name(user),
