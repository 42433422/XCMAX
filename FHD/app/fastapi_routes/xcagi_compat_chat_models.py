...
from app.application.workflow.multimodal_user_content import EmptyMultimodalResponseError, UnsupportedMultimodalModelError
...
class XcagiCompatChatBody(BaseModel):
    ...
    message: str = Field(..., min_length=1, validation_alias=AliasChoices('message', 'user_message', 'content', 'text', 'query'))
    context: dict | None = Field(default=None, validation_alias=AliasChoices('context', 'runtime_context', 'session_context', 'ddd_context', 'neuro_context', 'neuro_ddd_context'))
    system_prompt: str | None = Field(default=None, validation_alias=AliasChoices('system_prompt', 'system', 'instructions'))
    mode: str | None = Field(default=None, validation_alias=AliasChoices('mode', 'llm_mode'))
    db_read_token: str | None = Field(default=None, description='兼容旧客户端字段；当前版本不需要数据库查看授权。')
    db_write_token: str | None = Field(default=None, description='兼容旧客户端字段；当前版本不需要数据库写入授权。')
...
def _market_connection_label() -> str:
    raw = _facade().os.environ.get('XCAGI_MARKET_BASE_URL') or _facade().os.environ.get('MODSTORE_PLATFORM_URL') or ''
    try:
...
def _xcagi_chat_http_exc(exc: BaseException) -> HTTPException:
    ...
    try:
        ...
        if isinstance(exc, httpx.ConnectError):
            return HTTPException(status_code=503, detail=f'无法连接修茈平台 LLM（{_facade()._market_connection_label()}）')
    ...
    if isinstance(exc, ValueError):
        ...
        if '余额不足' in msg or '402' in msg:
            return HTTPException(status_code=402, detail='修茈市场模型余额不足，请在「模型支付」充值后重试。')
        if '平台错误' in msg:
...
def _xcagi_compat_reply_payload(reply: str | dict, *, runtime_context_update: dict[str, Any] | None=None, kitten_attachments: dict[str, Any] | None=None) -> dict:
    ...
    try:
        if reply_records:
            ...
            raw.setdefault('tool_key', last_record.get('tool_id') or last_record.get('tool_name') or '')
            raw.setdefault('tool_name', last_record.get('tool_name') or last_record.get('tool_id') or '')
    ...
    if err_code or last_result.get('success') is False:
        ...
        if notice not in text:
            text = f'{text.rstrip()}\n\n{notice}'.strip()
    data: dict[str, Any] = {'response': text, 'text': text, 'thinking_steps': thinking_steps, **tool_data}
