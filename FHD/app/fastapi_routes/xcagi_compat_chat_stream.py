...
def _xcagi_planner_stream_bytes(request: Request, body: XcagiCompatChatBody, *, ai_tier: str):
    ...
    sales_closed_loop_route = str(_sales_route.get('intent') or '') == 'sales_write' and str(_sales_route.get('action') or '') == 'execute_closed_loop' and isinstance(_sales_route.get('payload'), dict) and bool(_sales_route.get('payload'))
    controlled_entity_named = any((token in str(body.message or '') for token in ('客户', '购买单位', '产品', '商品', '原材料', '物料', '出货', '发货')))
    from app.infrastructure.tenant_scope import tenant_scope
    authenticated_tenant_id = runtime_context.get('tenant_id')
    authenticated_tenant_id = int(authenticated_tenant_id) if authenticated_tenant_id is not None else None
    if has_pending_workflow or sales_closed_loop_route or (controlled_entity_named and _looks_like_business_db_write(str(body.message or ''), str(body.message or '').lower())):
        with tenant_scope(authenticated_tenant_id):
            payload = chat_service.process_chat(user_id=scoped_user_id, message=body.message, context=runtime_context, source=body.source, file_context={})
    ...
    business_payload = try_handle_business_chat_action(body.message, runtime_context=runtime_context, user_id=body.user_id, request=request)
    ...
    if not ok_read and read_req:
        yield _facade()._sse_event_line({'type': 'token', 'text': f"[需要授权: {read_req.get('token_description')}]"})
        yield _facade()._sse_event_line({'type': 'requires_token', 'token_name': read_req.get('token_name'), 'token_description': read_req.get('token_description')})
        return
    ...
    if intr is not None:
        ...
        payload = _facade().attach_chat_trace_run(payload, message=body.message, runtime_context=cleared, user_id=body.user_id, source=body.source, channel='compat_chat_stream')
        yield _facade()._sse_event_line({'type': 'done', 'result': payload})
        return
    ...
    try:
        pre_run = _facade().start_legacy_chat_run(message=body.message, runtime_context=planner_runtime_context, user_id=body.user_id, source=body.source, channel='compat_chat_stream')
    ...
    try:
        ...
        for ev in _facade()._xcagi_guarded_planner_stream_events(body, runtime_context=planner_runtime_context, workspace_root=workspace_root, client=llm_client):
            et = ev.get('type')
            if et == 'error':
                if pre_run is not None:
                    payload = {'success': False, 'message': str(ev.get('message') or '流式 planner 执行失败'), 'response': str(ev.get('message') or '流式 planner 执行失败'), 'data': {'error': str(ev.get('message') or '流式 planner 执行失败'), 'status_code': ev.get('status_code')}}
                    _facade().finalize_legacy_chat_run(pre_run.run_id, payload, message=body.message, runtime_context=planner_runtime_context, user_id=body.user_id, source=body.source, channel='compat_chat_stream')
                yield _facade()._sse_event_line(_facade()._sse_payload_with_run_id(ev, getattr(pre_run, 'run_id', None)))
                return
            if et == 'token':
                ...
                if pre_run is not None:
                    payload = {'success': True, 'requires_token': True, 'token_name': ev.get('token_name'), 'token_description': ev.get('token_description'), 'message': ev.get('message'), 'response': ev.get('message'), 'data': {'requires_token': True, 'token_name': ev.get('token_name'), 'token_description': ev.get('token_description')}}
                    _facade().finalize_legacy_chat_run(pre_run.run_id, payload, message=body.message, runtime_context=planner_runtime_context, user_id=body.user_id, source=body.source, channel='compat_chat_stream')
                yield _facade()._sse_event_line(_facade()._sse_payload_with_run_id(ev, getattr(pre_run, 'run_id', None)))
        ...
        if not merged.strip():
            ...
            if pre_run is not None:
                payload = {'success': False, 'message': msg, 'response': msg, 'data': {'error': msg}}
                _facade().finalize_legacy_chat_run(pre_run.run_id, payload, message=body.message, runtime_context=planner_runtime_context, user_id=body.user_id, source=body.source, channel='compat_chat_stream')
            yield _facade()._sse_event_line(_facade()._sse_payload_with_run_id({'type': 'error', 'message': msg}, getattr(pre_run, 'run_id', None)))
            return
        ...
        if pre_run is not None:
            payload = _facade().finalize_legacy_chat_run(pre_run.run_id, payload, message=body.message, runtime_context=planner_runtime_context, user_id=body.user_id, source=body.source, channel='compat_chat_stream')
        else:
            payload = _facade().attach_chat_trace_run(payload, message=body.message, runtime_context=runtime_context, user_id=body.user_id, source=body.source, channel='compat_chat_stream')
        yield _facade()._sse_event_line({'type': 'done', 'result': payload})
    except _facade().RECOVERABLE_ERRORS as e:
        exc = _facade()._xcagi_chat_http_exc(e)
        message = exc.detail if isinstance(exc.detail, str) else str(exc.detail)
        if pre_run is not None:
            payload = {'success': False, 'message': message, 'response': message, 'data': {'error': message, 'status_code': exc.status_code}}
            _facade().finalize_legacy_chat_run(pre_run.run_id, payload, message=body.message, runtime_context=planner_runtime_context, user_id=body.user_id, source=body.source, channel='compat_chat_stream')
        yield _facade()._sse_event_line(_facade()._sse_payload_with_run_id({'type': 'error', 'message': message, 'status_code': exc.status_code}, getattr(pre_run, 'run_id', None)))
