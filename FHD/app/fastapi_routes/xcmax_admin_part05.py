...
def _collect_codex_usage() -> dict[str, _facade().Any]:
    ...
    jsonl_files = sorted(
        f
        for f in (_facade().os.path.join(archived, x) for x in _facade().os.listdir(archived))
        if f.endswith(".jsonl")
    )
...
def _collect_trae_usage() -> dict[str, _facade().Any]:
    ...
    state_db = _facade().os.path.expanduser(
    ...
    return {
        ...
        "note": f"Trae API 被 WAF 403 拦截，按 {total_turns} 轮 × 1050 万 tokens/轮 估算（prompt 1000 万 + completion 50 万）",
        "estimated": True,
...
def _estimate_cost_usd(source_key: str, data: dict[str, _facade().Any]) -> float:
    ...
    if source_key == "codex":
        ...
        return (
            uncached * 5 / 1000000 + cached * 1.25 / 1000000 + (output + reasoning) * 10 / 1000000
        )
    if source_key == "trae":
        prompt = _facade()._to_int(data.get("prompt_tokens"))
        output = _facade()._to_int(data.get("completion_tokens"))
...
def _collect_mimo_usage() -> dict[str, _facade().Any]:
    ...
    return {
        ...
        "note": f"Credits {credits_used:,} / {credits_quota:,}（{usage_pct}%），实际 token {actual_tokens:,}",
...
def _build_token_usage_summary() -> dict[str, _facade().Any]:
    ...
    summary = {
        ...
        "sources": sources,
        "collected_at": _facade().time.strftime("%Y-%m-%d %H:%M:%S"),
    }
    try:
