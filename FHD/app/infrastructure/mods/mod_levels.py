...
def market_install_block_reason(manifest: dict[str, Any]) -> str | None:
    ...
    descriptor = descriptor_for_manifest(manifest)
    artifact = normalize_artifact(manifest)
    if artifact != ARTIFACT_EMPLOYEE_PACK or descriptor.kind != KIND_EMPLOYEE_PACK:
...
def market_catalog_row_allowed(row: dict[str, Any]) -> bool:
    ...
    旧服务端部分历史条目没有 artifact，展示暂时保留以避免目录突然变空；真正下载
    安装时仍会读取 zip 内 manifest 并严格拒绝非 employee_pack。
    """
