...
class _ModManagerPart01Mixin:
    ...
    def _refresh_mods_root_if_needed(self) -> None:
        ...
        """
        env_raw = (_facade().os.environ.get('XCAGI_MODS_ROOT') or _facade().os.environ.get('XCAGI_MODS_DIR') or '').strip()
        if env_raw:
            p = _facade().os.path.abspath(env_raw)
            if _facade().os.path.isdir(p):
                if self.mods_root != p:
                    _facade().logger.info('[ModManager] Updating mods_root from env: %s -> %s', self.mods_root, p)
                    self.mods_root = p
                    self._ensure_attempts = 0
                return
            _facade().logger.warning('[ModManager] XCAGI_MODS_ROOT / XCAGI_MODS_DIR is set but not a directory: %s (keeping %s)', p, self.mods_root)
        if not _facade().os.path.isdir(self.mods_root):
            fb = _facade()._default_mods_root()
            if fb != self.mods_root:
                _facade().logger.warning('[ModManager] mods_root was missing or invalid (%s), re-resolved -> %s', self.mods_root, fb)
                self.mods_root = fb
                self._ensure_attempts = 0

    def _record_load_failure(self, mod_id: str, stage: str, message: str) -> None:
        self._recent_load_failures.append({'mod_id': mod_id, 'stage': stage, 'message': message[:500]})
    ...
    def ensure_mods_loaded(self, app: _facade().Any) -> None:
        ...
        try:
            ...
            _facade().logger.warning('[ModManager] 注册表无 Mod 但磁盘有 manifest，第 %s 次尝试加载：mods_root=%s，manifest 数=%s', self._ensure_attempts, self.mods_root, len(discovered))
            self.load_all_mods()
            _facade().load_mod_routes(app, self)
        except _facade().RECOVERABLE_ERRORS as e:
            _facade().logger.exception('[ModManager] ensure_mods_loaded failed (mods_root=%s): %s', getattr(self, 'mods_root', None), e)
    ...
    def resolve_mod_directory(self, mod_id: str) -> str | None:
        ...
        def _direct(candidate: str) -> str | None:
            ...
            for root in self.all_mods_roots():
                ...
                if mod_path and _facade()._trusted_child_path(mod_path, 'manifest.json', directory=False):
                    return mod_path
            return None
        hit = _direct(mid)
        if hit:
    ...
    def _scan_mods_from_build_index(self, fp: str) -> list[_facade().ModMetadata] | None:
        ...
        for root in self.all_mods_roots():
            ...
            for row in rows:
                ...
                if not mod_path or not _facade().os.path.isfile(_facade().os.path.join(mod_path, 'manifest.json')):
                    continue
            ...
            if mods:
                _facade().logger.info('[ModManager] scan_mods via mods-index.json (%s mods)', len(mods))
                return mods
        return None
    ...
    def scan_mods(self, *, use_cache: bool=True) -> list[_facade().ModMetadata]:
        ...
        for mods_root in self.all_mods_roots():
            if not _facade().os.path.isdir(mods_root):
                _facade().logger.warning('[ModManager] Mods directory does not exist: %s', mods_root)
                continue
            for entry in _facade().os.listdir(mods_root):
                ...
                _facade().logger.debug('[ModManager] Checking %s/%s, manifest exists: %s', mods_root, entry, _facade().os.path.isfile(manifest_path))
                metadata = _facade().parse_manifest(mod_path)
                if metadata:
                    ...
                    mods.append(metadata)
                    _facade().logger.debug('[ModManager] Found mod: %s (%s) v%s @ %s', metadata.id, metadata.name, metadata.version, mod_path)
                else:
                    _facade().logger.warning('[ModManager] Failed to parse manifest for mod entry: %s/%s', mods_root, entry)
                    self._scan_manifest_errors.append({'entry': entry, 'mods_root': mods_root, 'message': 'manifest.json 缺失或无法解析（检查 JSON 与必填字段 id）'})
    ...
    def load_mod(self, mod_id: str) -> bool:
        ...
        if registry.get_mod_metadata(mod_id):
            ...
            if mod_id not in self._loaded_mods:
                _facade().logger.warning('[ModManager] Mod %s in registry but missing from _loaded_mods; syncing list', mod_id)
        ...
        if not mod_path:
            self._record_load_failure(mod_id, 'fs', f'目录不存在（已搜索 mods 根: {self.all_mods_roots()}）')
            return False
        metadata = _facade().parse_manifest(mod_path)
        if not metadata:
            ...
            return False
        _facade().logger.info('[ModManager] Mod metadata parsed: id=%s, name=%s, version=%s', metadata.id, metadata.name, metadata.version)
        if _facade().normalize_artifact({'artifact': metadata.artifact}) == _facade().ARTIFACT_BUNDLE:
            if registry.get_mod_metadata(mod_id):
                return True
            if registry.register_mod(metadata):
                self._loaded_mods.append(mod_id)
                _facade().logger.info('[ModManager] Registered bundle metadata only (no backend): %s', mod_id)
                return True
        ...
        if not _facade().validate_dependencies(metadata, deps):
            ...
            self._record_load_failure(mod_id, 'dependencies', '依赖未满足（需先加载所依赖的 mod，或检查 manifest dependencies）')
            return False
        try:
            ...
            _facade().logger.info('[ModManager] Mod loaded successfully: %s%s', effective_id, f' (requested {mod_id})' if effective_id != mod_id else '')
            return True
        except _facade().RECOVERABLE_ERRORS as e:
            _facade().logger.error('[ModManager] Failed to load mod %s: %s', mod_id, e, exc_info=True)
                ...
                if hasattr(module, metadata.backend_init):
                    ...
                    if callable(init_fn):
                        try:
                            ...
                            _facade().logger.warning('mod init hook %s for %s failed: %s', metadata.backend_init, mod_id, exc)
            except _facade().RECOVERABLE_ERRORS as e:
                _facade().logger.error('Failed to load backend entry for %s: %s', mod_id, e, exc_info=True)
                raise
    ...
    def install_mod_package(self, package_path: str, verify_signature: bool=True, activate: bool=True) -> tuple[bool, str, _facade().ModMetadata | None]:
        """
        安装 MOD 包
        ...
        try:
            ...
            with _facade().tempfile.TemporaryDirectory() as temp_dir:
                try:
                    (extract_path, manifest) = _facade().ModPackage.extract_package(package_path, temp_dir, verify_signature=verify_signature)
                ...
                if _facade().os.path.exists(target_path):
                    ...
                    _facade().logger.info('MOD %s already exists (v%s), updating to v%s', mod_id, existing_version, new_version)
