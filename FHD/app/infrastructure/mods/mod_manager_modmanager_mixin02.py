...
class _ModManagerPart02Mixin:
    ...
    def update_mod(self, mod_id: str, package_path: str, verify_signature: bool=True) -> tuple[bool, str, _facade().ModMetadata | None]:
        """
        更新 MOD
        ...
        try:
            ...
            _facade().logger.info('Updating MOD %s: v%s -> v%s', mod_id, current_version, new_version)
            ...
            with _facade().tempfile.TemporaryDirectory() as temp_dir:
                try:
                    (extract_path, _) = _facade().ModPackage.extract_package(package_path, temp_dir, verify_signature=verify_signature)
    ...
    def validate_mod_package(self, package_path: str) -> tuple[bool, str, dict[str, _facade().Any]]:
        ...
        try:
            ...
            with _facade().tempfile.TemporaryDirectory() as temp_dir:
                (extract_path, manifest) = _facade().ModPackage.extract_package(package_path, temp_dir, verify_signature=True)
                ...
                if art == _facade().ARTIFACT_BUNDLE:
                    ...
                    if _facade().os.path.isdir(frontend_path):
                        ...
                        if frontend_routes:
                            routes_file = _facade().os.path.join(frontend_path, f'{frontend_routes}.js')
                            if not _facade().os.path.isfile(routes_file):
                                errors.append(f'前端路由文件不存在：{frontend_routes}.js')
                is_valid = len(errors) == 0
                return (is_valid, '验证通过' if is_valid else '; '.join(errors), {'id': mod_id, 'name': manifest.get('name', ''), 'version': version, 'author': manifest.get('author', ''), 'artifact': art, 'errors': errors, 'warnings': warnings})
    ...
    def _metadata_to_api_dict(m: _facade().ModMetadata) -> dict[str, _facade().Any]:
        ...
        row: dict[str, _facade().Any] = {'id': m.id, 'name': m.name, 'version': m.version, 'author': m.author or '', 'description': m.description or '', 'primary': bool(m.primary), 'artifact': art, 'industry': dict(m.industry) if isinstance(m.industry, dict) else {}, 'ui_labels': dict(m.ui_labels) if isinstance(m.ui_labels, dict) else {}, 'ui_starter_pack': list(m.ui_starter_pack) if isinstance(m.ui_starter_pack, list) else [], 'menu': list(m.frontend_menu) if m.frontend_menu else [], 'frontend': {'pro_entry_path': str(getattr(m, 'frontend_pro_entry_path', '') or '').strip()}, 'menu_overrides': list(m.frontend_menu_overrides) if m.frontend_menu_overrides else [], 'workflow_employees': list(m.workflow_employees) if m.workflow_employees else [], 'comms_exports': list(m.comms_exports) if m.comms_exports else []}
        if art == _facade().ARTIFACT_BUNDLE:
            row['type'] = 'bundle'
        return row
        ...
        for metadata in mods:
            try:
                ...
                if not is_mod_visible_for_enterprise(metadata.id):
                    _facade().logger.info('[ModManager] Skipping mod %s (enterprise entitlement)', metadata.id)
                    continue
            except _facade().RECOVERABLE_ERRORS:
                pass
            ...
            if metadata.dependencies:
                ...
                if not deps_satisfied:
                    _facade().logger.warning('[ModManager] Skipping mod %s due to unsatisfied dependencies', metadata.id)
                    self._record_load_failure(metadata.id, 'dependencies', 'load_all 阶段依赖未满足（可能需先加载其他 mod）')
                    continue
