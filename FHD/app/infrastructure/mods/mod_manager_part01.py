...
def _default_mods_root() -> str:
    ...
    _facade().logger.debug('[_default_mods_root] Resolving mods root, CWD: %s', _facade().os.getcwd())
    env = (_facade().os.environ.get('XCAGI_MODS_ROOT') or _facade().os.environ.get('XCAGI_MODS_DIR') or '').strip()
    if env:
        p = _facade().os.path.abspath(env)
        if _facade().os.path.isdir(p):
            _facade().logger.debug('[_default_mods_root] Mods root from env: %s', p)
            return p
        _facade().logger.warning('[_default_mods_root] XCAGI_MODS_ROOT / XCAGI_MODS_DIR is set but not a directory: %s', p)
    file_here = _facade().os.path.abspath(__file__)
    from_pkg_layout = _facade().os.path.join(_facade().os.path.dirname(_facade().os.path.dirname(_facade().os.path.dirname(_facade().os.path.dirname(file_here)))), 'mods')
    _facade().logger.debug('[_default_mods_root] Checking package-relative path: %s, exists: %s', from_pkg_layout, _facade().os.path.isdir(from_pkg_layout))
    if _facade().os.path.isdir(from_pkg_layout):
        _facade().logger.debug('[_default_mods_root] Mods root (next to app package): %s', from_pkg_layout)
        return from_pkg_layout
    cwd_mods = _facade().os.path.join(_facade().os.getcwd(), 'mods')
    _facade().logger.debug('[_default_mods_root] Checking CWD mods: %s, exists: %s', cwd_mods, _facade().os.path.isdir(cwd_mods))
    ...
    for i in range(8):
        ...
        _facade().logger.debug('[_default_mods_root] Walking up: %s, exists: %s', trial, _facade().os.path.isdir(trial))
        ...
        if parent == cur:
            break
        cur = parent
    _facade().logger.warning('[_default_mods_root] No mods directory found; using package-relative path (may be empty): %s. Set XCAGI_MODS_ROOT or run from project root.', from_pkg_layout)
    return from_pkg_layout

def _repo_layout_mods_candidates() -> list[str]:
    """Find additional repository MOD roots missing from a packaged deployment."""
    file_here = _facade().os.path.abspath(__file__)
    repo_root = _facade().os.path.dirname(_facade().os.path.dirname(_facade().os.path.dirname(_facade().os.path.dirname(file_here))))
    out: list[str] = []
...
def import_mod_backend_py(mod_path: str, mod_id: str, stem: str):
    ...
    import hashlib
    path_digest = hashlib.sha256(_facade().os.path.normpath(_facade().os.path.abspath(mod_path)).encode()).hexdigest()[:16]
    ...
    for (event, handler_spec) in metadata.hooks.items():
        ...
        try:
            ...
            if not module_name or not attr:
                _facade().logger.error('Invalid hook handler spec for mod %s: %r', mod_id, handler_spec)
                continue
            module = _facade().import_mod_backend_py(mod_fs_path, mod_id, module_name)
            handler = getattr(module, attr, None)
            if not callable(handler):
                _facade().logger.error('Hook handler not callable for mod %s: %r', mod_id, handler_spec)
                continue
...
def _invoke_mod_init_hook(init_fn: _facade().Any, *, mod_id: str | None=None) -> None:
    ...
    for p in params:
        if p.name == 'app':
            ...
            _facade().logger.warning('Skip mod init %s: cannot satisfy required parameter %r', getattr(init_fn, '__qualname__', init_fn), p.name)
            return
    try:
        sig.bind(**kwargs)
    except TypeError:
