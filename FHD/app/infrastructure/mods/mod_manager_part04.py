...
def register_employee_pack_routes(app, mod_manager: _facade().ModManager | None, pack_id: str, *, force: bool=False) -> bool:
    ...
    if not employee_pack_allowed_in_runtime(resolved_id, data):
        _facade().logger.debug('skip control-plane employee route in enterprise client: %s', resolved_id)
        return False
    ...
    try:
        ...
        _facade().logger.error('employee_pack route registration failed %s: %s', resolved_id, e, exc_info=True)
...
def _resolve_mod_metadata_for_http(mod_manager: _facade().ModManager, mod_id: str) -> _facade().ModMetadata | None:
    ...
    needs_disk = metadata is None or not (metadata.backend_entry or '').strip() or (not (metadata.mod_path or '').strip())
    if needs_disk:
...
def _register_single_mod_http_routes(app, mod_manager: _facade().ModManager, mod_id: str, *, force: bool=False) -> bool:
        ...
        if hasattr(module, 'register_fastapi_routes'):
            ...
            if callable(register_fastapi_fn):
                ...
                for old_route in routes_before:
                    endpoint_name = str(getattr(getattr(old_route, 'endpoint', None), '__name__', ''))
                    ...
                    if any((getattr(new_route, 'path', None) == old_path and old_methods.intersection(set(getattr(new_route, 'methods', set()) or set())) for new_route in new_routes)):
                        if app_router is not None:
                            app_router.routes.remove(old_route)
                        _facade().logger.info('Removed superseded host alias after loading Mod %s: %s', mid, old_path)
...
def _mod_allowed_for_api_load(mod_id: str, session_id: str | None=None) -> bool:
    ...
    if not mid:
        return False
    try:
        from app.enterprise.mod_entitlements import enterprise_mod_filter_active, is_mod_visible_for_enterprise
...
def ensure_mod_api_ready(mod_id: str, session_id: str | None=None) -> bool:
    ...
    employee_pack_ready = _facade().ensure_employee_pack_api_ready(mm, mid, registered_pack_ids=_facade()._employee_pack_routes_registered, register_routes=_facade().register_employee_pack_routes)
    ...
    if mid not in mm._loaded_mods:
        ...
        if retry_at > _facade().time.monotonic():
            _facade().logger.debug('[ModManager] ensure_mod_api_ready: load_mod(%s) retry delay active', mid)
            return False
        if not mm.load_mod(mid):
            _facade()._MOD_API_FAILURE_RETRY_AT[mid] = _facade().time.monotonic() + _facade()._MOD_API_FAILURE_BACKOFF_SECONDS
            from app.runtime_integrity import record_runtime_issue
            record_runtime_issue(f'industry_mod:{mid}', f'Industry MOD failed to load: {mid}', ttl_seconds=max(_facade()._MOD_API_FAILURE_BACKOFF_SECONDS * 2, 30.0))
...
def _entitled_client_mod_ids_for_api_mount(session_id: str | None=None) -> list[str]:
    ...
    try:
        from app.enterprise.mod_entitlements import enterprise_mod_filter_active, get_cached_entitled_client_mod_ids, is_client_mod_id, is_mod_visible_for_enterprise
    except _facade().RECOVERABLE_ERRORS:
        return []
    if not enterprise_mod_filter_active():
        return []
    ...
    try:
        ...
        candidates = {mid for mid in candidates if mid not in open_seed_ids or (mods_root / mid).is_dir()}
    except _facade().RECOVERABLE_ERRORS:
...
def load_mod_blueprints(app, mod_manager: _facade().ModManager | None=None) -> None:
    ...
    """
    _facade().logger.info('load_mod_blueprints: skipped (Mod routes use FastAPI load_mod_routes on main app)')
