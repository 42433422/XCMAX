...
class DatasetVectorSQLiteIndex:
            ...
            for chunk in chunks:
                ...
                conn.execute(
                    ...
                    (
                        ...
                        chunk.text,
                        _facade().json.dumps(
                            _facade()._embedding_from_metadata(metadata), ensure_ascii=False
                        ),
                        _facade().json.dumps(metadata, ensure_ascii=False),
                        chunk.source_url or "",
    ...
    def delete_dataset(self, dataset_id: str) -> bool:
        ...
        with self._get_conn() as conn:
            result = conn.execute(
                "DELETE FROM dataset_vector_indexes WHERE index_id = ?", (index_id,)
            )
            conn.commit()
    ...
    def query(
        ...
        *,
        ...
        chunks = _facade()._filter_chunks(
            chunks, tenant_id=tenant_id, version=version, metadata_filter=metadata_filter or {}
        )
