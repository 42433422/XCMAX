...
def view_excel_content(
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=1, max_row=min(max_rows, ws.max_row)):
            ...
            if row_data:
                content.append({"row": row[0].row, "cells": row_data})
                row_count += 1
...
def get_merged_cells(file_path: str, sheet_name: str | None = None) -> dict[str, Any]:
    """
    获取Excel文件中的合并单元格信息
...
def get_cell_styles(
    file_path: str, sheet_name: str | None = None, max_rows: int = 10
) -> dict[str, Any]:
    """
    获取单元格样式信息
                ...
                if cell.value is not None:
                    style_info = {
                        ...
                        "font": {
                            ...
                            "color": (
                                str(cell.font.color) if cell.font and cell.font.color else None
                            ),
                        },
                        "alignment": {
                        ...
                        "fill": {
                            ...
                            "fg_color": (
                                str(cell.fill.fgColor) if cell.fill and cell.fill.fgColor else None
                            ),
                        },
                    }
...
def analyze_structure(file_path: str, sheet_name: str | None = None) -> dict[str, Any]:
    """
    分析Excel文件结构
    ...
    try:
        ...
        for col in range(1, ws.max_column + 1):
            ...
            column_info.append({"column": col_letter, "index": col, "sample_values": sample_values})
...
class ExcelToolkitSkill:
    ...
    def execute(
        self, file_path: str, action: str = "view", sheet_name: str | None = None, **kwargs
    ) -> dict[str, Any]:
        """
        执行Excel工具操作
...
_skill_instance: ExcelToolkitSkill | None = None
