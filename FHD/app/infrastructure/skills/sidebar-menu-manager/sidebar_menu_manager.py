...
def get_menu_items() -> list[dict[str, str]]:
    ...
    for match in matches:
        menu_items.append({"key": match[0], "name": match[1], "icon": match[2]})

    return menu_items


def find_menu_items_block(content: str) -> str | None:
...
def remove_menu_item(key: str) -> dict[str, Any]:
    ...
    return {"success": True, "message": f"Removed menu item: {key}"}
...
def update_menu_item(key: str, name: str = None, icon: str = None) -> dict[str, Any]:
    ...
    return {"success": True, "message": f"Updated menu item: {key}"}
...
def reorder_menu_items(key: str, new_index: int) -> dict[str, Any]:
    ...
    if new_index < 0 or new_index >= len(items):
        return {
            "success": False,
            "message": f"Invalid new_index {new_index}. Must be between 0 and {len(items) - 1}",
        }
    ...
    items_str = ",\n".join(
        [f"  {{ key: '{i['key']}', name: '{i['name']}', icon: '{i['icon']}' }}" for i in items]
    )
