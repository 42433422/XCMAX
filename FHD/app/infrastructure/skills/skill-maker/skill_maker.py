...
def list_existing_skills() -> list[dict[str, str]]:
    ...
    for name in os.listdir(skills_dir):
        ...
        if os.path.isdir(skill_path):
            ...
            skills.append(
                {"name": name, "path": skill_path, "has_skill_md": os.path.exists(skill_md)}
            )
    return skills
...
def create_skill_directory(skill_name: str) -> dict[str, Any]:
    ...
    if os.path.exists(skill_path):
        return {"success": False, "message": f'Skill "{skill_name}" already exists at {skill_path}'}
...
def validate_skill(skill_name: str) -> dict[str, Any]:
    ...
    if not os.path.exists(skill_path):
        return {"valid": False, "message": f'Skill "{skill_name}" does not exist'}
