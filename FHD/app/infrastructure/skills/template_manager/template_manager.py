        ...
        result = db.execute(
            ...
            ),
            {
                ...
                "editable_config": json.dumps(
                    kwargs.get("editable_config", {}), ensure_ascii=False
                ),
                "zone_config": json.dumps(kwargs.get("zone_config", {}), ensure_ascii=False),
                "merged_cells_config": json.dumps(
                    kwargs.get("merged_cells_config", {}), ensure_ascii=False
                ),
...
def get_template_file(filename: str) -> dict | None:
    base_dir = get_base_dir()
    template_dir = os.path.join(base_dir, "templates")
    temp_dir = os.path.join(base_dir, "temp_excel")
