...
def _agent_node_output(run: Any, node_id: str) -> dict[str, Any]:
    ...
    output: dict[str, Any] = dict(node_outputs.get(node_id) or {})
    if not output:
...
def _products_compat_preflight(
    ...
    if gate:
        return cast("dict[Any, Any] | None", gate)
...
@router.get("/mod/taiyangniao-pro/shipment/shipment-records/units/", include_in_schema=False)
...
def products_list(
    ...
    try:
        ...
        if is_erp_products_via_service_enabled():
            return cast("dict[Any, Any]", products_list_via_service(request, page=page, per_page=per_page, keyword=keyword, unit=unit))
    except RECOVERABLE_ERRORS:
...
@router.post("/products/update/", response_model=None, include_in_schema=False)
def products_update(
...
@router.post("/products/add/", response_model=None, include_in_schema=False)
def products_add(
    request: Request, body: dict = Body(default_factory=dict)
) -> dict[str, Any] | JSONResponse:
...
@router.post("/products/delete/", response_model=None, include_in_schema=False)
def products_delete(
    request: Request, body: dict = Body(default_factory=dict)
) -> dict[str, Any] | JSONResponse:
    payload = dict(body or {})
...
@router.post("/products/batch-delete/", response_model=None, include_in_schema=False)
