...
async def tool_query_trae_usage(params: dict[str, _facade().Any], ctx: dict[str, _facade().Any]) -> dict[str, _facade().Any]:
    ...
    result_data: dict[str, _facade().Any] = {'sources': [], 'api_usage': None, 'local_state': None, 'config': None, 'trae_summary': {}}
    ...
    if jwt_path.is_file():
        ...
        try:
            ...
            resp = _httpx.get('https://trae.cn/api/v1/user/usage', headers={'Authorization': f'Bearer {jwt_token}', 'User-Agent': 'Trae/1.10.0', 'Content-Type': 'application/json'}, timeout=8, follow_redirects=True)
            if resp.status_code == 200:
                result_data['api_usage'] = resp.json()
            else:
                result_data['api_usage'] = {'status_code': resp.status_code, 'note': f'Trae API 返回 {resp.status_code}，token 用量需去 Trae 网页设置页查看'}
    ...
    if state_db.is_file():
        ...
        try:
            ...
            conn.close()
            result_data['local_state'] = {'user_id': user_id, 'accumulated_chat_turns': accumulated_turns, 'current_models': current_models, 'available_models_by_mode': {mode: [m.get('name', '') for m in models if isinstance(m, dict)] for (mode, models) in available_models.items()}, 'feedback_keys': list(feedback.keys())}
        except _facade().RECOVERABLE_ERRORS as exc:
    ...
    if argv_file.is_file():
        ...
        try:
            result_data['config'] = {'argv': _facade().json.loads(argv_file.read_text(encoding='utf-8'))}
        except _facade().RECOVERABLE_ERRORS:
            pass
    local = result_data.get('local_state') or {}
    result_data['trae_summary'] = {'chat_turns': local.get('accumulated_chat_turns', 0), 'current_models': local.get('current_models', {}), 'user_id': local.get('user_id', ''), 'api_accessible': bool(result_data.get('api_usage') and isinstance(result_data.get('api_usage'), dict) and ('status_code' not in result_data.get('api_usage', {}))), 'note': 'Trae token 用量 API 被 403 拦截。本地能提取聊天轮次和模型列表，精确 token 用量需去 Trae 设置页查看。'}
    return _facade()._ok(f"Trae 使用统计：{local.get('accumulated_chat_turns', 0)} 轮聊天，{len(result_data['sources'])} 个数据源", **result_data)
...
async def handle_specialized(employee_id: str, payload: dict[str, _facade().Any], ctx: dict[str, _facade().Any]) -> dict[str, _facade().Any]:
    """专属工具调度入口。
    ...
    if not tool_name:
        ...
        return _facade()._ok(f'员工 {employee_id} 可用 {len(available)} 个专属工具', employee_id=employee_id, available_tools=available, handler='specialized')
    allowed = _facade().get_employee_tools(employee_id)
    if tool_name not in allowed:
        return _facade()._err(f'工具 {tool_name!r} 不在员工 {employee_id} 的专属工具清单中。可用: {allowed}', employee_id=employee_id, available_tools=allowed)
    ...
    if tool_name in _facade()._code_write_tools():
        ...
        if not gate_verdict.get('ok', True):
            return _facade()._err(f"写操作被 gate 拦截: {gate_verdict.get('reason', '')}", blocked=True, gate_result=gate_verdict, pending_approval=bool(gate_verdict.get('pending_approval')), approval_request_ids=list(gate_verdict.get('approval_request_ids') or []))
    try:
