...
class _DeadLetterQueuePart02Mixin:
    ...
    def triage_entries(self) -> dict[str, list[str]]:
        ...
        """
        retriable_reasons = {_facade().DeadLetterReason.RETRY_EXHAUSTED, _facade().DeadLetterReason.TIMEOUT}
        fixable_reasons = {_facade().DeadLetterReason.INVALID_PAYLOAD, _facade().DeadLetterReason.HANDLER_NOT_FOUND}
        poison_reasons = {_facade().DeadLetterReason.UNRECOVERABLE, _facade().DeadLetterReason.CIRCUIT_BREAKER}
    ...
    def _trigger_alert(self, entry: _facade().DeadLetterEntry):
        ...
        告警内容含"X 条同类失败"。
        """
        (should_alert, count) = self._alert_suppressor.record_and_check(entry.reason, entry.original_event.event_type)
        if not should_alert:
            return
    ...
    def schedule_retry(self, entry_id: str, base: float=_facade().RETRY_BASE_DELAY, cap: float=_facade().RETRY_MAX_DELAY) -> float | None:
        """
        ...
        if entry is None:
            _facade().logger.warning('[DeadLetterQueue] schedule_retry 失败：找不到条目 %s', entry_id)
            return None
        ...
        _facade().logger.info('[DeadLetterQueue] 安排重试: entry_id=%s, retry=%s, delay=%.3fs', entry_id, new_retry_count, delay)
        return float(delay)
    ...
    def _evict_oldest(self):
        ...
        if self._conn is not None:
            with self._lock:
                ...
                self._conn.execute('DELETE FROM neuro_dead_letters WHERE entry_id = ?', (oldest_id,))
