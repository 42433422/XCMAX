...
class _EventStorePart01Mixin:
    ...
    def __init__(self, mode: _facade().EventStoreMode=_facade().EventStoreMode.MEMORY, storage_path: str | None=None, max_events: int=100000, max_snapshots_per_stream: int=3, upcaster_registry: _facade().UpcasterRegistry | None=None):
        self._mode = mode
        ...
        _facade().logger.info('[EventStore] 初始化完成 (mode=%s, max_events=%s, max_snapshots=%s)', mode.value, max_events, max_snapshots_per_stream)

    def _init_sqlite(self) -> None:
        """初始化 SQLITE 持久化存储"""
        if not self._storage_path:
            raise ValueError('[EventStore] SQLITE 模式必须提供 storage_path')
        self._conn = _facade().sqlite3.connect(self._storage_path, check_same_thread=False, isolation_level=None)
        ...
        self._conn.execute('CREATE INDEX IF NOT EXISTS idx_events_stream ON neuro_events(stream_id, sequence_number)')
        self._conn.execute('CREATE INDEX IF NOT EXISTS idx_events_type ON neuro_events(event_type)')
        self._conn.execute('CREATE INDEX IF NOT EXISTS idx_events_stored_at ON neuro_events(stored_at)')
        self._conn.execute('\n            CREATE TABLE IF NOT EXISTS neuro_snapshots (\n                snapshot_id TEXT PRIMARY KEY,\n                stream_id TEXT NOT NULL,\n                sequence_number INTEGER NOT NULL,\n                state TEXT NOT NULL,\n                created_at TEXT NOT NULL,\n                version INTEGER DEFAULT 1,\n                metadata TEXT\n            )\n            ')
        self._conn.execute('CREATE INDEX IF NOT EXISTS idx_snapshots_stream ON neuro_snapshots(stream_id)')
        ...
        cursor = self._conn.execute('SELECT COALESCE(MAX(sequence_number), 0) AS max_seq FROM neuro_events')
        row = cursor.fetchone()
        if row and row['max_seq']:
            self._sequence_counter = row['max_seq']
        _facade().logger.info('[EventStore] SQLITE 模式初始化完成 (path=%s, seq_start=%s)', self._storage_path, self._sequence_counter)

    def _row_to_stored_event(self, row: _facade().sqlite3.Row) -> _facade().StoredEvent:
        """将数据库行转换为 StoredEvent（自动应用 upcaster 链）"""
        event = _facade().NeuroEvent.from_dict(_facade().json.loads(row['event_data']), preserve_queue_identity=True)
        metadata = _facade().json.loads(row['metadata']) if row['metadata'] else {}
        if self._upcaster_registry is not None:
            schema_version = metadata.get('event_schema_version', 1)
            (new_payload, new_version) = self._upcaster_registry.upcast(event.event_type, event.payload, schema_version)
            if new_version != schema_version:
                event.payload = new_payload
                metadata['event_schema_version'] = new_version
        return _facade().StoredEvent(store_id=row['store_id'], event=event, stored_at=_facade().datetime.fromisoformat(row['stored_at']), sequence_number=row['sequence_number'], stream_id=row['stream_id'], metadata=metadata)
    ...
    def _apply_upcasters_to_stored(self, stored: _facade().StoredEvent) -> _facade().StoredEvent:
        ...
        (new_payload, new_version) = self._upcaster_registry.upcast(stored.event.event_type, stored.event.payload, schema_version)
        if new_version == schema_version:
            return stored
        new_metadata = dict(stored.metadata)
        new_metadata['event_schema_version'] = new_version
        new_event = _facade().NeuroEvent(event_type=stored.event.event_type, payload=new_payload, priority=stored.event.priority, metadata=stored.event.metadata, preserve_queue_identity=True)
        return _facade().StoredEvent(store_id=stored.store_id, event=new_event, stored_at=stored.stored_at, sequence_number=stored.sequence_number, stream_id=stored.stream_id, metadata=new_metadata)
    ...
    def _get_stream_version(self, stream_id: str) -> int:
        ...
        if self._mode == _facade().EventStoreMode.SQLITE and self._conn is not None:
            cursor = self._conn.execute('SELECT COUNT(*) AS cnt FROM neuro_events WHERE stream_id = ? AND is_deleted = 0', (stream_id,))
            row = cursor.fetchone()
    ...
    def append(self, event: _facade().NeuroEvent, stream_id: str | None=None, expected_version: int | None=None) -> str:
        """
        存储事件
        ...
        with self._lock:
            ...
            if self._upcaster_registry is not None:
                event_metadata['event_schema_version'] = self._upcaster_registry.get_current_version(event.event_type)
            stored = _facade().StoredEvent(store_id=store_id, event=event, stored_at=_facade().datetime.now(), sequence_number=sequence_number, stream_id=stream_id, metadata=event_metadata)
            if self._mode == _facade().EventStoreMode.SQLITE and self._conn is not None:
        ...
        _facade().logger.debug('[EventStore] 事件存储: %s (store_id=%s)', event.event_type, store_id)
    ...
    def _append_sqlite(self, stored: _facade().StoredEvent) -> None:
        ...
        if current_count >= self._max_events:
            self._cleanup_oldest()
        with self._conn:
            self._conn.execute('\n                INSERT INTO neuro_events\n                    (store_id, event_type, event_data, stream_id,\n                     sequence_number, stored_at, metadata, is_deleted)\n                VALUES (?, ?, ?, ?, ?, ?, ?, 0)\n                ', (stored.store_id, stored.event.event_type, stored.event.to_json(), stored.stream_id, stored.sequence_number, stored.stored_at.isoformat(), _facade().json.dumps(stored.metadata, ensure_ascii=False) if stored.metadata else None))

    def append_many(self, events: list[_facade().NeuroEvent], stream_id: str | None=None, expected_version: int | None=None) -> list[str]:
        """
        批量存储事件
            ...
            with self._lock:
                ...
                with self._conn:
                    for event in events:
                        ...
                        if self._upcaster_registry is not None:
                            event_metadata['event_schema_version'] = self._upcaster_registry.get_current_version(event.event_type)
                        stored = _facade().StoredEvent(store_id=store_id, event=event, stored_at=_facade().datetime.now(), sequence_number=self._sequence_counter, stream_id=stream_id, metadata=event_metadata)
                        self._conn.execute('\n                            INSERT INTO neuro_events\n                                (store_id, event_type, event_data, stream_id,\n                                 sequence_number, stored_at, metadata, is_deleted)\n                            VALUES (?, ?, ?, ?, ?, ?, ?, 0)\n                            ', (stored.store_id, stored.event.event_type, stored.event.to_json(), stored.stream_id, stored.sequence_number, stored.stored_at.isoformat(), _facade().json.dumps(stored.metadata, ensure_ascii=False) if stored.metadata else None))
    ...
    def append_with_retry(self, stream_id: str, build_events: _facade().Callable[[list[_facade().StoredEvent]], list[_facade().NeuroEvent]], max_retries: int=3, base_delay: float=0.01) -> list[str]:
        """
        乐观并发重试追加
        ...
        for attempt in range(max_retries + 1):
            ...
            if not new_events:
                return []
            try:
                return self.append_many(new_events, stream_id=stream_id, expected_version=expected_version)
            except _facade().WrongExpectedVersionError as e:
                last_error = e
                if attempt < max_retries:
                    delay = base_delay * 2 ** attempt
                    _facade().logger.warning('[EventStore] 乐观并发冲突，%ss 后重试 (attempt=%s/%s, stream=%s)', delay, attempt + 1, max_retries, stream_id)
    ...
    def get(self, store_id: str) -> _facade().StoredEvent | None:
        """获取单个事件"""
        if self._mode == _facade().EventStoreMode.SQLITE and self._conn is not None:
            cursor = self._conn.execute('SELECT * FROM neuro_events WHERE store_id = ? AND is_deleted = 0', (store_id,))
            row = cursor.fetchone()
            if row is None:
                return None
    ...
    def get_all(self, start_time: _facade().datetime | None=None, end_time: _facade().datetime | None=None, event_type: str | None=None) -> _facade().Iterator[_facade().StoredEvent]:
        """
        获取事件流
    ...
    def get_stream_events(self, stream_id: str, from_sequence: int=0) -> list[_facade().StoredEvent]:
        """
        获取事件流中的所有事件
    ...
    def _compute_state_hash(state: dict[str, _facade().Any]) -> str:
        ...
        return _facade().hashlib.sha256(_facade().json.dumps(state, sort_keys=True, ensure_ascii=False).encode()).hexdigest()

    def save_snapshot(self, stream_id: str, state: dict[str, _facade().Any], sequence_number: int) -> str:
        """
        保存聚合根快照
        ...
        snapshot = _facade().Snapshot(snapshot_id=snapshot_id, stream_id=stream_id, sequence_number=sequence_number, state=state, created_at=_facade().datetime.now(), metadata={'state_hash': state_hash})
        if self._mode == _facade().EventStoreMode.SQLITE and self._conn is not None:
            with self._lock:
                with self._conn:
                    self._conn.execute('\n                        INSERT INTO neuro_snapshots\n                            (snapshot_id, stream_id, sequence_number,\n                             state, created_at, version, metadata)\n                        VALUES (?, ?, ?, ?, ?, ?, ?)\n                        ', (snapshot.snapshot_id, snapshot.stream_id, snapshot.sequence_number, _facade().json.dumps(snapshot.state, ensure_ascii=False), snapshot.created_at.isoformat(), snapshot.version, _facade().json.dumps(snapshot.metadata, ensure_ascii=False)))
        ...
        _facade().logger.debug('[EventStore] 快照保存: %s (seq=%s, snap_id=%s, hash=%s)', stream_id, sequence_number, snapshot_id, state_hash[:8])
        return snapshot_id

    def _row_to_snapshot(self, row: _facade().sqlite3.Row) -> _facade().Snapshot:
        """将数据库行转换为 Snapshot"""
        metadata = _facade().json.loads(row['metadata']) if row['metadata'] else {}
        return _facade().Snapshot(snapshot_id=row['snapshot_id'], stream_id=row['stream_id'], sequence_number=row['sequence_number'], state=_facade().json.loads(row['state']), created_at=_facade().datetime.fromisoformat(row['created_at']), version=row['version'], metadata=metadata)
    ...
    def replay(self, start_time: _facade().datetime | None=None, end_time: _facade().datetime | None=None, event_types: list[str] | None=None, stream_id: str | None=None, callback: _facade().Callable[[_facade().NeuroEvent], None] | None=None) -> int:
        """
        重播事件
        ...
        for stored in events:
            ...
            if callback:
                ...
                _facade().logger.info('[EventStore] 重播: %s (store_id=%s)', stored.event.event_type, stored.store_id)
                count += 1
