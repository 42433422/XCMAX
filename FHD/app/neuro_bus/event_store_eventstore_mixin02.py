...
class _EventStorePart02Mixin:
    def replay_stream(
                ...
                if stored_hash is not None:
                    ...
                    if stored_hash != actual_hash:
                        _facade().logger.warning(
                            "[EventStore] 快照 state_hash 校验失败，丢弃快照全量重放: stream=%s snap_id=%s expected=%s actual=%s",
                            stream_id,
        ...
        if snapshot:
            ...
            result["snapshot_age_seconds"] = (
                _facade().datetime.now() - snapshot.created_at
            ).total_seconds()
    ...
    def get_stats(self) -> dict[str, _facade().Any]:
        ...
        return {
            ...
            "by_stream": by_stream,
            "oldest_event": min(e.stored_at for e in self._events.values()).isoformat()
            if self._events
            else None,
            "newest_event": max(e.stored_at for e in self._events.values()).isoformat()
            if self._events
            else None,
        }
        ...
        if self._mode == _facade().EventStoreMode.SQLITE and self._conn is not None:
            with self._lock:
                with self._conn:
                    ...
                    self._conn.execute(
                        "DELETE FROM neuro_snapshots WHERE stream_id = ?", (stream_id,)
                    )
