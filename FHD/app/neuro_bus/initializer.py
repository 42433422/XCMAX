...
class NeuroBusInitializer:
    ...
    def initialize(self, event_store_mode: EventStoreMode = EventStoreMode.MEMORY) -> bool:
        ...
        try:
            ...
            logger.info("[NeuroBusInitializer] ================================")
            logger.info("[NeuroBusInitializer] 已初始化 Level 4 组件:")
            logger.info("  - 去重器 (Deduplicator)")
            logger.info("  - 熔断保护 (Circuit Breaker)")
            logger.info("  - 重试处理器 (Retry Handler)")
            logger.info("  - 死信队列 (DLQ)")
            logger.info("  - 事件存储 (Event Store)")
            logger.info("  - 健康监控 (Health Monitor)")
            logger.info("[NeuroBusInitializer] 总线级可靠性机制（按环境变量懒加载）:")
            logger.info("  - 去重器 (Dedup) / 限流器 (Rate Limiter) / 熔断器 (Circuit)")
            logger.info("  - 保命通道 (Lifeline) / 链路追踪 (Tracer)")
            logger.info("  - SLA 控制器 (SLAController) / 重试 (Retry)")
            logger.info("  - 死信队列自动转移 (DLQ_AUTO)")
