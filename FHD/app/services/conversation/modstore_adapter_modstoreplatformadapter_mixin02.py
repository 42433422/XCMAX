...
class _ModstorePlatformAdapterPart02Mixin:
    ...
    def stream_chat_completion_sync(self, messages: _facade().List[_facade().Dict[str, _facade().Any]], temperature: float=0.7, max_tokens: int=2000, provider: str | None=None, model: str | None=None, **kwargs) -> _facade().Iterator[str]:
        ...
        for (idx, (prov, mdl)) in enumerate(candidates):
            ...
            payload: _facade().Dict[str, _facade().Any] = {'provider': prov, 'model': mdl, 'messages': prepared, 'temperature': temperature, 'max_tokens': max_tokens, 'stream': True, 'allow_failover': idx == 0, **kwargs}
            if self.user_id:
                payload['user_id'] = self.user_id
            _facade().logger.info('[Modstream] 启动同步流式请求: %s/%s (attempt %s/%s)', prov, mdl, idx + 1, len(candidates))
            retry_next = False
            with _facade()._httpx_sync_client(timeout=_facade().httpx.Timeout(self.timeout, connect=_facade()._market_connect_timeout()), limits=_facade().httpx.Limits(max_keepalive_connections=10, max_connections=30), headers=self._build_headers()) as client:
                with client.stream('POST', url, json=payload) as response:
                    ...
                    if status_code >= 400:
                        ...
                        if idx + 1 < len(candidates) and _facade().is_market_chat_failoverable(status_code, error_text):
                            _facade().logger.warning('[Modstream] 流式换模 %s/%s -> %s', prov, mdl, candidates[idx + 1])
                            last_error = err
                            retry_next = True
                        else:
                            _facade().logger.error('[Modstream] 平台同步流式返回错误 %s: %s', status_code, error_text)
                            raise err
                    else:
    ...
    async def get_credential_status(self, provider: str | None=None) -> _facade().Dict[str, _facade().Any]:
        """
        获取指定供应商的密钥状态
