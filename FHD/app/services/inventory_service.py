...
class InventoryService:
    ...
    # ── 按单履行入口（W1-03）：预留 / 扣减 / 回补 ─────────────────
    # 这些方法作为 stock-move 载体，全部落于 inventory_transactions + inventory_ledger，
    # 支持传入 db 复用在订单事务内（由履行层保证原子提交），也可独立调用（自开会话）。
        ...
        *,
        ...
        def _run(db):
            ...
            txn = InventoryTransaction(
                ...
                batch_no=batch_no,
                quantity=Decimal("0"),
                before_quantity=Decimal(str(ledger.quantity or 0)),
                after_quantity=Decimal(str(ledger.quantity or 0)),
            ...
            return {
                "success": True,
                "ledger_id": ledger.id,
                "reserved_quantity": float(ledger.reserved_quantity),
                "available_quantity": float(ledger.available_quantity),
            }
    ...
    def deduct_for_order(
        ...
        *,
        ...
        def _run(db):
            ...
            ledger.available_quantity = Decimal(str(ledger.quantity)) - Decimal(
                str(ledger.reserved_quantity)
            )
            ledger.updated_at = now
            db.flush()
            txn = InventoryTransaction(
                ...
                after_quantity=Decimal(str(ledger.quantity)),
                ordered_quantity=qty,
        ...
        with get_db() as db:
            try:
                ...
                db.rollback()
                logger.error("按单扣减失败: %s", e)
                return {"success": False, "message": str(e)}

    def restock_for_order(
        self,
        product_id: int,
        warehouse_id: int,
        quantity: float,
        *,
        ...
        def _run(db):
            ...
            return {
                ...
                "available_quantity": float(ledger.available_quantity or 0),
            }
