...
class KittenReportExportService:
    ...
    def collect_plugin_results(self, payload: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        return [self._plugin_to_dict(plugin.run(plugin_payload)) for plugin in self.plugins]
    ...
    def build_report(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        plugin_results = self.collect_plugin_results(payload)
        ...
        *,
        ...
        if dataset:
            ...
            ws_summary.append(
                [
                    "数据规模",
                    f"{int(dataset.get('rows') or 0)} 行 / {int(dataset.get('columns') or 0)} 列",
                ]
            )
    ...
    def _add_financial_sheet(self, wb: Workbook, plugin_results: list[dict[str, Any]]) -> None:
        financial_plugin = next(
            (p for p in plugin_results if p.get("key") == "financial_report"), None
        )
        inventory_plugin = next(
            (p for p in plugin_results if p.get("key") == "inventory_valuation"), None
        )
        ...
        current_row = 1
        ws_fin.cell(row=current_row, column=1, value="📊 小猫财务分析报告").font = Font(
            bold=True, size=16
        )
        ...
        if financial_plugin:
            ...
            ws_fin.cell(row=current_row, column=1, value="核心财务指标").font = Font(
                bold=True, size=12
            )
            current_row += 1
            ...
            if monthly_data:
                current_row += 2
                ws_fin.cell(row=current_row, column=1, value="月度营收趋势").font = Font(
                    bold=True, size=12
                )
                current_row += 1
                ...
                for month in monthly_data:
                    ...
                    ws_fin.cell(
                        row=current_row, column=3, value=month.get("order_count", 0)
                    ).border = thin_border
                    current_row += 1
            ...
            if product_analysis:
                current_row += 2
                ws_fin.cell(row=current_row, column=1, value="产品销售排行 (Top 10)").font = Font(
                    bold=True, size=12
                )
                current_row += 1
                ...
                for product in product_analysis[:10]:
                    ws_fin.cell(
                        row=current_row, column=1, value=product.get("product_name")
                    ).border = thin_border
                    ws_fin.cell(
                        row=current_row, column=2, value=round(product.get("total_revenue", 0), 2)
                    ).border = thin_border
                    ws_fin.cell(
                        row=current_row, column=3, value=round(product.get("total_qty", 0), 2)
                    ).border = thin_border
                    ws_fin.cell(
                        row=current_row, column=4, value=product.get("order_count", 0)
                    ).border = thin_border
                    current_row += 1
            ...
            if customer_analysis:
                current_row += 2
                ws_fin.cell(row=current_row, column=1, value="客户销售排行 (Top 10)").font = Font(
                    bold=True, size=12
                )
                current_row += 1
                ...
                for customer in customer_analysis[:10]:
                    ws_fin.cell(
                        row=current_row, column=1, value=customer.get("customer")
                    ).border = thin_border
                    ws_fin.cell(
                        row=current_row, column=2, value=round(customer.get("total_amount", 0), 2)
                    ).border = thin_border
                    ws_fin.cell(
                        row=current_row, column=3, value=customer.get("order_count", 0)
                    ).border = thin_border
                    ws_fin.cell(
                        row=current_row,
                        column=4,
                        value=round(customer.get("avg_order_value", 0), 2),
                    ).border = thin_border
                    current_row += 1
        ...
        if inventory_plugin:
            ...
            ws_fin.cell(row=current_row, column=1, value="📦 库存价值评估").font = Font(
                bold=True, size=12
            )
            current_row += 1
            ...
            if low_stock:
                current_row += 2
                ws_fin.cell(row=current_row, column=1, value="⚠️ 低库存预警").font = Font(
