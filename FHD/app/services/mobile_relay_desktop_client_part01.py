...
def _migrate_legacy_config_once() -> None:
    ...
    try:
        ...
        _facade()._CONFIG_FILE.write_text(_facade()._LEGACY_CONFIG_FILE.read_text(encoding='utf-8'), encoding='utf-8')
        _facade().logger.info('迁移历史云中继配对凭证 %s -> %s', _facade()._LEGACY_CONFIG_FILE, _facade()._CONFIG_FILE)
    except OSError:
...
def _ensure_super_employee_service_classes() -> None:
    ...
    if _facade().ClaudeSuperEmployeeService is not None and _facade().CodexSuperEmployeeService is not None and (_facade().CursorSuperEmployeeService is not None) and (_facade().TraeSuperEmployeeService is not None):
        return
    if _facade().ClaudeSuperEmployeeService is None:
        from app.application.claude_super_employee_service import ClaudeSuperEmployeeService as _ClaudeSuperEmployeeService
        _facade().ClaudeSuperEmployeeService = _ClaudeSuperEmployeeService
    if _facade().CodexSuperEmployeeService is None:
        from app.application.codex_super_employee_service import CodexSuperEmployeeService as _CodexSuperEmployeeService
        _facade().CodexSuperEmployeeService = _CodexSuperEmployeeService
    if _facade().CursorSuperEmployeeService is None:
        from app.application.cursor_super_employee_service import CursorSuperEmployeeService as _CursorSuperEmployeeService
        _facade().CursorSuperEmployeeService = _CursorSuperEmployeeService
    if _facade().TraeSuperEmployeeService is None:
        from app.application.trae_super_employee_service import TraeSuperEmployeeService as _TraeSuperEmployeeService
...
def _relay_base_url() -> str:
    value = (_facade().os.environ.get('XCAGI_RELAY_BASE_URL') or _facade().os.environ.get('XCAGI_PUBLIC_FHD_BASE_URL') or 'https://xiu-ci.com/fhd-api').strip()
...
def _read_config() -> dict[str, _facade().Any]:
    try:
        ...
        _facade().logger.warning('mobile relay desktop config is unreadable: %s', _facade()._CONFIG_FILE, exc_info=True)
        return {}

def _public_payload_from_config(config: dict[str, _facade().Any]) -> dict[str, _facade().Any] | None:
    ...
    if exp <= 0:
        ...
        if registered_at > 0:
            exp = registered_at + int(_facade().os.environ.get('XCAGI_RELAY_PAIRING_TTL_SEC') or '86400')
    if exp > 0 and exp <= int(_facade().time.time()):
        return None
    expires_at = str(config.get('expires_at') or '').strip()
    return {'relay_id': relay_id, 'pairing_code': pairing_code, 'paired': bool(config.get('paired')), 'mobile_username': str(config.get('mobile_username') or '').strip(), 'last_relay_sync_at': int(config.get('last_relay_sync_at') or 0), **({'expires_at': expires_at} if expires_at else {}), **({'exp': exp} if exp > 0 else {}), 'relay_base_url': base_url, 'qr_json': {'v': 3, 'kind': 'xcagi_relay_pairing', 'relay_id': relay_id, 'code': pairing_code, 't': pairing_code, 'relay_base_url': base_url}}
...
def cached_desktop_relay_payload() -> dict[str, _facade().Any] | None:
    ...
    if not relay_id:
        return None
    return {'relay_id': relay_id, 'relay_base_url': str(config.get('relay_base_url') or '') or _facade()._relay_base_url(), 'paired': True, 'mobile_username': str(config.get('mobile_username') or '').strip(), 'last_relay_sync_at': int(config.get('last_relay_sync_at') or 0)}

def _write_config(data: dict[str, _facade().Any]) -> None:
    _facade()._CONFIG_FILE.parent.mkdir(parents=True, exist_ok=True)
    _facade()._CONFIG_FILE.write_text(_facade().json.dumps(data, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')

def register_desktop_relay(*, host: str, port: int, label: str='', force_new: bool=False) -> dict[str, _facade().Any] | None:
    ...
    if not force_new:
        ...
        has_identity = bool(str(existing.get('relay_id') or '').strip() and str(existing.get('desktop_token') or '').strip())
        ...
        if has_identity and (existing.get('paired') or valid_payload):
            ...
            return valid_payload or {'relay_id': str(existing.get('relay_id') or ''), 'relay_base_url': str(existing.get('relay_base_url') or '') or _facade()._relay_base_url(), 'paired': bool(existing.get('paired'))}
    base_url = _facade()._relay_base_url()
    device_label = label.strip() or f'XCAGI 桌面执行端 - {_facade().socket.gethostname()}'
    body = {'label': device_label, 'device_id': _facade().get_stable_device_id(), 'relay_base_url': base_url, 'capabilities': {'codex': True, 'codex_cli': True, 'claude': True, 'claude_cli': True, 'cursor': True, 'cursor_cli': True, 'trae': True, 'trae_cli': True, 'desktop': True, 'host': host, 'port': int(port), 'platform': _facade().platform.platform()}}
    ...
    try:
        with _facade()._relay_http_client(timeout) as client:
            resp = client.post(_facade()._api_url('/api/mobile/v1/relay/desktop/register', base_url), json=body)
    ...
    if not isinstance(data, dict) or not data.get('desktop_token') or (not data.get('relay_id')):
        ...
        return None
    config = {'relay_id': str(data.get('relay_id') or ''), 'desktop_token': str(data.get('desktop_token') or ''), 'relay_base_url': str(data.get('relay_base_url') or base_url), 'pairing_code': str(data.get('pairing_code') or ''), 'expires_at': str(data.get('expires_at') or ''), 'exp': int(data.get('exp') or 0), 'registered_at': int(_facade().time.time()), 'label': device_label}
...
def _gc_orphan_workspaces() -> int:
    ...
    """
    try:
        max_age = float(_facade().os.environ.get('XCAGI_RELAY_WORKSPACE_GC_MAX_AGE_SEC') or str(6 * 3600))
    except (TypeError, ValueError):
        max_age = 6 * 3600.0
...
def start_desktop_relay_poller() -> bool:
    ...
    with _facade()._STATE_LOCK:
        ...
        _facade()._WORKER_THREAD = _facade().threading.Thread(target=_facade()._poll_loop, name='xcagi-mobile-relay-desktop', daemon=True)
...
def stop_desktop_relay_poller() -> None:
    ...
    if worker.is_alive():
        facade.logger.warning('mobile relay poller did not stop within %.1fs', request_timeout + 1.0)

def _relay_poll_backoff_seconds(failure_count: int, *, base_interval: float, max_interval: float) -> float:
    ...
    return _facade().cast('float', min(max_interval, max(1.0, base_interval) * 2 ** min(failures - 1, 8)))
...
def _poll_loop() -> None:
    ...
    while not _facade()._STOP_EVENT.is_set():
        ...
        try:
            _facade()._poll_once()
            if failure_count:
                _facade().logger.info('mobile relay poll recovered after %d failure(s)', failure_count)
            failure_count = 0
        except (_facade().httpx.HTTPError, _facade().httpx.InvalidURL, ValueError) as exc:
            failure_count += 1
            wait_seconds = _facade()._relay_poll_backoff_seconds(failure_count, base_interval=interval, max_interval=max_backoff)
            if failure_count == 1 or failure_count & failure_count - 1 == 0:
                _facade().logger.warning('mobile relay unavailable; retry in %.0fs (failure %d): %s', wait_seconds, failure_count, exc)
            else:
                _facade().logger.debug('mobile relay remains unavailable: %s', exc)
        except _facade().RECOVERABLE_ERRORS as exc:
            failure_count += 1
            wait_seconds = _facade()._relay_poll_backoff_seconds(failure_count, base_interval=interval, max_interval=max_backoff)
            _facade().logger.warning('mobile relay poll failed; retry in %.0fs: %s', wait_seconds, exc)
        _facade()._STOP_EVENT.wait(wait_seconds)

def _complete_relay_task(task: dict[str, _facade().Any], relay_id: str, desktop_token: str, base_url: str) -> None:
    ...
    try:
        ...
        if relay_status in _facade()._FAILED_STATUSES or relay_status in _facade()._BLOCKED_STATUSES:
        ...
        with _facade()._relay_http_client(timeout) as client:
            client.post(_facade()._api_url(f'/api/mobile/v1/relay/desktop/tasks/{task_id}/complete', base_url), json={'relay_id': relay_id, 'desktop_token': desktop_token, 'status': relay_status, 'result': result}).raise_for_status()
    except _facade().RECOVERABLE_ERRORS:
...
def _poll_once() -> None:
    ...
    timeout = float(_facade().os.environ.get('XCAGI_RELAY_POLL_TIMEOUT_SEC') or '30')
    with _facade()._relay_http_client(timeout) as client:
        resp = client.post(_facade()._api_url('/api/mobile/v1/relay/desktop/poll', base_url), json={'relay_id': relay_id, 'desktop_token': desktop_token, 'max_tasks': free})
    ...
    if isinstance(desktop, dict):
        is_paired = str(desktop.get('status') or '') == 'paired' or int(desktop.get('mobile_user_id') or 0) > 0
    ...
    for task in tasks:
        ...
        _facade().threading.Thread(target=_facade()._complete_relay_task, args=(task, relay_id, desktop_token, base_url), name=f'relay-task-{task_id[:8]}', daemon=True).start()

def _extract_tool_calls(assistant: dict[str, _facade().Any], tool_label: str) -> list[dict[str, _facade().Any]]:
    ...
    if m:
        calls.append({'action': 'create_branch', 'icon': 'branch', 'label': f'创建分支 {m.group(1)}', 'detail': m.group(1)})
    m = re.search('验证[：:]\\s*(通过|未通过)[（(]([^)）]*)', body)
    if m:
        ok = m.group(1) == '通过'
        calls.append({'action': 'verify', 'icon': 'check', 'label': f"验证{('通过' if ok else '未通过')}", 'detail': m.group(2)[:200], 'success': ok})
    m = re.search('推送[：:]\\s*(.+?)(?:\\n|$)', body)
    if m:
        push_text = m.group(1).strip()[:200]
        calls.append({'action': 'push', 'icon': 'upload', 'label': '推送分支', 'detail': push_text, 'success': '成功' in push_text or '已推送' in push_text})
    calls.insert(0, {'action': 'cli_run', 'icon': 'terminal', 'label': f'{tool_label} CLI 执行', 'detail': '调用无头 agent 修改代码'})
    return calls
