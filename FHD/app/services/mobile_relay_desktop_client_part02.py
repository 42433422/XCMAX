...
def _execute_task(task: dict[str, _facade().Any]) -> dict[str, _facade().Any]:
    ...
    message = str(payload.get('message') or payload.get('body') or payload.get('prompt') or payload.get('task') or '').strip()
    if not message:
    ...
    if claude_service_class is None or codex_service_class is None or cursor_service_class is None or (trae_service_class is None):
    ...
    if not isinstance(context, dict):
        context = {}
    workspace_root = str(context.get('workspace_root') or payload.get('workspace_root') or '').strip()
    orig_surface = str(context.get('client_surface') or '').strip().lower()
    is_work_order = orig_surface == 'ai_group' or bool(str(context.get('work_order_id') or '').strip()) or bool(str(context.get('assigned_task') or '').strip())
    context = {**context, 'source': 'mobile_relay', 'relay_task_id': str(task.get('task_id') or ''), 'client_surface': orig_surface or 'mobile', 'target_devices': ['all'], 'force_cli_direct': True}
    ...
    if is_work_order:
        context['mode'] = 'code'
    elif str(context.get('mode') or '').strip().lower() in {'code', 'task', 'dispatch', 'dev', 'develop'}:
    ...
    try:
        ...
        if dispatch_status == 'completed':
            ...
            (ok, relay_status, error) = _facade()._classify_terminal_result(assistant, message=message)
            tool_calls = _facade()._extract_tool_calls(assistant, tool_label)
            if tool_calls:
                result['tool_calls'] = tool_calls
            if ok:
                return {'ok': True, 'codex': result, '_relay_status': 'completed'}
            return {'ok': False, 'error': error or f'{tool_label} 回写显示任务未完成', 'codex': result, '_relay_status': relay_status}
        ...
        timeout = max(0.0, float(_facade().os.environ.get('XCAGI_RELAY_CODEX_WAIT_TIMEOUT_SEC') or '300'))
        interval = max(0.05, float(_facade().os.environ.get('XCAGI_RELAY_CODEX_WAIT_INTERVAL_SEC') or '2'))
        deadline = _facade().time.monotonic() + timeout
        while True:
            terminal = _facade()._terminal_codex_message(service.list_messages(user_id=user_id, limit=200), request_id=request_id, task_id=task_id)
            if terminal:
                result['assistant_message'] = terminal
                (ok, relay_status, error) = _facade()._classify_terminal_result(terminal, message=message)
                if ok:
                    return {'ok': True, 'codex': result, '_relay_status': 'completed'}
                return {'ok': False, 'error': error or f'{tool_label} 回写显示任务未完成', 'codex': result, '_relay_status': relay_status}
        ...
        return {'error': f'{tool_label} 已派发，但在 {timeout:g} 秒内未回写{suffix}', 'codex': result, '_relay_status': 'blocked'}
    except _facade().RECOVERABLE_ERRORS as exc:
        _facade().logger.exception('mobile relay Codex task failed')
        return {'error': str(exc)[:1000]}

def _git_op_from_message(payload: dict[str, _facade().Any], message: str) -> tuple[str, dict[str, _facade().Any]] | None:
    ...
    if explicit in _facade().GIT_OP_KINDS:
        ...
        return None
    context = payload.get('context') if isinstance(payload.get('context'), dict) else {}
    text_source = _facade()._extract_branch_after(text, 'SOURCE_', '_TARGET') or _facade()._extract_merge_source(text)
    allow_selected_branch = explicit_git_op or _facade()._text_mentions_branch_op(text, lowered)
    if not isinstance(context, dict):
        context = {}
    source = str(payload.get('source_branch') or '').strip() or text_source or (str(payload.get('branch') or '').strip() if allow_selected_branch else '') or (str(context.get('branch') or '').strip() if allow_selected_branch else '')
    target = str(payload.get('target_branch') or payload.get('target') or payload.get('base') or '').strip() or _facade()._extract_target_branch(text)
...
def _text_mentions_branch_op(text: str, lowered: str) -> bool:
    return any((marker in text or marker in lowered for marker in ('合并分支', '这个分支', '当前分支', '待合并分支', 'merge branch', 'current branch', 'source branch', 'target branch', '查看分支', '丢弃分支', '删除分支')))

def _extract_branch_after(text: str, prefix: str, suffix: str) -> str:
    pattern = _facade().re.compile(f'{_facade().re.escape(prefix)}(?P<branch>[A-Za-z0-9._/-]+?){_facade().re.escape(suffix)}', _facade().re.IGNORECASE)
    match = pattern.search(text)
    return _facade()._trim_branch_token(match.group('branch')) if match else ''

def _extract_target_branch(text: str) -> str:
    match = _facade().re.search('TARGET(?:_CURRENT)?_(?P<branch>[A-Za-z0-9._/-]+)', text, _facade().re.IGNORECASE)
    return _facade()._trim_branch_token(match.group('branch')) if match else ''

def _extract_merge_source(text: str) -> str:
    match = _facade().re.search('(?:合并|merge)\\s+(?:分支\\s*)?(?P<branch>[A-Za-z0-9._/-]+)', text, _facade().re.IGNORECASE)
    return _facade()._trim_branch_token(match.group('branch')) if match else ''

def _extract_merge_target(text: str) -> str:
    match = _facade().re.search('(?:到|至|into|->)\\s*(?:分支\\s*)?(?P<branch>[A-Za-z0-9._/-]+)', text, _facade().re.IGNORECASE)
    return _facade()._trim_branch_token(match.group('branch')) if match else ''

def _trim_branch_token(value: str) -> str:
    branch = str(value or '').strip().strip('，,。.;；')
    for marker in ('_CHECK', '_IF', '_RUN', '_REPORT', '_DO', '_SAFE', '_STATUS', '_FIRST', '_THEN'):
...
def _classify_terminal_result(row: dict[str, _facade().Any], *, message: str) -> tuple[bool, str, str]:
    ...
    if body and _facade()._body_indicates_unfinished(body):
        ...
        return (False, relay_status, _facade()._terminal_error_summary(body, '执行端回写显示未完成'))
    if _facade()._message_requires_execution_evidence(message) and (not _facade()._body_has_execution_evidence(body)):
        return (False, 'blocked', _facade()._terminal_error_summary(body, '执行端回写缺少改动文件、命令、测试、构建或手机复测证据'))
...
def _body_indicates_unfinished(body: str) -> bool:
    ...
    return any((marker in body or marker.replace(' ', '') in compact for marker in _facade()._FAILURE_BODY_MARKERS))

def _body_indicates_failed(body: str) -> bool:
    return any((marker in body for marker in ('失败', 'failed', '合并有冲突', 'merge conflict', '验证未通过', '❌', 'error', 'Error')))
...
def _terminal_codex_message(messages: list[dict[str, _facade().Any]], *, request_id: str, task_id: str) -> dict[str, _facade().Any] | None:
