...
def install_modstore_office_compat_alias() -> bool:
    """Expose this dependency-free contract through its legacy import path.

    Official Office employee packs are also distributed by MODstore and some
    released packs dynamically import ``modstore_server.office_plaintext_generate``.
    Desktop builds intentionally omit the full MODstore service, so the host
    supplies only this shared contract when the real package is unavailable.
    """

    legacy_name = "modstore_server.office_plaintext_generate"
    try:
        importlib.import_module(legacy_name)
        return False
    except ModuleNotFoundError as exc:
        if exc.name not in {"modstore_server", legacy_name}:
            raise

    package = sys.modules.get("modstore_server")
    if package is None:
        package = types.ModuleType("modstore_server")
        package.__path__ = []  # type: ignore[attr-defined]
        package.__package__ = "modstore_server"
        sys.modules["modstore_server"] = package

    current = sys.modules[__name__]
    sys.modules[legacy_name] = current
    setattr(package, "office_plaintext_generate", current)
    return True
