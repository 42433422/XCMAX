...
def _registered_router_excel_import(
    ...
    if action == "execute_import":
        ...
        result = _facade()._execute_excel_import_records(
...
def execute_registered_workflow_tool(tool_id: str, action: str, params: dict | None = None) -> dict:
    ...
    if router is not None:
        ...
        if tool_id == "business_db" and action == "write" and isinstance(result, dict):
            ...
            if isinstance(payload, dict):
                result = _facade()._remember_business_db_target(
                    runtime_context,
                    _facade()._normalize_business_db_entity(params.get("entity"), user_message),
    ...
    try:
        ...
        _facade().logger.debug(
            "employee tool direct dispatch skipped tool=%s", tool_id, exc_info=True
        )
