...
def serve_model(model_path: str, port: int = 5000):
    """
    启动独立意图推理进程（FastAPI + uvicorn）。

    主产品栈请使用 ``python run.py``；本函数仅供本地/脚本拉起最小 predict 服务。
    """
    import uvicorn
    ...
    classifier = BertIntentClassifier(model_path=model_path)
    app = FastAPI(title="xcagi-intent-serve")

    @app.post("/api/intent/predict")
    def predict(body: dict = Body(default_factory=dict)) -> dict:
        text = str((body or {}).get("text") or "")
        return classifier.predict(text, return_probs=True)

    logger.info("启动意图模型服务 http://0.0.0.0:%s（FastAPI）", port)
    logger.info("意图识别 API: POST http://0.0.0.0:%s/api/intent/predict", port)
