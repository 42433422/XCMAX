...
class _UserMemoryServicePart02Mixin:
    ...
    def _calculate_similarity(self, slots1: dict[str, _facade().Any], slots2: dict[str, _facade().Any]) -> float:
        """计算槽位相似度"""
    ...
    def add_feedback(self, user_id: str, message: str, recognized_intent: str, feedback: str, corrected_intent: str | None=None, slots: dict[str, _facade().Any] | None=None) -> None:
        """
        添加用户反馈
        ...
        record = _facade().FeedbackRecord(timestamp=_facade().datetime.now().isoformat(), message=message[:200] if message else '', recognized_intent=recognized_intent, user_feedback=feedback, corrected_intent=corrected_intent, slots=slots or {})
        ...
        _facade().logger.debug('用户 %s 反馈已记录: feedback=%s, recognized=%s', user_id, feedback, recognized_intent)

    def _adjust_pattern_weights(self, memory: _facade().UserMemory, recognized_intent: str, corrected_intent: str | None, feedback: str) -> None:
        """调整模式权重"""
        weight_delta = 0.0
    ...
    def get_feedback_stats(self, user_id: str) -> dict[str, _facade().Any]:
        ...
        intent_error_rates: dict[str, dict[str, int]] = _facade().defaultdict(lambda : {'total': 0, 'errors': 0})
        ...
        return {'total': len(memory.feedback_history), 'confirmed': feedback_counts.get('confirmed', 0), 'negated': feedback_counts.get('negated', 0), 'corrected': feedback_counts.get('corrected', 0), 'error_rates': error_rates}
    ...
    def get_habit_suggestions(self, user_id: str) -> list[dict[str, _facade().Any]]:
        ...
        for seq in action_sequence:
            if seq['confidence'] >= 0.8 and len(seq['actions']) >= 2:
                suggestions.append({'type': 'action_sequence', 'actions': seq['actions'], 'confidence': seq['confidence'], 'suggestion': f"执行 {seq['actions'][0]} 后主动提示 {seq['actions'][1]}"})
        return suggestions

    def _analyze_action_sequence(self, memory: _facade().UserMemory) -> list[dict[str, _facade().Any]]:
        """分析操作序列"""
        sequences: dict[str, dict[str, _facade().Any]] = _facade().defaultdict(lambda : {'count': 0, 'first_action': ''})
        ...
        for (seq_key, stats) in sequences.items():
            if stats['count'] >= 2:
                ...
                result.append({'actions': actions, 'confidence': min(0.95, stats['count'] * 0.15), 'count': stats['count']})
        return result

    def apply_preference_to_slots(self, user_id: str, intent: str, slots: dict[str, _facade().Any]) -> dict[str, _facade().Any]:
        """
        将用户偏好应用到槽位
    ...
    def get_memory_summary(self, user_id: str) -> dict[str, _facade().Any]:
        ...
        return {'has_memory': True, 'preference_count': len(memory.preferences), 'action_count': len(memory.frequent_actions), 'feedback_count': len(memory.feedback_history), 'memory_v2_count': len(memory.memory_v2_records), 'memory_v2_pending_count': len([m for m in memory.memory_v2_records if m.get('status') == 'pending']), 'memory_v2_active_count': len([m for m in memory.memory_v2_records if m.get('status') == 'active']), 'last_updated': memory.updated_at, 'top_intents': [a.get('intent') for a in memory.frequent_actions[:3]]}
