...
def _apply_model_config(item: dict[str, Any]) -> None:
    ...
    try:
        ...
        with get_db() as db:
            ...
            if user:
                user.default_llm_json = _facade().json.dumps(payload.get('llm_config') or {}, ensure_ascii=False)
                db.commit()
...
def _apply_ecosystem(item: dict[str, Any]) -> None:
    ...
    try:
        ...
        conn.execute('INSERT OR REPLACE INTO sync_meta (key, value) VALUES (?, ?)', (key, _facade().json.dumps(payload, ensure_ascii=False, default=str)))
        conn.commit()
        conn.close()
    except _facade().RECOVERABLE_ERRORS as exc:
...
def _apply_im_message(item: dict[str, Any]) -> None:
    ...
    try:
        ...
        with get_db() as db:
            ...
            if obj:
                if incoming_ms:
                    stored_ms = int((_facade()._read_sync_meta(meta_key) or {}).get('updated_at_ms') or 0)
                ...
                obj = ImMessage(id=message_id if message_id else None, conversation_id=conversation_id, sender_user_id=sender_user_id, body=body[:4000])
                db.add(obj)
            ...
            db.refresh(obj)
            if meta_key:
                _facade()._write_sync_meta(meta_key, {'updated_at_ms': incoming_ms or _facade().utc_now_ms(), 'id': int(obj.id)})
    except _facade().RECOVERABLE_ERRORS as exc:
...
def _apply_im_read_state(item: dict[str, Any]) -> None:
    ...
    try:
        ...
        with get_db() as db:
            member = db.execute(select(ImConversationMember).where(ImConversationMember.conversation_id == conversation_id, ImConversationMember.user_id == user_id)).scalar_one_or_none()
            if not member:
                return
            applied_read = max(int(member.last_read_message_id or 0), new_read)
            member.last_read_message_id = applied_read
            db.commit()
        _facade()._write_sync_meta(meta_key, {'updated_at_ms': max(incoming_ms, stored_ms) if incoming_ms else stored_ms, 'last_read_message_id': applied_read})
    except _facade().RECOVERABLE_ERRORS as exc:
        _facade().logger.warning('apply_im_read_state failed conv=%s user=%s: %s', conversation_id, user_id, exc)
...
def _apply_account_entitlements(item: dict[str, Any]) -> None:
    ...
    if not username:
        return
    try:
        stored_payload = {**payload, 'market_user_id': entity_id, 'mod_ids': _facade()._sync_payload_list(payload.get('mod_ids'))}
        ...
        _facade().logger.warning('apply_account_entitlements failed user=%s id=%s: %s', username, entity_id, exc)
