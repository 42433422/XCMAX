...
from app.utils.async_task.models import (
...
from app.utils.async_task.models import (
    register_default_tasks as _register_default_tasks,
)
...
class AsyncTaskManager:
    ...
    def register_task(
        self, config: AsyncTaskConfig, func: Callable[..., Any] | None = None
    ) -> None:
        """注册任务配置"""
        self._task_configs[config.name] = config
        if func is not None:
            self._task_functions[config.name] = func
    ...
    def _execute_sync(
        ...
        try:
            func = self._task_functions.get(task_name)
            if func is None:
    ...
    def _execute_async(
        ...
        if os.environ.get("XCAGI_DESKTOP_MODE", "0").lower() in {"1", "true", "yes", "on"}:
            ...
            def runner():
                self._execute_sync(task_name, args, kwargs, result, config)

            future = submit_background(runner)
            result.metadata["desktop_future"] = str(id(future))
            result.metadata["queue"] = "desktop-thread"
            return result
...
def async_task(
    ...
    **kwargs,
    ...
    def decorator(func: Callable) -> Callable:
        ...
        get_async_task_manager().register_task(config, func)
        ...
        wrapper_with_api.get_task_status = staticmethod(get_task_status)
        wrapper_with_api.config = config
