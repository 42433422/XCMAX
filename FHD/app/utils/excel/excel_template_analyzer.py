...
class ExcelTemplateAnalyzer:
    ...
    def _extract_structure(self):
        ...
        for col in range(1, self.max_col + 1):
            ...
            self.worksheet.column_dimensions[col_letter].width  # noqa: B018

        for row in range(1, self.max_row + 1):
            self.worksheet.row_dimensions[row].height  # noqa: B018
    ...
    def _guess_merged_purpose(self, min_row: int, max_row: int, min_col: int, max_col: int) -> str:
        ...
        if cell_value:
            if "送货单" in str(cell_value) or "收据" in str(cell_value):
                ...
                kw in str(cell_value) for kw in ["型号", "名称", "数量", "规格", "单价", "金额"]
            ):
                return "表头"
    ...
    def _classify_cell_type(self, cell, row: int, col: int, is_merged: bool) -> str:
        ...
        if any(
            keyword in value_str for keyword in ["合计", "小计", "总计", "签名", "日期：", "单位："]
        ):
            return "template"
    ...
    def _extract_style(self, cell) -> CellStyle | None:
        style = CellStyle()
    ...
    def _identify_zones(self):
        ...
        if header_rows:
            self.zones.append(
                ContentZone(
                    name="header", rows=header_rows, type="template", description="表头和标题区域"
                )
            )
        ...
        if data_rows:
            self.zones.append(
                ContentZone(
                    name="data", rows=data_rows, type="editable", description="数据输入区域"
                )
            )
        ...
        if summary_rows:
            self.zones.append(
                ContentZone(
                    name="summary", rows=summary_rows, type="template", description="汇总和签名区域"
                )
            )
    ...
    def _identify_editable_ranges(self):
        editable_cells = [
            (cell.row, cell.col) for cell in self.cells.values() if cell.type == "editable"
        ]

        if not editable_cells:
            return

        rows = sorted({r for r, c in editable_cells})
        cols = sorted({c for r, c in editable_cells})
        ...
        for row_group in row_groups:
            ...
            col_group = [
                c
                for c in cols
                if any(c == cell_col for r, cell_col in editable_cells if r in row_group)
            ]
...
def analyze_template(file_path: str, sheet_name: str | None = None) -> dict[str, Any]:
    """分析 Excel 模板并返回结构信息"""
    analyzer = ExcelTemplateAnalyzer(file_path)
    return analyzer.analyze(sheet_name)


def analyze_to_json(file_path: str, output_path: str, sheet_name: str | None = None):
    """分析 Excel 模板并保存为 JSON 文件"""
    analyzer = ExcelTemplateAnalyzer(file_path)
    return analyzer.save_json(output_path)


def extract_entries(file_path: str, sheet_name: str | None = None) -> dict[str, Any]:
    """提取 Excel 模板中的可编辑词条"""
