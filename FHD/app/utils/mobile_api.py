...
def format_mobile_response(
    data: Any, message: str = "success", success: bool = True, code: int = 200
) -> dict[str, Any]:
    """
    格式化移动端统一响应格式
...
def format_error_response(error: str, message: str = "error", code: int = 400) -> dict[str, Any]:
    """
    格式化错误响应
    ...
    Returns:
        dict: 统一格式的错误响应
    """
    return {"code": code, "message": message, "success": False, "data": {"message": error}}
...
def parse_search_params(
    request_args: dict[str, Any], allowed_fields: list[str] | None = None
) -> dict[str, Any]:
    """
    解析搜索参数
...
def format_mobile_product_list(
    products: list[dict[str, Any]], total: int, page: int = 1, per_page: int = 20
) -> dict[str, Any]:
    """
    格式化产品列表为移动端格式
    ...
    return format_mobile_response(data=paginate_list(optimized_products, total, page, per_page))


def format_mobile_customer_list(
    customers: list[dict[str, Any]], total: int, page: int = 1, per_page: int = 20
) -> dict[str, Any]:
    """
    格式化客户列表为移动端格式
    ...
    return format_mobile_response(data=paginate_list(optimized_customers, total, page, per_page))


def format_mobile_shipment_list(
    shipments: list[dict[str, Any]], total: int, page: int = 1, per_page: int = 20
) -> dict[str, Any]:
    """
    格式化发货单列表为移动端格式
    ...
    return format_mobile_response(data=paginate_list(optimized_shipments, total, page, per_page))
