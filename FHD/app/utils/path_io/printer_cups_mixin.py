...
class PrinterCupsMixin:
    ...
    def _is_print_backend_available() -> bool:
        return bool(_facade()._PRINT_BACKEND_AVAILABLE or _facade().PrinterUtils._is_cups_backend_available())

    @staticmethod
    def _is_cups_backend_available() -> bool:
        return _facade().sys.platform == 'darwin' and _facade().win32print is None and (_facade().win32api is None) and _facade().os.path.isfile(_facade()._CUPS_LPSTAT) and _facade().os.access(_facade()._CUPS_LPSTAT, _facade().os.X_OK) and _facade().os.path.isfile(_facade()._CUPS_LP) and _facade().os.access(_facade()._CUPS_LP, _facade().os.X_OK)
    ...
    @classmethod
    def _run_cups(cls, command: str, arguments: tuple[str, ...], *, timeout: float=15, input_stream: BinaryIO | None=None) -> Any:
        if command == 'lp':
        ...
        return _facade().subprocess.run([executable, *arguments], stdin=input_stream, capture_output=True, text=True, timeout=timeout, check=False, env=cls._cups_env())
    ...
    def _get_cups_default_printer(self) -> str | None:
        try:
            ...
            for prefix in ('system default destination:', '系统默认目的位置：', '系统默认目的位置:'):
    ...
    def _cups_status_text(raw: str) -> str:
        ...
        if 'disabled' in normalized or 'paused' in normalized or '已停用' in raw or ('暂停' in raw):
            return '已暂停'
        if 'printing' in normalized or 'processing' in normalized or '正在打印' in raw or ('正在处理' in raw):
            return '打印中'
        ...
        try:
            ...
            for raw_line in result.stdout.splitlines():
                ...
                if parsed is None:
                    continue
                (name, status) = parsed
                printers.append({'name': name, 'status': status, 'is_default': name == default_printer})
            return printers
        except _facade()._CUPS_ERRORS as exc:
    ...
    def _allowed_print_roots() -> tuple[str, ...]:
        ...
        roots = {app_data, _facade().os.path.join(app_data, 'shipment_outputs'), _facade().os.path.realpath(_facade().tempfile.gettempdir())}
            ...
            try:
                with _facade().os.scandir(normalized_root) as entries:
                    for entry in entries:
                        ...
                        if _facade().os.path.normcase(candidate) == _facade().os.path.normcase(requested):
    ...
    def _print_cups(self, file_path: str, printer_name: str) -> dict:
        ...
        if validated_printer is None:
            return {'success': False, 'message': '打印失败：打印机不存在或名称不安全', 'printer': printer_name}
        validated_file = self._resolve_allowed_print_path(file_path)
        if validated_file is None:
            return {'success': False, 'message': '打印失败：文件不在允许的打印目录中', 'printer': validated_printer}
        try:
            with open(validated_file, 'rb') as source:
                result = self._run_cups('lp', ('-d', validated_printer), timeout=30, input_stream=source)
            if result.returncode != 0:
                _facade().logger.warning('CUPS rejected print job for %s: %s', validated_printer, result.stderr.strip() or result.stdout.strip() or 'lp command failed')
                return {'success': False, 'message': '打印失败：macOS 打印服务拒绝了任务', 'printer': validated_printer}
            return {'success': True, 'message': '打印任务已提交到 macOS CUPS', 'file': _facade().os.path.basename(validated_file), 'printer': validated_printer, 'method': 'cups_lp'}
        except _facade()._CUPS_ERRORS as exc:
            _facade().logger.error('CUPS print failed: %s', exc)
            return {'success': False, 'message': '打印失败：macOS 打印服务暂不可用', 'printer': validated_printer}
