...
class PrinterFileMixin:
    ...
    def _print_excel(self, file_path: str, printer_name: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                return {'success': True, 'message': '打印任务已发送（os.startfile）', 'file': _facade().os.path.basename(file_path), 'printer': printer_name}
                ...
                try:
                    ...
                    if result > 32:
                        ...
                        return {'success': True, 'message': '打印任务已发送到打印机', 'file': _facade().os.path.basename(file_path), 'printer': printer_name}
                    else:
                    ...
                    try:
                        ...
                        return {'success': True, 'message': '文件已打开，请手动打印', 'file': _facade().os.path.basename(file_path), 'printer': printer_name, 'manual': True}
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    return {'success': True, 'message': 'PDF文件已发送到打印机', 'file': _facade().os.path.basename(file_path), 'printer': printer_name, 'method': 'win32print'}
                finally:
            ...
            try:
                adobe_paths = ['C:\\Program Files (x86)\\Adobe\\Acrobat Reader DC\\Reader\\AcroRd32.exe', 'C:\\Program Files\\Adobe\\Acrobat Reader DC\\Reader\\AcroRd32.exe']
                for adobe_path in adobe_paths:
                    if _facade().os.path.exists(adobe_path):
                        _facade().logger.info('使用Adobe Reader: %s', adobe_path)
                        _facade().subprocess.run([adobe_path, '/t', file_path, printer_name], check=True, timeout=30, capture_output=True)
                        _facade().logger.info('PDF文件已通过Adobe Reader发送到打印机')
                        return {'success': True, 'message': 'PDF文件已通过Adobe Reader发送到打印机', 'file': _facade().os.path.basename(file_path), 'printer': printer_name, 'method': 'adobe_cli'}
    ...
    def _print_default(self, file_path: str, printer_name: str, show_app: bool=False) -> dict:
        ...
        try:
            ...
            _facade().win32api.ShellExecute(0, 'print', file_path, f'/d:"{printer_name}"', '.', show_cmd)
            app_status = '（显示应用窗口）' if show_app else '（隐藏应用窗口）'
            _facade().logger.info('文件已发送到打印机%s: %s', app_status, file_path)
            return {'success': True, 'message': f'打印任务已发送到打印机{app_status}', 'file': _facade().os.path.basename(file_path), 'printer': printer_name, 'show_app': show_app}
        except _facade().RECOVERABLE_ERRORS as e:
