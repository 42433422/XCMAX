...
**位置：仓库根下 `docker/` 目录**（本机例：`E:\FHD\docker`）。本目录内为 Mod 沙箱「最小镜像」的 **Dockerfile、构建脚本、说明**；构建上下文仍须为**仓库根**（脚本会拼装最小临时目录后 `docker build`）。
...
| 文件 | 说明 |
|------|------|
| `Dockerfile.mod-sandbox` | 多阶段 Mod 沙箱镜像 |
| `mod-sandbox.dockerignore` | 与 `docker build --ignorefile` 联用，缩小 build context |
| `build-mod-sandbox.ps1` / `build-mod-sandbox.sh` | 在仓库根执行，无需 `--ignorefile` |
| `MOD_SANDBOX.md` | 本说明（即当前文件） |

## 构建

### 方式 A（推荐，任意 Docker 版本）

在**仓库根**（`E:\FHD`）执行；脚本会拼装最小 context 再 `docker build`，无需 `--ignorefile`：

```powershell
powershell -ExecutionPolicy Bypass -File docker/build-mod-sandbox.ps1
```

```bash
bash docker/build-mod-sandbox.sh
```
...
```bash
docker build --ignorefile docker/mod-sandbox.dockerignore -f docker/Dockerfile.mod-sandbox -t xcagi-mod-sandbox .
```
...
- **PyAudio**：`Dockerfile.mod-sandbox` 的 builder 阶段已安装 `portaudio19-dev`；若自行改依赖仍从源码编 PyAudio，需保留该包。
...
若 worker 因 **`PermissionError`** 在 `/app/config`、`/app/templates` 等路径 `makedirs` 失败，请使用当前镜像（`Dockerfile.mod-sandbox` 已对 `/app` 作 `chown` 以允许 `appuser` 在仓库根下创建子目录）。若遇 **`ModuleNotFoundError`**，先确认已按 [`XCAGI/requirements.txt`](../XCAGI/requirements.txt) 完整构建（含 `openai` 等），再查容器日志。
