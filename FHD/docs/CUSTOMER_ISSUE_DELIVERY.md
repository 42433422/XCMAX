...
本次仍通过集成 PR #1785 交付，基线为 origin/main 的 893a6a36d。历史变更按下表保留，发布身份以最终合入提交为准。

| 历史改动 | 集成处理 |
| --- | --- |
| #1768 宿主入门、智能对话置顶与 CLI 入口 | 已由 a286f585f 进入 main，继续保留 |
| #1793 通用 attendance_engine 及 SDK 加载 | 保留 main 实现；私包表头解析复用共享 SDK，消除重复源码 |
| #1796 常驻考勤入口、#1797 账号数据隔离 | 保留 main 界面与现有数据库；私包按实际登录用户名读取已标注归属的人员，不复制旧库 |
| #1801 智能对话意图闸、#1802 Windows 通用考勤打包 | 保留 main 实现与检查，不回退 |
| 本次原 6dc322c5f 工单、签包、运行回执和重试 | 适配上述主线后继续集成；运行回执仍要求真实签包及业务验证 |

共享数据库未具备明确归属列时，私包只使用自己的账号工作区；读取桥接不执行历史数据归属迁移。共享名单已经建立但当前账号为空时，保持空名单，不以旧私包 seed 复活已删除人员。
