...
| 交付物 | 路径 / 命令 | 验收 |
|--------|-------------|------|
| Windows 企业版安装包 | `build-installer.ps1 -ProductSku enterprise`（版本省略时取脚本默认值） | `release/xcagi-v<产品版本>/enterprise/` |
| Windows 安装包（macOS/Linux 交叉构建） | `bash scripts/package/build-windows-installer.sh <产品版本> enterprise` | Docker/Wine 构建，必须包含 `resources/backend/xcagi-backend.exe` |
| macOS 安装包（单 SKU） | `bash scripts/package/build-installer.sh enterprise`（版本省略时取脚本默认值） | 与 Windows 共用 SKU 资源契约，后端二进制按平台生成 |
...
确认 [`VERSION.md`](../VERSION.md) 的产品版本与工具链映射，并通过版本锚点校验（数字不在此复述）。
