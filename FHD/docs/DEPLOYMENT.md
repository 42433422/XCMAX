...
```bash
# 启动 FastAPI 服务（唯一 HTTP 入口，入口工厂：app.fastapi_app:get_fastapi_app）
python run.py
...
server {
    ...
    # 反向代理到 FastAPI（uvicorn 默认 5000；保留 8000 示例仅供旧部署参考）
    location /api {
...
# 业务侧通常再套一层 @redis_cached(key, ttl=300) 之类的简易装饰器
```
