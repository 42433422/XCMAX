...
| `release_ready` | `false`（G7 正式包 bundle 写回归待 1.0.0.3 重测；T6 重启复验未执行） | download_release.json + manifest.json |
...
| ZIP（arm64，OTA 载荷） | `https://xiu-ci.com/releases/stable/enterprise/XCAGI-Enterprise-1.0.0.2-mac-arm64.zip` | 264,517,668 | SHA256 `44c1a3807100c9805af0fe276af790e083cf77da942e710c75e63c1495a6e7b6`（本机 OTA 下载实测一致）；SHA512 `1ryEtv+nlqe72gUZvMK56fNjp5Ncb7MslaACYv3p43q/ifYGRwSMiZpdimmo1wCzMH6qsASQv2/NPm/TYDchUw==`（latest-mac.yml；ed25519 公钥验签 **VALID** 2026-09-13） |
...
> **当前结论（2026-09-13 T4 后）：NOT Release Ready（正式包 1.0.0.2 发现 G7 新断点——模板上传写签名 bundle 内 Permission denied；修复已提交，待 1.0.0.3 重建重测）。** 1.0.0.2 正式发布闭环已达成：release run [34732266439](https://github.com/42433422/XCMAX/actions/runs/34732266439)（checkout `6eda2203d` 构建+签名+公证+扫描对门禁通过）→ CVM 上传步骤 180min 超时 → artifact 手动恢复发布（五文件 SHA256 与 artifact 逐字节一致，官方+stable 双路径，manifest/公开 feed 复验通过，见 §2 交付说明与 [恢复证据](evidence/e2e/macos-release-1.0.0.2/ota-release-cvm-recovery-20260913.md)）。G1/G3 GREEN；**T4 真机 OTA 全链（2026-09-13）实跑完成：G8/G9 升 GREEN、G10 补 OTA 路径 PASS、G11 升 YELLOW**；G7 判 **RED**（正式包 bundle 内写入回归，候选包 v2 结论对 9a4a06bc4 有效；复现+日志+修复 commit 见 G7 行）。剩余未闭环：G7（修复发版重测）、T2（UI 登录截图）、T6（G12 重启复验）。**G13 GREEN（降级演练 PASS）**。
...
| G7 | 真实业务任务 | RED | **1.0.0.2 正式包（6eda2203d）真机复测（2026-09-13 T4 OTA 后）发现新断点**：`POST /api/templates/upload` 500 `Permission denied: '/Applications/XCAGI.app/Contents/Resources/backend/_internal/app'`——模板分析链路把上传文件存到 `analyzer.py` 旁的 `uploads/templates`（打包后位于签名 bundle 内，只读）；登录修复（auth/login 200）后暴露。generate 连锁 TEMPLATE_NOT_FOUND（模板未入库）。复现：1.0.0.1 真机 OTA→登录→multipart upload `尹玉华1.xlsx`→500；日志 [g7-retest-post-ota.txt](evidence/e2e/macos-release-1.0.0.2/g7-retest-post-ota.txt)。历史（候选包 v2 = 9a4a06bc4，2026-09-12）：upload 405→401 断链根治，登录态全链 200 出单 [g7-full-chain-success-20260912.json](evidence/e2e/macos-release-1.0.0.2/g7-full-chain-success-20260912.json)——该结论对 9a4a06bc4 有效，但正式包 6eda2203d（#1912 新并提交）引入 bundle 内写入回归 | 复现步骤+日志已齐（见左）；修复 commit：`get_upload_dir()` 重定向（本回填同分支提交） | **阻断**：正式包用户无法导入模板完成发运单业务 | 本分支 fix（`get_upload_dir()`） | 修复并入 main → 1.0.0.3 重建 → 真机 OTA 重测 G7（upload 2xx + generate 出单） |
| G8 | 更新发现 | GREEN | **真机实跑（2026-09-13 T4）**：1.0.0.1 真机（feed 1.0.0.2 上线后）应用内自动检查更新——`update_available` 15:33:01（[updater-events.jsonl](../../logs/) 事件流，落盘 [g8-update-discovery-10001-to-10002.json](evidence/e2e/macos-release-1.0.0.2/g8-update-discovery-10001-to-10002.json)）：isUpdateAvailable=true，productVersion=1.0.0.2，buildSha=6eda2203d，ed25519 签名 present，sha512 与 feed 一致 | — | 无 | #583 | — |
| G9 | 更新安装 | GREEN | **真机实跑（2026-09-13 T4）**：download_start→update_downloaded（15:33:28，SHA512 校验匹配，增量 blockmap 命中缓存）→ install_start（15:36:59）→ ShipIt「Detected this as an install request」→「Successfully launched application」（15:41:52 ShipIt status 0）→ 自动重启后 `/api/health` version=1.0.0.2 healthy（15:41:55 后端 PID 新起）；`/Applications/XCAGI.app` Info.plist 实测 1.0.0.2。证据 [t4-post-ota-verify.json](evidence/e2e/macos-release-1.0.0.2/t4-post-ota-verify.json) | — | 无 | #583 | — |
| G10 | 数据保留（升级后） | GREEN | 三次实机覆盖升级全部 PASS：① 1.0.0.1 同版本重装零丢失（[data-retention.json](evidence/e2e/macos-release-1.0.0.1/data-retention.json)）；② 1.0.0.1→1.0.0.2(3d3ec8d98) 跨版本（[pre](evidence/e2e/macos-release-1.0.0.2/data-digest-preupgrade-10002.json)/[post](evidence/e2e/macos-release-1.0.0.2/data-digest-after-10002.json)）；③ 1.0.0.2(3d3ec8d98)→1.0.0.2(9a4a06bc4, 含 G7 修复) 覆盖升级：DB 字节级相同（986,615,808B）、uploads 151、mods 847、backups 4、标记存活，data-retention result=PASS（[data-retention-cand2.json](evidence/e2e/macos-release-1.0.0.2/data-retention-cand2.json)、[run log](evidence/e2e/macos-release-1.0.0.2/acceptance-10002-cand2-run.zip)）；④ **OTA 路径实测（2026-09-13 T4/T5）**：1.0.0.1→1.0.0.2(6eda2203d) 真机 OTA 前后 digest 逐项一致——DB 986,886,144B 字节级相同、uploads 155、mods 847、backups 4、升级后启动备份自动生成（xcagi-startup-20260913074200.db）；[pre](evidence/e2e/macos-release-1.0.0.2/t4-ota-digest-pre-10001.json)/[post](evidence/e2e/macos-release-1.0.0.2/t4-ota-digest-post-10002.json)、[after-ota](evidence/e2e/macos-release-1.0.0.2/data-digest-after-ota-10002.json) | — | 无 | #1870 | — |
| G11 | 更新后重新执行业务 | YELLOW | **T4 实测（2026-09-13）**：OTA 后 health healthy/1.0.0.2、mods API 200、employees API 200、登录 `auth/login`→`auth/me` 200（xcagi-enterprise-demo 企业账号，1.0.0.2 启动错误已修复）；证据 [t4-post-ota-verify.json](evidence/e2e/macos-release-1.0.0.2/t4-post-ota-verify.json)、[g7-retest-post-ota.txt](evidence/e2e/macos-release-1.0.0.2/g7-retest-post-ota.txt) | G7 业务链（upload→generate 出单）因 bundle 内写入新断点未走通（G7 RED，修复后 1.0.0.3 重测） | 无（受 G7 阻断，非 G11 本身缺陷） | — | G7 修复发版后重测出单 |
...
11. **G7 正式包新断点（1.0.0.2，6eda2203d）：模板上传分析写入签名 bundle 内 `_internal/` 报 Permission denied**：`analyzer.py` `_analyze_template_with_upload_inner` 把上传 Excel 存到 `os.path.dirname(os.path.dirname(__file__)) / uploads/templates`——源码态解析为 `app/uploads/templates`，但打包态 `__file__` 位于 `XCAGI.app/Contents/Resources/backend/_internal/app/...`（签名 bundle 内只读），写入报 `Permission denied`；此前候选包 v2（9a4a06bc4）登录态 upload 200 是因为该候选包经 `XCAGI_DATA_DIR`/未公证等环境差异未命中 bundle 只读路径，T4 OTA 后正式包实测暴露。修复（本分支）：改用 `get_upload_dir()`（打包感知：源码态落仓库、打包态落 `~/Library/Application Support/XCAGI/uploads/templates`），与路由策略文件重定向（#1905）同一原则。证据 [g7-retest-post-ota.txt](evidence/e2e/macos-release-1.0.0.2/g7-retest-post-ota.txt)、[g7-retest-login-prefix.txt](evidence/e2e/macos-release-1.0.0.2/g7-retest-login-prefix.txt)。**教训：候选包（未公证、环境特异）验证结果不能外推为正式包结论，正式包上线后必须重跑核心业务链。**
...
| T4 | ~~下次发版后：旧版真机→检查更新→下载→安装→观察期→复验~~ **已完成（2026-09-13 真机全链）**：1.0.0.1（99854233）→发现 1.0.0.2（15:33:01）→下载验签（15:33:28）→ShipIt 安装（15:41:52）→自动重启 healthy/1.0.0.2（15:41:55）；G8/G9 GREEN、G10 OTA 路径 PASS、G11 YELLOW（业务链受 G7 阻断）；证据 t4-post-ota-verify.json、g8-update-discovery-10001-to-10002.json | G8/G9/G11 | 真机 |
| T5 | ~~跨版本覆盖升级数据保留~~ **已完成（2026-09-13 OTA 路径）**：真机 OTA 前后 digest 逐项一致（DB 986,886,144B 字节级相同、uploads 155、mods 847、backups 4、启动备份自动生成），见 G10-④ | G10 | 真机 |
