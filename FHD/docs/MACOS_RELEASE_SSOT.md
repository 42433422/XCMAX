...
| `release_ready` | `false`（G12 重启复验 UNKNOWN 待 T6；G9 应用内一键安装 YELLOW 待 #1930 修复下版复测） | download_release.json + manifest.json |
...
> **当前结论（2026-09-14 T4 复跑后）：NOT Release Ready——差 G12 重启复验（T6 待执行）；G9 应用内一键安装 YELLOW（缺陷已定位，修复 PR [#1930](https://github.com/42433422/XCMAX/pull/1930)，下版复测转 GREEN）。** 1.0.0.3 发布与真机 OTA 链已达成：release run [34773838698](https://github.com/42433422/XCMAX/actions/runs/34773838698)（checkout `4bfb23365c` 构建+签名+公证+扫描对 passed）→ step14 CVM 直传超时 → artifact 本机中转恢复发布（§2）→ 真机 1.0.0.2 应用内发现并下载 1.0.0.3（G8）→ ShipIt 安装替换（G9，含退出拦截缺陷、手动退出配合完成）→ 数据保留 PASS（G10）→ 登录态业务全链 upload+generate 出单 200（G7/G11）。历史 RED（G7 bundle 写回归）已修复并重测转 GREEN（复现+日志记录保留于 §7-11）；G9 新缺陷记录见 §7-12。**安装→使用→更新→数据保留→更新后继续使用主链已有完整真机证据，唯 G12 重启复验未执行，不宣布闭环。**
...
| G5 | 登录绑定 | YELLOW | **1.0.0.3 API 实证**：`auth/login` 200（xcagi-enterprise-demo）→ 会话有效（attendance capabilities 200 权益门控生效，[g6-post-ota-10003.json](evidence/e2e/macos-release-1.0.0.3/g6-post-ota-10003.json)）；T2 UI 登录主窗口截图 [t2-logged-in-main-window.png](evidence/e2e/macos-release-1.0.0.3/t2-logged-in-main-window.png)（2026-09-14 00:17，OTA 前 1.0.0.2 会话） | 新版（1.0.0.3）UI 登录动作截图（并入 T6 复验） | 无 | — | T6 复验时补新版 UI 登录截图 |
...
| G12 | 重启 Mac 后核心功能复验 | UNKNOWN | — | 未执行真实重启（避免中断在用会话） | 闭环阻断项（Runbook §9-6） | — | T6：重启后复验登录/Mod/业务/数据 |
...
| T2 | 真机登录绑定（市场账号），截图+日志——**1.0.0.2 会话已完成**（[t2-logged-in-main-window.png](evidence/e2e/macos-release-1.0.0.3/t2-logged-in-main-window.png)，2026-09-14 00:17）；1.0.0.3 UI 登录截图并入 T6 | G5 | 任意真机 |
...
| T6 | 重启 Mac 后复验：登录会话、Mod/AI 员工、业务链、数据完整（脚本 `t6-post-reboot-verify.sh` 已备） | G12 | 真机 |
