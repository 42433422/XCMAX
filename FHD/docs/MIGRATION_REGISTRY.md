...
| **Alembic**                            | 单一迁移链：仓库根 `**alembic/versions/`**；`XCAGI/alembic.ini` 的 `script_location` 指向 `**../alembic**`；活链根为 `**2026_06_22_baseline**`（squashed baseline），当前 head 以 `alembic heads` 实测为准（2026-08-01 为 `2026_07_24_shipment_etl_fingerprints`）；归档前旧迁移在 `versions/_archive_pre_baseline_2026_06_22/`。新模型/字段必须同时提交迁移，防漂移门禁 `scripts/dev/check_schema_drift.py`，公约详见 `[ALEMBIC_MIGRATION_GUIDE.md](guides/ALEMBIC_MIGRATION_GUIDE.md)`「现状与公约」                                                                |
...
| **D. 数据库 schema（Alembic）**                   | 单一活链 + 漂移门禁已接入 CI（观察期）                                | `**alembic/versions/`**、`**scripts/dev/check_schema_drift.py**`                                                          | 旧迁移归档于 `versions/_archive_pre_baseline_2026_06_22/`                                                                                        | `[ALEMBIC_MIGRATION_GUIDE.md](guides/ALEMBIC_MIGRATION_GUIDE.md)`「现状与公约」节（以此为准）             |
