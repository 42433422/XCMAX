...
```bash
# 应无输出（仅 allowlist 内 *_app_service_v2.py / sdk_v2.py）
git ls-files 'FHD/app/**/*_v*.py' | while read f; do
  case "$f" in
    *app_service_v2.py|*/sdk_v2.py) grep -qxF "$f" scripts/ci/v2_versioned_py_allowlist.txt || echo "NOT IN ALLOWLIST: $f" ;;
  esac
done
