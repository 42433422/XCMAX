...
- **Windows 精确 SHA 重建包对齐当前 main（2026-09-12 交付）**：`XCAGI-Enterprise-Setup-1.0.0.1-x64-unsigned-7ffcf47dd.exe`（sha256 `a1c080d3…2cdc3`、248,373,120 字节、`git_sha 7ffcf47dd`=#1874 合入后的 main、`runner_install_smoke:passed`，run 34642774803 `windows_installer_only` 精确 SHA 构建，delivery-receipt 随产物归档）上传至 `https://xiu-ci.com/releases/testing/enterprise/XCAGI-Enterprise-Setup-1.0.0.1-x64-unsigned-7ffcf47dd.exe`。本包 = 项1「当前 main 更新后重新生成精确 SHA 安装包」的交付物；含 #1874 故障注入脚本、#1876 LLM 端点自愈、#1877/#1830/#1838/#1825 全部合入内容。故障注入 runner 级三轮迭代（旧包 fe690c9c→新包 a1c080d3）：run 34642734307 全 FAIL 暴露三缺陷→run 34646944874 kill-all/kill-orphan PASS 暴露备份与 env 缺口→**run 34648499865 终态 PASS=4 / FAIL=0 / SKIP=2（仅人工场景）**，其中 corrupt-main「坏库改名留证 + 备份还原 + health 可达」自动恢复证据首次取得；已知限制：孤儿后端直拉未真正监听 17500（WARN 留证），端口占用路径待实机覆盖。
