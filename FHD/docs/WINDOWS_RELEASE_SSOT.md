...
| B14 | **P0→已按业主决策关闭（2026-09-17，限期 1096 天）** | 未签名 Windows 包此前只能停在内网产物：`generate-windows-hotfix-pointer.py` 硬写 `download_allowed=false`，且没有任何 job 发布 `download-windows-hotfix.json`，官网 Windows 行恒为「待开放」 | 业主 2026-09-17 决策：**限期风险接受替代签名硬门禁**，授权至 `2029-09-17`（[windows_signing_acceptance.json](../config/windows_signing_acceptance.json)，双人复核 + 决策记录 hash + 3 条已披露风险）。落地两处：① `--risk-acceptance`（授权期内 `download_allowed=true`、`channel=enterprise-interim`，过期/弱证据 fail-closed，不带该参数仍是隔离语义）；② 新 job `publish-unsigned-windows-pointer`（只在 `windows_installer_only` 车道，凭真回执公开 `-unsigned.exe`、`.sha256` 与下载指针，公网 HTTP + 远端 SHA256 双验证）。**Windows 稳定更新通道保持关闭**（该 job 不写任何更新元数据）；护栏 `tests/release_gate/test_windows_interim_release_policy.py`。**待实跑**：该 job 尚未在真实 run 上执行，下载页开闸需一次 `windows_installer_only` 派发实证；该次派发前需先把 `config/download_release.json` 的 `version_lock`/`download_version`/`release_history[0].version` 锁到本次发布版本（与既有 macalign 车道同一前置条件；未锁定时生成器报 `release metadata version does not match` 并 fail-closed，不会发布陈旧指针） |
