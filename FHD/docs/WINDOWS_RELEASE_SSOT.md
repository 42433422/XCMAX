...
> 兄弟文档：[MACOS_RELEASE_SSOT.md](MACOS_RELEASE_SSOT.md)（macOS 域，G1–G12 同构；mac 侧无 G13，见 §7 同步注记）。
...
| B3 | **P1** | 发布非原子：mac dmg 09-04 重构建后 manifest 未同步（290,432,409→293,401,820 B）→ 官方 sha256 校验必失败 | `generate-download-manifest.py` 在产物替换后强制重跑并原子发布（修复归属 macos-release 域，见 §7） |
...
1. **dmg/manifest 漂移实锤**（其 §7 偏差-3 的 live 确认）：dmg 实测 293,401,820 B（09-05 重构建 `99854233c`）≠ manifest `7ab4fdc1…/290,432,409 B` → 其 G1「SHA256 实测一致」已失效，须重发布或重生成 manifest。
2. **x64 dmg live 404**（其偏差-4 的 live 确认）。
3. **testing 通道签名 INVALID**（B7，双平台共用测试通道）。
4. **mac 侧缺回滚门**：本文件 G13（回滚/恢复）为用户目标强制项，mac 文档 G1–G12 未含 → 建议 mac 域补 G13 并列入 T 任务。
