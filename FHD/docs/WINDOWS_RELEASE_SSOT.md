...
> 最近一次完整更新闭环实跑：2026-09-15（T9 最终收口，基座 1.0.0.2 → OTA 1.0.0.4；证据 [t9/](evidence/e2e/windows-release-1.0.0.2/t9/t9-chain-runs.json)）。
...
## 2. 构建产物与校验和（历史记录截至 2026-09-11；当前 Windows 包见最新行）
...
| Win 临时交付包 `XCAGI-Enterprise-Setup-1.0.0.5-x64-unsigned.exe` | `54325894c` | 248,514,275 B | sha256 `45f761cf1ad968c5abeae466e8b0749364e1d1edfabf0821ef28db52f5cb572e`（Windows 1.0.0.5 基线回执与实测一致） | **未签名（限期风险接受）** | 官网版本目录；2026-09-19 干净隔离安装冷启动 41.4s，证据见 [base-rbac-run.json](evidence/e2e/windows-real-machine-1.0.0.5/base-rbac-run.json)；稳定 OTA feed 未推进 |
...
| B14 | **P0→已按业主决策关闭（2026-09-17，限期 1096 天）** | 未签名 Windows 包此前只能停在内网产物；`generate-windows-hotfix-pointer.py` 与公开指针 job 已落地，2026-09-19 的 1.0.0.5 实机记录证明下载/安装链可用 | 业主决策授权未签名官网临时下载至 `2029-09-17`，授权记录见 [windows_signing_acceptance.json](../config/windows_signing_acceptance.json)。当前 job `publish-interim-windows-pointer` 按回执同时支持有效签名包与限期风险接受的未签名包；后者 fail-closed 校验授权和有效期。两种包都只公开 `.exe`、`.sha256` 与下载指针，不写 `latest.yml`，Windows 稳定 OTA 仍关闭；护栏 `tests/release_gate/test_windows_interim_release_policy.py` |
