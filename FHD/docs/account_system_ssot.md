...
| 桌面端软件 | 软件下载 / 企业交付包 | Windows / macOS | enterprise / admin | 必须绑定企业或管理员会话；不接受 personal 账号进入本地工作台；**安装包须内置 `admin-vue-dist`（`/admin` 独立运维台）**，管理员登录与网页同构跳转 `/admin` |

| 手机 App | 应用商店 / 企业分发 | **Flutter**（`mobile-flutter-poc`，覆盖 Android/iOS） | enterprise / admin | 企业移动工作台；账号能力跟随企业授权 |
...
| 桌面端 Windows/macOS | 不允许进入 | 本地业务工作台、行业 Mod、文件/打印/本地数据能力 | **独立运维台 `/admin`**（运维诊断、代管排障、企业配置审计）；与网页管理端同构建产物，禁止回落到企业 SPA |
| 手机端（Flutter） | 不允许 personal | 消息、审批、移动看板、扫码绑定桌面端 | 移动运维告警、会话排障 |

约束：个人账号只服务网站个人消费场景；企业账号是业务系统主体；管理员账号是平台运营主体。桌面端和手机端只能接受企业或管理员会话，不能因为前端入口传了 `account_kind` 就放行 personal。  
桌面端「管理员登录」必须进入 `/admin`（`templates/admin-vue-dist`）；若安装包缺少该产物，`/admin` 会回落到企业登录页，表现为「只有网页能进管理端」——属发版缺陷，不是产品设计。
