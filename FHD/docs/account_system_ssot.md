...
- 网页与桌面端共用修茈市场 `GET /market/register` 和 `POST /api/auth/register`；不得在桌面端再建第二套公开注册数据源。
- 注册只必填用户名和密码。邮箱选填：留空时不要求验证码；一旦填写邮箱，必须先完成邮箱验证，禁止保存未验证邮箱。
- 桌面端以固定查询参数 `source=xcagi-desktop` 打开同一注册页；该参数只控制来源说明和注册成功后的交接提示，不能提升账号身份或绕过企业授权。
- 普通网页注册成功后进入网页工作台；桌面来源注册成功后停留在成功页，明确告知用户关闭网页并回到桌面端登录。桌面端是否放行仍以已开通的 enterprise 身份/授权为准。
- 登录与注册保持两个独立页面；禁止用注册来源参数自动登录、自动购买或任意外链回跳。
