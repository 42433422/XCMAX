...
| 客户私有交付 | [config/customer_delivery.json](../config/customer_delivery.json) | 客户品牌、`legacy_mod_id`（定制）与 `industry_mod_id`（通用行业包）映射；生产员工私有交付入口只暴露定制包 | 禁止把 `attendance-industry` 等通用行业包当作生产员工/私有交付项目 |
