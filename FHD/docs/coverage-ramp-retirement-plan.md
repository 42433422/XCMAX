...
> 目标：停止用 72 个 `test_coverage_ramp_*` stub 稀释真实覆盖率；对外口径统一为
> **行为覆盖率（B2 转正后实测 84.39% 行 / 76.52% 分支，2026-08-24）**。
...
| **行为（排除 stub）** | **84.39%** | **76.52%** | 真实契约覆盖，唯一硬 gate（ADR-0001） |

- stub 规模：**72 个文件 / 60,714 行**（`tests/test_coverage_ramp_phase*.py`，B2 转正后）
- pyproject markers 自承：「断言弱、杀变体能力低；非行为契约测试」
- 测试膨胀棘轮（`metrics/test-bloat-history.jsonl`）：ratio 已从 1.848 回落至 **1.663**
  （B1+B2 共转正 279 个行为测试脱离 stub 口径），长期目标 ≤1.5。
...
| B2 | ✅ 完成（2026-08-24） | 7 个 `phase1_p1_*` 文件**全部转正**：逐文件审查断言质量，163 个测试绝大多数为真实行为契约（auth/me 三态、错误码映射 504/402/503、agent_run 元数据深度断言、审批节点推进、工具注册表）。改名为 `test_ai_chat_system_auth_behavior.py` 等 7 个 `_behavior.py`，用探测脚本实测 ~20 处弱断言的真实行为后逐一补强为精确契约（含 1 处按实现修正 `_extract_excel_paths_from_context` 键名），163 测试全通过。行为覆盖率 84.15%→84.39% 行 / 76.34%→76.52% 分支，stub 基线 79→72。 |
