...
| Windows（故障注入，项7/8） | `FHD/scripts/package/fault-injection-windows.ps1` | `powershell -ExecutionPolicy Bypass -File fault-injection-windows.ps1`（默认跑 kill-all / kill-orphan / corrupt-backup / corrupt-main 四个可自动化场景并输出 JSON 收据；`corrupt-main` 有安全闸，仅允许 `C:\XCAGI-acceptance` 隔离安装；disk-full 与 power-cut 为人工场景，脚本给指引并记录结果） |
