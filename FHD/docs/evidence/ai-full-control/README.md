...
静态注册盘点发现 37 类能力、190 个动作。wechat 缺少统一 dispatcher。
...
- 完整功能分母：逐页面/API/授权 Mod 对账，补齐缺失 dispatcher，不能用 190 个注册动作冒充全软件覆盖率。
- 控件与身份：文件上传、复杂自绘控件、账号切换时连接更新，以及旧外部 AIOPEN 通道的身份策略仍需核验。
- 业务结果：导航/点击回执只证明交互层结果；各模块持久化读取、权限隔离、撤销和异常补偿需真实数据闭环。
- 任务与自治：跨模块任务、配置和 Mod 更新；长期记忆、主动发现、调度、去重、暂停恢复和故障修复需按账号贯通验证。
