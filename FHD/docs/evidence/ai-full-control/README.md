...
四阶段全量目标尚未完成。集成分支已同步主线 `8f9c2abf3`（合并提交 `8d8c69c98`），产品改动仍在 PR #1804，未作为主线安装版交付。

- 最近一次 22 项隔离 SQLite 业务试验为 **15/22**，源提交 `dd2bd5a1f`，见 `business-task-context-propagation.jsonl` 与 metadata。它早于后续泛化导出日期澄清改动，不能视作当前提交全量复测；无真实模型调用。
- 真实数据库销售查询经并行工作流进入 XLSX，验证当前租户金额 51，排除另一租户金额 900；失败查询不生成附件。见 `workflow-export-database.json`。附件读取的授权范围在测试中被替换，真实登录 HTTP 下载与安装版尚未验收。
- 未指定日期的销售导出现在生成日期澄清、销售查询、导出依赖图；支持本月和明确起止日期，拒绝无效或倒置日期。29 项相关测试通过，聊天恢复测试保留原计划和数据引用，但实际执行器在该测试中被替换。
- 并行线程复制独立上下文，身份与租户传播、线程间隔离通过专门并发测试。当前 `b1fc51970` 的应用 mypy 检查通过（1819 源文件，沿用现有未注解函数检查配置）。

剩余业务验收包括销售口语下单/报价、入库、发货单、澄清后的真实聊天执行，以及所有能力的真实模型和运行时覆盖。CI、主线合入、主线精确提交产物、安装版身份/UI与客户效果仍须分别验收。下文为历史证据，不覆盖或提高本摘要的验收结论。
