...
核查同时确认旧 /api/customers/import 路由仍引用缺失的 run_customers_excel_import_bytes，旧覆盖测试以替身返回成功；/api/customers/export 兼容路由仍明确返回 501。这两个旧入口待与实际部署、Mod 路由和现有 ETL 主链对账并修复，不能因通用 API 协议接通而标记完成。当前工作仍在同一集成 PR #1804，未合入 main 或更新已安装软件。
