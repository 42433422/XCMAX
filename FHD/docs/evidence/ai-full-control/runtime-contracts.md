...
single_label 和 pdf_labels 现均使用已有标签任务服务，明确产品、模板、份数和尺寸后生成预览，再走独立确认和提交；真实 HTTP 已验证预览及确认链路，物理打印使用替身。使用真实登录会话的 AI API 调用另验证了 PDF 生成与完整回读。single_label 只提供型号和数量的旧请求须补齐配置，前端工作流显示待配置提示。物理出纸、任务到预览页的一键续接和安装端仍待验收。
