...
# Default ref is main: the handoff script, the per-platform framework and the catalog all live
# there, so the Windows side works off the current baseline instead of a stale branch.
$Ref = if ($env:XCAGI_REF) { $env:XCAGI_REF } else { 'main' }
...
if (Test-Path (Join-Path $Out 'base-login-windows-run.json')) {
    Write-Host '--- base-login-windows-run.json (head) ---'
    Get-Content (Join-Path $Out 'base-login-windows-run.json') -TotalCount 40
}