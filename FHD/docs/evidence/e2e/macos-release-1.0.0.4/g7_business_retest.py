#!/usr/bin/env python3
"""G7 重测：登录 → 模板上传 → 列表 → 发货单生成出单 → 文件下载（1.0.0.4 正式包实例）。

用法: python3 g7_business_retest.py [--base http://127.0.0.1:17500] [--xlsx 模板.xlsx]
环境: XCAGI_TEST_USER / XCAGI_TEST_PASS（本地验收租户账号，不入库）
输出证据: FHD/docs/evidence/e2e/macos-release-1.0.0.4/g7-retest-10004.json
"""
...
EV_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
NO_PROXY = {"http": None, "https": None}

opener = urllib.request.build_opener(urllib.request.ProxyHandler({}))
TOKEN = None
RESULT = {"phase": "G7-retest-10004", "steps": {}}
...
def main():
    ...
    ap.add_argument("--xlsx", default=os.path.join(EV_DIR, "g7-template-probe.xlsx"))
    args = ap.parse_args()
    ...
    RESULT["steps"]["health"] = {"status": s, "version": b.get("version"),
        "git_sha": ((b.get("build") or {}).get("git_sha") or b.get("git_sha") or "")[:12]}
    ...
    # templates list（全文搜索，勿截断——1.0.0.4 首轮误报根因：dict 响应被 [:400] 截断）
    s, b = jcall("GET", "/api/templates", headers=H)
    names = json.dumps(b, ensure_ascii=False)
    RESULT["steps"]["templates_list"] = {"status": s, "probe_present": "G7重测探针" in names,
        "total_listed": len((b or {}).get("templates") or []) if isinstance(b, dict) else None}

    # 确保客户存在（幂等：已存在返 400「客户名称已存在」视为通过）
    ...
    RESULT["steps"]["customer_ensure"] = {"status": s, "detail": detail,
        "already_exists": s == 400 and isinstance(detail, str) and "已存在" in detail,
        "customer_id": (b.get("data", {}) or {}).get("id") if isinstance(b, dict) else None}
    ...
    # 文件下载回读（GET /api/shipment/download/{doc_name}，校验 xlsx 魔数 + 非空 + SHA256）
    if doc_name:
        s2, raw = call("GET", f"/api/shipment/download/{quote(doc_name)}", headers=H)
        RESULT["steps"]["file_download"] = {
            "status": s2, "bytes": len(raw),
            "xlsx_magic": raw[:4] == b"PK\x03\x04",
            "sha256": hashlib.sha256(raw).hexdigest(),
            "filename": doc_name,
        }
    else:
        RESULT["steps"]["file_download"] = {"skipped": True, "reason": "no doc_name from generate"}

    print("\n==== G7 RESULT ====")
    print(json.dumps(RESULT, ensure_ascii=False, indent=2))
    out = os.path.join(EV_DIR, "g7-retest-10004.json")
