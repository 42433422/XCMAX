...
`POST /api/auth/login-with-phone-code` — body `{ phone, code, account_kind? }`；后置链与密码登录相同（市场 JWT + 本地 session + Mod 权益 + tenant）。

`POST /api/market/send-phone-code` — body `{ phone }`；登录页发送短信验证码（代理市场）。

### PC 扫码登录

1. Web `POST /api/auth/qr/issue` → `{ qr_id, poll_secret, expires_at }`
2. Web 轮询 `GET /api/auth/qr/status?qr_id=&poll_secret=` → `pending` / `confirmed` / `expired`
3. Android `POST /api/mobile/v1/auth/qr/confirm` — body `{ qr_id, username, password }`（二维码内容 `xcagi://auth-qr?qr_id=…`）
4. Web poll 收到 `confirmed` 后 Set-Cookie + 进首页
...
登录成功响应与 `/api/auth/me` 可含 `tenant_id`、`tenant_name`（`sessions.tenant_id` + `users.tenant_id`）。
...
| FHD Web 手机验证码 | 企业版 | `POST /api/market/send-phone-code` → `POST /api/auth/login-with-phone-code` |
| FHD Web 扫码 | 企业版 | `qr/issue` + `qr/status` poll；App `auth/qr/confirm` |
| FHD Web SSO | OIDC 启用 | `GET /api/auth/oidc/start` |
| Android | 电脑在线 + 已配置 host | `POST /api/mobile/v1/auth/login` → `GET session-handoff` |
| Android 手机验证码 + PC 在线 | | `POST /api/mobile/v1/auth/login-with-phone-code` |
| Android SSO | OIDC | Custom Tabs → `POST /api/mobile/v1/auth/oidc/exchange` |
...
- [mobile_repository.dart](../../mobile-flutter/lib/src/data/mobile_repository.dart)
