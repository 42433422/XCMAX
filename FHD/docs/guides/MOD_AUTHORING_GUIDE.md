...
> 版本对齐：XCAGI 10.0（Neuro-DDD + FastAPI 主栈；含桌面壳交付，v10 线内迭代）。
...
本 monorepo 存在 **`FHD/mods/`** 与 **`FHD/XCAGI/mods/`** 两份 Mod 目录，但**只允许在一个地方改**：

| 路径 | 角色 |
|------|------|
| **`FHD/mods/<mod_id>/`** | **SSOT（唯一编辑源）**。本地 `start-enterprise-dev.sh` 的 `XCAGI_MODS_ROOT` 与 Vite `@mod-views` 均优先读此处。 |
| **`FHD/XCAGI/mods/<mod_id>/`** | **导出副本**。供 Docker 挂载、`run_packed.py`、文档中的 `XCAGI/mods` 路径；**禁止手改**。 |

**工作流**

1. 新建或修改 Mod → 只编辑 `FHD/mods/<mod_id>/`。
2. 同步到导出副本（发版前或改完 bridge Mod 后）：

   ```bash
   cd FHD
   python scripts/dev/mods_ssot.py sync
   # 或：bash scripts/dev/sync-mods-to-xcagi.sh
   ```
...
可选：`sync --mod xcagi-erp-domain-bridge` 只同步单个 Mod；`sync --prune` 删除导出副本中 SSOT 已移除的文件。

**反模式**：在 `XCAGI/mods` 改 Vue/Python 而不同步到 `mods/` — 浏览器仍显示旧 UI（Vite 优先读 `mods/`）。
...
客户品牌与账号定制包的映射 SSOT 见 [`config/customer_delivery.json`](../../config/customer_delivery.json)：`industry_mod_id` = 通用行业包，`legacy_mod_id` = 客户定制包。**生产员工私有交付只列 `legacy_mod_id`（∩ entitlement），不得把 `*-industry` 行业包列入该入口**（企业 entitlement 里两者常同时存在，属预期；入口过滤属口径，不是权益错误）。**转返工须填写问题说明并开现有客服变更工单**（`user_cs_change_request` · `bug_fix` · `source=private_mod_rework`），禁止另建工单系统。
...
| 方法 | 路径 | 行为 |
|---|---|---|
| `GET` | `/phone-agent/status?channel=wechat\|adb` | **始终 HTTP 200 + JSON**；即便内部失败也要返回 `{"success": true/false, "data": {...}}`，避免前端只看到「HTTP 500」而拿不到任何诊断字段。 |
...
**模板脚手架**：`MODstore/templates/skeleton/`（变量：`__MOD_ID__`、`__MOD_NAME__`）。
