...
|------|-------:|---------:|-------:|---------:|--------|------|
| **已提交快照**（`1569dfa4` · 06-14） | **52.74%** | **37.17%** | **55.82%** | **62.80%** | 3,785 / 0 fail | 低于当前 floor / 目标，不可按当前门禁宣称绿 |
| **WIP**（工作区 · 06-17） | **74.56%** | **61.80%** | **74.15%** | **69.54%** | 12,675 / **196 fail + 7 err** | 未提交 · 不可对外宣称 |

**棘轮 floor**：后端行 **80%** / 分支 **70%**；前端 lines **73%** / branches **68%** / functions **64%** / statements **73%**。

**距终态（按 WIP 计）**：后端行差 **~15pp**（目标 90%）；前端 lines 差 **~6pp**（目标 80%）。
**阻塞**：须先清零 196 个失败 + 7 个 error，并用完整测试 + 覆盖率产物复跑通过后，再 `--bump`。
