...
> **⚠️ 本文档为 2026-04-18 历史快照，已退役**。其中覆盖率、行数、测试维度等数字**均已过期**，**禁止对外引用**。
> 当前权威口径见：
> - 覆盖率提升计划 [`reports/COVERAGE_RAMP.md`](COVERAGE_RAMP.md)
> - 覆盖率唯一数字口径 [`metrics/coverage-dual-summary.json`](../../metrics/coverage-dual-summary.json)（覆盖率 `SSOT`）
> - 对外声称 vs 实测 [`docs/CLAIMED_VS_ACTUAL.md`](../CLAIMED_VS_ACTUAL.md)
>
> 本文件保留作历史评估快照，正文不再维护；过期数字处已加删除线与实测批注。
...
**统计数据**:
- 后端 Python 代码：~~~22,500 行~~ **（已过期；实测 `app/` 真实 199,349 行，`find FHD/app -name '*.py' | xargs wc -l`，2026-06-21）**
...
### 4.1 测试覆盖率：~~**18%**~~ ❌ **（已过期；实测后端行 85.07% / 前端行 90.03%，见 [`metrics/coverage-dual-summary.json`](../../metrics/coverage-dual-summary.json)）**

**统计**:
- 单元测试：45 个 (严重不足)
- 集成测试：12 个
- E2E 测试：~~0 个~~ ❌ **（已过期；实测 `frontend/e2e/` 有 10 个 spec，`frontend/e2e/*.spec.ts`）**
- 性能测试：0 个 ❌
- 安全测试：0 个 ❌
...
- 安全性：6.0/10 ⭐⭐⭐
- 可维护性：6.5/10 ⭐⭐⭐
- 测试覆盖：~~2.0/10~~ ❌ **（已过期；实测覆盖率已达后端行 85.07% / 前端行 90.03%，见 [`metrics/coverage-dual-summary.json`](../../metrics/coverage-dual-summary.json)）**
- 文档完善：7.0/10 ⭐⭐⭐⭐
- 性能优化：6.5/10 ⭐⭐⭐
- 工程实践：5.5/10 ⭐⭐⭐
...
| 模块化 | Mod 系统 | 插件系统 | ⭐⭐⭐⭐ |
| 代码质量 | 6.5/10 | 7.0/10 | ⭐⭐⭐ |
| 测试覆盖 | ~~18%~~（已过期；实测后端行 85.07% / 前端行 90.03%） | 60% | ⭐ |
| 安全性 | 6.0/10 | 7.5/10 | ⭐⭐ |
