...
| 智能对话(Chat) | `frontend/src/views/ChatView.vue` `frontend/src/api/chat.ts` `frontend/src/composables/useChatView.ts` | `static/js/modules/chat.js`（生产构建默认不加载） | vue |
...
- Vue 页面生产构建默认 **不** 加载 `chat.js`（`__ENABLE_LEGACY__` 须显式开启）；开发环境仍可按需加载其余 legacy 脚本：
  - `modules/chat.js`
  - `modules/products.js`
