...
test.describe('XCAGI 前端冒烟 @5001', () => {
  ...
  test('首页可加载且主壳在超时内可见（非持久 opacity:0）', async ({ page, request }) => {
    ...
    if (!apiProbe || !apiProbe.ok()) {
      test.info().annotations.push({
        type: 'notice',
        description: '经 Vite 代理的 /api/health 未成功：多为 5000 后端未启动或仍初始化。页面应仍能出壳，但聊天等功能会失效。',
      })
    }
  ...
  test('http://localhost:5001 与 127.0.0.1 行为一致（Windows IPv6 localhost）', async ({ page }) => {
    const webBase = (process.env.PLAYWRIGHT_BASE_URL || 'http://127.0.0.1:5001').replace(/127\.0\.0\.1/, 'localhost')
    if (isFullStack()) {
