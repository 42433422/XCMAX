...
function emitUploadState(state, extra = {}) {
  try {
    ...
    window.dispatchEvent(
      new CustomEvent('ai-upload-status', {
        detail: {
          state,
          purpose: currentImportPurpose || 'general',
          ...extra,
...
function buildExcelNextStepPrompt(suggestedUseCustomersImport) {
  ...
  return [
    ...
    ...options,
    '',
    suggestedUseCustomersImport
      ? '💡 若为购买单位列表，回复「导入购买单位」或「上传客户管理」并重新选择该文件即可自动导入。'
      : '当前模板能力：发货单模板、出货记录模板（后续可继续扩展）。',
  ].join('\n')
}
  ...
  for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          try {
            if (rawText.trim().startsWith('<')) {
              throw new Error(
                '服务器返回了网页而非数据，请确认后端已启动且接口 /api/customers/import 可用（状态码 ' + importResp.status + '）',
              )
            }
    ...
    try {
      ...
      if (result && result.success) {
        ...
        const messageParts = [`📎 文件分析完成: ${file.name}`, `解析器: ${parser}`, `分析结果:\n${resultText.slice(0, 700)}`]
        if (preview && summary && preview !== summary) {
          messageParts.push(`内容预览:\n${preview.slice(0, 350)}`)
        }
        // unit_products_db：优先自动导入；仅在无法确定单位名或导入失败时再进入聊天补充。
        if (result.suggested_use === 'unit_products_db' && result.saved_name) {
          ...
          const candidates = Array.isArray(result.unit_candidates) ? result.unit_candidates : []
          const fallbackName = String(candidates[0] || '').trim()
          const unitNameForImport = guess || fallbackName
          const savePending = (stageName) => {
            try {
              let sessionId = ''
              try {
              ...
              window.__xcagiPendingUnitProductsImport = {
                ...
                unit_name_guess: guess,
                unit_candidates: candidates,
                stage: stageName,
                session_id: sessionId,
              }
              try {
                localStorage.setItem('xcagiPendingUnitProductsImport', JSON.stringify(window.__xcagiPendingUnitProductsImport))
              } catch (_) {}
            } catch (_) {
              /* best-effort */
            }
          ...
          if (unitNameForImport) {
            try {
              const importResp = await fetch(API_BASE + '/api/ai/sqlite/import_unit_products', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                  saved_name: result.saved_name,
                  unit_name: unitNameForImport,
              ...
              if (importResult && importResult.success) {
                ...
                const skipped = Number(importResult.skipped_duplicates || 0)
                messageParts.push(`已自动导入购买单位「${unitNameForImport}」产品：新增 ${imported} 条，重复跳过 ${skipped} 条。`)
                try {
                ...
                const errMsg =
                  importResult && (importResult.message || importResult.error)
                    ? importResult.message || importResult.error
                    : `自动导入失败 (HTTP ${importResp.status})`
                messageParts.push(`自动导入失败：${errMsg}`)
                savePending('choose_candidate')
                messageParts.push('可在聊天中回复「改成 <名称>」或输入候选序号，继续导入。')
              }
            } catch (e) {
              messageParts.push(`自动导入请求失败：${e && e.message ? e.message : String(e)}`)
              savePending('choose_candidate')
              messageParts.push('可在聊天中回复「改成 <名称>」或输入候选序号，继续导入。')
            }
          } else {
            savePending('choose_candidate')
            messageParts.push('未能识别购买单位名称。可在聊天中回复「改成 <名称>」或输入候选序号直接导入。')
          }
        }
