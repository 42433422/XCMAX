...
function renderTools(tools) {
  ...
  tools.forEach((tool) => {
    const categoryKey =
      tool && tool.category && tool.category.category_key ? tool.category.category_key : tool && tool.category ? tool.category : 'other'
...
function filterTools() {
  ...
  let filtered = allTools.filter((tool) => {
    const categoryKey =
      tool && tool.category && tool.category.category_key ? tool.category.category_key : tool && tool.category ? tool.category : ''
    const matchSearch =
      !search || (tool.name || '').toLowerCase().includes(search) || (tool.description || '').toLowerCase().includes(search)
