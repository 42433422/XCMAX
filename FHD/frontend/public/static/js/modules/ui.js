...
function bindUiEvents() {
  ...
  bind('importCustomersExcelInput', 'change', function () {
    ...
    fetch(apiBase + '/api/customers/import', { method: 'POST', body: form })
      ...
      .catch(function (err) {
        return alert('导入失败：' + (err.message || '网络错误'))
      })
      .finally(() => {
        input.value = ''
      })
...
function showTemplateOverviewModal(apiBase) {
  ...
  fetch(apiBase + '/api/shipment-template/overview?filename=' + encodeURIComponent('发货单模板.xlsx'))
    ...
    .then(function (data) {
      ...
      if (entries.length) {
        html +=
          '<p><strong>可编辑词条：</strong></p><p style="margin:0 0 8px;font-size:13px;color:#666;">' +
          entries
            .map(function (e) {
              return e.name + '(' + (e.column || '') + ')'
            })
            .join('、') +
          '</p>'
      }
  ...
  fetch(apiBase + '/api/distillation/versions')
    ...
    .then(function (data) {
      ...
      if (versions.length === 0) {
        el.innerHTML =
          '<p class="muted">暂无训练产物。专业版对话会参与蒸馏，积累数据后运行「蒸馏/每周重训」即可生成 last.pt、best.pt。</p><p class="muted">已积累样本数：' +
          samples +
          '</p>'
        return
      }
      var html =
        '<table class="table" style="width:100%; margin-bottom:12px;"><thead><tr><th>文件</th><th>说明</th><th>修改时间</th><th>大小</th></tr></thead><tbody>'
