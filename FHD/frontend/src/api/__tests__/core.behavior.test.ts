...
function makeResponse(opts: { ok?: boolean; status?: number; contentType?: string; json?: unknown }) {
...
describe('error handling', () => {
  ...
  it('uses nested error.message', async () => {
    fetchMock.mockResolvedValue(makeResponse({ ok: false, status: 422, json: { error: { message: '校验失败' } } }))
  ...
  it('dispatches prompt-lan-gate on 401 license_required', async () => {
    ...
    fetchMock.mockResolvedValue(
      makeResponse({
        ok: false,
        status: 401,
        json: { error: 'license_required', message: '需授权' },
      }),
    )
