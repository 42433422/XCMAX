...
function claudeEndpoint(scope: ClaudeSuperEmployeeApiScope = 'admin'): string {
  return scope === 'mobile' ? '/api/mobile/v1/admin/claude-super-employee/messages' : '/api/admin/claude-super-employee/messages'
}
...
export async function fetchClaudeSuperEmployeeMessages(options: ClaudeSuperEmployeeApiOptions = {}): Promise<ClaudeSuperEmployeeMessage[]> {
