...
export const manualInductApi = {
  preview(payload: { target_scope: string; purchase_unit?: string; rows: Record<string, unknown>[] }) {
    return api.post<{
      success: boolean
      message?: string
  ...
  commit(payload: {
    ...
    return api.post<{ success: boolean; message?: string; summary?: Record<string, unknown> }>('/api/excel/manual-induct/commit', payload)
  },
