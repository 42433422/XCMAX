 ...
 * 搜索 MOD
 */
export async function searchMods(query?: string, author?: string, installed?: boolean, limit: number = 50): Promise<ModSearchResult> {
 ...
 * 上传 MOD 包
 */
export async function uploadModPackage(file: File, activate: boolean = false): Promise<UploadResult> {
...
export async function installMod(
  mod: string | Pick<ModCatalogItemUi, 'id' | 'pkg_id' | 'version' | 'package_file'>,
 ...
 * 更新 MOD
 */
export async function updateMod(modId: string, packageFile: string): Promise<InstallResult> {
  const payload = {
    mod_id: modId,
...
export async function bootstrapEditionPack(
  ...
  const response = await apiFetch(`/api/mod-store/bootstrap-edition-pack?edition=${encodeURIComponent(edition)}`, { method: 'POST' })
  const data = await response.json()
  clearDeliverableStatusCache()
  if (!response.ok) {
    const detail = (data && (data.detail || data.message)) || response.statusText
...
export async function reloadEmployeePacks(packId?: string): Promise<{ success: boolean; message: string; data?: Record<string, unknown> }> {
...
export async function installOfficeEmployeePack(options?: {
  ...
  const errors: string[] = []
  const targets = OFFICE_EMPLOYEE_PKG_IDS.map((id) => available.find((m) => (m.pkg_id || m.id) === id)).filter(
    Boolean,
  ) as ModCatalogItemUi[]
