...
export const productsApi = {
  ...
  async getProductUnits(): Promise<{ success: boolean; data: string[]; count: number }> {
    const resp = await api.get<{ data?: unknown[] | { units?: unknown[] } }>(resolveErpApiPath('/api/products/units'))
    const raw = resp?.data
    const nested = raw && typeof raw === 'object' && !Array.isArray(raw) ? (raw as { units?: unknown[] }) : null
  ...
  searchProductNames(keyword: string): Promise<ApiResponse<unknown[]>> {
    return api.get<ApiResponse<unknown[]>>(`${erpBase()}/products/product_names/search`, {
      keyword,
    })
  },
