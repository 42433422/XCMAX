...
describe('tutorialV2Api', () => {
  ...
  it('loads course, current-run, and team-report contracts', async () => {
    ...
    apiMock.get.mockResolvedValueOnce({ data: courses }).mockResolvedValueOnce({ data: run }).mockResolvedValueOnce({ data: reports })
    ...
    expect(apiMock.get.mock.calls).toEqual([['/api/tutorial/v2/courses'], ['/api/tutorial/v2/runs/current'], ['/api/tutorial/v2/reports']])
  })
  ...
  it('uses the exact start, lifecycle, verification, and reset routes', async () => {
    ...
    await expect(
      tutorialV2Api.verify('run-1', 'create-customer', {
