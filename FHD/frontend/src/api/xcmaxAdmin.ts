...
export const xcmaxAdminApi = {
  ...
  setUserEnterprise(userId: number, isEnterprise: boolean) {
    return api.put(`/api/xcmax/admin/market/users/${userId}/enterprise?is_enterprise=${isEnterprise}`)
  },
  getUserProfiles() {
  ...
  pullSync() {
    return api.post<{ data?: { pull?: Record<string, unknown>; apply?: Record<string, unknown> } }>('/api/xcmax/sync/pull', {})
  },
  getCurrentEntitlementsSyncStatus() {
    return api.get<{ data?: CurrentEntitlementsSyncStatus; message?: string }>('/api/xcmax/sync/entitlements/current')
  },
  ...
  checkDeployUpdates(channel?: string) {
    if (channel) {
      return api.get<{ data?: DeployCheckData; message?: string }>('/api/xcmax/admin/deploy/check', { channel })
    }
  ...
  startDeployPush(body: Record<string, unknown>) {
    return api.post<{ data?: DeployJobData; message?: string }>('/api/xcmax/admin/deploy/push', body)
  },
  getDeployJob(jobId: string) {
    return api.get<{ data?: DeployJobData; message?: string }>(`/api/xcmax/admin/deploy/jobs/${encodeURIComponent(jobId)}`)
  },
}
