...
export const xcmaxDeployApi = {
  ...
  applyEnterpriseUpdate(
    ...
    return api.post<{ success?: boolean; data?: DeployJobData; message?: string }>('/api/xcmax/deploy/apply', body)
  },
  getEnterpriseJob(jobId: string) {
    return api.get<{ success?: boolean; data?: DeployJobData }>(`/api/xcmax/deploy/jobs/${encodeURIComponent(jobId)}`)
  },
}
