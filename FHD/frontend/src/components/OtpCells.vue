...
const digits = computed(() => props.modelValue.replace(/\D/g, '').slice(0, props.length).split(''))
...
<template>
  <div class="otp-cells" role="group" aria-label="短信验证码">
    <div class="otp-cells__row">
      <div v-for="index in length" :key="index" class="otp-cells__cell" :class="{ 'is-focused': !disabled && digits.length === index - 1 }">
        {{ digits[index - 1] || '' }}
      </div>
      <input
        ref="inputRef"
