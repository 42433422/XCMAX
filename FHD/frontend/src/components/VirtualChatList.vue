...
const visibleRange = computed(() => {
  ...
  const end = Math.min(props.messages.length - 1, findEndIndex(scrollTop.value + containerHeight.value))
...
function calculateMessageHeights() {
  ...
  const heights = batchEstimateMessageHeights(messages, props.maxMessageWidth - 32)
