  ...
  <div class="agent-task-runtime">
    ...
    <details v-if="toolCalls.length" class="agent-tool-sessions">
      ...
      <div v-for="tool in toolCalls" :key="tool.call_id" class="agent-tool-session">
        ...
        <small
          >{{ tool.status }}<template v-if="tool.duration_ms"> · {{ tool.duration_ms }}ms</template></small
        >
      </div>
    </details>
    ...
    <div class="agent-task-evidence">
      结果证据：{{ payload.eventCount || 0 }} 条事件 · {{ payload.completedToolCount || 0 }} 次已完成工具调用 ·
      {{ payload.artifactCount || 0 }} 个产物
    </div>
    ...
    <div class="task-actions">
      ...
      <button v-if="isRetryable" class="btn btn-primary btn-sm" @click="$emit('retry')">
        {{ $t('chat.retryTask') }}
      </button>
      <button v-if="isPausable" class="btn btn-secondary btn-sm" @click="$emit('pause')">
        {{ $t('chat.pauseTask') }}
      </button>
      <button v-if="task.status === 'paused'" class="btn btn-primary btn-sm" @click="$emit('resume')">
        {{ $t('chat.resumeTask') }}
      </button>
      <button v-if="isCancellable" class="btn btn-secondary btn-sm" @click="$emit('cancel')">
        {{ $t('chat.cancel') }}
      </button>
    </div>
  </div>
...
.agent-step-status--retrying {
  color: #1d4ed8 !important;
}
.agent-step-status--waiting_user {
  color: #b45309 !important;
}
.agent-step-status--failed {
  color: #b91c1c !important;
}
.agent-tool-sessions summary {
  cursor: pointer;
  font-size: 11px;
  font-weight: 600;
  color: #334155;
...
.agent-result-evidence summary {
  cursor: pointer;
  font-size: 11px;
  font-weight: 600;
  color: #334155;
}
.agent-result-evidence pre {
  max-height: 220px;
  margin: 6px 0 0;
  overflow: auto;
...
.agent-result-evidence ul {
  margin: 6px 0 0;
  padding-left: 18px;
  font-size: 10px;
  color: #334155;
}
.agent-result-evidence small {
  display: block;
  word-break: break-all;
  color: #64748b;
}
