...
import { PERSY_NODE_THEME as nodeTheme } from './persyKnowledgeGraphTheme'
...
function prioritizeGraphNodes(nodes: KnowledgeGraphNode[]): KnowledgeGraphNode[] {
  const stickyTypes = new Set(['core','erp_constraint','erp_domain','erp_ontology','erp_rule','onboarding','recall','topic','memory'])
...
function buildOption(): EChartsCoreOption {
  ...
  const data = nodes.map((node) => {
    ...
    const showLabel =
      selected || recalled || ['core','topic','onboarding','erp_ontology','erp_domain','erp_constraint'].includes(node.type)
    return {
      id: node.id,
      name: node.label,
      ...
      label: {
        ...
        position:
          node.type === 'core'
            ? 'inside'
            : ['topic','memory','erp_ontology','erp_domain','erp_constraint'].includes(node.type)
              ? 'top'
              : 'right',
