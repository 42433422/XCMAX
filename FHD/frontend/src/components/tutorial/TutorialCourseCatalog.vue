<template>
  <section class="tutorial-v2-catalog" aria-label="进阶教程课程目录">
    ...
    <div class="tutorial-v2-upgrade" role="status">这是新版动手教程。旧教程记录还在，但建议从第 1 门重新学一次。</div>
    ...
    <ul v-else class="tutorial-v2-course-list">
      <li v-for="(course, index) in store.courses" :key="course.id" class="tutorial-v2-course-card">
        ...
        <div class="tutorial-v2-meta">
          ...
          <span v-if="course.prerequisite_ids.length"> 开始前先完成：{{ prerequisiteLabel(course.prerequisite_ids) }} </span>
        ...
        <p v-if="course.locked" class="tutorial-v2-locked">先完成：{{ prerequisiteLabel(course.missing_prerequisite_ids) }}</p>
        <div class="tutorial-v2-actions">
          <button type="button" class="btn btn-primary btn-sm" :disabled="course.locked || store.loading" @click="openCourse(course)">
            {{ actionLabel(course) }}
          </button>
          <button v-if="course.run" type="button" class="btn btn-secondary btn-sm" :disabled="store.loading" @click="resetCourse(course)">
            重置
          </button>
        </div>
      </li>
    ...
    <details class="tutorial-v2-team">
      ...
      <ul v-if="store.reports.length" class="tutorial-v2-report-list">
        <li v-for="(report, index) in store.reports" :key="`${report.user_id}-${report.course_id}-${index}`">
          {{ report.user_name || `成员 #${report.user_id}` }} · {{ courseLabel(String(report.course_id || '')) }} ·
          {{ reportStatusLabel(String(report.status || '')) }} · {{ report.progress }}% · 尝试 {{ report.attempt_count }} 次
        </li>
      </ul>
    </details>
  </section>
...
async function resetCourse(course: TutorialCourseDTO) {
  ...
  const confirmed = window.confirm(
    '重新学习会清空本轮练习并从第一步开始。你公司的正式数据不会受影响；旧练习数据会在 7 天后自动清理。确定重新开始吗？',
  )
  if (!confirmed) return
...
.tutorial-v2-catalog__head {
  display: flex;
  justify-content: space-between;
  gap: 12px;
  align-items: flex-start;
}
.tutorial-v2-catalog__head p,
.tutorial-v2-course-card p {
  margin: 4px 0 0;
  color: #56697a;
  line-height: 1.45;
}
.tutorial-v2-space-badge {
  flex: 0 0 auto;
  border-radius: 999px;
  padding: 4px 9px;
  color: #7a3d00;
...
.tutorial-v2-course-card__top {
  display: flex;
  justify-content: space-between;
  gap: 10px;
}
.tutorial-v2-course-card__number {
  display: block;
  margin-bottom: 3px;
  color: #2678ad;
  font-size: 11px;
...
.tutorial-v2-status[data-status='completed'] {
  color: #137247;
  font-weight: 700;
}
.tutorial-v2-meta {
...
.tutorial-v2-progress {
  height: 6px;
  overflow: hidden;
  border-radius: 999px;
  background: #e8eef3;
}
.tutorial-v2-progress span {
  display: block;
  height: 100%;
  background: linear-gradient(90deg, #238bd3, #33b985);
}
.tutorial-v2-locked {
...
.tutorial-v2-team summary {
  cursor: pointer;
  font-weight: 600;
  margin-bottom: 9px;
}
.tutorial-v2-report-list {
  padding-left: 18px;
  max-height: 140px;
  overflow: auto;
}
.tutorial-v2-report-hint {
  color: #66798a;
}
