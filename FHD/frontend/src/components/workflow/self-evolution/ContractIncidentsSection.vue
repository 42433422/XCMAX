...
<template>
  <div v-if="allSurfaceIncidents.length" class="selp-contract-incidents" aria-label="模块异常事件">
    <div class="selp-contract-incidents-head">
      ...
      <small>{{
        asArray(surfaceIncidentSummary.surfaces)
          .map((s) => asString(s))
          .slice(0, 4)
          .join(' / ') || '异常事件'
      }}</small>
    ...
    <div class="selp-contract-incidents-grid" role="list">
      ...
      >
        ...
        <small
          >{{ incident.requires_admin ? '仅管理员' : '操作员' }} · {{ incident.executable ? '可执行' : '仅导航' }} ·
          {{ firstText(incident.id, '状态:模块') }}</small
        >
        <small
          >{{ firstText(incident.source, '状态校验') }} · {{ firstText(incident.schema_version, schemaVersion) }} ·
          {{ firstText(incident.created_at, '时间未知') }}</small
        >
        <em>{{
          asArray(incident.missing)
            .map((item) => asString(item))
            .filter(Boolean)
            .slice(0, 5)
            .join(' / ') || firstText(incident.detail, '缺少依赖')
        }}</em>
      </div>
    </div>
  </div>
