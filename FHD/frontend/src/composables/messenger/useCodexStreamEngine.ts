...
export function useCodexStreamEngine(input: CodexStreamEngineInput) {
  const { codexMessages, codexScrollEl, activeSystemEntry, fetchActiveSuperMessages } = input
  ...
  function ensureCodexTypewriter(): void {
    ...
    codexStreamTimer = setInterval(() => {
      ...
      if (current.length >= codexStreamTarget.length) {
        stopCodexTypewriter()
        codexStreamActive.value = codexStreamMessageId.value === CODEX_STREAM_PLACEHOLDER_ID && Boolean(codexPollTimer)
        return
      }
  ...
  function startCodexTypewriter(options: {
    ...
    if (options.reset || messageChanged || !target.startsWith(codexStreamBody.value) || codexStreamBody.value.length > target.length) {
  ...
  function syncCodexStreamFromMessages(items: CodexSuperEmployeeMessage[], requestId = ''): boolean {
    const effectiveRequestId = requestId || String(latestCodexDispatcherMessage(items)?.dispatch_request_id || '')
