...
import { dutyEmployeesFromEmployeeSsot, type EmployeeSsotPayload } from '@/utils/employeeSsotContacts'
...
import { fetchClaudeSuperEmployeeMessages, sendClaudeSuperEmployeeMessage } from '@/api/claudeSuperEmployee'
import { fetchCursorSuperEmployeeMessages, sendCursorSuperEmployeeMessage } from '@/api/cursorSuperEmployee'
...
export function useSuperEmployeeDispatch(params: UseSuperEmployeeDispatchParams) {
  ...
  const codexApiScope = computed<CodexSuperEmployeeApiScope>(() => (isAdminConsoleSpa() ? 'admin' : 'mobile'))
  ...
  const codexSenderLabel = computed(() => (codexApiScope.value === 'mobile' ? '手机端' : '管理端'))

  const codexContextSource = computed(() => (codexApiScope.value === 'mobile' ? 'mobile_im' : 'admin_im'))
  ...
  const codexVisibleMessages = computed<CodexDisplayMessage[]>(() => {
    ...
    if (codexStreamBody.value && codexStreamMessageId.value === CODEX_STREAM_PLACEHOLDER_ID) {
      visible.push({
  ...
  async function loadDutyEmployees(): Promise<void> {
    ...
    try {
      ...
      const normalized = rawItems.map(normalizeDutyEmployee).filter((item): item is DutyEmployeeEntry => Boolean(item))
      if (normalized.length) {
        dutyEmployees.value = uniqueDutyEmployees(normalized)
      }
    } catch (error) {
      showAppToast(productErrorMessage(error, '员工通讯录暂时不可用，已使用本地编制兜底'), 'warning')
    }
  }
  ...
  async function onDutyEmployeeSend(): Promise<void> {
    ...
    try {
      const result = await api.post<EmployeeExecuteResponse>(`/api/xcmax/local/employees/${encodeURIComponent(entry.id)}/execute`, {
        task: text,
