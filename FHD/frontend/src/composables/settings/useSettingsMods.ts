...
export function useSettingsMods() {
  ...
  const modelPaymentBridgeInstalled = computed(() => mods.value.some((m) => String(m.id || '').trim() === MODEL_PAYMENT_BRIDGE_ID))
  ...
  const selectableExtensionMods = computed(() => mods.value.filter((m) => isSelectableExtensionModId(String(m.id || ''))))

  const workflowEmployeeMods = computed(() => mods.value.filter((m) => isWorkflowEmployeeModId(String(m.id || ''))))
  ...
  const modRoutesStatusText = computed(() => {
    ...
    if (isLoaded.value && mods.value.length > 0 && modRoutes.value.length === 0) {
  ...
  const modSettingsFoldMeta = computed(() => {
    ...
    const wf = workflowEmployeeMods.value.length ? t('settings.modFoldWorkflow', { count: workflowEmployeeMods.value.length }) : ''
  ...
  async function onUninstallMod(modId: string) {
    ...
    try {
      ...
      if (!data || !data.success) {
        await appAlert(
          t('settings.uninstallFailed', {
            detail: (data && (data.message || data.error)) || t('settings.unknownError'),
          }),
        )
        return
      }
  ...
  const installedAccountCustomMod = computed(() => mods.value.find((m) => accountCustomModIds.has(String(m.id || '').trim())) || null)
  ...
  const aboutDisplayLine = computed(() => {
    ...
    if (brand) {
      const industryPart = industry ? t('settings.aboutIndustryLine', { industry }) : t('settings.aboutCustomLine')
  ...
  const intentPackageEntries = computed(() => {
    ...
    return INTENT_PACKAGE_ORDER.filter((key) => pkgs[key]).map((key) => ({
      ...
      keywords: Array.isArray(pkgs[key].keywords) ? pkgs[key].keywords.filter((kw) => String(kw || '').trim()).slice(0, 12) : [],
    }))
  })
  ...
  async function loadIndustries() {
    try {
      ...
      if (response.success) {
        ...
        const cur = (!Array.isArray(payload) ? payload?.current : undefined) ?? industryStore.currentIndustry?.id ?? DEFAULT_INDUSTRY_ID
  ...
  async function loadCurrentIndustryDetail() {
    try {
      ...
      if (response.success) {
        ...
        currentIndustryUnit.value = (typeof fromMod === 'string' && fromMod.trim()) || asString(units?.primary) || '天'
