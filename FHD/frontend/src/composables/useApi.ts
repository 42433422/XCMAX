...
function resolveApiError(err: unknown, fallback = ''): Error {
  if (err instanceof ApiError) {
    ...
    const nested = data?.error && typeof data.error === 'object' ? (data.error as { code?: string; message?: string }) : null
...
export function useApi<T>(apiFn: (params: Record<string, unknown>) => Promise<T>, options: UseApiOptions<T> = {}): UseApiReturn<T> {
