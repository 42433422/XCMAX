...
type LinkedExcelSheet = { sheet_name: string; sheet_index: number }

function resolveExcelFilePathFromAnalysis(result: any): string {
  const candidates = [
    result?.file_path,
    result?.preview_data?.file_path,
    result?.data?.file_path,
    result?.data?.preview_data?.file_path,
    result?.document?.filepath,
    result?.document?.file_path,
    result?.meta?.file_path,
...
function resolveExcelSheetOptionsFromContext(ctx: any): LinkedExcelSheet[] {
  ...
  if (allSheets.length) {
    allSheets.forEach((s: any, idx: number) => {
      ...
      if (!name) return
      result.push({ sheet_name: name, sheet_index: Number(s?.sheet_index) || idx + 1 })
    })
    return result
  }
...
function resolveLinkedSheetGridPreview(ctx: any, linkedSheet: LinkedExcelSheet | null): Record<string, any> | null {
  if (!ctx || typeof ctx !== 'object' || !linkedSheet?.sheet_name) return null
  const allSheets = Array.isArray(ctx?.preview_data?.all_sheets) ? ctx.preview_data.all_sheets : []
  const target = allSheets.find((s: any) => {
  ...
  const fieldNames = (Array.isArray(target?.fields) ? target.fields : [])
    ...
    .filter(Boolean)
    .slice(0, 40)
  const rows = Array.isArray(target?.grid_preview?.rows) ? target.grid_preview.rows.slice(0, 60) : []
  const previewText = [
    ...
    rows.length ? `网格前 ${rows.length} 行：${JSON.stringify(rows)}` : ''
  ].filter(Boolean).join('\n')
  return {
...
function readPersistedTaskPanelState(sessionKey: string): PersistedTaskPanelState | null {
  ...
  try {
    ...
    const taskList = Array.isArray((parsed as any).taskList) ? (parsed as any).taskList : []
    const activeTaskId = String((parsed as any).activeTaskId || '').trim()
    const expandedTaskIds = Array.isArray((parsed as any).expandedTaskIds)
      ? (parsed as any).expandedTaskIds.map((x: any) => String(x || '').trim()).filter(Boolean)
      : []
    const filterRaw = String((parsed as any).taskFilter || '').trim()
    const taskFilter = (['all', 'running', 'success', 'failed'].includes(filterRaw) ? filterRaw : 'all') as PersistedTaskPanelState['taskFilter']
    const currentTask = ((parsed as any).currentTask && typeof (parsed as any).currentTask === 'object')
      ? (parsed as any).currentTask as ShipmentTask
      : null
    return {
      taskList,
      activeTaskId,
      ...
      taskFilter,
      currentTask,
      savedAt: Number((parsed as any).savedAt || 0) || 0,
    }
  } catch {
    return null
  }
...
function persistTaskPanelState(sessionKey: string, state: PersistedTaskPanelState): void {
  const sid = String(sessionKey || '').trim()
...
function clearPersistedTaskPanelState(sessionKey: string): void {
  const sid = String(sessionKey || '').trim()
  if (!sid || typeof sessionStorage === 'undefined') return
  try {
...
export function useChatView(options: UseChatViewOptions) {
  ...
  // TTS 语音播报开关（从 localStorage 读取设置）
  ...
  /** 流式/JSON 返回 requires_token（写入）时暂存，用于弹窗确认后自动重试同一条用户话 */
  const pendingDbWriteChatRetryMessages = ref<string[] | null>(null)
  /** 写入令牌弹窗出现前，Planner 流式已输出的正文（用于续跑时注入 db_write_stream_resume，避免同一对话里从头再读 Excel） */
  ...
  function persistTaskPanelStateForSession(targetSessionId?: string): void {
    ...
    persistTaskPanelState(sid, {
      ...
      currentTask: (currentTask.value ? { ...(currentTask.value as any) } : null) as ShipmentTask | null,
      savedAt: Date.now(),
    })
  }
  ...
  /** 下一条 Planner 请求将带入 ``context.multimodal_attachments``（发送后由 consume 清空） */
  ...
  // 切换扩展（Mod）时：历史面板如果正在展开，拉取当前 Mod 的会话列表覆盖原列表；
  // 未展开时将本地列表重置为当前 Mod 的本地缓存（下次打开会再覆盖为服务端最新）。
  watch(
  ...
  function normalizeHistorySessions(rawSessions: any[]): any[] {
    return (Array.isArray(rawSessions) ? rawSessions : [])
      .map((session: any, idx: number) => {
        const sid = String(session?.session_id || session?.id || '').trim()
        if (!sid) return null
        const title = String(session?.title || session?.summary || '').trim() || `会话 ${idx + 1}`
        const count = Number(
        ...
        return {
          ...
          last_message_at: String(session?.last_message_at || session?.updated_at || session?.created_at || '').trim(),
          is_local_only: false
        }
      })
      .filter(Boolean)
  ...
  function readLocalMessagesBySession(targetSessionId: string): ChatMessage[] {
    ...
    try {
      const raw = localStorage.getItem(buildChatMessagesKey(sid, String(modsStore.activeModId || '')))
      if (!raw) return []
      const parsed = JSON.parse(raw)
      ...
      return parsed
        .map((msg: any) => ({
          ...
          time: String(msg?.time || '').trim() || new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })
        }))
  ...
  function readLocalSessionMeta(targetSessionId: string): Record<string, any> | null {
    const sid = String(targetSessionId || '').trim()
    if (!sid || typeof localStorage === 'undefined') return null
    try {
      const raw = localStorage.getItem(buildChatSessionMetaKey(sid, String(modsStore.activeModId || '')))
      if (!raw) return null
      const parsed = JSON.parse(raw)
      return parsed && typeof parsed === 'object' ? parsed : null
    } catch {
  ...
  function buildLocalHistorySession(targetSessionId: string): any | null {
    const sid = String(targetSessionId || '').trim()
    if (!sid) return null
  ...
  function listLocalHistorySessions(limit = 50): any[] {
    if (typeof localStorage === 'undefined') return []
    // 按当前激活的 Mod 过滤：不同 Mod 的本地会话互相隔离。
    const currentMod = String(modsStore.activeModId || '')
    ...
    const sessions = Array.from(ids)
      ...
      .filter(Boolean)
    sessions.sort((a: any, b: any) => toHistoryTimestamp(b.last_message_at) - toHistoryTimestamp(a.last_message_at))
    return sessions.slice(0, Math.max(1, limit))
  }

  function mergeHistorySessions(serverRawSessions: any[]): any[] {
    ...
    serverSessions.forEach((session: any) => {
    ...
    merged.sort((a: any, b: any) => toHistoryTimestamp(b.last_message_at) - toHistoryTimestamp(a.last_message_at))
    return merged
  }

  function clearLocalHistoryCache(): void {
    if (typeof localStorage === 'undefined') return
    // 仅清空"当前 Mod"对应的本地会话缓存，避免跨 Mod 误删。
    const currentMod = String(modsStore.activeModId || '')
  ...
  /** 与 ``requestChatByMode``、Planner SSE 共用，保证上下文与专业/普通链路一致 */
  ...
  onMounted(() => {
    ...
    window.addEventListener(FHD_DB_WRITE_UNLOCKED_EVENT, onDbWriteUnlockedForChatRetry)
  ...
  onBeforeUnmount(() => {
    ...
    window.removeEventListener(FHD_DB_WRITE_UNLOCKED_EVENT, onDbWriteUnlockedForChatRetry)
      ...
      const sessionsRaw = Array.isArray((data as any)?.sessions)
        ...
        : Array.isArray((data as any)?.data)
          ...
          : Array.isArray((data as any)?.conversations)
            ...
            : []
      historySessions.value = mergeHistorySessions(sessionsRaw)
    } catch (e) {
      const localFallback = mergeHistorySessions([])
