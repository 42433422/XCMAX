...
import { XCMAX_SYNC_IM_MESSAGE_EVENT, XCMAX_SYNC_IM_READ_EVENT } from '@/composables/useXcmaxSync'
...
function isPublicRoute(name: unknown): boolean {
  const n = String(name || '')
  return n === 'login' || n.startsWith('login-') || n === 'lan-gate' || n === 'product-onboarding'
}
