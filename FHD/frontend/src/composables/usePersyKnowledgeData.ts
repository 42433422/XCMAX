...
export function usePersyKnowledgeData(options: { viewMode: Ref<PersyKnowledgeViewMode> }) {
  ...
  const graphNodeCount = computed(() => graph.value?.nodes.filter((node) => node.type !== 'core').length ?? 0)
  ...
  const retrievalModeText = computed(() => (semanticAvailable.value ? '语义召回' : '关键词召回'))
  const knowledgeNodes = computed(() => (graph.value?.nodes || []).filter((node) => node.type !== 'core'))
  ...
  const omniscientHint = computed(() => {
    ...
    const activeExpected = Number(omniscient.value.datasets?.[activeDatasetId.value]?.document_count || 0)
  ...
  async function refreshStatus(
    ...
    const includeDocuments = refreshOptions.includeDocuments === true || viewMode.value === 'sources'
  ...
  async function refreshGraph(expectedDatasetId?: string, epoch?: number): Promise<void> {
    ...
    try {
      ...
      selectedNode.value =
        nextGraph.nodes.find((node) => node.id === selectedId) || nextGraph.nodes.find((node) => node.type === 'core') || null
    } finally {
  ...
  async function refreshAll(): Promise<void> {
    ...
    const loadedNodes = Number(graph.value?.nodes?.filter((node) => node.type !== 'core' && node.type !== 'onboarding').length || 0)
