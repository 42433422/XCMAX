...
describe('useVisibleNavItems · 企业完整包客户行业侧栏', () => {
  ...
  it('非平台壳模式下选中太阳鸟也注入宿主业务卡片', () => {
    ...
    expect(visibleNavItems.value.find((i) => i.key === 'erp-hr')?.name).toBe('人事考勤')
    expect(visibleNavItems.value.find((i) => i.key === 'attendance-employees')?.name).toBe('人员管理')
    expect(visibleNavItems.value.find((i) => i.key === 'attendance-departments')?.name).toBe('部门管理')
