...
export function superEmployeeAvatarKeyForId(employeeId: string | null | undefined): SuperEmployeeAvatarKey | null {
  const id = String(employeeId || '').trim()
  return SUPER_EMPLOYEE_ID_TO_AVATAR_KEY[id] || null
}

export function superEmployeeAvatarSrcForKey(key: SuperEmployeeAvatarKey | null | undefined): string | null {
  if (!key) return null
  return SUPER_EMPLOYEE_AVATAR_SRC[key] || null
}

export function superEmployeeAvatarSrcForId(employeeId: string | null | undefined): string | null {
  return superEmployeeAvatarSrcForKey(superEmployeeAvatarKeyForId(employeeId))
}

export function resolveSuperEmployeeAvatarSrc(employeeId: string | null | undefined, avatarUrl?: string | null): string | null {
