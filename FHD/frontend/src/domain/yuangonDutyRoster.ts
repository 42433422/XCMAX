...
/** 编制员工说明，来源于本机员工包 manifest 的 description。 */
export const YUANGON_PKG_DESCRIPTIONS: Record<string, string> = {
  ...
  'doc-knowledge-curator': '维护 xiu-ci.com 与 MODstore 平台的全部文档资产：README、ESkill.md、docs/ 目录、需求/方案 Markdown，以及 yuangon/ 各员工 README 同步；可调用 py-doc-generator.xcemp 与 project-doc-generator.xcemp 辅助生成文档；不修改源码。员工包专属文档由 employee-pack-curator 全权负责，本员工不主动修改。',
  'employee-interview-assistant': '面向内部编制与协作场景：通过结构化提问与表单补全，帮助其他员工及岗位包补全元数据、能力说明、运行依赖与风险字段；不替代 HR 正式录用流程。',
  'employee-pack-quality-interviewer': '对候选 employee_pack 做结构化入职面试：基于 manifest 节选、同步测试日志或沙盒 JSON，对照职责边界与平台契约给出录用、条件录用或驳回结论。',
  'intent-analyst': '解析用户自然语言需求，提取结构化意图、领域关键词与建议能力；识别用户身份与权限。',
  'employee-planner': '根据结构化需求规划员工包架构，拆分员工职责、脚本工作流与 Skill 组；输出一站式员工蓝图。',
  'artifact-generator': '根据规划蓝图生成员工包产物，包括 manifest、Python 实现、资产文件；支持 LLM 驱动和资产驱动两种模式。',
  'quality-validator': '对生成的产物进行服务端校验，包括 manifest 合规性、Python 语法、资产完整性与一致性检查。',
  'miniapp-builder': '为员工包生成配套脚本工作流，将自然语言需求转化为可执行的脚本逻辑。',
  'script-binder': '将生成的脚本工作流嵌入员工包，更新 manifest 能力声明与目录结构。',
  'workflow-automator': '为员工包创建自动化工作流，通过自然语言生成画布节点与连线。',
  ...
  'enterprise-adoption-officer': '跟踪 O6 企业使用阶段：租户激活、功能采纳、用量遥测与回访触发；本岗偏数据与里程碑。',
  ...
  'mobile-ios-release-officer': 'P-S iOS 渠道发布：TestFlight / App Store 工程、notarize 协同与 release 门禁。',
  ...
  'ecosystem-investor-portal-officer': 'O-B B4 投资方或伙伴只读 Portal、里程碑与风险视图、进度只读 API。',
