...
function extractRoutesFromModule(m: Record<string, unknown>): RouteRecordRaw[] {
  ...
  for (const v of Object.values(m)) {
    ...
    if (first && typeof first.path === 'string' && (typeof first.component === 'function' || typeof first.components === 'object')) {
...
async function waitForRouterReadyOrTimeout(router: Router): Promise<void> {
  await Promise.race([router.isReady().catch(() => undefined), new Promise<void>((resolve) => window.setTimeout(resolve, 500))])
}
...
function findGlobKeyForMod(modId: string): string | undefined {
  const suffixes = [`/mods/${modId}/frontend/routes.js`]
 ...
 */
export async function registerModRoutes(router: Router, entries: ModRouteApiEntry[] | undefined | null): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    if (!key || !modRouteLoaders[key]) {
      if (import.meta.env.DEV && !missingBundleNotifiedModIds.has(mod_id)) {
        ...
        console.info(`[mods] Skip mod "${mod_id}": no frontend routes bundle (expected mods/${mod_id}/frontend/routes.js)`)
      }
      continue
    }
    try {
      ...
      for (const r of routes) {
        ...
        const beforeNames = new Set(currentBefore.matched.map((m) => String(m.name || '')).filter(Boolean))
        router.addRoute(r)
        const currentAfter = router.currentRoute.value
        const resolvedAfter = router.resolve(currentRefreshFullPath(router))
        const resolvedNames = resolvedAfter.matched.map((m) => String(m.name || '')).filter(Boolean)
        if (
      ...
      if (routes.length) {
        ...
        console.warn(`[mods] No routes extracted from "${mod_id}" bundle — check routes.js exports (named route array).`)
      }
    } catch (e) {
      console.error(`[mods] Failed to load routes for "${mod_id}":`, e)
      if (import.meta.env.DEV) {
        console.info('[mods] Hint: after adding mods/*/frontend/routes.js, run `npm run build` (or dev restart) so Vite includes the glob.')
      }
    }
  ...
  if (shouldRefreshCurrentRoute || shouldReplaceWithRefreshTarget(router, finalTargetFullPath)) {
