...
export const useModStoreStore = defineStore('modStore', () => {
  ...
  async function searchModsAction(query?: string, author?: string, installed?: boolean) {
    loading.value = true
    error.value = null
  ...
  async function rateModAction(modId: string, rating: number, comment: string = '', userId: string = '') {
    try {
