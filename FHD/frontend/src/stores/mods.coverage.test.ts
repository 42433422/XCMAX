...
const mockAugmentEntitledModIdsForAccount = vi.fn((_u: unknown, ids: string[] | undefined) => ids || [])
...
vi.mock('@/utils/apiBase', () => ({
  ...
  isApiFetchTimeoutError: (e: unknown) => e instanceof DOMException && e.name === 'AbortError' && e.message.includes('apiFetch timeout'),
...
vi.mock('@/constants/accountModBinding', () => ({
  ...
  augmentEntitledModIdsForAccount: (u: unknown, ids: string[] | undefined) => mockAugmentEntitledModIdsForAccount(u, ids),
...
function routeApiFetch(routes: {
  ...
  mockApiFetch.mockImplementation(async (url: string) => {
    ...
    if (url === '/api/system/industry') return routes.industry ? routes.industry() : makeResponse({ success: true, data: { id: '' } })
    if (url.includes('erp-domain') && url.includes('status')) return routes.probeErp ? routes.probeErp() : makeResponse({ success: true })
...
const sampleMods = [
    ...
    menu: [
      {
        ...
        label: '智能对话',
        icon: 'chat',
        path: '/mod/xcagi-planner-bridge/chat',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    menu: [
      {
        id: 'mod-taiyangniao-products',
        label: '产品管理',
        icon: 'box',
        path: '/mod/taiyangniao-pro/products',
      },
    ],
  },
]
    ...
    menu: [
      {
        ...
        label: '流程可视化',
        icon: 'flow',
        path: '/mod/xcagi-core-workflow-employees/viz',
      },
    ],
  },
  {
    ...
    menu: [
      {
        ...
        label: '审批中心',
        icon: 'check',
        path: '/mod/xcagi-approval-bridge/hub',
      },
    ],
  },
  {
    ...
    menu: [
      {
        ...
        label: '局域网',
        icon: 'network',
        path: '/mod/xcagi-lan-license-bridge/gate',
      },
    ],
  },
  {
    ...
    menu: [
      {
        ...
        label: '模型支付',
        icon: 'pay',
        path: '/mod/xcagi-model-payment-bridge/pay',
      },
    ],
  },
  {
    ...
    menu: [
      {
        ...
        label: '客服',
        icon: 'headset',
        path: '/mod/xcagi-customer-service-bridge/svc',
      },
    ],
  },
  {
    ...
    menu: [
      {
        ...
        label: '办公包',
        icon: 'office',
        path: '/mod/xcagi-office-employee-pack-bridge/office',
      },
    ],
  },
]
...
describe('mods store – coverage 补齐', () => {
  ...
  beforeEach(async () => {
    ...
    mockAugmentEntitledModIdsForAccount.mockImplementation((_u: unknown, ids: string[] | undefined) => ids || [])
  ...
  describe('菜单函数', () => {
          ...
          menu: [
            {
              ...
              label: '测试面板',
              icon: 'box',
              path: '/mod/test-extension-mod/panel',
            },
          ],
        },
      ] as never[]
    ...
    it('getModMenu 在 fromUi 为空时回退到 fromFull 并包含 aux pack', () => {
      ...
      mockIsAuxEmployeePackModId.mockImplementation((id: string) => id === 'test-aux-pack')
      ...
      store.mods = [
        ...sampleMods,
        {
          ...
          menu: [
            {
              id: 'mod-test-aux-panel',
              label: '测试面板',
              icon: 'box',
              path: '/mod/test-aux-pack/panel',
            },
          ],
        },
      ] as never[]
    ...
    it('shouldHideModMenuEntry 隐藏考勤侧栏菜单', () => {
      mockShouldHideAttendanceModSidebarMenu.mockImplementation((menuId: string) => menuId === 'mod-attendance-dashboard')
      const store = useModsStore()
      store.mods = sampleMods as never[]
    ...
    it('shouldHideModMenuEntry 隐藏客户 ERP 重复菜单', () => {
      mockShouldSuppressClientErpModMenuId.mockImplementation((menuId: string) => menuId === 'mod-planner-chat')
      const store = useModsStore()
      store.mods = sampleMods as never[]
          ...
          menu: [
            {
              ...
              label: '流程可视化',
              icon: 'flow',
              path: '/mod/xcagi-workflow-visualization-bridge/viz',
            },
          ],
        },
        {
    ...
    it('validateModMenuPaths 对正确路径不发出警告', () => {
      ...
      store.getModMenu()
      // 正确路径不应触发路径警告
      const pathWarnings = warnSpy.mock.calls.filter((c) => String(c[0] || '').includes('does not match mod id prefix'))
    ...
    it('getModMenu 跳过无 menu 数组的 Mod', () => {
      const store = useModsStore()
      store.mods = [{ id: 'no-menu-mod', name: 'No Menu', version: '1.0', author: '', description: '' }, ...sampleMods] as never[]
