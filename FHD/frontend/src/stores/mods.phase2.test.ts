...
vi.mock('@/constants/protectedMods', () => ({
  isProtectedClientModId: (id: string) => ['attendance-industry', 'coating-industry', 'taiyangniao-pro', 'sz-qsm-pro'].includes(id),
}))
...
describe('mods store – getModMenu', () => {
  ...
  it('skips mods without menu arrays', () => {
    const store = useModsStore()
    store.mods = [{ id: 'no-menu-mod', name: 'No Menu', version: '1.0', author: '', description: '' }] as never[]
    const menu = store.getModMenu()
    expect(menu).toEqual([])
  })
