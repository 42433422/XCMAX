...
export function buildOfficeEmployeePackSteps(): TutorialStep[] {
  return [
    createStep({
      ...
      title: '智能生态 · 入口',
      description: '办公员工包（CSV/Excel/PDF 等表格类 AI 员工）是软件的基础能力，也适合教学演示。请先点击侧栏「智能生态」。',
    ...
    createStep({
      ...
      title: '员工商店',
      description: '在智能生态首页点击「员工商店」，进入能力库浏览与安装本机/市场员工包（与侧栏「能力库」同页）。',
      routeName: 'ai-ecosystem',
    ...
    createStep({
      ...
      title: '办公员工包',
      description: '左侧选择「办公员工包」分类。装齐后 Planner 会自动加载对应工具（无需重启），可用于日常办公与教程演示。',
      routeName: 'mod-store',
    ...
    createStep({
      ...
      title: '一键安装并入驻',
      description: '点击「一键安装并入驻」装齐 10 个办公员工。安装完成后系统会热加载 HTTP 路由与 Planner 工具注册表。',
      routeName: 'mod-store',
      routeQuery: { tab: 'office' },
    ...
    createStep({
      ...
      title: '对话验收',
      description: '回到对话页，可尝试发送「读取这个 CSV 文件」等指令；Planner 应能看到并调用刚安装的办公员工工具。',
      routeName: 'chat',
