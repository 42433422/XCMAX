import type { AgentArtifact, AgentRun, AgentRunEvent, AgentRunStep, AgentTaskSummary, AgentToolCall } from '@/api/agentRuns'
...
function taskContextOf(run: AgentRun): AgentTaskContext {
  ...
  return {
    ...
    workspace_path: asString(raw.workspace_path || runtime.worktree_path || runtime.workspace_path || runtime.cwd).trim(),
...
function latestRun(runs: AgentRun[]): AgentRun {
  ...
  const active = sorted.find((run) =>
    ['planning', 'running', 'retrying', 'waiting_user', 'blocked', 'paused', 'queued'].includes(asString(run.status)),
  )
...
function evidenceOf(runs: AgentRun[]) {
  ...
  return {
    ...
    completed_tool_count: runs.reduce(
      (count, run) => count + asArray<AgentToolCall>(run.tool_calls).filter((call) => asString(call.status) === 'completed').length,
      0,
    ),
  }
}
...
function capabilitiesOf(run: AgentRun) {
  ...
  return {
    ...
    cancel: ['queued', 'planning', 'running', 'retrying', 'waiting_user', 'paused', 'blocked'].includes(status),
    retry: ['failed', 'cancelled', 'blocked'].includes(status) && !asRecord(run.metadata).non_retryable,
    evidence: true,
  }
}
...
export function groupAgentRunsIntoTasks(runs: AgentRun[]): TaskItem[] {
  ...
  return [...groups.entries()]
    .map(([taskId, groupedRuns]) => {
      const ordered = [...groupedRuns].sort((left, right) => timestamp(left.created_at) - timestamp(right.created_at))
...
export function taskSummariesToTaskItems(tasks: AgentTaskSummary[]): TaskItem[] {
  return asArray<AgentTaskSummary>(tasks)
    .map((summary) => {
      ...
      const pendingControl = controlCommand?.status === 'requested' ? controlCommand.action : ''
      const taskStage =
