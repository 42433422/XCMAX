...
export function parseApprovalCardFromPayload(data: ChatPlannerPayload): ChatApprovalCard | null {
  ...
  if (!Object.keys(raw).length) {
    ...
    if (action !== 'workflow_confirmation_required' && action !== 'approval_pending') {
      return null
    }
    return {
      kind: action,
      ...
      blocking_nodes: asArray(inner.blocking_nodes)
        .map((x) => asString(x))
        .filter(Boolean),
      ...
      approval_request_ids: asArray(inner.approval_request_ids)
        .map((x) => asString(x))
        .filter(Boolean),
      approval_path: asString(inner.approval_path),
      todo: asArray(inner.todo)
        .map((x) => asString(x))
        .filter(Boolean),
  ...
  return {
    ...
    blocking_nodes: asArray(raw.blocking_nodes)
      .map((x) => asString(x))
      .filter(Boolean),
    ...
    approval_request_ids: asArray(raw.approval_request_ids)
      .map((x) => asString(x))
      .filter(Boolean),
    approval_path: asString(raw.approval_path),
    todo: asArray(raw.todo)
      .map((x) => asString(x))
      .filter(Boolean),
