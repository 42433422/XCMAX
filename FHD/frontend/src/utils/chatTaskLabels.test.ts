...
describe('chatTaskLabels', () => {
  ...
  it('preserves a server message timestamp instead of replacing it with now', () => {
    ...
    expect(formatChatMessageTime(raw)).toBe(
      new Date(raw).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }),
    );
  ...
  it('removes internal legacy and agent terminology from task text', () => {
    expect(normalizeTaskDisplayText('Agent Progress 100% Legacy planner run 执行完成'))
      .toBe('执行进度 100% 智能任务 执行完成');
