...
import { ETL_MAX_FILE_BYTES, formatEtlBytes, selectEtlSourceFiles } from './etlFileSelection'
...
describe('selectEtlSourceFiles', () => {
  it('keeps supported files and their folder-relative paths', () => {
    const selection = selectEtlSourceFiles(
      [folderFile('a', 'customers.csv', '华东数据/客户/customers.csv'), folderFile('b', 'products.xlsx', '华东数据/产品/products.xlsx')],
      1024,
    )

    expect(selection.folderName).toBe('华东数据')
    expect(selection.accepted.map((item) => item.relativePath)).toEqual(['华东数据/客户/customers.csv', '华东数据/产品/products.xlsx'])
  ...
  it('ignores unsupported, oversized, and duplicate files with stable reasons', () => {
    ...
    const selection = selectEtlSourceFiles(
      [first, first, folderFile('1234', 'notes.txt', '批次/notes.txt'), folderFile('123456', 'large.csv', '批次/large.csv')],
      4,
    )

    expect(selection.accepted).toHaveLength(1)
    expect(selection.ignored.map((item) => item.reason)).toEqual(['duplicate', 'unsupported', 'too_large'])
  })

  it('removes traversal segments from display-only relative paths', () => {
    const selection = selectEtlSourceFiles([folderFile('a', 'data.csv', '../unsafe/./data.csv')], 1024)
  ...
  it('accepts 100MB exactly and rejects the next byte', () => {
    ...
    expect(selection.ignored).toEqual([{ name: '批次/over.csv', reason: 'too_large' }])
  })
})
