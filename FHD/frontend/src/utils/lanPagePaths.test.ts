...
import { resolveLanPagePath, resolveLanPageRedirectForRouteName, useLanModPages } from './lanPagePaths'
...
describe('lanPagePaths', () => {
  ...
  it('maps lan-gate when facade on', () => {
    ...
    expect(resolveLanPageRedirectForRouteName('lan-gate')).toBe('/mod/xcagi-lan-license-bridge/lan-gate')
  })

  it('preserves query string', () => {
    const store: Record<string, string> = { [LS_LAN_MOD_FACADE_ENABLED]: '1' }
    vi.stubGlobal('localStorage', { getItem: (k: string) => store[k] ?? null })
    expect(resolveLanPagePath('/lan-gate?foo=1')).toBe('/mod/xcagi-lan-license-bridge/lan-gate?foo=1')
  })
})
