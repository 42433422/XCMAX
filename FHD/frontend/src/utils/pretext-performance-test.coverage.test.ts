...
import { runSingleMessageTest, runBatchMessageTest, runFullTestSuite, default as defaultExport } from './pretext-performance-test'

// Mock pretext 模块以避免依赖实际测量
vi.mock('./pretext', () => ({
  measureText: vi.fn(() => ({ width: 100, height: 20, lines: 1 })),
  batchMeasure: vi.fn((items: unknown[]) => items.map(() => ({ width: 100, height: 20, lines: 1 }))),
}))
