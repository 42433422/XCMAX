 ...
 * 运行单条消息测试
 */
export function runSingleMessageTest(message: string, width: number = 600, iterations: number = 100): TestResult {
  // 预热
  for (let i = 0; i < 10; i++) {
 ...
 * 运行批量消息测试
 */
export function runBatchMessageTest(messages: string[], width: number = 600): TestResult {
...
export function runFullTestSuite(): TestResult[] {
  ...
  const longMessage =
    '这是一条很长的消息，模拟 AI 回复的详细内容。在实际应用中，AI 可能会返回较长的回复，包含多个段落和详细说明。这种消息需要更复杂的布局计算，包括换行、段落间距等。Pretext.js 的优势在这种情况下会更加明显，因为它可以完全避免 DOM 重排带来的性能损耗。'
