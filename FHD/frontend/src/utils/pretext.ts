 ...
 * @returns 高度数组
 */
export function batchEstimateMessageHeights(messages: { content: string; fontSize?: number }[], maxWidth: number): number[] {
  return messages.map((msg) => estimateMessageHeight(msg.content, maxWidth, msg.fontSize || 14))
}
 ...
 * @returns 行高数组
 */
export function calculateTableRowHeights(cellContents: string[], columnWidth: number, fontSize: number = 12): number[] {
...
export function getPerformanceStats(): {
  ...
  return {
    totalMeasurements,
    cachedMeasurements,
    cacheHitRate: totalMeasurements > 0 ? cachedMeasurements / totalMeasurements : 0,
  }
}
