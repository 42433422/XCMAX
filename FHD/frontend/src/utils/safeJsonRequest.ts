 ...
 */
export async function safeJsonRequest<T = unknown>(url: string, options: RequestInit = {}): Promise<SafeJsonResult<T>> {
  ...
  try {
    ...
    if (!resp.ok) {
      const msg =
        typeof (data as { message?: string })?.message === 'string' ? (data as { message: string }).message : `请求失败（${resp.status}）`
      return {
        ok: false,
        status: resp.status,
        data,
