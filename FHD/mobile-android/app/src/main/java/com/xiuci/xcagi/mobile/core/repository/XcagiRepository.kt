...
class XcagiRepository @Inject constructor(
    ...
    suspend fun loadCursorSuperEmployeeMessages(limit: Int = 80): Result<List<ChatMsg>> =
        try {
            syncRouterFromStore()
            preferCloudIfLanUnreachable()
    ...
    suspend fun loadTraeSuperEmployeeMessages(limit: Int = 80): Result<List<ChatMsg>> =
        try {
            syncRouterFromStore()
            preferCloudIfLanUnreachable()
    ...
    private fun Map<*, *>.toWorkflowEmployeeInfo(): WorkflowEmployeeInfo {
        return WorkflowEmployeeInfo(
            ...
            // 缺字段（旧后端）默认 true，不误标未安装；新后端回传真实安装态。
            installed = this["installed"] as? Boolean ?: true,
            runnable = this["runnable"] as? Boolean ?: true,
