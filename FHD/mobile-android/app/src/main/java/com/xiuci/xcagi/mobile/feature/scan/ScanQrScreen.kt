...
fun ScanQrScreen(vm: AppViewModel, onBack: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize()) {
                                    ...
                                    BarcodeScanning.getClient().process(image)
                                        .addOnSuccessListener { barcodes ->
                                            ...
                                            if (!raw.isNullOrBlank()) {
                                                ...
                                                if (raw.contains("auth-qr") && authMatch != null) {
                                                    authQrId = authMatch.groupValues[1]
                                                } else {
        ...
        if (authQrId.isNotBlank()) {
            ...
            Button(
                {
                    vm.confirmAuthQr(authQrId, authUsername, authPassword) {
                        if (it) onBack() else scanned = false
                    }
                },
