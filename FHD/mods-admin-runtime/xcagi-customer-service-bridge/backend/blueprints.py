...
async def _run_user_cs_employee(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": str(exc)[:500], "data": {"success": False, "error": str(exc)[:500]}}

    return {
        "success": False,
        "error": "user-customer-service-officer 未安装",
        "data": {"success": False, "error": "请确认 mods/_employees/user-customer-service-officer 已安装并在岗"},
    }
...
def register_fastapi_routes(app, mod_id: str) -> None:
    ...
    async def user_cs_crm_pull_external(body: PipelineBody):
        ...
        try:
            ...
            if isinstance(pull, dict) and not pull.get("success") and not pull.get("skipped"):
