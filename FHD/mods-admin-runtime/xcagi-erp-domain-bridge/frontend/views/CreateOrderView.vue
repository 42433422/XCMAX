...
async function loadPurchaseUnits() {
  try {
    const response = await fetch(resolveErpApiPath('/api/purchase_units'))
    const data = await response.json()
    if (data.success) {
  ...
  if (form.purchaseUnit) {
    try {
      const response = await fetch(
        resolveErpApiPath(`/api/purchase_units/by_name/${encodeURIComponent(form.purchaseUnit)}`),
      )
      const data = await response.json()
      if (data.success) {
...
async function loadAllProducts() {
  try {
    const response = await fetch(resolveErpApiPath('/api/products/product_names'))
    const data = await response.json()
    if (data.success) {
...
async function generateOrderNumber() {
  try {
    const response = await fetch(resolveErpApiPath('/api/orders/next_number?suffix=A'))
    const data = await response.json()
    if (data.success) {
