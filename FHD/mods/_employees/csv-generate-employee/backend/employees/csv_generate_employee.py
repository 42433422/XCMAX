...
EMPLOYEE_ID = "csv-generate-employee"
EMPLOYEE_LABEL = "CSV 生成员"
SYSTEM_PROMPT = "你是CSV 生成员。你必须按 direct_python 方式处理真实文件，读取 payload 中的 file_path/path/excel_path，必要时使用打包模板，成功条件是实际写出输出文件。任何输入缺失、模板缺失、转换模块异常都要返回明确错误，禁止编造已完成。"
RULE_SPEC = {
    "brief": "CSV生成员工：上传 .json（含 columns 与 rows）写出 outputs/output.csv，使用 direct_python，禁止编造表格数据。",
    ...
    "output_schema": ["columns", "rows", "row_count", "delimiter", "encoding"],
    "requirements": [
...
async def run(payload: Dict[str, Any], ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        from csv_generate.convert import convert_file
