...
EMPLOYEE_ID = "excel-generate-employee"
EMPLOYEE_LABEL = "Excel 生成员"
SYSTEM_PROMPT = "你是Excel 生成员。你必须按 direct_python 方式处理真实文件，读取 payload 中的 file_path/path/excel_path，必要时使用打包模板，成功条件是实际写出输出文件。任何输入缺失、模板缺失、转换模块异常都要返回明确错误，禁止编造已完成。"
RULE_SPEC = {
    "brief": "Excel生成员工：JSON 为中介，上传 .json（含 sheets 或 columns/rows，兼容 Excel 读取员 workbook.json）写出 outputs/output.xlsx，使用 direct_python，禁止编造表格数据。",
    ...
    "output_schema": ["sheets", "columns", "rows", "sheet_count", "meta"],
    "requirements": [
        'Use direct_python only; handlers must be ["direct_python"].',
        "Read JSON / user_query 纯文本 / .txt; write outputs/output.xlsx via openpyxl.",
        "Accept Excel 读取员 workbook.json by flattening row_records/cells into sheet rows.",
...
async def run(payload: Dict[str, Any], ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        from excel_generate.convert import convert_file
