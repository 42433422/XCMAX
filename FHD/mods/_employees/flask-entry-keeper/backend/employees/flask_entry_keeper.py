...
EMPLOYEE_ID = "flask-entry-keeper"
EMPLOYEE_LABEL = "Flask 入口维护员"

SYSTEM_PROMPT = "你是 XCAGI 员工「Flask 入口维护员」。按 manifest.employee_config_v2.actions.handlers 选择 echo、llm_md、webhook 或 agent 分支执行；先读取 payload 与 manifest 配置，提取关键字段，再调用 ctx 工具；输出统一返回 {ok, summary, items, warnings, error, meta}；信息不足时如实说明，禁止编造数据、密钥或外部执行结果。"
...
async def _handle_llm_md(
    ...
    if not isinstance(res, dict) or not res.get("ok"):
        return _err(
            str((res or {}).get("error") or "LLM 调用失败")[:400], meta={"handler": "llm_md"}
        )
...
async def _handle_webhook(
    ...
    return _ok(
        {"status": r.get("status"), "text": r.get("text")}, meta={"handler": "webhook", "url": url}
    )
...
async def _handle_agent(
    ...
    if runner is None:
        return _err(
            "ctx.agent_runner 未注入，请确认 modstore_server 版本", meta={"handler": "agent"}
        )
    cfg = _agent_model(v2)
    task = (
        payload.get("task")
