...
def run(payload: dict[str, Any], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    if not checks or any(str(item.get("status") or "").lower() != "success" for item in checks if isinstance(item, dict)):
        issues.append({"code": "checks_not_green", "path": "pr_snapshot.checks"})
    if not any(str(item.get("state") or "").lower() == "approved" for item in reviews if isinstance(item, dict)):
    ...
    return {
        ...
        "evidence": ["input.pr_snapshot.checks", "input.pr_snapshot.reviews", "input.pr_snapshot.mergeable"],
        "read_only": True,
        "side_effects": [],
    }
