...
def run(payload: dict[str, Any], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, raw in enumerate(inventory[:500]):
        ...
        references = item.get("referenced_by") if isinstance(item.get("referenced_by"), list) else []
        ...
        if not references and float(days) >= 90:
            if not recovery:
                issues.append({"code": "missing_recovery_path", "path": f"inventory[{index}].recovery_path"})
            else:
                candidates.append({"path": path, "last_used_days": float(days), "recovery_path": recovery})
    return {
        "ok": True,
        ...
        "issues": issues,
        "evidence": ["input.inventory", "input.inventory.referenced_by", "input.inventory.recovery_path"],
        "read_only": True,
        "side_effects": [],
    }
