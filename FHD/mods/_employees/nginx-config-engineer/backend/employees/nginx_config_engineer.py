...
def run(payload: dict[str, Any], _ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(receipt, dict):
        issues.append({"code": "missing_config_check_receipt", "detail": "config_check_receipt is required"})
        receipt = {}
    ...
    if int(receipt.get("certificate_days_remaining") or 0) < 30:
        issues.append({"code": "certificate_expiry_near", "detail": "certificate threshold is 30 days"})
    if receipt.get("route_drift") is not False:
        issues.append({"code": "route_drift_present", "detail": "supplied receipt reports route drift"})
    if receipt.get("production_change_requested") is not False:
        issues.append({"code": "production_change_not_excluded", "detail": "fixture must exclude production mutation"})
    approved = not issues
    return {
        "ok": True,
        "status": "approved" if approved else "needs_review",
        "summary": "supplied Nginx check receipt is healthy" if approved else "Nginx receipt needs review",
        "issues": issues,
