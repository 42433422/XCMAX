...
def run(payload: dict[str, Any], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "totals": {"orders_cents": order_total, "payments_cents": payment_total, "refunds_cents": refund_total, "difference_cents": difference},
        "issues": issues,
        "reconciled": not issues,
        "evidence": ["input.ledger.orders", "input.ledger.payments", "input.ledger.refunds"],
        "read_only": True,
        "side_effects": [],
    }
