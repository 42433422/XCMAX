...
EMPLOYEE_ID = "pdf-generate-employee"
EMPLOYEE_LABEL = "PDF 生成员"
SYSTEM_PROMPT = "你是PDF 生成员。你必须按 direct_python 方式处理真实文件，读取 payload 中的 file_path/path/excel_path，必要时使用打包模板，成功条件是实际写出输出文件。任何输入缺失、模板缺失、转换模块异常都要返回明确错误，禁止编造已完成。"
RULE_SPEC = {
    "brief": "制作 PDF 生成员工包。上传 .pdf → 读取原生文字/分页结构 → 输出结构化 JSON（document_parsed.json）作为中介 → 写入 outputs/generated_document.pdf；可选用 agent 润色。handlers 含 direct_python 与 agent。",
    ...
    "optional_llm_polish": True,
    "output_schema": ["pages", "blocks", "plain_text", "stats", "metadata"],
    "requirements": [
...
async def run(payload: Dict[str, Any], ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        from pdf_generate.convert import convert_file
