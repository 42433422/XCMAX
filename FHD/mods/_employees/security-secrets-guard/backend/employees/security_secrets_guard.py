...
_SENSITIVE_KEY = re.compile(r"(?i)(secret|token|password|api[_-]?key|private[_-]?key|credential|value)")
...
def run(payload: dict[str, Any], _ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    if _contains_sensitive_key(summary):
        return {
            ...
            "issues": [{"code": "sensitive_input_blocked", "detail": "provide redacted counts and rule ids only"}],
    ...
    if summary.get("redacted") is not True:
        issues.append({"code": "redaction_unproven", "detail": "fixture must declare redacted=true"})
    if int(summary.get("prohibited_hits") or 0) > 0:
        issues.append({"code": "prohibited_hit_detected", "detail": "prohibited findings require immediate block"})
    if int(summary.get("high_severity_count") or 0) > 0:
        issues.append({"code": "high_severity_finding", "detail": "high-severity findings require review"})
    approved = not issues
    return {
        "ok": True,
        "status": "approved" if approved else "needs_review",
        "summary": "redacted finding summary is clear" if approved else "security finding summary requires review",
        "issues": issues,
        "evidence": ["fixture_only", "redacted_counts_only", "no_repository_scan", "no_secret_output"],
        "read_only": True,
        "side_effects": [],
    }
