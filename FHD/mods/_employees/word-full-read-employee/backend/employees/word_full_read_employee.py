...
EMPLOYEE_LABEL = "Word 全量读取员"
SYSTEM_PROMPT = "你是Word 全量读取员。你必须按 direct_python 方式处理真实文件，读取 payload 中的 file_path/path/excel_path，必要时使用打包模板，成功条件是实际写出输出文件。任何输入缺失、模板缺失、转换模块异常都要返回明确错误，禁止编造已完成。"
RULE_SPEC = {
  "brief": "Word全量读取员工：上传 .docx，direct_python 全量提取段落/表格/样式，输出 document_full.json，禁止 LLM 编造。",
...
async def run(payload: Dict[str, Any], ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        from word_full_read.convert import convert_file
