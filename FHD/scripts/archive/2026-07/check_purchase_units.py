    ...
    try:
        ...
        for desc in descriptions:
            ...
            if match:
                ...
                # 统计该购买单位的产品数量
                cursor.execute(
                    "SELECT COUNT(*) FROM products WHERE description LIKE ?", (f"%[{unit_name}]%",)
                )
        ...
        # 检查没有购买单位标记的产品
        cursor.execute(
            "SELECT COUNT(*) FROM products WHERE description IS NULL OR description = '' OR description NOT LIKE '%[%]%'"
        )
