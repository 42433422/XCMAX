...
if result.get("success"):
    ...
    for i, tb in enumerate(text_blocks):
        print(
            f"  [{i}] '{tb.get('text')}' -> cell_row={tb.get('cell_row')}, cell_col={tb.get('cell_col')}"
        )
    ...
    for i, cell in enumerate(cells):
        print(
            f"  [{i}] 行{cell.get('row')} 列{cell.get('start_col')}-{cell.get('end_col')} "
    ...
    for i, field in enumerate(fields):
        print(
            f"  [{i}] label='{field.get('label')}' value='{field.get('value')}' is_merged={field.get('is_merged')}"
        )
else:
