...
def fix_purchase_unit():
    ...
    try:
        ...
        # 更新购买单位标记
        cursor.execute(
            "UPDATE products SET description = REPLACE(description, '[半岛家具]', '[半岛风情]') WHERE description LIKE '%半岛家具%'"
        )
        ...
        if new_count > 0:
            print("\n📋 更新后的半岛风情产品列表:")
            cursor.execute(
                "SELECT id, model_number, name, price, unit, description FROM products WHERE description LIKE '%半岛风情%'"
            )
            products = cursor.fetchall()
            for product in products:
                print(
                    f"  - ID: {product[0]}, 型号: {product[1]}, 名称: {product[2]}, 价格: {product[3]}元/{product[4]}"
                )
