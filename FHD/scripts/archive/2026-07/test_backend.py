...
if result["success"]:
    ...
    for cell in cells:
        print(
            f"单元格 [{cell['row']},{cell['col']}]: 位置=({cell['x']}, {cell['y']}), 尺寸={cell['width']}x{cell['height']}"
        )
    ...
    for i, field in enumerate(result["fields"], 1):
        print(
            f"{i}. {field['label']}: {field['value']} (类型：{field['type']}, 置信度：{field['confidence']:.2f})"
        )
    ...
    grid_config = {
        ...
        "grid_size": {"rows": grid.get("rows", 0), "cols": grid.get("cols", 0)},
        "cells": cells,
