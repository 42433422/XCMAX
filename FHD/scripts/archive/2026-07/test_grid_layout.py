...
for i, text in enumerate(rec_texts):
    if i < len(rec_polys) and len(rec_polys[i]) > 0:
        ...
        cell_match = None
        for cell in cells:
            if cell["x1"] <= center_x <= cell["x2"] and cell["y1"] <= center_y <= cell["y2"]:
                cell_match = cell
                break
        ...
        if cell_match:
            print(
                f"'{text}' (置信度：{score:.2f}) -> 单元格 [{cell_match['row']},{cell_match['col']}] Y={center_y:.1f}"
            )
        else:
...
grid_config = {
    ...
    "grid_size": {"rows": len(horizontal_lines) - 1, "cols": len(vertical_lines) - 1},
