...
cur.execute(
    "SELECT resource_id, message_id, type, size, create_time, status, data_index, packed_info FROM MessageResourceDetail LIMIT 5"
)
...
cur.execute(
    "SELECT resource_id, message_id, type, size, data_index FROM MessageResourceDetail WHERE size > 1000000 AND size < 2000000"
)
rows = cur.fetchall()
