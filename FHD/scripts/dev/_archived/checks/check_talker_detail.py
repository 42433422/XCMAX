...
cur.execute(
    f"SELECT message_content, WCDB_CT_message_content, talker, real_sender_id, create_time FROM [{table}] WHERE talker LIKE '%fxzqdqt%' ORDER BY create_time DESC LIMIT 5"
)
rows = cur.fetchall()
for row in rows:
    if row[0]:
