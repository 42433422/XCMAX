...
class TestShoeLabelGenerator:
    ...
    def _draw_content(
        self, draw: ImageDraw.ImageDraw, product_data: Dict[str, Any], order_number: str
    ):
        ...
        draw.text(
            (180, y_offset),
            str(product_data.get("item_number", "")),
            font=value_font,
            fill=self.text_color,
        )
        y_offset += line_height
        ...
        draw.text(
            (180, y_offset),
            str(product_data.get("grade", "合格品")),
            font=value_font,
            fill=self.text_color,
        )
    ...
    def generate_labels_for_order(self, order_number: str, products: list) -> list:
        ...
        for i, product in enumerate(products, 1):
            ...
            if filename:
                labels.append(
                    {"filename": filename, "order_number": order_number, "label_index": i}
                )
        return labels
...
def example_usage():
    ...
    product = {"product_name": "示例产品", "item_number": "12345", "grade": "合格品"}
