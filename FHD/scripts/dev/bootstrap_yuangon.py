...
- **当前状态**：`FHD/mobile-flutter/` 是唯一移动实现；`release-ios.yml` 负责 Flutter iOS 构建、签名和上传
""",
...
def _make_employee_yaml(pkg_id: str, mf: dict) -> str:
    ...
    if pkg_id == "mobile-ios-release-officer":
        ios_note = "# NOTE: scope FHD/mobile-flutter/** 为唯一移动实现，release-ios.yml 为 iOS 发版入口\n"
...
def _make_readme(pkg_id: str, mf: dict) -> str:
    ...
    ios_warning = ""
    if pkg_id == "mobile-ios-release-officer":
        ios_warning = "\n> **发版入口**：`FHD/mobile-flutter/` 是唯一移动实现，`FHD/.github/workflows/release-ios.yml` 负责 Flutter iOS 构建、签名、TestFlight 与 App Store Connect 上传。\n"
...
`FHD/mobile-flutter/` 是唯一移动端实现。本岗当前职责：
