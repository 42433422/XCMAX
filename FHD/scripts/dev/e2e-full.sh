...
# P0 为每个浏览器上下文创建独立会话。保持认证限流启用，但为这个隔离的
# 验收后端提供足够预算，避免测试自身的有意登录流量触发 10 次/分钟默认值。
export XCAGI_AUTH_RATE_LIMIT="${XCAGI_AUTH_RATE_LIMIT:-1}"
export XCAGI_AUTH_RATE_LIMIT_MAX="${XCAGI_AUTH_RATE_LIMIT_MAX:-100}"
unset DATABASE_URL
