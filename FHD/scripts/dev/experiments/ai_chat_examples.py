...
def test_chat():
    ...
    for message in test_messages:
        ...
        response = requests.post(
            f"{BASE_URL}/chat", json={"message": message, "user_id": "test_user"}
        )
...
def test_intent_recognition():
    ...
    for message in test_messages:
        ...
        response = requests.post(f"{BASE_URL}/intent/test", json={"message": message})
...
def test_context():
    ...
    for message in messages:
        print(f"\n发送：{message}")
        requests.post(f"{BASE_URL}/chat", json={"message": message, "user_id": user_id})

    # 获取上下文
    print("\n获取对话上下文...")
    response = requests.get(f"{BASE_URL}/context", params={"user_id": user_id})
    ...
    # 清除上下文
    print("\n清除上下文...")
    response = requests.post(f"{BASE_URL}/context/clear", json={"user_id": user_id})
