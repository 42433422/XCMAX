...
ANCHORS: list[tuple[str, str, str]] = [
    ...
    ("mobile-flutter/android/app/build.gradle.kts", r'injectedVersionName[\s\S]*?\?:\s*"([\d.]+)"', PRODUCT_VERSION),
    ("mobile-flutter/pubspec.yaml", r'(?m)^version:\s*([\d.]+)\+\d+', TOOLCHAIN_VERSION),
    ("mobile-flutter/ios/Flutter/Version.xcconfig", r'(?m)^FLUTTER_BUILD_NAME=([\d.]+)', TOOLCHAIN_VERSION),
    ("desktop/resources/build-info.json", r'"version"\s*:\s*"([\d.]+)"', PRODUCT_VERSION),
    ("mobile-flutter/lib/src/api/mobile_api.dart", r"versionName\s*=\s*'([\d.]+)'", PRODUCT_VERSION),
