...
class ProfessionalUploadHandler:
    ...
    def __init__(self):
        self.target_db = "e:/FHD/98k/AI助手/AI助手/products.db"
        ...
        if file_type == "sqlite_database":
            ...
            if analysis.get("suggested_action") == "import_products":
                ...
                if user_response.lower() in ["是", "yes", "y", "确认", "确认导入"]:
                    ...
                    return {"success": True, "message": "用户取消了导入操作"}
                ...
                return {"success": False, "message": "数据库中没有找到产品表，无法进行导入"}
        else:
    ...
    def _extract_unit_name(self, response):
        ...
        patterns = [r"改成\s*(.+)", r"改为\s*(.+)", r"change to\s*(.+)", r"rename to\s*(.+)"]
    ...
    def _import_products_from_db(self, source_db, purchase_unit):
        ...
        try:
            ...
            # 确保目标数据库表存在
            self._ensure_target_table_exists(target_cursor)
            ...
            for product in source_products:
                ...
                target_cursor.execute(
                    "SELECT id FROM products WHERE model_number = ?", (model_number,)
                )
                ...
                description_with_unit = (
                    f"[{purchase_unit}] {description if description else ''}".strip()
                )
    ...
    def _ensure_target_table_exists(self, cursor):
        ...
        if not table_exists:
            ...
            # 创建索引
            cursor.execute("CREATE INDEX idx_products_model_number ON products(model_number)")
            cursor.execute("CREATE INDEX idx_products_name ON products(name)")


def simulate_professional_upload():
    """模拟专业版上传流程"""
    handler = ProfessionalUploadHandler()
