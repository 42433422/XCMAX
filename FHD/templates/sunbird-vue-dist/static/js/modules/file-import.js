                    ...
                    if (unitNameForImport) {
                        try {
                            ...
                            if (importResult && importResult.success) {
                                ...
                                messageParts.push(
                                    `已自动导入购买单位「${unitNameForImport}」产品：新增 ${imported} 条，重复跳过 ${skipped} 条。`
                                );
                                try { delete window.__xcagiPendingUnitProductsImport; } catch (_) {}
                                try { localStorage.removeItem('xcagiPendingUnitProductsImport'); } catch (_) {}
                            } else {
                                const errMsg = (importResult && (importResult.message || importResult.error))
