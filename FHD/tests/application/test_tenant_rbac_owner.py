...
def _workspace(*, owned: bool = True) -> tuple[int, int, str, str]:
    ...
    with get_host_db() as db:
        owner = User(
            ...
            role="user",
            tier="enterprise",
            market_user_id=market_id if owned else None,
            created_at=now,
        )
        db.add(owner)
        db.flush()
...
def test_legacy_owner_requires_live_market_identity_and_single_founder() -> None:
    ...
    with get_host_db() as db:
        db.get(Tenant, tenant_id).created_at = db.get(User, owner_id).created_at + timedelta(
            seconds=1
        )
    bind_verified_market_identity(
        user_id=owner_id,
        market_user_id=_market_id(),
...
def test_owner_can_manage_tenant_roles_without_platform_privilege_or_cross_tenant_access() -> None:
    ...
    assert (
        client.put(f"/api/rbac/users/{other_id}/role", json={"role": role_key}).status_code == 404
    )
...
def test_invitation_binds_only_target_market_identity_once_and_never_moves_other_workspace() -> (
    None
):
    owner_id, tenant_id, _owner_name, _sid = _workspace()
    target = f"member-{uuid4().hex[:12]}"
    invite = create_tenant_invitation(
        inviter_user_id=owner_id, tenant_id=tenant_id, target_username=target
    )
    with get_host_db() as db:
        assert (
            db.query(TenantInvitation)
            .filter(TenantInvitation.token_sha256 == invite["code"])
            .first()
            is None
        )
        ...
        db.add(
            Session(
                session_id=sid, user_id=member_id, expires_at=utc_now_naive() + timedelta(days=1)
            )
        )
    ...
    with get_host_db() as db:
        ...
        assert (member.tenant_id, member.market_user_id, member.role) == (
            tenant_id,
            market_id,
            f"tenant:{tenant_id}:member",
        )
    ...
    second = create_tenant_invitation(
        inviter_user_id=owner_id, tenant_id=tenant_id, target_username=outsider_name
    )
    with pytest.raises(TenantIdentityError):
        accept_verified_tenant_invitation(
            user_id=outsider_id,
            market_user_id=_market_id(),
            market_username=outsider_name,
            code=second["code"],
            session_id=outsider_sid,
        )
    with get_host_db() as db:
