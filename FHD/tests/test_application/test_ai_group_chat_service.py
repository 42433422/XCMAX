...
def make_service(
    ...
    return AiGroupChatService(
        ...
        department_loader=(
            fake_enterprise_departments if mode == "enterprise" else fake_departments
        ),
        employee_loader=(
            employees if callable(employees) else (lambda: employees or [])
        ),
        mode=mode,
    )
...
async def test_post_message_all_members_reply(tmp_path: Path):
    ...
    svc.add_member(
        user_id=1, group_id=gid, member={"employee_id": "e2", "name": "小服"}
    )
...
def test_enterprise_seed_populates_members_by_layer(tmp_path: Path):
    ...
    emps = [
        ...
        },
        {
            ...
            "name": "流程编排器",
            "summary": "管理",
            "department_key": resolve_enterprise_org_layer(
                "workflow_automator", "流程编排器"
            ),
        },
    ]
...
def test_resolve_enterprise_org_layer_manifest_priority():
    ...
    assert (
        resolve_enterprise_org_layer("unknown_id", "未知", "", "service") == "service"
    )
    # ID 表优先于关键词
...
@pytest.mark.asyncio
async def test_execute_after_discussion_dispatches_super_employee(
    ...
    class FakeService:
        def invoke(self, *, user_id, message, context):
            ...
            return {
                "status": "completed",
                "assistant_message": {"body": "已完成：改了复购页文案"},
            }
    ...
    result = await svc.post_message(
        user_id=1, group_id=gid, text="实现复购优化", execute=True
    )
...
async def test_execute_noop_when_no_super_employee(tmp_path: Path):
    ...
    result = await svc.post_message(
        user_id=1, group_id=gid, text="做个表", execute=True
    )
