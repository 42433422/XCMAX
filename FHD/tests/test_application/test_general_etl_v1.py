...
def test_structured_file_knowledge_preview_uses_uploaded_document_path(tmp_path):
    xlsx_path = tmp_path / "采购订单.xlsx"
    xlsx_path.write_bytes(b"placeholder")

    dataset = parse_file(xlsx_path, target_type="knowledge")

    assert dataset.headers == ["document_path"]
    assert dataset.rows[0].values["document_path"] == str(xlsx_path.resolve())
    assert dataset.source_features == {"kind": "document", "knowledge_only": False}
...
def test_ocr_preview_routes_from_reconstructed_document_evidence(
    etl_db,
    tmp_path,
    monkeypatch,
):
    monkeypatch.setenv("FHD_ETL_LLM", "off")
    derived = tmp_path / "ocr-purchase-order.xlsx"
    workbook = Workbook()
    ...
    with tenant_scope(6):
        ...
        assert run["target_type"] == "purchase_orders"
        understanding = run["source_features"]["document_understanding"]
        assert understanding["documents"][0]["document_type"] == "purchase_order"
        assert understanding["recommended_target_type"] == "purchase_orders"
        assert run["source_features"]["kind"] == "ocr"
        assert run["draft"]["ocr_confirmed"] is False
        assert run["draft"]["document_confirmed"] is False
        db.close()


def test_auto_workbook_creates_one_preview_run_per_document_range(
    etl_db,
    monkeypatch,
    tmp_path,
):
    path = tmp_path / "three-documents.xlsx"
    workbook = Workbook()
    mixed = workbook.active
    mixed.title = "报价混排"
    for values in (
        ["报价单 A"],
        ["品名", "数量", "单价", "金额"],
    ...
    attendance.append(["姓名", "部门", "日期"])
    attendance.append(["张三", "生产部", "2026-07-30"])
    workbook.save(path)
    workbook.close()
            ...
            "tables": [
                {
                    ...
                    "last_column": 3,
                    "columns": [
                        {"column": 1, "role": "employee_name"},
                        {"column": 2, "role": "department"},
                        {"column": 3, "role": "attendance_date"},
                    ],
                }
            ],
            "confidence": 0.99,
    ...
    understanding = {
        "version": 1,
        "source": "llm",
        "file_structure": "mixed_workbook",
        "summary": "两张财务单据和一张考勤表",
        "recommended_target_type": "export_xlsx",
        "document_count": 3,
        "documents": documents,
        ...
        "requires_review": False,
        "plan_hash": "test-plan",
        "evidence": {"sheet_count": 2},
        "llm": {"used_llm": True, "degraded": False, "model": "test-model"},
    }
    understanding["document_routes"] = build_document_routes(
        understanding,
        hinted_target_type="export_xlsx",
    )
    monkeypatch.setattr(
        "app.application.etl.document_understanding.understand_workbook",
    ...
    with tenant_scope(6):
        ...
        assert len(root["details"]["sheet_inventory"]) == 2
        assert root["details"]["sheet_inventory"][0]["structure"] == "multi_document"
        routes = root["details"]["document_routes"]
        assert len(routes) == 3
        assert {route["status"] for route in routes} == {"preview_ready"}
        assert [route["target_type"] for route in routes] == [
            "export_xlsx",
            "export_xlsx",
            "attendance",
        ]
        stale_root = db.get(EtlRun, root["id"])
        stale_details = json.loads(stale_root.summary_json)
        stale_details["document_routes"][1].update(
            {"status": "previewing", "progress": 5, "total_rows": 0}
        )
        stale_root.summary_json = json.dumps(stale_details, ensure_ascii=False)
        db.commit()
        refreshed_root = service.get_run(db, run_id=root["id"], owner_user_id=10)
        assert refreshed_root["details"]["document_routes"][1]["status"] == "preview_ready"
        assert refreshed_root["details"]["document_routes"][1]["progress"] == 100
        assert refreshed_root["details"]["document_routes"][1]["total_rows"] == 1

        all_runs = db.query(EtlRun).filter(EtlRun.upload_id == upload["upload_id"]).all()
        assert len(all_runs) == 3
        parsed_ranges = []
        for route in routes:
            route_run = service.get_run(
                db,
                run_id=route["run_id"],
                owner_user_id=10,
            )
            assert "_document_plan" not in route_run["details"]
            assert len(route_run["source_features"]["document_understanding"]["documents"]) == 1
            rows = service.get_rows(
                db,
                run_id=route["run_id"],
                owner_user_id=10,
            ...
            assert len(rows) == 1
            parsed_ranges.append((rows[0]["source_sheet"], rows[0]["source_row"]))
            assert all(
                issue["code"] != "ETL_DOCUMENT_TARGET_MISMATCH"
                for issue in rows[0]["validation_issues"]
            )
        assert parsed_ranges == [("报价混排", 3), ("报价混排", 7), ("考勤", 2)]
        db.close()


def test_preview_persists_sheet_inventory_before_business_classification(
    etl_db,
    tmp_path,
    monkeypatch,
):
    path = tmp_path / "inventory-first.xlsx"
    workbook = Workbook()
    workbook.active.title = "送货单"
    workbook.active.append(["品名", "数量"])
    workbook.create_sheet("产品目录").append(["产品名称", "型号"])
    workbook.save(path)
    workbook.close()

    observed: dict[str, object] = {}

    def stop_after_inventory(*_args, evidence=None, **_kwargs):
        observer = etl_db()
        persisted = observer.query(EtlRun).one()
        details = json.loads(persisted.summary_json)
        observed.update(
            {
        ...
        observer.close()
        raise RuntimeError("stop after workbook inventory")

    service = EtlService()
    monkeypatch.setattr(
        "app.application.etl.document_understanding.understand_workbook",
        stop_after_inventory,
    )
    monkeypatch.setattr(
        service,
        "_submit_preview",
    ...
    with tenant_scope(6):
        ...
        assert observed == {
            ...
            "progress": 15,
            "sheet_count": 2,
            "sheet_names": ["送货单", "产品目录"],
            "evidence_sheet_count": 2,
        }
...
def test_structured_workbook_execution_requires_confirmed_document_plan(etl_db, monkeypatch):
    service = EtlService()
    monkeypatch.setenv("FHD_ETL_LLM", "off")
    monkeypatch.setattr(
        service,
    ...
    with tenant_scope(7):
        ...
        assert run["source_features"]["document_understanding"]["requires_confirmation"] is True
        with pytest.raises(EtlConflict) as blocked:
            service.execute(
                db,
                run_id=run["id"],
        ...
        assert blocked.value.code == "ETL_DOCUMENT_CONFIRMATION_REQUIRED"

        record = db.get(EtlRun, run["id"])
        draft = json.loads(record.draft_json)
        draft["document_confirmed"] = True
        record.draft_json = json.dumps(draft, ensure_ascii=False)
        db.commit()
        monkeypatch.setattr(service, "_submit_execution", lambda *_args: None)
...
def test_degraded_document_preview_can_reanalyze_original_upload(etl_db, monkeypatch):
    service = EtlService()
    monkeypatch.setattr(
        service,
        "_submit_preview",
    ...
    with tenant_scope(8):
        ...
        source_features = json.loads(record.source_features_json)
        source_features["document_understanding"]["llm"]["degraded"] = True
        record.source_features_json = json.dumps(source_features, ensure_ascii=False)
        draft = json.loads(record.draft_json)
        draft["document_confirmed"] = True
        record.draft_json = json.dumps(draft, ensure_ascii=False)
        db.commit()
        ...
        monkeypatch.setattr(
            service,
            "_submit_preview",
            lambda run_id, tenant_id, owner_user_id: submitted.update(
                run_id=run_id,
                tenant_id=tenant_id,
                owner_user_id=owner_user_id,
            ),
        )
        retried = service.reanalyze_with_llm(
            db,
            run_id=run["id"],
            owner_user_id=12,
        )

        assert retried["status"] == "previewing"
        assert retried["stage"] == "parsing"
        assert retried["draft"]["document_confirmed"] is False
        assert retried["draft"]["field_mappings"] == []
        assert submitted["run_id"] == run["id"]
        assert submitted["owner_user_id"] == 12
        db.close()
...
def test_purchase_order_preview_surfaces_master_data_candidates_without_auto_matching(etl_db):
    adapter = PurchaseOrderAdapter()
    with tenant_scope(140):
        db = etl_db()
        db.add_all(
            [
                Supplier(tenant_id=140, code="SUP-140", name="星光涂料工厂"),
                Product(
                    tenant_id=140,
                    unit="内部采购",
                    model_number="",
        ...
        preview = adapter.preview(
            db,
            {
                "external_order_no": "PO-CANDIDATE",
                "supplier_name": "星光涂料厂",
        ...
        assert "ETL_SUPPLIER_MATCH_CANDIDATES" in codes
        assert "ETL_PRODUCT_MATCH_CANDIDATES" in codes
        assert db.query(PurchaseOrder).count() == 0
        db.close()
...
def test_shipment_adapter_allows_distinct_lines_of_one_order_in_same_run(etl_db):
    ...
    with tenant_scope(115):
        ...
        db.commit()
        assert (
            db.query(ShipmentRecord).filter(ShipmentRecord.purchase_unit == "甲公司").count() == 2
        )
