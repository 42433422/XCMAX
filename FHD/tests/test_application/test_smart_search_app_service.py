...
class TestSmartSearchApplicationService:
    ...
    def test_search_products(self, mock_get: MagicMock, svc: SmartSearchApplicationService):
        ...
        mock_get.return_value.get_products.assert_called_once_with(
            keyword="PU", page=1, per_page=20
        )
    ...
    def test_search_excel_vector_no_indexes(
        self, mock_get: MagicMock, svc: SmartSearchApplicationService
    ):
    ...
    @patch("app.application.excel_vector_app_service.get_excel_vector_search_app_service")
    def test_search_excel_vector_query(
        self, mock_get: MagicMock, svc: SmartSearchApplicationService
    ):
        ...
        mock_get.return_value.query.assert_called_once_with(
            index_id="idx1", query_text="q", top_k=3
        )

    @patch("app.application.excel_vector_app_service.get_excel_vector_search_app_service")
    def test_search_excel_vector_query_failure(
        self, mock_get: MagicMock, svc: SmartSearchApplicationService
    ):
    ...
    @patch.object(SmartSearchApplicationService, "search_products")
    def test_search_products_scope_only(
        self, mock_prod: MagicMock, svc: SmartSearchApplicationService
    ):
        mock_prod.return_value = {"success": True, "data": []}
    ...
    @patch.object(SmartSearchApplicationService, "search_products")
    def test_search_products_failure_still_returns_block(
        self, mock_prod: MagicMock, svc: SmartSearchApplicationService
    ):
