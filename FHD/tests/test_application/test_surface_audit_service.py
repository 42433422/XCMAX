...
class TestLanePngDayDirs:
    def test_nonexistent_root(self, tmp_path):
        with patch("app.application.surface_audit_service._lane_png_root", return_value=tmp_path / "nonexistent"):
    ...
    def test_with_day_dirs(self, tmp_path):
        ...
        with patch("app.application.surface_audit_service._lane_png_root", return_value=png_root), \
             patch("app.application.surface_audit_service._today_key", return_value=today.isoformat()):
            result = _lane_png_day_dirs("test")
            assert len(result) >= 1
    ...
    def test_future_day_excluded(self, tmp_path):
        ...
        with patch("app.application.surface_audit_service._lane_png_root", return_value=png_root), \
             patch("app.application.surface_audit_service._today_key", return_value=date.today().isoformat()):
            result = _lane_png_day_dirs("test")
...
class TestResolveLanePagePngPath:
    def test_saved_path_exists(self, tmp_path):
        ...
        result = resolve_lane_page_png_path(
            "P-W", 0, {"screenshot_saved": str(png)}
        )
        assert result == png
    ...
    def test_slug_match_in_day_dir(self, tmp_path):
        ...
        with patch("app.application.surface_audit_service._lane_png_day_dirs", return_value=[day_dir]):
            result = resolve_lane_page_png_path(
                "P-W", 0, {"id": "dashboard"}
            )
...
class TestMostRecentPriorCache:
    def test_no_cache_dir(self, tmp_path):
        with patch("app.application.surface_audit_service._lane_cache_dir", return_value=tmp_path / "nonexistent"):
...
class TestAdbHasDevice:
    ...
    def test_adb_device_connected(self, tmp_path):
        with patch("app.application.surface_audit_service._FHD_ROOT", tmp_path), \
            ...
            adb_path = tmp_path / "mobile-android" / ".toolchain" / "android-sdk" / "platform-tools" / "adb"
...
class TestPlaywrightAvailable:
    def test_both_exist(self, tmp_path):
        ...
        with patch("app.application.surface_audit_service._SCRIPT_PATH", script), \
             patch("app.application.surface_audit_service._NODE_MODULES", tmp_path / "frontend" / "node_modules"):
    ...
    def test_script_missing(self, tmp_path):
        pw = tmp_path / "frontend" / "node_modules" / "@playwright" / "test"
        pw.mkdir(parents=True)
        with patch("app.application.surface_audit_service._SCRIPT_PATH", tmp_path / "missing.mjs"), \
             patch("app.application.surface_audit_service._NODE_MODULES", tmp_path / "frontend" / "node_modules"):
            result = _playwright_available()
            assert result is False
...
class TestRunSurfaceAuditLane:
    ...
    def test_unknown_lane(self):
        with patch("app.application.surface_audit_service._load_config", return_value={"lanes": {"P-W": {}}}):
    ...
    def test_cached_result(self):
        with patch("app.application.surface_audit_service._load_config", return_value={"lanes": {"P-W": {}}}), \
             patch("app.application.surface_audit_service._read_cache", return_value={"success": True, "data": "cached"}):
    ...
    def test_playwright_not_available(self):
        with patch("app.application.surface_audit_service._load_config", return_value={"lanes": {"P-W": {}}}), \
             ...
             patch("app.application.surface_audit_service._playwright_available", return_value=False):
...
class TestListConfiguredLanes:
    def test_with_lanes(self):
        with patch("app.application.surface_audit_service._load_config", return_value={"lanes": {"P-W": {}, "P-App": {}}}):
            result = list_configured_lanes()
            assert set(result) == {"P-W", "P-App"}

    def test_empty(self):
        with patch("app.application.surface_audit_service._load_config", return_value={"lanes": {}}):
            result = list_configured_lanes()
            assert result == []
