...
V2_SERVICES = [
    (
        ...
        "ExcelVectorAppServiceV2",
        "ai",
        "excelvectorappservice_v2",
        "get_excel_vector_app_service_v2",
    ),
    (
        "file_analysis_app_service_v2",
        "FileAnalysisAppServiceV2",
        "ai",
        "fileanalysisappservice_v2",
        "get_file_analysis_app_service_v2",
    ),
    (
        "material_app_service_v2",
        "MaterialAppServiceV2",
        "material",
        "materialappservice_v2",
    ...
    (
        ...
        "ProductImportAppServiceV2",
        "product",
        "productimportappservice_v2",
        "get_product_import_app_service_v2",
    ),
    (
        "template_app_service_v2",
        "TemplateAppServiceV2",
        "print",
        "templateappservice_v2",
        "get_template_app_service_v2",
    ),
    (
        "unit_products_import_app_service_v2",
        "UnitProductsImportAppServiceV2",
        "product",
        "unitproductsimportappservice_v2",
    ...
    (
        ...
        "UserMemoryVectorAppServiceV2",
        "auth",
        "usermemoryvectorappservice_v2",
        "get_user_memory_vector_app_service_v2",
    ),
    (
        "user_preference_app_service_v2",
        "UserPreferenceAppServiceV2",
        "auth",
        "userpreferenceappservice_v2",
        "get_user_preference_app_service_v2",
    ),
    (
        "wechat_contact_app_service_v2",
        "WechatContactAppServiceV2",
        "wechat",
        "wechatcontactappservice_v2",
        "get_wechat_contact_app_service_v2",
    ),
    (
        "wechat_task_app_service_v2",
        "WechatTaskAppServiceV2",
        "wechat",
        "wechattaskappservice_v2",
        "get_wechat_task_app_service_v2",
    ),
    (
        "extract_log_app_service_v2",
        "ExtractLogAppServiceV2",
        "log",
        "extractlogappservice_v2",
...
class TestV2ServiceCommon:
    ...
    def test_create_correlation_id_prefix(
        self, module_name, class_name, prefix, source, getter_name
    ):
    ...
    def test_create_correlation_id_format(
        self, module_name, class_name, prefix, source, getter_name
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_command_event_type(
        self, module_name, class_name, prefix, source, getter_name
    ):
        ...
        with patch("app.neuro_bus.events.base.NeuroEvent", return_value=mock_event) as mock_neuro:
            ...
            call_kwargs = (
                mock_neuro.call_args[1]
                if mock_neuro.call_args[1]
                else mock_neuro.call_args[0]
                if isinstance(mock_neuro.call_args[0], dict)
                else {}
            )
            ...
            if mock_neuro.call_args[1]:
                ...
                # positional args
                assert mock_neuro.call_args[0][0] == f"{prefix}.update" or "event_type" in str(
                    mock_neuro.call_args
                )

    @pytest.mark.asyncio
    async def test_execute_command_error_returns_failure(
        self, module_name, class_name, prefix, source, getter_name
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_command_value_error_returns_failure(
        self, module_name, class_name, prefix, source, getter_name
    ):
