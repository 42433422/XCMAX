...
def test_corporate_deploy_syncs_the_versioned_download_snippet_before_nginx_reload() -> (
    None
):
    ...
    sync_script = (
        ...
        / "deploy"
        / "scripts"
        / "sync-nginx-xiu-ci-snippets.sh"
    ).read_text(encoding="utf-8")

    assert (
        'bash "${SITE_ROOT}/deploy/scripts/sync-nginx-xiu-ci-snippets.sh"' in workflow
    )
...
def test_release_page_uses_public_history_order_for_the_current_version() -> None:
    ...
    assert (
        "if (releaseVersion === '1.0.0.0') entry.className += ' is-current'"
        not in release_page
    )
...
def test_download_page_prefers_a_same_or_newer_explicit_unsigned_interim_pointer() -> (
    None
):
    ...
    assert (
        "fetch('/download-windows-hotfix.json', { cache: 'no-store' })" in download_page
    )
    ...
    assert (
        "row.platform === 'android' && Boolean(state.androidVersion)" in download_page
    )
...
def test_macos_release_flow_publishes_download_center_metadata_and_has_recovery_path() -> (
    None
):
    mac_workflow = (
    ...
    assert (
        'step_ok("Enforce exact-SHA two-day security clearance")' in recovery_workflow
    )
