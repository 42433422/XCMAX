...
STAGE_SCRIPT = FHD_ROOT / "mobile-harmony" / "scripts" / "stage-release-packages.sh"
...
def test_android_only_mode_does_not_weaken_harmony_release_default() -> None:
    workflow = WORKFLOW.read_text()
    stage_script = STAGE_SCRIPT.read_text()

    assert "android_only:" in workflow
    assert "default: false" in workflow
    assert 'if [[ "${{ inputs.android_only }}" == "true" ]]' in workflow
    assert "args+=(--android-only)" in workflow
    assert "--allow-missing-harmony" not in workflow
    assert 'stage-release-packages.sh "${args[@]}"' in workflow

    assert "ANDROID_ONLY=0" in stage_script
    assert "--android-only)" in stage_script
    assert 'if [[ "${ANDROID_ONLY}" -eq 1 ]]' in stage_script
    assert "Android-only 发布" in stage_script
    assert "ALLOW_MISSING_HARMONY=0" in stage_script
