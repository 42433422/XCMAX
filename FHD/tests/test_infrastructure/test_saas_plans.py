...
def test_permanent_plan_prices_by_budget() -> None:
    ...
    cases = [
        ("5 万以内", "saas-permanent-starter", 4_999_900),
        ("5–20 万", "saas-permanent-growth", 19_999_900),
        ("20–50 万", "saas-permanent-max", 49_999_900),
        ("50 万以上", "saas-permanent-ultra", 99_999_900),
    ]
...
def test_pricing_plans_for_budget_returns_trial_and_permanent() -> None:
    sp.load_saas_plans_config.cache_clear()
    plans = sp.pricing_plans_for_budget("20–50 万")
    ids = [p["id"] for p in plans]
    assert ids == ["saas-trial-30", "saas-permanent-max"]


def test_normalize_budget_range_aliases() -> None:
    assert sp.normalize_budget_range("200k-500k") == "20–50 万"
