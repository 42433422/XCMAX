...
class TestImRoutes:
    ...
    def test_admin_session_excludes_enterprise_dedicated_cs_from_conversations(self, client):
        ...
        with (
            ...
            patch("app.fastapi_routes.im_routes.ImApplicationService") as mock_svc_cls,
            patch(
                "app.fastapi_routes.im_routes._include_enterprise_dedicated_cs", return_value=False
            ),
        ):
            mock_db = MagicMock()
            mock_db_cls.return_value = mock_db
    ...
    def test_codex_super_employee_lists_messages_for_admin(self, client):
        with (
            ...
            mock_svc.list_messages.return_value = [
                {"id": "m1", "role": "assistant", "body": "ready"}
            ]
            mock_svc_cls.return_value = mock_svc
    ...
    def test_cursor_super_employee_invokes_with_factory_context(self, client, monkeypatch):
        monkeypatch.setenv("XCMAX_FACTORY_CAPABILITY_TOKEN", "factory-token")
        with (
            patch("app.fastapi_routes.im_routes.HostSessionLocal") as mock_db_cls,
            patch(
            ...
            resp = client.post(
                ...
                json={"message": "修登录", "workspace_id": "xcmax"},
            )
        assert resp.status_code == 200
        mock_svc.invoke.assert_called_once()
        kwargs = mock_svc.invoke.call_args.kwargs
        assert kwargs["message"] == "修登录"
        assert kwargs["context"]["_factory_token"] == "factory-token"
        assert kwargs["context"]["workspace_id"] == "xcmax"
