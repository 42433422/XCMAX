...
class TestWechatRedirect:
    def test_wechat_redirect_no_url_returns_503(self, client_mp):
        ...
        with patch.dict(
            "sys.modules", {"app.infrastructure.payment.modstore_payment_proxy": mock_proxy}
        ):
...
class TestCheckout:
    def test_checkout_modstore_sot_with_plan_success(self, client_mp):
        ...
        with (
            patch("app.fastapi_routes.model_payment.is_modstore_payment_sot", return_value=True),
            ...
            r = client_mp.post(
                "/api/model-payment/checkout", json={"plan_id": "demo-starter", "channel": "alipay"}
            )
            assert r.status_code == 200
    ...
    def test_checkout_invalid_channel_returns_400(self, client_mp):
        with (
            patch("app.fastapi_routes.model_payment.is_modstore_payment_sot", return_value=False),
            patch(
                "app.fastapi_routes.model_payment.is_json_legacy_payment_sot", return_value=False
            ),
            patch.dict("os.environ", {}, clear=False),
        ):
            r = client_mp.post(
                "/api/model-payment/checkout", json={"plan_id": "demo-starter", "channel": "wechat"}
            )
    ...
    def test_checkout_demo_mode_alipay_not_ready(self, client_mp):
        with (
            ...
            patch(
                "app.fastapi_routes.model_payment._plan_by_id",
                return_value={"id": "demo-starter", "title": "体验档", "amount_cents": 990},
            ),
            patch("app.fastapi_routes.model_payment.mp_ali") as mock_ali,
            patch.dict("os.environ", {}, clear=False),
        ):
...
class TestAlipayNotify:
    def test_notify_not_local_sot_returns_410(self, client_mp):
        with patch(
            "app.fastapi_routes.model_payment.is_local_model_payment_sot", return_value=False
        ):
    ...
    def test_notify_no_signature_returns_400(self, client_mp):
        with patch(
            "app.fastapi_routes.model_payment.is_local_model_payment_sot", return_value=True
        ):
    ...
    def test_notify_credentials_not_ready_returns_503(self, client_mp):
        with (
            patch("app.fastapi_routes.model_payment.is_local_model_payment_sot", return_value=True),
            patch("app.fastapi_routes.model_payment.mp_ali") as mock_ali,
        ):
            mock_ali.credentials_ready.return_value = False
            r = client_mp.post(
    ...
    def test_notify_verify_success_returns_success(self, client_mp):
        with (
            ...
            mock_orders.apply_notify_paid.return_value = (
                "marked_paid",
                {"plan_id": "p1", "amount_cents": 990, "entitlement": {"purchase_count": 1}},
            )
            r = client_mp.post(
                "/api/model-payment/notify/alipay",
                data={
...
class TestDiagnostics:
    ...
    def test_diagnostics_postgres_sot_includes_order_count(self, client_mp):
        with (
            ...
            patch("app.fastapi_routes.model_payment.mp_orders") as mock_orders,
            patch(
                "app.fastapi_routes.model_payment.model_payment_backend", return_value="postgres"
            ),
            patch(
                "app.fastapi_routes.model_payment.is_fhd_postgres_payment_sot", return_value=True
            ),
        ):
...
class TestQueryTrade:
    ...
    def test_query_trade_failure(self, client_mp):
        with (
            ...
            mock_ali.query_order.return_value = {"success": False, "message": "查询失败"}
...
class TestRefund:
    ...
    def test_refund_success(self, client_mp):
        with (
            ...
            mock_ali.refund_order.return_value = {
                "success": True,
                "raw": {"refund_status": "REFUND_SUCCESS"},
            }
            mock_orders.update_order_status.return_value = None
            r = client_mp.post(
                "/api/model-payment/refund",
                json={"out_trade_no": "mp-123", "refund_amount": "9.90"},
            )
            assert r.status_code == 200
            body = r.json()
            assert body["success"] is True
    ...
    def test_refund_alipay_failure(self, client_mp):
        with patch("app.fastapi_routes.model_payment.mp_ali") as mock_ali:
            mock_ali.refund_order.return_value = {"success": False, "message": "退款失败"}
            r = client_mp.post(
                "/api/model-payment/refund",
                json={"out_trade_no": "mp-123", "refund_amount": "9.90"},
            )
    ...
    def test_refund_write_failure_still_returns_success(self, client_mp):
        with (
            ...
            mock_ali.refund_order.return_value = {"success": True, "raw": {}}
...
class TestRefundQuery:
    ...
    def test_refund_query_failure(self, client_mp):
        with patch("app.fastapi_routes.model_payment.mp_ali") as mock_ali:
            mock_ali.query_refund.return_value = {"success": False, "message": "查询失败"}
            r = client_mp.get("/api/model-payment/refund/query", params={"out_trade_no": "mp-123"})
            assert r.status_code == 200
            assert r.json()["success"] is False
