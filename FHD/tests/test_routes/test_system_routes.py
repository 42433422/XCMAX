...
class TestSystemInfoGet:
    def test_success(self, client: TestClient):
        ...
        with patch("app.application.facades.session_facade.get_system_service", return_value=mock_svc):
            r = client.get("/api/system/info")
            assert r.json()["success"] is True

    def test_error(self, client: TestClient):
        with patch("app.application.facades.session_facade.get_system_service", side_effect=Exception("fail")):
            r = client.get("/api/system/info")
...
class TestDatabaseBackups:
    def test_list_success(self, client: TestClient):
        ...
        with patch("app.application.facades.session_facade.get_database_service", return_value=mock_svc):
            r = client.get("/api/database/backups")
            assert r.json()["success"] is True

    def test_list_error(self, client: TestClient):
        with patch("app.application.facades.session_facade.get_database_service", side_effect=Exception("fail")):
            r = client.get("/api/database/backups")
            assert r.status_code == 500
...
class TestPerformanceStatus:
    def test_not_initialized(self, client: TestClient):
        ...
        with patch("app.utils.performance_initializer.get_performance_optimizer", return_value=mock_opt):
    ...
    def test_error(self, client: TestClient):
        with patch("app.utils.performance_initializer.get_performance_optimizer", side_effect=Exception("fail")):
            r = client.get("/api/performance/status")
            assert r.status_code == 500
...
class TestPerformanceHealth:
    ...
    def test_degraded(self, client: TestClient):
        mock_opt = MagicMock()
        mock_opt.get_health_check.return_value = {"status": "degraded", "timestamp": 0, "checks": {}}
        with patch("app.utils.performance_initializer.get_performance_optimizer", return_value=mock_opt):
            r = client.get("/api/performance/health")
            assert r.status_code == 503
    ...
    def test_unhealthy(self, client: TestClient):
        mock_opt = MagicMock()
        mock_opt.get_health_check.return_value = {"status": "unhealthy", "timestamp": 0, "checks": {}}
        with patch("app.utils.performance_initializer.get_performance_optimizer", return_value=mock_opt):
            r = client.get("/api/performance/health")
            assert r.status_code == 500
    ...
    def test_with_issues(self, client: TestClient):
        mock_opt = MagicMock()
        mock_opt.get_health_check.return_value = {"status": "healthy", "timestamp": 0, "checks": {}, "issues": ["slow"]}
        with patch("app.utils.performance_initializer.get_performance_optimizer", return_value=mock_opt):
            r = client.get("/api/performance/health")
...
class TestPerformanceOptimizeReinitialize:
    def test_success(self, client: TestClient):
        mock_opt = MagicMock()
        mock_opt.get_status.return_value = {"status": "ok"}
        with patch("app.utils.performance_initializer.init_performance_optimization", return_value=mock_opt):
            r = client.post("/api/performance/optimize/reinitialize")
            assert r.json()["success"] is True

    def test_error(self, client: TestClient):
        with patch("app.utils.performance_initializer.init_performance_optimization", side_effect=Exception("fail")):
            r = client.post("/api/performance/optimize/reinitialize")
            assert r.status_code == 500
...
class TestTemplatesProgress:
    def test_success(self, client: TestClient):
        with patch("app.fastapi_routes.domains.system.routes.get_template_analysis_progress", return_value={"progress": 50, "success": True, "task_id": "task1", "step": 1, "message": "done", "completed": False}):
...
class TestTemplatesDelete:
    def test_missing_id(self, client: TestClient):
        r = client.request("DELETE", "/api/templates/delete", content=json.dumps({}), headers={"Content-Type": "application/json"})
        assert r.status_code == 400

    def test_fs_template_empty_filename(self, client: TestClient):
        r = client.request("DELETE", "/api/templates/delete", content=json.dumps({"id": "fs:"}), headers={"Content-Type": "application/json"})
        assert r.status_code == 400

    def test_fs_template_not_found(self, client: TestClient):
        with patch("app.fastapi_routes.domains.system.routes.get_base_dir", return_value="/nonexistent"):
            r = client.request("DELETE", "/api/templates/delete", content=json.dumps({"id": "fs:missing.docx"}), headers={"Content-Type": "application/json"})
            assert r.status_code == 404

    def test_fs_template_success(self, client: TestClient, tmp_path):
        test_file = tmp_path / "test.docx"
        test_file.write_text("content")
        with patch("app.fastapi_routes.domains.system.routes.get_base_dir", return_value=str(tmp_path)):
            r = client.request("DELETE", "/api/templates/delete", content=json.dumps({"id": "fs:test.docx"}), headers={"Content-Type": "application/json"})
            assert r.json()["success"] is True
    ...
    def test_db_template_not_found(self, client: TestClient):
        ...
        with patch("app.db.session.get_db", return_value=mock_db), \
            ...
            r = client.request("DELETE", "/api/templates/delete", content=json.dumps({"id": "db:9999"}), headers={"Content-Type": "application/json"})
            assert r.status_code == 404

    def test_unsupported_type(self, client: TestClient):
        r = client.request("DELETE", "/api/templates/delete", content=json.dumps({"id": "other:123"}), headers={"Content-Type": "application/json"})
        assert r.status_code == 400

    def test_error(self, client: TestClient):
        with patch("app.fastapi_routes.domains.system.routes.get_base_dir", side_effect=RuntimeError("fail")):
            r = client.request("DELETE", "/api/templates/delete", content=json.dumps({"id": "fs:test.docx"}), headers={"Content-Type": "application/json"})
            assert r.status_code == 500
...
class TestSkillsInfo:
    def test_found(self, client: TestClient):
        mock_reg = MagicMock()
        mock_reg.get.return_value = {"name": "test", "description": "desc", "keywords": [], "category": "gen"}
        with patch("app.infrastructure.skills.get_skill_registry", return_value=mock_reg):
