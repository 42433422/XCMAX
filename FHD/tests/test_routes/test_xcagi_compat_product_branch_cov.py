...
class TestAgentNodeOutput:
    def test_final_output_none_falls_back_to_steps(self):
        run = SimpleNamespace(
            final_output=None, steps=[], status="completed", error="", run_id="r1"
        )
    ...
    def test_steps_not_found_uses_status(self):
        ...
        run = SimpleNamespace(
            final_output=None, steps=[step], status="completed", error="", run_id=""
        )
        out = xp._agent_node_output(run, "n1")
        assert out["success"] is True
    ...
    def test_status_not_completed_sets_success_false(self):
        run = SimpleNamespace(
            final_output=None, steps=[], status="failed", error="boom", run_id="r4"
        )
        out = xp._agent_node_output(run, "n1")
    ...
    def test_steps_none(self):
        run = SimpleNamespace(
            final_output=None, steps=None, status="completed", error="", run_id="r8"
        )
        out = xp._agent_node_output(run, "n1")
        assert out["success"] is True
...
class TestProductsCompatStatusCode:
    ...
    def test_error_code_tool_exception(self):
        assert (
            xp._products_compat_status_code(
    ...
    def test_error_code_other(self):
        assert (
            xp._products_compat_status_code(
                {"success": False, "status_code": 0, "error_code": "other"}
            )
            == 200
        )
...
class TestProductsCompatViaServiceEnabled:
    def test_enabled_true(self):
        with patch(
            "app.mod_sdk.erp_products_facade.is_erp_products_via_service_enabled", return_value=True
        ):
...
class TestProductsCompatPreflight:
    ...
    def test_via_service_enabled_returns_none(self):
        with patch(
            "app.fastapi_routes.xcagi_compat_product._products_compat_via_service_enabled",
            return_value=True,
        ):
...
class TestExecuteProductsCompatAction:
    ...
    def test_via_service_update(self):
        with (
            ...
            patch(
                "app.mod_sdk.erp_products_facade.products_update", return_value={"success": True}
            ),
        ):
    ...
    def test_via_service_delete(self):
        with (
            ...
            patch(
                "app.mod_sdk.erp_products_facade.products_delete", return_value={"success": True}
            ),
        ):
    ...
    def test_via_service_batch_delete(self):
        with (
            ...
            assert (
                xp._execute_products_compat_action("batch_delete", {"ids": [1, 2]})["success"]
                is True
            )
    ...
    def test_via_service_recoverable_error(self):
        with (
            ...
            with patch(
                "app.fastapi_routes.xcagi_compat_product.products_pg_insert_row", return_value=1
            ):
    ...
    def test_pg_update_invalid_id(self):
        ...
        try:
            with (
                ...
                patch(
                    "app.fastapi_routes.xcagi_compat_product._product_parse_id", return_value=None
                ),
            ):
...
class TestRunProductsCompatAgent:
    ...
    def test_action_meta_not_dict(self):
        ...
        with patch(
            ...
            result = xp._run_products_compat_agent(
                request=self._req(), action="unknown", params={}, route_path="/p"
            )
            assert result["success"] is False
            assert "未注册" in result["message"]
    ...
    def test_registry_products_missing(self):
        with patch(
            "app.services.tools_execution.registry.get_workflow_tool_registry", return_value={}
        ):
            result = xp._run_products_compat_agent(
                request=self._req(), action="create", params={}, route_path="/p"
            )
            assert result["success"] is False
    ...
    def test_run_status_waiting_user_continues(self):
        ...
        run1 = SimpleNamespace(
            status="waiting_user", run_id="r1", final_output=None, steps=[], error=""
        )
        run2 = SimpleNamespace(
            status="completed",
            run_id="r1",
            final_output={"node_outputs": {"products_create_compat": {"success": True}}},
            steps=[],
            error="",
        )
        orch = MagicMock()
    ...
    def test_run_normal_completed(self):
        ...
        run = SimpleNamespace(
            status="completed",
            run_id="r9",
            final_output={"node_outputs": {"products_delete_compat": {"success": True}}},
            steps=[],
            error="",
        )
        orch = MagicMock()
        ...
        with (
            ...
            result = xp._run_products_compat_agent(
                request=self._req(), action="delete", params={"message": "hi"}, route_path="/p"
            )
            assert result["success"] is True
...
class TestProductsPriceListWordResponse:
    def test_business_data_not_exposed_with_reason(self):
        with (
            ...
            patch(
                "app.shell.mod_business_scope.business_data_hidden_reason", return_value="not ready"
            ),
        ):
            with pytest.raises(HTTPException) as exc_info:
