...
class TestModuleHelpers:
    ...
    def test_get_deepseek_api_key_missing_returns_empty(self, monkeypatch):
        ...
        with patch("app.utils.path_io.path_utils.get_resource_path", return_value="/no/such/config.py"):
...
class TestGetApiKey:
    ...
    def test_empty_when_no_key_anywhere(self, monkeypatch):
        monkeypatch.delenv("DEEPSEEK_API_KEY", raising=False)
        with patch("app.utils.path_io.path_utils.get_resource_path", return_value="/no/such/cfg.py"):
            r = DeepSeekIntentRecognizer(api_key=None)
