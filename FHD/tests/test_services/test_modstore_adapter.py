...
class TestFromSession:
    ...
    def test_from_session_with_request_auth(self, monkeypatch):
        """Authorization 是 FHD JWT，不得当作 MODstore market token。"""
    ...
    def test_from_session_request_token_overrides_env(self, monkeypatch):
        """请求头 Authorization（FHD JWT）不得覆盖 MODSTORE_AUTH_TOKEN。

        market token 只来自显式 kwargs / 环境变量 / session 绑定；
        把 FHD JWT 当 market token 会触发 MODstore 401。
        """
