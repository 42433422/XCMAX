...
class TestFromSession:
    ...
    def test_from_session_env_market_token_ignores_request_jwt(self, monkeypatch):
        """FHD Authorization JWT must not replace a MODstore market token.

        The two tokens have different issuers and signing secrets. Forwarding the
        request JWT to MODstore would produce a 401 and weaken the auth boundary.
        """
        ...
        adapter = ModstorePlatformAdapter.from_session(request=request)
        assert adapter.auth_token == "stale_env_token"
        assert adapter._source == "env"
