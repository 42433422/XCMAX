...
class TestProductModel:
    ...
    def test_product_optional_fields(self, test_session):
        """测试产品可选字段"""
        product = Product(name="只有名称的产品", is_active=1)
        test_session.add(product)
        test_session.commit()
...
class TestUserModel:
    ...
    def test_create_user(self, test_session):
        """测试创建用户"""
        user = User(
            username="testuser", password="hashed_password", display_name="测试用户", role="user"
        )
...
class TestUserSessionModel:
    ...
    def test_create_user_session(self, test_session):
        ...
        session = UserSession(
            session_id="sess_001", user_id=user.id, expires_at=datetime.now() + timedelta(hours=24)
        )
...
class TestMaterialModel:
    ...
    def test_create_material(self, test_session):
        """测试创建原材料"""
        material = Material(material_code="MAT-001", name="测试原材料", quantity=100.0, unit="kg")
...
class TestAIModels:
    ...
    def test_create_ai_tool_category(self, test_session):
        """测试创建 AI 工具分类"""
        category = AIToolCategory(category_key="text_generation", category_name="文本生成")
...
class TestProductRelationships:
    ...
    def test_product_with_shipment_records(self, test_session):
        """测试产品与发货记录的关系"""
        product = Product(name="测试产品", model_number="TEST-001", price=100.0, is_active=1)
        test_session.add(product)
        test_session.commit()
...
class TestShipmentRecordValidation:
    ...
    def test_shipment_required_fields(self, test_session):
        """测试发货记录必需字段"""
        shipment = ShipmentRecord(
            purchase_unit="测试单位", product_name="测试产品", quantity_kg=100.0, quantity_tins=4
        )
        test_session.add(shipment)
        test_session.commit()
...
class TestMaterialValidation:
    ...
    def test_material_low_stock(self, test_session):
        """测试低库存检测"""
        material = Material(
            material_code="MAT-001", name="低库存原料", quantity=5.0, min_stock=10.0, unit="kg"
        )
        test_session.add(material)
        test_session.commit()
    ...
    def test_material_required_fields(self, test_session):
        """测试原材料必需字段"""
        material = Material(material_code="MAT-002", name="测试原料")
        test_session.add(material)
        test_session.commit()
...
class TestUserAuthentication:
    ...
    def test_user_unique_username(self, test_session):
        """测试用户名唯一性"""
        user1 = User(username="uniqueuser", password="pwd1", display_name="用户 1", role="user")
...
class TestBulkOperations:
    ...
    def test_bulk_insert_products(self, test_session):
        """测试批量插入产品"""
        products = [
            Product(name=f"产品_{i}", price=100.0 * i, is_active=1, tenant_id=1) for i in range(10)
        ]
