...
class TestCustomersToolDelete:
    def test_delete_by_id(self):
        with patch("app.services.unified_query_service.query_service") as mock_qs:
            ...
            result = _unwrap(dispatch_legacy_tool_payload(
                "customers", "delete", {"customer_id": "1"},
    ...
    def test_delete_by_name(self):
        with patch("app.services.unified_query_service.query_service") as mock_qs:
            mock_qs.delete.return_value = 1
            result = _unwrap(dispatch_legacy_tool_payload(
                "customers", "delete", {"unit_name": "公司A"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is True
    ...
    def test_delete_not_found(self):
        with patch("app.services.unified_query_service.query_service") as mock_qs:
            ...
            with patch("app.infrastructure.lookups.purchase_unit_resolver.resolve_purchase_unit", return_value=None):
                result = _unwrap(dispatch_legacy_tool_payload(
                    "customers", "delete", {"unit_name": "不存在公司"},
                    json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                    parse_order_text_fn=_parse_order_text,
                ))
        assert result["success"] is True
    ...
    def test_delete_with_resolve_fallback(self):
        with patch("app.services.unified_query_service.query_service") as mock_qs:
            ...
            with patch("app.infrastructure.lookups.purchase_unit_resolver.resolve_purchase_unit", return_value=mock_resolved):
                result = _unwrap(dispatch_legacy_tool_payload(
                    "customers", "delete", {"unit_name": "公司A"},
                    json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                    parse_order_text_fn=_parse_order_text,
    ...
    def test_delete_with_order_text_name_extraction(self):
        with patch("app.services.unified_query_service.query_service") as mock_qs:
            mock_qs.delete.return_value = 1
            result = _unwrap(dispatch_legacy_tool_payload(
                "customers", "delete", {"order_text": "删除客户某某公司"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is True
    ...
    def test_delete_resolve_error(self):
        with patch("app.services.unified_query_service.query_service") as mock_qs:
            mock_qs.delete.return_value = 0
            with patch("app.infrastructure.lookups.purchase_unit_resolver.resolve_purchase_unit", side_effect=RuntimeError("fail")):
                result = _unwrap(dispatch_legacy_tool_payload(
                    "customers", "delete", {"unit_name": "公司A"},
                    json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                    parse_order_text_fn=_parse_order_text,
...
class TestCustomersToolSupplement:
    ...
    def test_supplement_customer_not_found(self):
        with (
            ...
            mock_ctx.return_value = mock_ctx_inst
            result = _unwrap(dispatch_legacy_tool_payload(
                "customers", "supplement", {"field_name": "contact_person", "field_value": "张三"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is False
...
class TestCustomersToolCreate:
    ...
    def test_create_already_exists(self):
        ...
        with (
            patch("app.application.get_customer_app_service", return_value=mock_svc),
            patch("app.services.unified_query_service.find_purchase_unit", return_value=mock_exists),
        ):
            result = _unwrap(dispatch_legacy_tool_payload(
                "customers", "添加", {"unit_name": "已存在公司"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is True
    ...
    def test_create_with_contact_info_in_unit_name(self):
        ...
        with (
            ...
            mock_ctx.return_value = mock_ctx_inst
            result = _unwrap(dispatch_legacy_tool_payload(
                "customers", "添加", {"unit_name": "七彩乐园联系人向总"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
...
class TestShipmentGenerateToolDeep:
    def test_generate_with_order_text(self):
        ...
        with patch("app.services.shipment_number_mode_service.ShipmentNumberModeService", return_value=mock_svc):
            result = _unwrap(dispatch_legacy_tool_payload(
                "shipment_generate", "generate", {"order_text": "给公司A发10桶涂料"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is True
    ...
    def test_generate_exception(self):
        with patch("app.services.shipment_number_mode_service.ShipmentNumberModeService", side_effect=RuntimeError("fail")):
            result = _unwrap(dispatch_legacy_tool_payload(
                "shipment_generate", "generate", {"order_text": "test"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is False
...
class TestPrintToolDeep:
    ...
    def test_print_label(self):
        ...
        with patch("app.services.get_printer_service", return_value=mock_svc):
            result = _unwrap(dispatch_legacy_tool_payload(
                "print", "print_label", {"file_path": "/label.pdf"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is True
    ...
    def test_print_document(self):
        ...
        with patch("app.services.get_printer_service", return_value=mock_svc):
            result = _unwrap(dispatch_legacy_tool_payload(
                "print", "print_document", {"file_path": "/doc.pdf"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is True
...
class TestPrinterListToolDeep:
    ...
    def test_set_default(self):
        ...
        with patch("app.services.get_system_service", return_value=mock_svc):
            result = _unwrap(dispatch_legacy_tool_payload(
                "printer_list", "set_default", {"printer_name": "HP"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is True
...
class TestMaterialsToolDeep:
    ...
    def test_update(self):
        ...
        with patch("app.application.get_material_application_service", return_value=mock_svc):
            result = _unwrap(dispatch_legacy_tool_payload(
                "materials", "update", {"id": "1", "name": "涂料B"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is True
    ...
    def test_batch_delete(self):
        ...
        with patch("app.application.get_material_application_service", return_value=mock_svc):
            result = _unwrap(dispatch_legacy_tool_payload(
                "materials", "batch_delete", {"ids": [1, 2, 3]},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is True
...
class TestWechatToolDeep:
    ...
    def test_refresh_cache(self):
        with patch("app.services.wechat_contact_cache_import.ensure_decrypted_wechat_dbs", return_value={"success": True}):
            result = _unwrap(dispatch_legacy_tool_payload(
...
class TestExcelAnalyzerToolDeep:
    ...
    def test_with_file_path(self):
        ...
        with patch("app.infrastructure.skills.excel_analyzer.excel_template_analyzer.get_excel_analyzer_skill", return_value=mock_skill):
            result = _unwrap(dispatch_legacy_tool_payload(
                "excel_analyzer", "analyze", {"file_path": "/test.xlsx"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is True
...
class TestShipmentRecordsToolDeep:
    def test_update(self):
        ...
        with patch("app.bootstrap.get_shipment_app_service", return_value=mock_svc):
            result = _unwrap(dispatch_legacy_tool_payload(
                "shipment_records", "update", {"id": "1", "status": "shipped"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is True
    ...
    def test_export(self):
        ...
        with patch("app.bootstrap.get_shipment_app_service", return_value=mock_svc):
            result = _unwrap(dispatch_legacy_tool_payload(
                "shipment_records", "export", {"unit": "公司A"},
                json_response_fn=_json_fn, hdr_getter=_hdr_getter,
                parse_order_text_fn=_parse_order_text,
            ))
        assert result["success"] is True
...
class TestBusinessDockingToolDeep:
    ...
    def test_file_not_exists(self):
        result = _unwrap(dispatch_legacy_tool_payload(
            "business_docking", "extract", {"file_path": "/nonexistent.xlsx"},
            json_response_fn=_json_fn, hdr_getter=_hdr_getter,
            parse_order_text_fn=_parse_order_text,
        ))
        assert result["success"] is False
