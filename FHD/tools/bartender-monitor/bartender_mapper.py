...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("bartender_mapping.log"), logging.StreamHandler()],
)
...
class WhiteRegionDetector:
    ...
    def detect_white_regions(
        self, image: Image.Image, is_full_window: bool = False
        ...
        for contour in contours:
            ...
            w / image_width if image_width > 0 else 0

            is_valid_label = (
                area > 10000  # 面积足够大
                and 0.3 < aspect_ratio < 4  # 宽高比合理
                and w > image_width * 0.3  # 宽度占图像30%以上
                and x > left_exclude  # 不在左侧边缘
                and x + w < image_width - right_exclude  # 不在右侧边缘
                and y > top_exclude  # 不在顶部边缘
            )
    ...
    def save_debug_image(
        self, image: Image.Image, regions: List[Tuple[int, int, int, int]], output_path: str
    ):
        """保存调试图像，标记检测到的白色区域"""
        ...
        for i, (x1, y1, x2, y2) in enumerate(regions):
            ...
            # 添加编号
            cv2.putText(
                img_array, f"Label {i + 1}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2
            )
...
注意事项：存放于阴凉干燥处，远离火源"""
    ...
    def classify_regions_with_position(
        self, text_items: List[Dict], image_width: int, image_height: int
    ) -> Dict:
        """对提取的文字按位置进行分类"""
        ...
        logger.info(
            f"位置分类完成: 上区={len(regions['left_top']) + len(regions['center_top']) + len(regions['right_top'])}, "
    ...
    def classify_regions(self, text: str) -> Dict:
        ...
        regions = {"product_name": "", "model_spec": "", "production_info": [], "other_info": []}
        ...
        for line in lines:
            ...
            if "产品名称" in line:
                regions["product_name"] = (
                    line.replace("产品名称：", "").replace("产品名称:", "").strip()
                )
            elif "型号规格" in line:
                regions["model_spec"] = (
                    line.replace("型号规格：", "").replace("型号规格:", "").strip()
                )
            elif any(
                keyword in line
                for keyword in ["生产", "执行标准", "日期", "保质期", "净含量", "厂家"]
            ):
    ...
    def generate_position_map_table(
        self, regions: Dict, image_width: int, image_height: int, image_index: int = 0
    ) -> str:
        """生成带位置信息的映射表格"""
        table = []
        table.append("=" * 80)
        table.append(
            f"标签位置映射结果 - 区域 {image_index + 1} (尺寸: {image_width}x{image_height})"
        )
        table.append("=" * 80)
        table.append(
            "{:<8} | {:<20} | {:<8} | {:<8} | {:<8} | {:<8} | {:<6}".format(
    ...
    def save_excel_style_table(
        self, text_items: List[Dict], output_path: str, image_index: int = 0
    ):
        """生成Excel样式的表格布局图像"""
        # 按Y坐标排序
        sorted_items = sorted(text_items, key=lambda x: x["center_y"])
        ...
        for j, header in enumerate(headers):
            ...
            cv2.putText(
                img_array, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2
            )
        ...
        for i, item in enumerate(sorted_items):
            ...
            for j, data in enumerate(row_data):
                ...
                cv2.putText(
                    img_array, data, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1
                )
    ...
    def generate_combined_layout(
        ...
        """生成左侧原图+右侧表格的组合布局"""
        # 按Y坐标排序
        sorted(text_items, key=lambda x: x["center_y"])
        ...
        for i in range(border_size):
            ...
            draw.rectangle(
                [0, 0, orig_width + border_size * 2, orig_height + border_size * 2],
                outline=(0, 0, 0),
                width=border_size,
            )
    ...
    def process(
        self, output_dir: str = ".", save_debug: bool = False, save_position_map: bool = True
    ) -> List[str]:
        """执行完整的处理流程"""
        results = []
        ...
        for w_idx, window in enumerate(windows):
            ...
            try:
                ...
                white_regions = self.white_detector.detect_white_regions(
                    window_image, is_full_window=True
                )
                ...
                for i, cropped in enumerate(cropped_images):
                    ...
                    # 位置分类
                    regions = self.classify_regions_with_position(
                        text_items, cropped.width, cropped.height
                    )
                    ...
                    combined_path = os.path.join(
                        output_dir, f"combined_layout_{w_idx + 1}_{i + 1}.png"
                    )
                    self.generate_combined_layout(cropped, text_items, combined_path, i)

                    # 生成位置映射表格
                    if save_position_map:
                        pos_table = self.generate_position_map_table(
                            regions, cropped.width, cropped.height, i
                        )
...
def main():
    ...
    if args.no_window:
        ...
        results = mapper.process(
            output_dir=args.output, save_debug=args.debug, save_position_map=save_position_map
        )
