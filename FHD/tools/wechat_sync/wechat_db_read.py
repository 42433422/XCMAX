...
SSOT 说明：本副本为 tools/wechat_sync 受管版本（服务 wechat_sync.py 同步代理）；
FHD/mutants/resources/wechat_cv/wechat_db_read.py 为沙箱原始版本（CV 脚本仍引用），
两处独立维护，改同步相关逻辑请改本副本。
...
本模块不负责从内存提取 key，仅从 wechat_db_key.json 读取 key_hex 后解密。配置示例：
  wechat_sync/wechat_db_key.json  {"key_hex": "64位十六进制密钥", "wechat_data_dir": "C:\\xwechat_files"}
"""
import os
import sqlite3
import json
    ...
    for table in ("MSG", "Message"):
        ...
        if r.get("success"):
            ...
            for row in (r.get("rows") or []):
                ...
                rows.append({"role": role, "text": content, "ts": row.get("createTime")})
        return {"success": True, "rows": rows, "count": len(rows)}
        # 可能表名或字段不同，尝试下一张表
