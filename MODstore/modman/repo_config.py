...
from modman.constants import DEFAULT_PORTAL_PLANS_URL, DEFAULT_XCAGI_BACKEND_URL
...
class RepoConfig:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "RepoConfig":
        return cls(
            ...
            portal_plans_url=str(d.get("portal_plans_url") or "").strip(),
            portal_wallet_sync_url=str(d.get("portal_wallet_sync_url") or "").strip(),
        )
...
def resolved_portal_plans_url(cfg: RepoConfig) -> str:
    u = (cfg.portal_plans_url or "").strip()
    if u:
        return u
    return DEFAULT_PORTAL_PLANS_URL
