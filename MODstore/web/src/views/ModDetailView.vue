      ...
      <div v-show="tab === 'overview'" class="stack">
        ...
        <section class="card">
          ...
          <div class="scaffold-block">
            <p class="hint muted small">{{ t('intro.modDetail.scaffoldHint') }}</p>
            <div class="grid2">
              <div>
                <label class="label">{{ t('intro.modDetail.scaffoldEmpId') }}</label>
                <input v-model="scaffoldEmpId" class="input" spellcheck="false" autocomplete="off" />
              </div>
              <div>
                <label class="label">{{ t('intro.modDetail.scaffoldLabel') }}</label>
                <input v-model="scaffoldLabel" class="input" spellcheck="false" />
              </div>
              <div class="full">
                <label class="label">{{ t('intro.modDetail.scaffoldPanelTitle') }}</label>
                <input v-model="scaffoldPanelTitle" class="input" spellcheck="false" />
              </div>
              <div class="full">
                <label class="label">{{ t('intro.modDetail.scaffoldPanelSummary') }}</label>
                <input v-model="scaffoldPanelSummary" class="input" spellcheck="false" />
              </div>
              <div class="full">
                <label class="label-inline">
                  <input v-model="scaffoldForceMerge" type="checkbox" />
                  {{ t('intro.modDetail.scaffoldForceMerge') }}
                </label>
              </div>
              <div class="full">
                <button
                  ...
                  @click="submitWorkflowEmployeeScaffold"
                >
                  {{ scaffoldBusy ? t('intro.modDetail.scaffoldBusy') : t('intro.modDetail.scaffoldSubmit') }}
                </button>
              </div>
            </div>
            <pre v-if="scaffoldMergeHint" class="scaffold-merge-hint mono small">{{ scaffoldMergeHint }}</pre>
          </div>
          <div class="scaffold-block sandbox-run-block">
            <h3 class="sub-card-title">{{ t('intro.modDetail.sandboxTitle') }}</h3>
            <p class="hint muted small">{{ t('intro.modDetail.sandboxHint') }}</p>
            <label class="label-inline" style="margin-bottom: 0.5rem">
              <input v-model="sandboxProbeHttp" type="checkbox" />
              {{ t('intro.modDetail.sandboxProbeHttp') }}
            </label>
            <div>
              <button
                type="button"
                ...
                @click="runEmployeeSandbox"
              >
                {{ sandboxBusy ? t('intro.modDetail.sandboxBusy') : t('intro.modDetail.sandboxRun') }}
              </button>
            </div>
            <pre v-if="sandboxResultText" class="scaffold-merge-hint mono small">{{ sandboxResultText }}</pre>
          </div>
        </section>
        ...
        <section class="card">
          ...
          <div class="wallet-portal-row">
            <button type="button" class="btn" @click="openPortalPlans">{{ t('intro.modDetail.portalOpenPlans') }}</button>
            <button type="button" class="btn" @click="pasteWalletFromClipboard">{{ t('intro.modDetail.portalPasteSecret') }}</button>
          </div>
          <p v-if="portalSyncConfigured" class="muted small portal-sync-hint">{{ t('intro.modDetail.portalOverviewSyncHint') }}</p>
        </section>
      </div>
      ...
      <div v-show="tab === 'edit'" class="panel">
        ...
        <div class="grid2">
          ...
          <div class="full token-block">
            ...
            <div v-if="portalSyncConfigured" class="portal-pull-block">
              <label class="label">{{ t('intro.modDetail.portalBearerLabel') }}</label>
              <input
                v-model="portalPullBearer"
                type="password"
                class="input"
                autocomplete="off"
                spellcheck="false"
              ...
              <button
                ...
                :disabled="portalPullBusy"
                @click="pullWalletFromPortal"
              >
                {{ portalPullBusy ? t('intro.modDetail.portalPullBusy') : t('intro.modDetail.portalPull') }}
              </button>
              <p class="muted small">{{ t('intro.modDetail.portalPullSecurityHint') }}</p>
            </div>
          </div>
        </div>
...
const scaffoldEmpId = ref('')
const scaffoldLabel = ref('')
const scaffoldPanelTitle = ref('')
const scaffoldPanelSummary = ref('')
const scaffoldForceMerge = ref(false)
const scaffoldBusy = ref(false)
const scaffoldMergeHint = ref('')

const sandboxProbeHttp = ref(false)
const sandboxBusy = ref(false)
const sandboxResultText = ref('')

/** 代理拉取 FHD GET /api/fhd/db-tokens/status 的返回体（或 null） */
const fhdHostTokens = ref(null)
const fhdHostErr = ref('')

/** 修茈门户：充值页（解析后 URL）、是否已配置 HTTPS 密钥同步接口 */
const portalPlansUrl = ref('')
const portalSyncConfigured = ref(false)
const portalPullBearer = ref('')
const portalPullBusy = ref(false)
...
async function runEmployeeSandbox() {
  sandboxBusy.value = true
  sandboxResultText.value = ''
  err.value = ''
  msg.value = ''
  try {
    const res = await api.employeeSandboxRun(id.value, { probe_http: !!sandboxProbeHttp.value })
    sandboxResultText.value = JSON.stringify(res, null, 2)
    if (res && res.ok) {
      msg.value = t('intro.modDetail.sandboxOk')
    } else {
      err.value = t('intro.modDetail.sandboxFail')
    }
  } catch (e) {
...
async function submitWorkflowEmployeeScaffold() {
  scaffoldBusy.value = true
  scaffoldMergeHint.value = ''
  err.value = ''
  msg.value = ''
  const idPart = scaffoldEmpId.value.trim()
  const labelPart = scaffoldLabel.value.trim()
  if (!idPart || !labelPart) {
    err.value = t('intro.modDetail.scaffoldNeedIdLabel')
    scaffoldBusy.value = false
    return
  }
  try {
    const res = await api.scaffoldWorkflowEmployee(id.value, {
      id: idPart,
      label: labelPart,
      panel_title: scaffoldPanelTitle.value.trim(),
      panel_summary: scaffoldPanelSummary.value.trim(),
      template: 'skeleton_router',
      force_auto_merge: !!scaffoldForceMerge.value,
    })
    scaffoldMergeHint.value = (res && res.merge_hint) || ''
    const warns = res && Array.isArray(res.manifest_warnings) ? res.manifest_warnings : []
    msg.value =
      warns.length > 0
        ? t('intro.modDetail.scaffoldOkWithWarnings', { w: warns[0] })
        : t('intro.modDetail.scaffoldOk')
    scaffoldEmpId.value = ''
    scaffoldLabel.value = ''
    scaffoldPanelTitle.value = ''
    scaffoldPanelSummary.value = ''
    scaffoldForceMerge.value = false
    await load()
  } catch (e) {
    err.value = e.message || String(e)
  } finally {
...
async function loadPortalCfg() {
  try {
    const c = await api.getConfig()
    portalPlansUrl.value = (c.portal_plans_url || '').trim()
    portalSyncConfigured.value = !!(c.saved_portal_wallet_sync_url && String(c.saved_portal_wallet_sync_url).trim())
  } catch {
    portalPlansUrl.value = ''
    portalSyncConfigured.value = false
  }
}

function openPortalPlans() {
  const u = (portalPlansUrl.value || '').trim()
  if (!u) return
...
async function pasteWalletFromClipboard() {
  ...
  try {
    if (!navigator.clipboard || typeof navigator.clipboard.readText !== 'function') {
      err.value = t('intro.modDetail.portalClipboardUnavailable')
      return
    }
    const text = await navigator.clipboard.readText()
    form.value.wallet_secret = text != null ? String(text).trim() : ''
    msg.value = t('intro.modDetail.portalClipboardOk')
  } catch {
    err.value = t('intro.modDetail.portalClipboardErr')
  }
}

async function pullWalletFromPortal() {
  err.value = ''
  msg.value = ''
  const auth = (portalPullBearer.value || '').trim()
  if (auth.length < 8) {
    err.value = t('intro.modDetail.portalBearerTooShort')
    return
  }
  portalPullBusy.value = true
  try {
    const res = await api.fetchPortalWalletSecret({ sync_url: '', authorization: auth })
    if (res && res.wallet_secret != null) {
      form.value.wallet_secret = String(res.wallet_secret).trim()
      portalPullBearer.value = ''
      msg.value = t('intro.modDetail.portalPullOk')
    }
  } catch (e) {
    err.value = e.message || String(e)
  } finally {
    portalPullBusy.value = false
  }
}
