<template>
  <div>
    ...
    <div class="panel">
      ...
      <label class="label" style="margin-top: 1rem">{{ t('intro.settings.portalPlansUrl') }}</label>
      <input
        v-model="portal_plans_url"
        class="input"
        :placeholder="t('intro.settings.portalPlansPlaceholder')"
      />
      <p class="muted small">{{ t('intro.settings.portalPlansHint') }}</p>

      <label class="label" style="margin-top: 1rem">{{ t('intro.settings.portalSyncUrl') }}</label>
      <input
        v-model="portal_wallet_sync_url"
        class="input"
        :placeholder="t('intro.settings.portalSyncPlaceholder')"
      />
      <p class="muted small">{{ t('intro.settings.portalSyncHint') }}</p>
      ...
      <div class="resolved" v-if="effective">
        ...
        <div><span class="muted">充值页（解析后）：</span> {{ effective.portal_plans_url || '—' }}</div>
        <div><span class="muted">密钥同步接口（已保存）：</span> {{ effective.saved_portal_wallet_sync_url || '（未配置）' }}</div>
      ...
      <input
        ...
        class="input"
        placeholder="默认：<FHD>/app/shell/fhd_shell_mods.json"
      />
...
const portal_plans_url = ref('')
const portal_wallet_sync_url = ref('')
...
async function reload() {
  ...
  portal_plans_url.value = c.saved_portal_plans_url || ''
  portal_wallet_sync_url.value = c.saved_portal_wallet_sync_url || ''
  effective.value = c
}
...
async function save() {
  ...
  try {
    await api.putConfig({
      ...
      portal_plans_url: portal_plans_url.value,
      portal_wallet_sync_url: portal_wallet_sync_url.value,
    })
    flash.value = '已保存'
    flashOk.value = true
    await reload()
