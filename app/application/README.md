...
shipment_router = APIRouter(prefix="/api/shipment", tags=["shipment"])

@shipment_router.post("/create")
async def create_shipment(payload: dict):
    unit_name = payload.get("unit_name")
    items = payload.get("items", [])
    app_service = get_shipment_application_service()
    result = app_service.create_shipment(unit_name=unit_name, items_data=items)
    return JSONResponse(content=result)
```
