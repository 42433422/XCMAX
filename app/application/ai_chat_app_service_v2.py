...
class AiChatAppServiceV2:
    """
    AiChatAppService V2 - 事件驱动版本
    """
    ...
    async def execute_command(self, command_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"[AiChatAppServiceV2] 命令已发布: {command_type} (event_id={event.metadata.event_id})")
            ...
            logger.exception(f"[AiChatAppServiceV2] 执行命令失败: {e}")
...
# 单例管理
_aichatappservice_v2_instance = None


def get_ai_chat_app_service_v2() -> AiChatAppServiceV2:
    """获取 AiChatAppServiceV2 单例"""
    global _aichatappservice_v2_instance
    if _aichatappservice_v2_instance is None:
        _aichatappservice_v2_instance = AiChatAppServiceV2()
    return _aichatappservice_v2_instance
