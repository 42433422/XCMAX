"""
conversation_app_service V2 - 事件驱动版本
...
class ConversationAppServiceV2:
    """
    ConversationAppService V2 - 事件驱动版本
    """
    
    def __init__(self):
        self._bus = get_neuro_bus()
        self._correlation_prefix = "conversation"
    ...
    async def execute_command(self, command_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 构建事件类型
            event_type = f"conversation.{command_type}"
            ...
            event = NeuroEvent(
                ...
                payload=payload,
                source="conversationappservice_v2",
            ...
            logger.info(f"[ConversationAppServiceV2] 命令已发布: {command_type} (event_id={event.metadata.event_id})")
...
instrument_application_service_class(ConversationAppServiceV2, service_name="ConversationAppServiceV2")

# 单例管理
_conversationappservice_v2_instance = None


def get_conversation_app_service_v2() -> ConversationAppServiceV2:
    """获取 ConversationAppServiceV2 单例"""
    global _conversationappservice_v2_instance
    if _conversationappservice_v2_instance is None:
        _conversationappservice_v2_instance = ConversationAppServiceV2()
    return _conversationappservice_v2_instance
