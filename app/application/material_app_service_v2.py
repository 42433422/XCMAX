"""
material_app_service V2 - 事件驱动版本
...
class MaterialAppServiceV2:
    """
    MaterialAppService V2 - 事件驱动版本
    """
    
    def __init__(self):
        self._bus = get_neuro_bus()
        self._correlation_prefix = "material"
    ...
    async def execute_command(self, command_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 构建事件类型
            event_type = f"material.{command_type}"
            ...
            logger.info(f"[MaterialAppServiceV2] 命令已发布: {command_type} (event_id={event.metadata.event_id})")
            ...
            logger.exception(f"[MaterialAppServiceV2] 执行命令失败: {e}")
...
# 单例管理
_materialappservice_v2_instance = None


def get_material_app_service_v2() -> MaterialAppServiceV2:
    """获取 MaterialAppServiceV2 单例"""
    global _materialappservice_v2_instance
    if _materialappservice_v2_instance is None:
        _materialappservice_v2_instance = MaterialAppServiceV2()
    return _materialappservice_v2_instance
