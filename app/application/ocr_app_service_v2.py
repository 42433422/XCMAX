...
class OcrAppServiceV2:
    """
    OcrAppService V2 - 事件驱动版本
    """
    
    def __init__(self):
        self._bus = get_neuro_bus()
        self._correlation_prefix = "ocr"
    ...
    async def execute_command(self, command_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"[OcrAppServiceV2] 命令已发布: {command_type} (event_id={event.metadata.event_id})")
            ...
            logger.exception(f"[OcrAppServiceV2] 执行命令失败: {e}")
...
# 单例管理
_ocrappservice_v2_instance = None


def get_ocr_app_service_v2() -> OcrAppServiceV2:
    """获取 OcrAppServiceV2 单例"""
    global _ocrappservice_v2_instance
    if _ocrappservice_v2_instance is None:
        _ocrappservice_v2_instance = OcrAppServiceV2()
    return _ocrappservice_v2_instance
