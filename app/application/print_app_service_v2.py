...
class PrintAppServiceV2:
    """
    PrintAppService V2 - 事件驱动版本
    """
    ...
    async def execute_command(self, command_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"[PrintAppServiceV2] 命令已发布: {command_type} (event_id={event.metadata.event_id})")
...
# 单例管理
_printappservice_v2_instance = None


def get_print_app_service_v2() -> PrintAppServiceV2:
    """获取 PrintAppServiceV2 单例"""
    global _printappservice_v2_instance
    if _printappservice_v2_instance is None:
        _printappservice_v2_instance = PrintAppServiceV2()
    return _printappservice_v2_instance
