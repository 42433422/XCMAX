"""
template_app_service V2 - 事件驱动版本
...
class TemplateAppServiceV2:
    """
    TemplateAppService V2 - 事件驱动版本
    """
    
    def __init__(self):
        self._bus = get_neuro_bus()
        self._correlation_prefix = "print"
    ...
    async def execute_command(self, command_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 构建事件类型
            event_type = f"print.{command_type}"
            ...
            logger.info(f"[TemplateAppServiceV2] 命令已发布: {command_type} (event_id={event.metadata.event_id})")
            ...
            logger.exception(f"[TemplateAppServiceV2] 执行命令失败: {e}")
...
# 单例管理
_templateappservice_v2_instance = None


def get_template_app_service_v2() -> TemplateAppServiceV2:
    """获取 TemplateAppServiceV2 单例"""
    global _templateappservice_v2_instance
    if _templateappservice_v2_instance is None:
        _templateappservice_v2_instance = TemplateAppServiceV2()
    return _templateappservice_v2_instance
