"""
user_preference_app_service V2 - 事件驱动版本
...
class UserPreferenceAppServiceV2:
    """
    UserPreferenceAppService V2 - 事件驱动版本
    """
    ...
    async def execute_command(self, command_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            event = NeuroEvent(
                ...
                payload=payload,
                source="userpreferenceappservice_v2",
            ...
            logger.info(f"[UserPreferenceAppServiceV2] 命令已发布: {command_type} (event_id={event.metadata.event_id})")
            ...
            logger.exception(f"[UserPreferenceAppServiceV2] 执行命令失败: {e}")
...
instrument_application_service_class(UserPreferenceAppServiceV2, service_name="UserPreferenceAppServiceV2")

# 单例管理
_userpreferenceappservice_v2_instance = None


def get_user_preference_app_service_v2() -> UserPreferenceAppServiceV2:
    """获取 UserPreferenceAppServiceV2 单例"""
    global _userpreferenceappservice_v2_instance
    if _userpreferenceappservice_v2_instance is None:
        _userpreferenceappservice_v2_instance = UserPreferenceAppServiceV2()
    return _userpreferenceappservice_v2_instance
