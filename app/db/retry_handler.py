...
def is_database_locked_error(error: Exception) -> bool:
    ...
    return any(indicator in error_str for indicator in locked_indicators) or \
           any(indicator in error_type for indicator in locked_indicators)
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if attempt < max_attempts:
                        logger.warning(
                            "Database locked in %s (attempt %d/%d): %s. "
                            "Retrying in %.2fs...",
