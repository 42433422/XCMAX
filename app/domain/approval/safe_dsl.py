...
ALLOWED_VARIABLES = {
    'amount', 'quantity', 'unit_price', 'total_price',
    'priority', 'department', 'user_role', 'customer_type',
    'order_value', 'risk_level', 'approval_count',
    ...
    def _validate_ast(self, node: ast.AST) -> None:
        ...
        if isinstance(node, ast.Name):
            if node.id not in ALLOWED_VARIABLES and node.id not in self.context:
                raise SafeEvaluationError(f"Disallowed variable: {node.id}. "
                                        f"Allowed: {ALLOWED_VARIABLES}")
...
def should_trigger_condition(node, context: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        logger.warning(f"Condition evaluation failed for node {getattr(node, 'id', 'unknown')}: {e}")
