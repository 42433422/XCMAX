...
from app.neuro_bus.sla_controller import SLALevel, with_sla
...
class SubconsciousProcessor:
    ...
    def register_handler(
        ...
        batch_handler: Optional[Callable[[List[NeuroEvent]], Any]] = None,
    ):
        """
        注册处理器
    ...
    async def _flush_batch(self, event_type: str):
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(handler):
                await asyncio.wait_for(
                    handler(events),
                    timeout=self.SLA_MAX_MS / 1000.0
                )
            else:
                loop = asyncio.get_running_loop()
                await asyncio.wait_for(
                    loop.run_in_executor(None, handler, events),
                    timeout=self.SLA_MAX_MS / 1000.0
                )
...
# 单例
_subconscious: Optional[SubconsciousProcessor] = None
