...
from app.infrastructure.mods.mod_auth import ModContextMiddleware, get_mod_context
...
def resolve_cors_allow_origins() -> list[str]:
    """
    浏览器在 fetch(..., credentials: 'include') 时，服务端不能使用 Access-Control-Allow-Origin: *，
    必须与请求 Origin 精确匹配。allow_credentials=True 时须配置显式源列表。
    """
...
def _lan_origin_regex_enabled() -> bool:
    ...
    if raw in {"0", "false", "no", "off"}:
        return False
    # 未显式配置：与 FLASK_DEBUG 默认 1 对齐，避免「没用启动器」时局域网全被 CORS 拦死
    return (os.environ.get("FLASK_DEBUG", "1").strip() == "1")


def resolve_cors_allow_origin_regex() -> str | None:
    """
    与 allow_origins 并列；用于开发机局域网 IP 访问 Vite（如 http://192.168.1.2:5001）
    且带 credentials 的 API 预检。生产请设 FLASK_DEBUG=0，并显式配置 CORS_ALLOW_ORIGINS。
    """
...
def _sqlite_db_file_from_url(database_url: Optional[str]) -> Optional[str]:
...
async def _init_neuro_ddd_async(app: FastAPI):
    ...
    if raw in {"0", "false", "off", "no"}:
        logger.info("神经总线未启用（XCAGI_NEURO_INTENT=%s）", os.environ.get("XCAGI_NEURO_INTENT", ""))
        return
    try:
...
def create_fastapi_app(
    ...
    if enable_cors:
        ...
        if _lan_origin_regex_enabled() and "allow_private_network" in inspect.signature(CORSMiddleware).parameters:
    ...
    logging.basicConfig(
        ...
        format=getattr(config_object, "LOG_FORMAT", "%(asctime)s - %(name)s - %(levelname)s - %(message)s")
    )
    ...
    try:
        from app.infrastructure.mods.mod_manager import get_mod_manager, is_mods_disabled, load_mod_routes
