...
async def auth_login(request: Request, body: dict = Body(default_factory=dict)):
    from app.application.auth_app_service import get_auth_app_service
    from app.fastapi_routes.market_account import login_market_with_password, save_session_market_token
    from app.mod_sdk.product_skus import resolve_product_sku
...
def auth_profile_patch(request: Request, body: dict = Body(default_factory=dict)):
    """更新当前用户展示名与邮箱。"""
    user, err = _require_login_user(request)
    if err:
        return err
...
def auth_profile_avatar_get(request: Request):
...
async def auth_update_company_brand(request: Request, body: dict = Body(default_factory=dict)):
    ...
    from app.application.session_account_meta import load_session_account_meta, persist_session_account_meta
...
def auth_refresh(body: dict = Body(default_factory=dict)):
    """刷新 **小程序** JWT（``mp_auth``）。FHD Web Cookie 会话请用 ``GET /api/market/session-handoff`` 或重新登录。"""
    from app.decorators.mp_auth import refresh_access_token

    refresh_token = (body.get("refresh_token") or "").strip()
    if not refresh_token:
        return JSONResponse(
            {"success": False, "error": {"code": "MISSING_TOKEN", "message": "refresh_token 不能为空"}},
            status_code=400,
        )
    result = refresh_access_token(refresh_token)
    if result is None:
        return JSONResponse(
            {"success": False, "error": {"code": "INVALID_TOKEN", "message": "refresh_token 无效或已过期"}},
            status_code=401,
        )
