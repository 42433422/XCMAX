...
#
# 会话桶按 (user_id, mod_id) 二元组分桶：不同扩展（Mod）之间的历史对话彼此隔离；
# 未选中 Mod（"原版模式"）使用空字符串作为 mod_id，与历史行为保持一致。
# mod_id 来源：
#   1) 中间件已设置的 ``X-XCAGI-Active-Mod-Id`` 请求头 → ContextVar
#   2) 兼容：请求 body/query 中显式传入的 ``mod_id`` 字段
# ---------------------------------------------------------------------------
_XCAGI_CONVERSATION_MAX_SESSIONS = 200
_conversation_lock = threading.Lock()
# (user_id, mod_id) -> session_id -> {"messages": list[dict], "created_ts": float, "updated_ts": float}
...
def _xcagi_resolve_session_scope(
    ...
    """返回 (user_id, mod_id) 二元组。

    - ``user_id`` 空时降级为 ``"default"``，与老行为一致；
    - ``mod_id`` 先看请求 body/query 的显式字段，再回落到当前请求上下文中的激活扩展；
      两者都不可用时返回空字符串（表示"原版模式 / 未选择扩展"）。
    """
...
# 聊天「一级数据库查看令牌」短期授权：同一请求主体在验证后 5 分钟内可复用。
...
def fhd_db_write_token_verify(body: dict = Body(default_factory=dict)) -> dict:
    """与 XCAGI ``POST /api/fhd/db-write-token/verify`` 一致：仅挂载本 Router 的部署也可用。"""
...
def fhd_db_read_token_verify(request: Request, body: dict = Body(default_factory=dict)) -> dict:
    """一级读令牌校验成功后，给当前请求主体发放 5 分钟聊天只读授权窗口。"""
    expected = effective_db_read_token()
    if not expected:
...
# 这三个 industry compat stub 只在 system_routes 注册失败时充当兜底；
# 正式业务路由在 ``app/fastapi_routes/system_routes.py`` 中已提供完整实现并进入 OpenAPI。
# 这里保留调用点但显式从 OpenAPI 隐藏，避免与业务路由产生 Duplicate Operation ID。
@router.get("/system/industries", include_in_schema=False)
def system_industries() -> dict:
    return {"success": True, "data": [_DEFAULT_INDUSTRY]}


@router.get("/system/industry", include_in_schema=False)
def system_industry_get() -> dict:
    return {"success": True, "data": _DEFAULT_INDUSTRY}


@router.post("/system/industry", include_in_schema=False)
def system_industry_post(body: dict = Body(default_factory=dict)) -> dict:
...
def _load_products_list_impl_pg(
    ...
    try:
        ...
        # COUNT(*) 在大表上可能非常慢；超时则回退为「当前页可见量 + 偏移」近似值，避免阻塞接口。
        total: int | None = None
        count_sql = f"SELECT COUNT(*) FROM products{where_sql}"
        with eng.connect() as conn:
            try:
                timeout_ms = int((os.environ.get("FHD_PRODUCTS_COUNT_TIMEOUT_MS") or "1500").strip())
            except Exception:
                timeout_ms = 1500
            try:
                if timeout_ms > 0:
        ...
        # 默认使用 PK 顺序，避免 created_at 缺索引时发生全表排序阻塞。
        prefer_created_at = (os.environ.get("FHD_PRODUCTS_ORDER_BY_CREATED_AT") or "").strip() == "1"
        order = (
        ...
        with eng.connect() as conn:
            try:
                query_timeout_ms = int((os.environ.get("FHD_PRODUCTS_QUERY_TIMEOUT_MS") or "8000").strip())
            except Exception:
...
def _customers_write_raise(request: Request) -> None:
    """二级写入令牌（与产品 CUD 一致）+ 校验 ``purchase_units`` 表存在。"""
...
def customers_all(
    ...
    携带 ``Authorization: Bearer`` 且为有效小程序 JWT 时，返回归档小程序分页列表（``code/message/data``）；
    无效 Bearer → 401；无 Bearer → 桌面全量 ``{success,data}``。
    """
    from app.fastapi_routes.miniprogram import (
        build_mp_customers_list_response,
        classify_miniprogram_bearer,
        _mp_json,
    )

    kind, _pl = classify_miniprogram_bearer(request.headers.get("Authorization"))
    if kind == "ok":
...
def customers_match(
    ...
    """
    from app.infrastructure.products.customer_matching import extract_customer_name, find_matching_customer
...
@router.put("/customers/{customer_id}/")
def customers_update(request: Request, customer_id: int, body: dict = Body(default_factory=dict)) -> dict:
    _customers_write_raise(request)
    name, cp, ph, addr = _customer_body_name_contact(body)
    if not name:
        raise HTTPException(status_code=400, detail="客户名称不能为空")
...
async def customers_import(request: Request, file: UploadFile = File(...)) -> dict:
    ...
    from app.application.excel_imports import run_customers_excel_import_bytes  # type: ignore[attr-defined]
...
def products_get_by_id(request: Request, product_id: int) -> dict | JSONResponse:
    """
    与归档 ``products`` 蓝图 ``GET /api/products/<product_id>`` 一致（Flask 中先于小程序视图匹配）。

    无 Bearer 或桌面：``{ success, data }``（须 ``verify_db_read_token_header``）；\
    有效小程序 JWT：``code/message/data``；无效 Bearer → 401。
    """
    from app.fastapi_routes.miniprogram import (
...
def products_resolve_name_hints(request: Request, body: dict = Body(default_factory=dict)) -> dict:
    ...
    # products/resolve-name-hints 历史上依赖 ``app.legacy.product_name_resolve``,
    # 该模块在 2026-04 backend → app.legacy 清理时已丢失。保留一个明确 501,
    # 避免命中该端点时让 xcagi_compat 查找已不存在的 legacy 模块。
    raise HTTPException(
        status_code=501,
        detail=(
...
def products_update(request: Request, body: dict = Body(default_factory=dict)) -> dict:
    ...
    from app.application.excel_imports import _parse_price  # type: ignore[attr-defined]
...
def products_add(request: Request, body: dict = Body(default_factory=dict)) -> dict:
    ...
    from app.application.excel_imports import _norm_model, _parse_price  # type: ignore[attr-defined]
...
def _extract_single_sheet_bundle(
    ...
    # 与 read_rows 上限一致：报价单等多行抬头时，仅前 20 行往往只有抬头/表头，数据行被裁掉会导致
    # 侧栏网格预览与模型上下文缺失关键行；网格构建已与 matrix 同步裁剪，体量可控。
...
def _xcagi_compat_reply_payload(
    ...
    *,
    ...
    last_result: dict = {}
    try:
        from app.application.workflow.legacy_chat_adapter import get_last_tool_result  # type: ignore[attr-defined]

        raw = get_last_tool_result()
        if isinstance(raw, dict) and raw:
            last_result = raw
            from app.application.tools import flatten_tool_result_dict_for_client  # type: ignore[attr-defined]
...
def _thinking_steps_from_planner_stream_text(merged: str) -> str | None:
    """流式 Planner 将工具进度以 ``[正在调用工具: …]`` 等形式写入正文；收束为 thinking_steps 供前端折叠展示。"""
...
def _xcagi_planner_stream_bytes(request: Request, body: XcagiCompatChatBody, *, ai_tier: str):
    ...
    if not ok_read and read_req:
        yield _sse_event_line({"type": "token", "text": f"[需要授权: {read_req.get('token_description')}]"})
...
def ai_unified_chat_stream(request: Request, body: XcagiCompatChatBody):
    assert_p2_elevated_claim_or_raise(request)
    tier = resolve_ai_tier(request)
    return StreamingResponse(
        _xcagi_planner_stream_bytes(request, body, ai_tier=tier),
...
async def _xcagi_execute_chat(request: Request, body: XcagiCompatChatBody) -> dict:
    ...
    try:
        ...
        _kitten_reply_extra = kitten_reply_attachments(runtime_context)
    except Exception:
    ...
    if intr is not None:
        ...
        return _xcagi_compat_reply_payload(
            intr, runtime_context_update=cleared, kitten_attachments=_kitten_reply_extra or None
        )

    vector_error = _ensure_vector_index_if_needed(body.message, runtime_context)
    if vector_error:
        return _xcagi_compat_reply_payload(vector_error, kitten_attachments=_kitten_reply_extra or None)
    ...
    return _xcagi_compat_reply_payload(reply, kitten_attachments=_kitten_reply_extra or None)


@router.post("/conversations/message")
@publish_route_event(RouteEvents.CONVERSATION_MESSAGE, domain="conversation", include_payload=False)
...
@publish_route_event(RouteEvents.CHAT_REQUEST, domain="ai", include_payload=False)
...
@publish_route_event(RouteEvents.CHAT_BATCH, domain="ai", include_payload=False)
...
def conversations_get(
    ...
    with _conversation_lock:
        ...
        # 兼容旧会话：若显式 mod 桶中不存在，且未显式指定 mod，则回落尝试同 user 的其它 mod 桶。
...
@publish_route_event(RouteEvents.TOOL_EXECUTE, domain="tools")
def compat_tools_list() -> dict:
    return get_tools_payload()


@router.get("/db-tools", summary="工具表目录别名（前端优先请求）")
@router.get("/db-tools/", summary="工具表目录别名（尾斜杠）")
def compat_db_tools_list() -> dict:
    return get_tools_payload()
...
# Neuro-DDD: 路由层 instrumentation
# 标记此路由模块已纳入 Neuro-DDD 架构
