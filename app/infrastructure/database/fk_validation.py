...
class ForeignKeyValidator:
    ...
    def __init__(self):
        self._valid_unit_ids: Optional[Set[int]] = None
        self._cache_ttl = 60  # 缓存60秒
    
    def validate_purchase_unit_exists(self, unit_id: Optional[int]) -> bool:
        """
        ...
        try:
            with SessionLocal() as db:
                ...
                if not exists:
                    logger.warning(
                        "Foreign key violation: purchase_units.id=%s does not exist",
                        unit_id
                    )
                return exists
    ...
    def validate_product_exists(self, product_id: Optional[int]) -> bool:
        """验证 product ID 是否存在"""
        ...
        try:
            with SessionLocal() as db:
                ...
                if not exists:
                    logger.warning(
                        "Foreign key violation: products.id=%s does not exist",
                        product_id
                    )
                return exists
        except Exception as e:
    ...
    def validate_customer_exists(self, customer_id: Optional[int]) -> bool:
        """验证 customer ID 是否存在"""
        ...
        try:
            with SessionLocal() as db:
                ...
                if not exists:
                    logger.warning(
                        "Foreign key violation: customers.id=%s does not exist",
                        customer_id
                    )
                return exists
        except Exception as e:
...
# 全局验证器实例
_fk_validator: Optional[ForeignKeyValidator] = None
...
def validate_shipment_unit_id(unit_id: Optional[int]) -> bool:
    """
    验证出货记录的单位ID是否有效
