    ...
    def _create_price_list_pdf(
        ...
        try:
            ...
            try:
                ...
                from app.infrastructure.documents.price_list_export import build_price_list_docx_bytes
                ...
                logger.warning(f"[PriceListGenerator] 导入模板生成失败：{e}，使用 python-docx 直接生成")
                ...
                logger.info(f"[PriceListGenerator] 使用 python-docx 生成 Word 成功")
                ...
                with open(txt_filepath, 'w', encoding='utf-8') as f:
                    ...
                    f.write(f"{'序号':<6}{'型号':<15}{'产品名称':<30}{'规格':<15}{'单位':<8}{'单价 (元)':<10}\n")
                    f.write("-" * 84 + "\n")
        ...
        try:
            if os.name == 'nt':  # Windows
                ...
                if printer_name not in printers:
                    logger.warning(f"[PriceListGenerator] 打印机 {printer_name} 不存在，使用默认打印机")
                ...
                if file_ext == '.docx' or file_ext == '.doc':
                    try:
                        ...
                        logger.warning(f"[PriceListGenerator] Word COM 打印失败：{e}，使用 Shell 打印")
