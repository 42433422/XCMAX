...
class SimpleSalesContractGenerator:
    ...
    def generate(
        ...
        return_expected_str = f"({return_buckets_expected})" if return_buckets_expected is not None else "(  )"
        return_actual_str = f"({return_buckets_actual})" if return_buckets_actual is not None else "(  )"
        note2_para.add_run(f"注：160KG桶需退回，如未退回一个按80元计算 应退桶{return_expected_str}个，实退桶{return_actual_str}个.").font.size = Pt(10)
        ...
        sig_para.add_run("核準：黄种霜    會計：胡小玲    經辨：姚胜华    倉庫：廖振卷").font.size = Pt(10)
