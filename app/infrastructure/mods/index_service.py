...
class ModIndexDatabase:
    ...
    def get_mod(self, mod_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM mod_metadata WHERE id = ?",
                (mod_id,)
            )
            row = cursor.fetchone()
    ...
    def update_install_status(self, mod_id: str, is_installed: bool):
        ...
        with self.get_connection() as conn:
            conn.execute(
                ...
                (1 if is_installed else 0, datetime.utcnow().isoformat(), mod_id)
            )
        ...
        with self.get_connection() as conn:
            conn.execute("""
                ...
                SET download_count = download_count + 1, 
                    updated_at = ?
                WHERE id = ?
            """, (datetime.utcnow().isoformat(), mod_id))
    ...
    def get_ratings(self, mod_id: str) -> List[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM mod_ratings WHERE mod_id = ? ORDER BY created_at DESC",
                (mod_id,)
            )
            return [dict(row) for row in cursor.fetchall()]
    
    def get_statistics(self, mod_id: str) -> Optional[Dict[str, Any]]:
        """获取 MOD 统计信息"""
        with self.get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM mod_statistics WHERE mod_id = ?",
                (mod_id,)
            )
            row = cursor.fetchone()
    ...
    def _recalculate_statistics(self, conn: sqlite3.Connection, mod_id: str):
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (mod_id, rating_count, avg_rating, datetime.utcnow().isoformat()))
...
class ModIndexService:
    ...
    def rebuild_index(self, store_dir: Optional[str] = None) -> Tuple[int, int]:
        """
        重建 MOD 索引
