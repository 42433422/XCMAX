...
from app.neuro_bus.events.base import NeuroEvent, EventPriority, EventHandler, AsyncEventHandler
...
class PriorityEventQueue:
    ...
    def get(self) -> Optional[NeuroEvent]:
        """取出最高优先级事件"""
        with self._lock:
            if not self._queue:
                return None
    ...
    def peek(self) -> Optional[NeuroEvent]:
        """查看最高优先级事件（不取出）"""
        with self._lock:
            if not self._queue:
                return None
...
class NeuroBus:
    ...
    def __init__(
        ...
        self._domain_handlers: Dict[str, Dict[str, List[HandlerSubscription]]] = defaultdict(lambda: defaultdict(list))
        self._global_handlers: List[HandlerSubscription] = []
        
        # 执行器
        self._executor = ThreadPoolExecutor(max_workers=worker_threads, thread_name_prefix="neurobus_")
        ...
        # 事件持久化（可选）
        self._persistent_events: List[Dict[str, Any]] = []
        self._enable_persistence = False

        # 可选可靠性层（默认关闭；见 README NeuroBus 机制表）
    ...
    def publish(self, event: NeuroEvent) -> bool:
        ...
        if self._enable_persistence:
            self._persistent_events.append(event.to_dict())

        span_id = None
        if self._rel_tracer is not None:
...
# 全局 NeuroBus 实例
_neuro_bus: Optional[NeuroBus] = None
