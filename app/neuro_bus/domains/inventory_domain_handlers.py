...
from app.neuro_bus.events.inventory_events import (
    InventoryStockInEvent, InventoryStockOutEvent, InventoryTransferEvent, InventoryCheckCompletedEvent
)
...
class InventoryServiceDomainHandlers:
    ...
    async def handle_stock_in(self, event: NeuroEvent) -> Dict[str, Any]:
        ...
        if os.environ.get("XCAGI_NEURO_UOW_ON_INVENTORY", "").strip().lower() in {"1", "true", "yes", "on"}:
