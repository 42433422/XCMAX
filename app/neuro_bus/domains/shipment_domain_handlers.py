...
class ShipmentDomainHandlers:
    ...
    async def handle_item_added(self, event: ShipmentItemAddedEvent) -> Dict[str, Any]:
        ...
        3. 记录操作日志
        """
        logger.info(f"[ShipmentDomain] 处理添加产品: shipment={event.payload.get('shipment_id')} "
    ...
    async def handle_exported(self, event: ShipmentExportedEvent) -> Dict[str, Any]:
        ...
        3. 可以触发文件上传/清理
        """
        logger.info(f"[ShipmentDomain] 处理导出: {event.payload.get('file_path')} "
    ...
    async def handle_inventory_deducted(self, event: ShipmentInventoryDeductedEvent) -> Dict[str, Any]:
        """
        处理库存扣减事件
        ...
        3. 检查库存预警
        """
        logger.info(f"[ShipmentDomain] 处理库存扣减: {event.payload.get('shipment_id')} "
