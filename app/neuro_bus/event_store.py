...
class StoredEvent:
    ...
    stored_at: datetime
    sequence_number: int
    stream_id: Optional[str] = None  # 事件流 ID（用于聚合根）
...
class EventStore:
    ...
    def get(self, store_id: str) -> Optional[StoredEvent]:
        """获取单个事件"""
    ...
    def save_snapshot(
        ...
        logger.debug(
            f"[EventStore] 快照保存: {stream_id} "
            f"(seq={sequence_number}, snap_id={snapshot_id})"
        )
    ...
    def replay(
        ...
        stream_id: Optional[str] = None,
        callback: Optional[Callable[[NeuroEvent], None]] = None,
    ) -> int:
        """
        重播事件
        ...
        for stored in events:
            ...
            if callback:
                ...
                # 默认：只记录
                logger.info(f"[EventStore] 重播: {stored.event.event_type} (store_id={stored.store_id})")
                count += 1
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        by_stream = {
            stream_id: len(store_ids)
            for stream_id, store_ids in self._stream_events.items()
        }
...
def store_event(event: NeuroEvent, stream_id: Optional[str] = None) -> str:
    """快捷函数：存储事件"""
