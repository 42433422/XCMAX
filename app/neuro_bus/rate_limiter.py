...
class DynamicRateLimiter:
    ...
    def __init__(
        ...
        priority_whitelist: Optional[list] = None,
    ):
        self._default_config = default_config or RateLimitConfig()
        self._priority_whitelist = priority_whitelist or [EventPriority.CRITICAL, EventPriority.HIGH]
    ...
    def allow(self, event: NeuroEvent) -> bool:
        ...
        with self._lock:
            ...
            if global_count > global_config.burst_size:
                ...
                logger.warning(f"Global rate limit exceeded: {global_count}/{global_config.burst_size}")
                return False
            ...
            if domain:
                ...
                if domain_count > domain_config.burst_size:
                    self._rejected_count += 1
                    logger.warning(f"Domain rate limit exceeded [{domain}]: {domain_count}/{domain_config.burst_size}")
                    return False
            ...
            if event_count > event_config.burst_size:
                self._rejected_count += 1
                logger.warning(f"Event type rate limit exceeded [{event_type}]: {event_count}/{event_config.burst_size}")
                return False
...
class NeuroRateLimiter:
    ...
    def __init__(self):
        self._limiter = DynamicRateLimiter(
            default_config=self.DOMAIN_LIMITS["default"]
        )
