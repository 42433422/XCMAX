...
class AIConversationService:
    ...
    def _is_pro_source(self, source: Optional[str]) -> bool:
        """兼容多种前端 source 写法。"""
    ...
    def _convert_neuro_intent_bridge(self, nr: Any) -> Dict[str, Any]:
        """将 NeuroIntentResult 转为与 Hybrid / rule 一致的意图字典。"""
    ...
    async def _recognize_intent(
        ...
        if is_offline_mode:
            ...
            if self._neuro_stack_enabled():
                from app.neuro_bus.integrations.intent_integration import get_neuro_intent_recognizer
    ...
    async def _get_or_create_context_async(
        ...
        """与 _get_or_create_context 相同，并在小猫分析开启联网时异步检索后合并进 request_context。"""
        conv_context = self.get_context(user_id)
        if not conv_context:
            conv_context = self.create_context(user_id)
        enriched = self._enrich_context_with_kitten_business_snapshot(context)
    ...
    async def chat(
        ...
        try:
            ...
            out = await self._execute_or_generate_response(message, intent_result, conv_context, user_id)
    ...
    def _apply_request_context(
        ...
        if ctx.get("kitten_analyzer"):
            ...
            if "web_search_results" in ctx:
                merged["web_search_results"] = self._sanitize_web_search_results(ctx.get("web_search_results"))
            ...
            if isinstance(meta, dict) and meta:
                merged["web_search_meta"] = {k: str(v)[:200] for k, v in meta.items() if v is not None}
