# -*- coding: utf-8 -*-
"""
模板与 Excel 预览辅助（从归档 templates 迁移，无 Flask）。
"""
...
def create_template_with_payload(payload: dict | None):
    """
    创建模板（兼容前端 /api/templates/create）；无 Flask request 依赖。
    """
...
def update_template_with_payload(payload: dict | None):
    """
    更新模板（兼容前端 /api/templates/update）；无 Flask request 依赖。
    - 支持基础编辑（名称等）
    - 支持网格替换并校验业务范围一致
    """
...
def analyze_template_with_upload(file, template_name: str = "", template_scope: str = ""):
    """
    分析上传的文件（无 Flask request 依赖）。
    ``file`` 须为具备 ``filename`` / ``save(path)`` 的对象
    （由 ``app.routes.document_templates_compat._UploadLikeFile`` 承担）。
    """
