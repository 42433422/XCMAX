...
def dispatch_legacy_tool_payload(
    ...
    *,
    ...
    if tool_id == 'products':
        ...
        return _j({"success": True, "redirect": "/console?view=ai-ecosystem", "message": "已打开AI生态"})
        ...
        return _j({"success": True, "redirect": "/console?view=materials-list", "message": "已打开原材料列表"})
        ...
        return _j({"success": True, "redirect": "/console?view=business-docking", "message": "已打开业务对接"})
        ...
        if action == "export":
            ...
            return _j(result, 200)
        return _j({"success": True, "redirect": "/console?view=shipment-records", "message": "已打开出货记录"})
        ...
        if effective_action in ('search', 'query') and keyword and not has_add_verb and not has_del_verb:
            ...
            return _j({"success": True, "redirect": f"/console?view=customers&keyword={keyword}", "message": f"已按关键词检索客户：{keyword}"})
        ...
        if should_supplement:
            ...
            if not last_customer and not field_value:
                return _j({"success": False, "message": "请先告诉我要补充哪个客户的联系人信息，例如：添加客户七彩乐园"}, 400)
        ...
        if action == "set_default":
            from app.services import get_system_service
            return _j(get_system_service().set_default_printer(str(params.get("printer_name") or "").strip()), 200)
        return _j({"success": True, "redirect": "/console?view=printer-list", "message": "已打开打印机列表"})
        ...
        if action in ("list", "query"):
            return _j(svc.get_all_materials(
                search=str(params.get("search") or params.get("keyword") or "").strip(),
                category=str(params.get("category") or "").strip() or None,
                page=int(params.get("page") or 1),
                per_page=int(params.get("per_page") or 20),
        ...
        return _j({"success": True, "redirect": "/console?view=template-preview", "message": "已打开模板预览"})
        ...
        return _j({"success": True, "redirect": "/console?view=settings", "message": "已打开系统设置"})
        ...
        if action in ("list", "query"):
            return _j({"success": True, "tools": ["database", "ocr", "excel_toolkit", "excel_analyzer", "template_extract"]}, 200)
        return _j({"success": True, "redirect": "/console?view=other-tools", "message": "已打开其他工具"})
