...
def resolve_fhd_repo_root(anchor: Optional[Path] = None) -> Optional[Path]:
    """
    解析 FHD 单仓根目录（含 app/、backend/、mods/ 等）。

    从 anchor 目录向上查找第一个存在 app/fastapi_app.py 的目录。
    """
    ...
    for d in (start, *start.parents):
        marker = d / "app" / "fastapi_app.py"
        if marker.is_file():
            return d
    return None


def ensure_fhd_repo_on_syspath(anchor: Optional[Path] = None) -> Optional[Path]:
    """
