...
| 工作区体积（`du -sh Desktop/XCMAX`） | ≤ 8 GB（[`plan-2026-06.md`](../../specs/plan-2026-06.md) M0） | **9.7G**（2026-06-05 复扫；自 ~11G 去掉与归档重复的 `FHD/.venv` + `frontend/node_modules` 实体 ~0.8G，工作区仅 `ARCHIVE_POINTER`；可重建依赖 SSOT `~/XCMAX-archives/m0-venv-20260605/` ~1.6G；bulk 见 `m0-fhd-bulk-20260605`；**主占用** `FHD/.git` ~9.4G；不含 `.git` 交付树 ~**0.3G**） | 夸大 | 发布工程 | 仓根 `ARCHIVE_POINTER.md` · Top：`FHD/.git` > `XCAGI`/templates/resources > `frontend`（已无 719M node_modules） |
