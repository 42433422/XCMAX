...
> **状态（2026-06）**：**待启动** — 骨架与验收口径已定点；demo 未跑通前不得在 [`CLAIMED_VS_ACTUAL.md`](CLAIMED_VS_ACTUAL.md) 标「已验证」。  
...
| `scripts/ai_agent_v0/run_demo.py` | ⬜ 待实现 | CLI：message → plan → run → 写 JSON |
| `docs/evidence/ai-agent-v0/demo-run-YYYYMMDD.json` | ⬜ demo 后 | 一次完整运行的结构化输出 |
