...
受管账号切换时，旧合并回写进程配置 `MERGE_WORKER_PARA_CREDENTIALS_FILE`，指向仅所有者可读、包含 `email/password/owner_id` 的凭证文件。它通过正常登录更新会话，并核对返回的所有者 UUID；配置缺失、登录失败或身份不符均停止本轮处理，不回退 guest。没有配置该文件的旧环境保留原有 guest 行为。新控制任务仍默认禁止自动合并，这项兼容适配不改变现有合并审批规则。

Mac 原代理的旧合并申请配置同一只读凭证文件 `DEVFLEET_PARA_CREDENTIALS_FILE`，同样以受管身份登录。先安装主线回执辅助模块，再启用两个进程的凭证文件配置；未配置的旧运行进程仍能在迁移前使用原通道，自更新不会提前要求新模块。
