...
| 阶段 | 当前证据 | 仍需完成 |
|---|---|---|
| 0 基线 | 独立工作区、生产身份、Para 运行接口和远端源码已检查 | 运行端差异与可发布版本核对 |
...
运行目录 `/Users/a4243342/XCMAX-runtime/para-api/devfleet` 不含 Git 元数据。与私有主线比对 api/scripts/src/tests 范围，124 个已跟踪文件相同、12 个不同，另有 4 个运行端新增源码或测试文件，详见 `para-source-reconciliation.json` 的逐文件 SHA256。差异包括任务预检、失败分类、自动合并队列和 WebSocket 心跳处理，需要逐项保留或适配；尚未将运行端修改直接吸收为正式主线。

已准备隔离源码工作区 `/private/tmp/xcmax-para-control-20260908`，分支 `codex/xcmax-control-contract`，保留 private/public 双远端用于核对。现有 JWT 权限依赖 `device.user_id`，设备归属于 guest，未发现项目级服务身份委托接口；不能仅创建另一个用户令牌就接管这些设备。生产也未配置新适配所需的服务凭证。源码查找不再是阻塞项，后续是在已找到的源码中补齐接口并验证运行差异。
