...
旧 Para `POST /api/devices/me/task-report` 没有事件幂等标识和尝试校验。依赖 PR 的新 `/control-report` 以数据库事务去重并检查尝试和序号；旧接口保持兼容。取消仍需执行器真实停止确认，界面不会提前显示已取消。
...
本地验证：28 项 MODstore 链路及交付阶段测试、11 项前端相关测试、管理端构建、Node 回执/源码摘要测试及隔离数据库身份切换测试通过。Para 完整 105 项测试通过，Windows 安装包由专用 CI 构建；PR 构建仅为候选。
