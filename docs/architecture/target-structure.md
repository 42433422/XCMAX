...
- Keep business code testable and independent from the web framework (FastAPI/Starlette), ORM, and file-system details.
...
- `app/domain/`
  ...
  - Must not depend on FastAPI/Starlette/SQLAlchemy/OpenPyXL.
...
- `domain` importing from `routes`, `infrastructure`, FastAPI/Starlette, SQLAlchemy, or file IO SDKs.
