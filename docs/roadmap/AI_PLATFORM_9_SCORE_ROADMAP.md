...
## 3.1 AI 员工对齐并超过人类员工 P0-P3 方案

本节目标：把“AI 员工像人一样”从体验描述改成可验收的岗位系统。对齐人类员工，不是增加头像或话术，而是做到：有职责边界、能接任务、能判断优先级、能执行、能验证、能交接、能复盘、能对失败负责。超过人类员工，则体现在并行执行、全量记忆、可审计证据、低成本重复劳动、24 小时值守和持续评测。

当前基线按 2026-06-29 本地检查口径：FHD 已有员工包、员工工具注册、`EmployeeAgent.run()`、`scheduler.py`、`agent_loop.py`、`write_approval.py`、`tool_scope.py`、`local_duty_graph_health.py` 等真实链路；MODstore daily-loop 也有 launchd 常驻服务。但离可替代人类员工还差在运行态统一、异常恢复、任务交接、审计闭环和长期质量评测。

### 3.1.1 人类员工对齐标准

| 人类员工能力 | AI 员工必须对齐的系统能力 | 超过人类员工的目标 |
|---|---|---|
| 知道自己负责什么 | 每个员工有 owner scope、输入/输出、禁区、SLA、验收门 | 自动从代码变更、工单、日志中更新职责画像 |
| 会接任务和拒绝任务 | `employee.list` -> 选人 -> `employee.execute`，未知任务必须澄清或转派 | 秒级路由到多个候选员工并给出置信度 |
| 会干活 | 工具调用、文件处理、业务读写、外部 API、审批后副作用 | 同一任务可并行拆给多个员工，并自动合并结果 |
| 会验证 | 每次输出绑定测试、smoke、curl、截图、DB readback 或 CI 证据 | 自动生成最小回归样本，加入 eval/nightly |
| 会交接 | 任务状态、阻塞原因、下一步、证据链接进入 run events 和 handoff | 换模型/换设备/重启后能恢复同一 run |
| 会复盘 | 失败进入 observation/repair/history，不只返回错误文本 | 自动聚类失败、形成修复建议和员工训练样本 |
| 守规矩 | 风险门、workspace guard、write approval、钱包/权益门 | 越权、超预算、低置信字段自动阻断并解释 |

### 3.1.2 P0：可信执行底座

目标：从“有员工”变成“能证明每个关键员工真的能跑”。P0 不追求智能超人，只追求不假、不丢、不误报。

| 改造项 | 当前落点 | 任务 | 验收标准 | 工作量 | 风险 |
|---|---|---|---|---|---|
| 运行态单一真相 | `FHD/app/application/local_duty_graph_health.py`, `FHD/app/fastapi_routes/xcmax_admin.py`, MODstore `/api/health` | 建立 `/api/xcmax/employee-readiness`，汇总 FHD 监听、employee scheduler、MODstore daily-loop、DB、LLM、钱包、员工包 runtime | 一条命令输出 green/yellow/red；必须区分“主任务可用”和“审计/旁路失败” | 2-4 人日 | 端口、launchd、runtime checkout 容易漂移 |
| 员工合同必填 | `FHD/mods/_employees/*/manifest.json`, `FHD/frontend/src/domain/employeeConfigV2.ts` | 每个员工必须有职责、scope_globs、forbidden_globs、SLA、输入输出、验证方式、失败升级人 | 66 个员工包合同校验通过；缺合同不能进入自动调度 | 3-5 人日 | 历史 manifest 格式不统一 |
| 最小真实 smoke | `FHD/tests/application/test_employee_runtime.py`, `FHD/tests/application/test_employee_runtime_scheduler.py`, `FHD/evals/` | 为核心 20 个员工建立真实输入样本，不用纯 mock 判定成功 | `direct_python`、`agent`、`specialized` 三类各有真实 smoke；结果写入 report | 4-6 人日 | LLM/外部 API 不稳定，需要离线夹具和在线分层 |
| 失败不可伪成功 | `employee_runtime/executor.py`, `agent.py`, `metrics.py` | 主任务、审计落库、事件发布、计费分别标状态，禁止旁路失败被吞成全绿 | run final_output 同时展示 task_success、audit_success、billing_success、handoff_success | 3-5 人日 | 旧接口只看 `success` |
| 管理端手动点火 | `xcmax_admin.py`, frontend employee cards | 每个员工卡片支持“运行 smoke / 查看最后一次失败 / 查看证据 / 重新执行” | 管理端能证明员工是否可托付，不靠日志 grep | 5-8 人日 | UI 容易先做展示，后端证据不足 |

P0 出口门：

1. `make` 或等价脚本可一次跑出 employee readiness report。
2. FHD 员工 API、MODstore daily-loop、DB、LLM、钱包、员工包 runtime 状态必须分项展示。
3. 核心员工 smoke 通过率达到 95% 以上；失败必须有 owner、原因、下一步。
4. 任意员工输出必须能点开 run events、ToolCall、测试/读回证据。

### 3.1.3 P1：达到合格人类员工

目标：达到一个可靠初级/中级人类员工的工作标准：能接任务、能完成、会自检、会交接。

| 改造项 | 当前落点 | 任务 | 验收标准 | 工作量 | 风险 |
|---|---|---|---|---|---|
| 岗位责任制 | `duty_roster`, `employee_runtime/orchestrator.py`, `employee_tool_registry.py` | 把员工绑定到代码路径、业务对象、工具权限和可接任务类型 | 任务路由先看责任人，再看能力，不再只靠 prompt | 5-8 人日 | 职责过细会增加维护成本 |
| Inbox/Queue | `AgentRun`, `scheduler.py`, MODstore `employee_executor.py` | 每个员工有 inbox、priority、deadline、blocked、waiting_review 状态 | 可查询“谁在干什么、卡在哪、多久没动” | 8-12 人日 | 多运行时状态一致性 |
| 自检清单 | `AgentRun` events, `tools_workflow_registered.py`, eval harness | 每类员工输出前必须跑岗位自检：测试、lint、readback、引用、截图或确认 | 没有 evidence 的任务不能标 completed | 6-10 人日 | 自检成本和速度 |
| 交接报告 | `employee_handoff.py`, `metrics.py`, frontend task panel | 自动生成“已做/未做/风险/证据/下一步/需要人决策” | 人类接手时不用翻日志 | 4-6 人日 | 报告可能冗长，需要结构化 |
| 审批与升级 | `write_approval.py`, `approval_gated_engine.py`, `employee_runtime/triggers.py` | 高风险写操作、低置信结果、连续失败自动升级给人 | 危险动作不会静默执行；升级记录可追踪 | 4-8 人日 | 过度审批会降低自动化价值 |

P1 出口门：

1. 50 条真实业务任务中，AI 员工独立完成率不低于 80%。
2. 人类只看交接报告即可复核任务，平均复核时间减少 50%。
3. 高风险任务 100% 进入审批或明确拒绝。
4. 连续失败不会无限重试，必须进入 blocked/escalated。

### 3.1.4 P2：超过单个人类员工

目标：利用 AI 的优势超过单个人类员工：并行、记忆、全天候、低成本重复执行、可量化学习。

| 改造项 | 当前落点 | 任务 | 验收标准 | 工作量 | 风险 |
|---|---|---|---|---|---|
| 多员工并行 | `employee_runtime/orchestrator.py`, `AgentRun` DAG | 一个任务可拆给多个员工并行执行，支持 join/merge/critic | 同一需求能并行分析、实现、测试、复核 | 10-15 人日 | 合并冲突和责任归属 |
| 记忆驱动执行 | `employee_runtime/memory.py`, `Memory v2`, Dataset/RAG | 员工自动引用历史 run、客户偏好、代码约束、失败模式 | 每次引用记忆都带 source/confidence；误用进入 eval | 8-12 人日 | 记忆污染 |
| 自动修复闭环 | `AgentOrchestrator` repair, MODstore `auto_fix_loop.py` | 失败后先分类，再尝试 deterministic repair、LLM repair、转派或升级 | 可恢复失败占比达到 60% 以上；修复次数受预算限制 | 8-12 人日 | LLM repair 造成新副作用 |
| 员工质量分 | `metrics.py`, `local_duty_graph_health.py`, Admin 质量看板 | 每个员工有成功率、返工率、平均耗时、证据完整率、升级率 | 管理端能按员工/部门/工具定位短板 | 6-10 人日 | 指标口径漂移 |
| Nightly Eval | `FHD/evals/`, CI, MODstore daily-loop | 每晚跑岗位样本、真实 API smoke、员工 smoke、成本/延迟趋势 | AI 员工能力变差会自动报警，不靠人发现 | 6-10 人日 | 夜间任务依赖外部服务 |

P2 出口门：

1. 100 条跨模块任务中，端到端完成率达到 85% 以上。
2. 同一复杂任务多员工并行比单员工串行节省 40% 以上时间。
3. 失败任务中至少 60% 可自动修复或自动生成可执行下一步。
4. 每个员工有趋势评分；低于阈值自动降级为人工确认或暂停调度。

### 3.1.5 P3：超过人类团队的自治部门

目标：从“一个个 AI 员工”升级为“可治理的 AI 部门”。P3 不等于完全无人，而是让人类从执行者变成授权、抽检和战略决策者。

| 改造项 | 当前落点 | 任务 | 验收标准 | 工作量 | 风险 |
|---|---|---|---|---|---|
| 部门级调度 | `duty_roster`, `workflow_scheduler.py`, `employee_orchestrator.py` | 以部门为单位做计划、排班、容量、升级和成果验收 | 能看到部门 backlog、吞吐、风险和交付物 | 12-18 人日 | 调度策略复杂 |
| 端到端业务闭环 | FHD business routes, MODstore daily chain, Retort engine | 从外部需求/日志/竞品吸收 -> 任务生成 -> 员工执行 -> 测试 -> 发布/回滚 | 每条自动改动都有 PR/CR、测试、部署、回滚策略 | 15-25 人日 | 自动发布风险高 |
| 人类抽检制度 | Admin review, eval reports, handoff | 人类不逐条执行，只抽检高风险、低置信、低评分任务 | 抽检样本可反向训练员工合同和 eval | 6-10 人日 | 过少抽检会漏风险 |
| 合规和成本治理 | ToolSpecV2 policy, billing, tenant entitlement | 部门预算、权限、审计、数据范围按租户/员工/工具控制 | 超预算、越权、数据越界全部可解释失败 | 8-12 人日 | 计费/权限错误高风险 |
| 自我进化边界 | Retort, employee health scan, prompt evolution | 外部吸收只能生成候选改进，必须通过评测、审批和回滚门 | AI 能持续变强，但不能绕过质量门 | 10-15 人日 | 自进化过度自动化 |

P3 出口门：

1. AI 部门可连续 7 天运行，关键链路没有无人处理的红色阻塞。
2. 业务任务从进入 inbox 到完成/升级/拒绝全程有证据，不出现“丢任务”。
3. 自动化交付必须经过测试、审批策略、回滚策略和人类抽检门。
4. 人类工作从“亲自做”转为“授权、抽检、处理例外”，执行工作量下降 70% 以上。

### 3.1.6 统一验收矩阵

| 验收项 | P0 | P1 | P2 | P3 |
|---|---|---|---|---|
| 真实可跑 | 核心员工 smoke 通过 | 任务执行有 evidence | 多员工并行真实完成 | 部门连续运行 |
| 任务交接 | run events 可读 | 自动 handoff report | 跨员工合并报告 | 部门级日报/周报 |
| 失败处理 | 不伪成功 | 自动 blocked/escalated | 自动 repair/转派 | 失败模式进入训练和治理 |
| 安全边界 | risk/workspace/write gate 生效 | 高风险全审批 | 预算/权限/低置信全拦截 | 租户/部门/员工级治理 |
| 质量评测 | 最小 smoke/eval | 50 条真实任务 | 100 条跨模块任务 + nightly | 连续 7 天趋势报告 |
| 超过人类点 | 不丢证据 | 交接更快 | 并行和记忆超过个人 | 部门自治超过普通小团队 |

### 3.1.7 不做清单

1. 不用“员工数量更多”替代“员工能闭环工作”。
2. 不把聊天回复当完成；必须有工具结果、读回、测试、截图、CI 或外部 API 证据。
3. 不让高风险写操作绕过审批和 workspace guard。
4. 不把调度器 running 当作任务成功；必须看具体 job/run 的状态。
5. 不让自我进化直接改生产；只能生成候选，经评测、审批和回滚门后合入。
