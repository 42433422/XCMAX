...
export const approvalApi = {
  ...
  async updateFlow(flowId: number, data: Partial<ApprovalFlow>) {
    try {
      return await api.put(`/api/approval/flows/${flowId}`, data)
    } catch (error) {
  ...
  async toggleFlowActive(flowId: number, isActive: boolean) {
    try {
      return await api.patch(`/api/approval/flows/${flowId}/active`, { is_active: isActive })
    } catch (error) {
  ...
  async deleteFlow(flowId: number) {
    try {
      return await api.delete(`/api/approval/flows/${flowId}`)
    } catch (error) {
