...
export const useModsStore = defineStore('mods', () => {
  ...
  function ensureActiveModSelection() {
    ...
    if (current && mods.value.some((m) => String(m.id || '').trim() === current)) return;
    setActiveModId(String(mods.value[0]?.id || '').trim());
  }
  ...
  async function fetchModsOnce(): Promise<{
    ...
    try {
      ...
      if (isApiFetchTimeoutError(error)) {
        loadError.value = `请求超时（${Math.round(DEFAULT_MOD_API_TIMEOUT_MS / 1000)}s 内无响应），请确认后端已启动或稍后刷新`;
      ...
      loadError.value = looksLikeTransport
        ? '无法连接开发环境（若控制台大量 ERR_CONNECTION_REFUSED :5001，请先在前端目录执行 npm run dev；再在 XCAGI 目录执行 python run.py :5000。热重启或断连时会出现 ERR_ABORTED，两端就绪后请刷新页面）。'
        : raw;
