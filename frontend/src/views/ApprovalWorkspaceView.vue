...
const loadData = async () => {
  ...
  try {
    // 加载待审批
    const pendingRes = await approvalApi.getPendingApprovals(userId)
    ...
    // 加载我发起的
    const initiatedRes = await approvalApi.getMyRequests(userId)
    if (initiatedRes.success && initiatedRes.data) {
      initiatedRequests.value = initiatedRes.data.requests || []
      stats.value.initiated = initiatedRequests.value.length
    }

    // 统计已通过/已拒绝
    const allRes = await approvalApi.getMyRequests(userId)
    if (allRes.success && 'data' in allRes && allRes.data) {
      const all = allRes.data.requests || []
      stats.value.approved = all.filter((r: ApprovalRequest) => r.status === 'approved').length
      stats.value.rejected = all.filter((r: ApprovalRequest) => r.status === 'rejected').length
    }
  } catch (error) {
