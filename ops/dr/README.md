...
- 生产机每 15 分钟强制切换并异地推送 PostgreSQL 10 WAL，`archive_timeout=15min`。
- 每周生成一次 PostgreSQL 10 物理基础备份；每天 03:30 的三库逻辑备份继续保留为跨版本回退。
- 备份用专用受限 SSH 账号写入 `incoming`；该账号不能执行远程命令，也不能读取归档。
- 温备机每 10 分钟校验并封存一次完整快照，每 15 分钟检测新快照并恢复。
- PostgreSQL 10 物理温备常驻 `127.0.0.1:15432` 并持续回放 WAL；PostgreSQL 16 逻辑恢复实例保留在 `127.0.0.1:5432`。
