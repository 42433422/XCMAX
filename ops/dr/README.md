...
- PostgreSQL 16 继续以 WAL 自动主备，故障提升必须通过防脑裂门禁。
  PostgreSQL 10 支付库已于 2026-07-31 退役（主 CVM 不再运行 PG10 实例，
  DR 上的 `xcmax-dr-postgres10` 容器已停用，数据卷保留在
  `/srv/xcmax-dr/wal/postgres10-data` 以备历史数据回溯）。
...
- 生产机对 PostgreSQL 16 应用库错峰强制切换并异地推送 WAL，`archive_timeout=15min`。
- 每周生成 PostgreSQL 16 物理基础备份；每天 03:30 的三库逻辑备份继续保留为跨版本回退。
- 备份用专用受限 SSH 账号写入 `incoming`；该账号不能执行远程命令，也不能读取归档。
- 温备机每 10 分钟校验并封存一次完整快照，每 15 分钟检测新快照并恢复。
- PostgreSQL 16 应用温备常驻 `127.0.0.1:15433`；PostgreSQL 16 逻辑恢复实例保留在 `127.0.0.1:5432`。
...
MODstore、PostgreSQL 16、Redis 和 payment API。它不能获得生产 shell。
...
3. PostgreSQL 16 standby 处于 recovery；
