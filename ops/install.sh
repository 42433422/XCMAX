...
fi
{
  ...
  echo ""
  echo "# PostgreSQL 16 应用库 WAL 增量异地同步（与 PG10 错峰）"
  echo "7,22,37,52 * * * * root set -a; . ${ENV_FILE}; set +a; bash ${INSTALL_DIR}/dr/xcmax_wal_ship_pg16.sh >> ${OPS_LOG_DIR:-/var/log/xcmax-ops}/wal-pg16-ship.log 2>&1"
  echo ""
  ...
  echo ""
  echo "# PostgreSQL 16 每周物理基础备份（周日 02:45）"
  echo "45 2 * * 0 root set -a; . ${ENV_FILE}; set +a; bash ${INSTALL_DIR}/dr/xcmax_wal_prepare_primary_pg16.sh >> ${OPS_LOG_DIR:-/var/log/xcmax-ops}/wal-pg16-base.log 2>&1"
  echo ""
  echo "# 夜间备份（北京 03:30）"
