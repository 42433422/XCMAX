...
def _build_absorption_task_plan(
    ...
    *,
    ...
    graph_gap = compare_repository_gaps(own_assessment.project, external_ref.local_path, max_files=12)
    gap_tasks = tasks_from_repository_gaps(graph_gap, own_map, limit=max(3, max_tasks // 2))
    ...
    oracle_loop = run_agent_oracle_loop(Path(__file__).resolve().parents[1], run_id="absorption-task-oracle")
    ...
    with tempfile.TemporaryDirectory(prefix="retort-absorb-loop-") as tmp:
        loop = run_bounded_agent_loop(
            ...
            judge=lambda _objective, trajectory: {
                "complete": bool(oracle.get("summary", {}).get("all_resolved")) and len(trajectory) >= len(candidates),
                "score": round(100 * len(trajectory) / len(candidates), 2) if candidates else 0,
                "missing": "" if oracle.get("summary", {}).get("all_resolved") else "heldout_oracle_not_all_resolved",
                "oracle_resolved_count": oracle.get("summary", {}).get("resolved_count"),
            },
...
def _task_from_depth_employee(external_ref: ExternalProjectRef, row: dict[str, Any], index: int) -> ImprovementTask:
    return ImprovementTask(
