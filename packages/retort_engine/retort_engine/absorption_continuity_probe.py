...
def build_absorption_continuity_probe(
    project: str | Path, *, min_runs: int = 5, output: str | Path = ""
) -> dict[str, Any]:
    root = Path(project).expanduser().resolve()
    runs = _latest_distinct_successful_runs(root, limit=max(min_runs, 1))
    inspected = [
        _inspect_run(root, run, latest=(index == 0)) for index, run in enumerate(runs)
    ]
...
def _inspect_run(root: Path, run: dict[str, Any], *, latest: bool) -> dict[str, Any]:
    ...
    checks = {
        ...
        and (
            all(bool(gate.get("ok")) for gate in gates if isinstance(gate, dict))
            if gates
            else True
        ),
...
def _latest_closed_loop(root: Path, latest_run: dict[str, Any]) -> dict[str, Any]:
    ...
    code_graph_verified = "code_graph_proof_passed=True" in evidence or bool(
        checks.get("per_run_code_graph_proof")
    )
    latest_run_referenced = (
        not run_id
        or any(run_id in item for item in evidence)
        or bool(hardening_merge_commit)
    )
    ...
    return {
        ...
        "required_flags": {
            ...
            "employee_execution_verified": bool(
                proof.get("employee_execution_verified")
            ),
            "post_absorption_tests_passed": bool(
                proof.get("post_absorption_tests_passed")
            ),
            "merge_verified": bool(proof.get("merge_verified")),
            "external_advantage_reassessed": bool(
                proof.get("external_advantage_reassessed")
            ),
