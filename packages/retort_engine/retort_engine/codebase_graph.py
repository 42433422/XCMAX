...
def build_codebase_graph(
    project: str | Path, *, include_tests: bool = False, max_files: int = 400
) -> dict[str, Any]:
    ...
    for path in files:
        ...
        nodes.append(
            {"id": rel, "kind": "file", "path": rel, "name": path.stem, "line": 1}
        )
        try:
    ...
    summary = {
        ...
        "local_dependency_edge_count": sum(
            1 for edge in edges if edge["kind"] == "depends_on"
        ),
...
def build_absorption_focus_map(
    ...
    *,
    ...
    external_graph = build_codebase_graph(
        external_root, include_tests=True, max_files=max_files
    )
...
def code_graph_absorption_proof(
    project: str | Path,
    ...
    *,
    ...
    focus_files = {
        str(item) for item in (pre_absorption_focus or {}).get("own_focus_files") or []
    }
    behavior_changed = [path for path in changed if _is_behavior_change(path)]
    changed_hotspots = sorted(
        path for path in behavior_changed if path in hotspot_files
    )
    changed_focus_files = sorted(
        path for path in behavior_changed if path in focus_files
    )
...
def _focus_terms(tasks: list[dict[str, Any]], signals: list[str]) -> list[str]:
    ...
    for signal in signals:
        terms.extend(
            SIGNAL_FOCUS_TERMS.get(
                str(signal), (str(signal), str(signal).replace("_", " "))
            )
        )
    task_text = " ".join(
        " ".join(
            str(task.get(key) or "") for key in ("task_id", "title", "dimension", "why")
        )
        for task in tasks
...
def _rank_focus_files(
    root: Path, graph: dict[str, Any], terms: list[str], *, limit: int = 10
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    hotspot_degree = {
        row["path"]: int(row["degree"]) for row in _hotspot_rows(graph, limit=64)
    }
    ...
    if not rows:
        rows = [
            {
                "path": row["path"],
                "score": int(row["degree"]),
                "term_hits": 0,
                "graph_degree": int(row["degree"]),
            }
            for row in _hotspot_rows(graph, limit=limit)
        ]
...
def _graph_file_paths(graph: dict[str, Any]) -> list[str]:
    return [
        ...
        if isinstance(node, dict) and node.get("kind") == "file" and node.get("id")
    ]
...
def _local_dependency_edges(
    module_index: dict[str, str], edges: list[dict[str, Any]]
) -> list[dict[str, str]]:
...
def _dependency_impact(
    graph: dict[str, Any], changed_files: list[str], *, limit: int = 24
) -> dict[str, Any]:
    ...
    return {
        ...
        "direct_dependencies": {
            path: sorted(dependencies.get(path, set())) for path in sorted(changed)
        },
        "direct_dependents": {
            path: sorted(dependents.get(path, set())) for path in sorted(changed)
        },
...
def _hotspots(
    nodes: list[dict[str, Any]], edges: list[dict[str, Any]], *, limit: int = 12
) -> list[dict[str, Any]]:
    node_kinds = {str(node["id"]): str(node.get("kind") or "") for node in nodes}
    scores: dict[str, dict[str, int]] = {
        node_id: {"incoming": 0, "outgoing": 0} for node_id in node_kinds
    }
    for edge in edges:
    ...
    return sorted(
        ...
        key=lambda row: (
            -row["degree"],
            -row["incoming"],
            kind_priority.get(str(row["kind"]), 9),
            row["id"],
        ),
    )[:limit]
...
def _dedupe_edges(edges: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for edge in edges:
        ...
        if key not in seen:
            ...
            result.append(edge)
    return sorted(
        result, key=lambda edge: (str(edge["kind"]), str(edge["from"]), str(edge["to"]))
    )
