...
def build_cross_project_replay(project: str | Path) -> dict[str, Any]:
    ...
    projects = [
        _project_entry(source, payload) for source, payload in sorted(runs.items())
    ]
    signals = sorted(
        {signal for item in projects for signal in item.get("signals", [])}
    )
    checks = [
        {
            "name": "at_least_three_external_sources",
            "passed": len(projects) >= 3,
            "observed": len(projects),
        },
        {
            "name": "behavior_code_absorbed",
            "passed": any(item.get("behavior_source_changed") for item in projects),
            "observed": sum(
                1 for item in projects if item.get("behavior_source_changed")
            ),
        },
        {
            "name": "real_pr_diff_reviewed",
            "passed": pr_report.get("status") == "reviewed",
            "observed": (pr_report.get("summary") or {}).get("comment_count", 0),
        },
        {
            "name": "publish_dry_run_ready",
            "passed": publish_report.get("status") == "dry_run_ready",
            "observed": (publish_report.get("summary") or {}).get(
                "would_post_comment_count", 0
            ),
        },
    ]
    residual_questions = [
    ...
    return {
        ...
        "summary": {
            ...
            "behavior_source_project_count": sum(
                1 for item in projects if item.get("behavior_source_changed")
            ),
            "real_pr_comment_count": int(
                (pr_report.get("summary") or {}).get("comment_count") or 0
            ),
            "publish_dry_run_comment_count": int(
                (publish_report.get("summary") or {}).get("would_post_comment_count")
                or 0
            ),
        },
        "projects": projects,
...
def _is_behavior_source(path: str) -> bool:
    value = path.replace("\\", "/")
    return "/retort_engine/" in value and value.endswith(
        (".py", ".js", ".ts", ".tsx", ".jsx")
    )
