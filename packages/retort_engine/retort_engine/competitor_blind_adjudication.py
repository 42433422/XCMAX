...
def build_competitor_blind_adjudication(
    ...
    *,
    ...
    comparison_file = (
        ...
        if comparison_path
        else root / "docs" / "retort_competitor_runtime_comparison.json"
    )
    ...
    input_path.write_text(
        json.dumps(blind_input, ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    ...
    summary = {
        ...
        "all_competitors_blind_accepted": bool(competitor_count)
        and accepted_count == competitor_count,
        ...
        "blind_delta_floor_met": int(adjudication_summary.get("minimum_delta") or 0)
        >= min_delta,
...
def _blind_input(comparison: dict[str, Any], *, min_delta: int) -> dict[str, Any]:
    ...
    runtimes = [
        item
        for item in competitor_output.get("runtimes") or []
...
def _redacted_comment(comment: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "has_publish_payload": isinstance(comment.get("publish_payload"), dict)
        and bool(comment.get("publish_payload")),
    }
