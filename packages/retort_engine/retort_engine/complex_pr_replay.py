...
def build_complex_pr_replay_report(
    ...
    *,
    ...
    for url in urls:
        ...
        comments = [
            item
            for item in (review.get("review") or {}).get("comments") or []
            if isinstance(item, dict)
        ]
        files = [
            item
            for item in (review.get("review") or {}).get("files") or []
            if isinstance(item, dict)
        ]
        ...
        severity_counts = {
            severity: sum(
                1 for item in comments if str(item.get("severity") or "") == severity
            )
        ...
        reviewed_new_changes = int(
            ((review.get("review") or {}).get("summary") or {}).get(
    ...
    total_reviewed_changes = sum(
        int(item["reviewed_new_change_count"]) for item in reviewed
    )
    status = (
        "ready"
        if len(reviewed) >= 3
        and len(complex_items) >= 3
        and total_comments >= 10
        and total_reviewed_changes >= 100
    ...
    return {
        ...
        "summary": {
            ...
            "distinct_extension_count": len(
                {ext for item in reviewed for ext in item["language_extensions"]}
            ),
        },
