...
def build_contract_runtime_rehearsal(
    ...
    *,
    ...
    rows = [
        _run_case(lab, dict(case), concurrent_workers=max(1, concurrent_workers))
        for case in (cases or DEFAULT_CONTRACT_CASES)
    ]
    ...
    summary = {
        ...
        "rollback_verified_count": sum(
            1 for row in rows if row["rollback"]["verified"]
        ),
        "valid_payload_accepted_count": sum(1 for row in rows if row["valid_accepted"]),
        "concurrent_worker_count": max(1, concurrent_workers),
        "concurrency_fault_injection_count": sum(
            len(row["concurrency_fault_injections"]) for row in rows
        ),
        "concurrent_violation_rejected_count": sum(
            1
            for row in rows
            for item in row["concurrency_fault_injections"]
            if item["invalid_rejected"]
        ),
        "concurrent_rollback_verified_count": sum(
            1
            for row in rows
            for item in row["concurrency_fault_injections"]
            if item["rollback_verified"]
        ),
        "all_violations_rejected": bool(rows)
        and all(row["invalid_rejected"] for row in rows),
        "all_rollbacks_verified": bool(rows)
        and all(row["rollback"]["verified"] for row in rows),
        "all_valid_payloads_accepted": bool(rows)
        and all(row["valid_accepted"] for row in rows),
        "all_concurrent_violations_rejected": bool(rows)
        and all(
            item["invalid_rejected"]
            for row in rows
            for item in row["concurrency_fault_injections"]
        ),
        "all_concurrent_rollbacks_verified": bool(rows)
        and all(
...
def _run_case(
    lab: Path, case: dict[str, Any], *, concurrent_workers: int
) -> dict[str, Any]:
    ...
    before_state = {
        "case_id": case_id,
        "contract": contract,
        "last_valid_payload": case["valid_payload"],
    }
    ...
    rollback_verified = (
        json.loads(state_path.read_text(encoding="utf-8")) == before_state
    )
    valid_check = validate_contract(contract, dict(case["valid_payload"]))
    concurrency = _run_concurrency_fault_injections(
        case_lab, case, concurrent_workers=concurrent_workers
    )
    ...
    return {
        ...
        "ready": (not invalid_check["valid"])
        and valid_check["valid"]
        and rollback_verified
        and all(
            item["invalid_rejected"] and item["rollback_verified"]
            for item in concurrency
        ),
    }


def _run_concurrency_fault_injections(
    case_lab: Path, case: dict[str, Any], *, concurrent_workers: int
) -> list[dict[str, Any]]:
    ...
    with ThreadPoolExecutor(max_workers=concurrent_workers) as pool:
        return list(
            pool.map(
                lambda worker: _concurrent_fault_worker(concurrency_dir, case, worker),
                range(1, concurrent_workers + 1),
            )
        )


def _concurrent_fault_worker(
    concurrency_dir: Path, case: dict[str, Any], worker: int
) -> dict[str, Any]:
    contract = str(case["contract"])
