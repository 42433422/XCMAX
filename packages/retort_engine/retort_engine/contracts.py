...
def contract_compatibility_report(
    name: str, previous_required_fields: tuple[str, ...] | list[str]
) -> dict[str, Any]:
    current_required = RETORT_CONTRACT_SCHEMAS[name]
    previous = tuple(previous_required_fields)
    removed_historical_fields = [
        field for field in previous if field not in current_required
    ]
    newly_required_fields = [
        field for field in current_required if field not in previous
    ]
