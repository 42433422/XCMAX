...
from retort_engine.absorption_workflow import (
    absorption_summary as _workflow_absorption_summary,
)
from retort_engine.absorption_workflow import (
    block_merge_after_failed_gates as _workflow_block_merge_after_failed_gates,
)
from retort_engine.absorption_workflow import (
    commit_absorption_execution as _workflow_commit_absorption_execution,
)
from retort_engine.absorption_workflow import (
    extract_json_from_stdout as _workflow_extract_json_from_stdout,
)
from retort_engine.absorption_workflow import (
    is_complete_absorption_stdout_json as _workflow_is_complete_absorption_stdout_json,
)
from retort_engine.absorption_workflow import (
...
from retort_engine.capability_audit import (
    capability_absorption_audit as _audit_capability_absorption_audit,
)
from retort_engine.capability_audit import (
    employee_result_files as _audit_employee_result_files,
)
from retort_engine.capability_audit import (
    latest_absorption_run as _audit_latest_absorption_run,
)
from retort_engine.capability_audit import (
    pr_review_runtime_evidence as _audit_pr_review_runtime_evidence,
)
...
from retort_engine.devour_session import (
    assessment_file_count as _devour_assessment_file_count,
)
from retort_engine.devour_session import assessment_score as _devour_assessment_score
from retort_engine.devour_session import (
    build_devour_session as _devour_build_devour_session,
)
...
from retort_engine.external_sources import (
    external_project_profile as _sources_external_project_profile,
)
from retort_engine.external_sources import (
    materialize_external_source as _sources_materialize_external_source,
)
...
from retort_engine.llm_absorption_evidence import (
    llm_absorption_evidence as _evidence_llm_absorption_evidence,
)
from retort_engine.llm_scoring import attach_llm_scoring as _scoring_attach_llm_scoring
from retort_engine.llm_scoring import (
    llm_disabled_review as _scoring_llm_disabled_review,
)
from retort_engine.llm_scoring import llm_enabled as _scoring_llm_enabled
from retort_engine.llm_scoring import (
    llm_external_reference as _scoring_llm_external_reference,
)
from retort_engine.llm_scoring import (
    maybe_request_llm_review as _scoring_maybe_request_llm_review,
)
from retort_engine.paibi_llm import (
    fetch_paibi_llm_review_status,
    fetch_paibi_parallel_review_status,
    record_paibi_llm_deep_result,
...
def assess_project(
    project: str, *, run_local_gates: bool = False, context_policy: str = "isolated"
) -> Assessment:
...
class RetortHistory:
    def __init__(self, path: str | Path) -> None:
        ...
        with sqlite3.connect(self.path) as conn:
            conn.executescript(
                "CREATE TABLE IF NOT EXISTS absorption_runs(id INTEGER PRIMARY KEY, payload TEXT); CREATE TABLE IF NOT EXISTS employee_tasks(id INTEGER PRIMARY KEY, payload TEXT);"
            )
    ...
    def record(self, table: str, payload: dict[str, Any]) -> None:
        with sqlite3.connect(self.path) as conn:
            ...
            if "payload" in columns:
                conn.execute(
                    f"INSERT INTO {table}(payload) VALUES (?)", (payload_json,)
                )
                ...
                conn.execute(
                    ...
                    (
                        ...
                        str((payload.get("external_ref") or {}).get("source") or ""),
                        payload_json,
                    ),
                )
                ...
                conn.execute(
                    ...
                    (
                        _now_iso(),
                        str(payload.get("queue_id") or uuid.uuid4()),
                        str(task.get("task_id") or ""),
                        str(task.get("owner_hint") or ""),
...
def absorb(payload: dict[str, Any]) -> dict[str, Any]:
    own = (
        Path(payload.get("own_project") or payload.get("project") or ".")
        .expanduser()
        .resolve()
    )
    ...
    try:
        ...
        if not keep_residue:
            ...
            closure = close_run_workspace(
                own,
                run_id=str(((result or {}).get("execution") or {}).get("run_id") or ""),
            )
            if isinstance(result, dict):
                result["workspace_closure"] = closure
                if not closure["summary"]["closed"] and result.get("status") not in {
...
def _absorb_unguarded(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    external = (
        payload.get("external_path")
        or payload.get("github_url")
        or payload.get("github")
        or ""
    )
    external_path = _materialize_external_source(
        str(external), own, bool(payload.get("refresh"))
    )
    branch_state = {
        "enabled": bool(payload.get("branch_workflow")),
        "status": "disabled",
    }
    if payload.get("branch_workflow"):
        try:
            branch_state = begin_absorption_branch(
                own,
                source=str(external),
                branch_name=payload.get("absorption_branch") or "",
                allow_dirty=bool(payload.get("allow_dirty_branch")),
            ).to_dict()
        except RuntimeError as exc:
            return {
                "status": "blocked_by_branch_workflow",
                "error": str(exc),
                "branch_workflow": branch_state,
                "tasks": [],
            }
    pre_assessment = assess_project(
        str(own), run_local_gates=bool(payload.get("run_local_gates"))
    ).to_dict()
    ...
    absorption_state = _record_absorption_shock(
        own, str(external), external_path, tasks
    )
    execution = _run_real_absorption_cli(
        own, str(external), external_path, tasks, external_assessment, payload
    )
    if execution.get("status") in {"applied", "noop"}:
        _record_execution_proof(own, execution, branch_state)
    if (
        payload.get("merge_after")
        and branch_state.get("enabled")
        and branch_state.get("created")
    ):
        try:
            execution["commit"] = _commit_absorption_execution(
                own, str(external), execution
            )
            if execution.get("status") in {"applied", "noop"} and not execution.get(
                "gates_passed"
            ):
                branch_state = _block_merge_after_failed_gates(own, branch_state)
                _record_execution_proof(own, execution, branch_state)
            else:
                result_branch = merge_absorption_branch(
                    own, BranchWorkflowState.from_dict(branch_state)
                ).to_dict()
                root = _git_root(own)
                if root is not None:
                    ...
                    execution["rollback_rehearsal"] = _rollback_rehearsal(
                        root, merge_commit
                    )
    ...
    result = {
        ...
        "external_ref": {
            "source": str(external),
            "local_path": "" if external_path is None else str(external_path),
        },
...
class RetortService:
    ...
    def absorption_lights(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        sources = [
            str(item)
            for item in audit.get("external_projects") or []
            if str(item).strip()
        ]
        return {
            "status": "ready",
    ...
    def self_evolve(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        result = {
            ...
            "rounds": [
                {"round_index": 1, "passed": False, "assessment": base, "tasks": []}
            ],
            "tasks": [],
            "round_policy": "single_paibi_llm_deep_review",
        }
        result["final_assessment"] = _attach_llm_scoring(
            llm_payload, base, project_path, "self_evolve", "", "", []
        )
        scores = [
            Score(
                str(score.get("dimension") or ""),
                float(score.get("value") or 0),
                str(score.get("reason") or ""),
            )
            for score in result["final_assessment"].get("scores", [])
            if isinstance(score, dict)
        ]
        weak = [
            score
            for score in scores
            if score.value <= float(payload.get("threshold") or 90.0)
        ]
        tasks = [
            _task_for_weak_score(score, float(payload.get("threshold") or 90.0), 1)
            for score in weak
        ]
        result["tasks"] = tasks
        result["rounds"] = [
            {
                "round_index": 1,
                "passed": not weak and bool(scores),
        ...
        result["stop_reason"] = (
            "all_llm_scores_strictly_above_threshold"
            if scores and not weak
            else "llm_questions_generated_for_weak_scores"
        )
        return result
    ...
    def similar_project_loop(self, payload: dict[str, Any]) -> dict[str, Any]:
        return run_similar_project_loop(
            str(payload.get("project") or payload.get("project_path") or "."),
            sources=[
                str(item) for item in payload.get("sources") or [] if str(item).strip()
            ],
    ...
    def llm_review(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return request_paibi_llm_review(
            ...
            external_source=str(
                payload.get("external_source") or payload.get("github_url") or ""
            ),
...
def _record_execution_proof(
    own: Path, execution: dict[str, Any], branch_state: dict[str, Any]
) -> None:
    _record_execution_proof_impl(
        own,
        execution,
        branch_state,
...
def _commit_absorption_execution(
    own: Path, source: str, execution: dict[str, Any]
) -> dict[str, Any]:
    return _workflow_commit_absorption_execution(
        own, source, execution, git_root=_git_root, git_command=_git
    )


def _block_merge_after_failed_gates(
    own: Path, branch_state: dict[str, Any]
) -> dict[str, Any]:
    return _workflow_block_merge_after_failed_gates(
        own, branch_state, git_root=_git_root, git_command=_git
    )
...
def _tasks_from_assessment(
    ...
    if external_path and external_path.is_dir():
        ...
        own_map = build_ranked_repository_map(
            own_root,
            focus_terms=("absorb", "agent", "review", "oracle"),
            max_files=12,
            max_chars=12_000,
        )
        ...
        depth = build_depth_absorption_workflow(
            group_review_files(own_root), group_review_files(external_path), tasks
        )
            ...
            tasks.append(
                {
                    ...
                    "dimension": str(
                        row.get("dimension") or "comparative_analysis_depth"
                    ),
                    "owner_hint": str(row.get("owner_hint") or "fhd-core-maintainer"),
        ...
        tasks = [
            {
                ...
                "owner_hint": "fhd-core-maintainer",
                "priority": "P1",
                "why": f"Compare implementation patterns from {source}.",
            },
            {
                "task_id": "retort-absorb-ux",
                "title": "Absorb better user experience",
                "dimension": "product_operability",
                "owner_hint": "market-frontend-dev",
                "priority": "P1",
                "why": f"Extract usable UX improvements from {source}.",
            },
            {
                "task_id": "retort-absorb-ops",
                "title": "Absorb better operational gates",
                "dimension": "operational_readiness",
                "owner_hint": "deploy-release-officer",
                "priority": "P2",
                "why": f"Adapt CI and release checks from {source}.",
            },
        ]
    ...
    if (
        profile.get("planet_frontend")
        or profile.get("atmosphere_shader")
        or profile.get("procedural_surface")
        or profile.get("webgl_scene")
    ):
        tasks.append(
            {
...
def _task_for_weak_score(
    score: Score, threshold: float, round_index: int
) -> dict[str, str]:
    return {
