...
EXTRACTED_BOUNDARY_MODULES = {
    "automation_surface": (
        ...
        "retort_engine/external_sources.py",
    ...
    "safety_policy": (
        "retort_engine/capability_audit.py",
        "retort_engine/absorption_state.py",
...
def load_core_refactor_plan(project_root: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "summary": {},
            "gate": {"passed": False, "status": "missing_plan"},
            "tasks": [],
        }
    return (
        payload
        if isinstance(payload, dict)
            ...
            "summary": {},
            "gate": {"passed": False, "status": "invalid_plan"},
            "tasks": [],
        }
    )


def verify_core_refactor_execution(
    project_root: str | Path, plan: dict[str, Any] | None = None
) -> dict[str, Any]:
    root = Path(project_root)
    ...
    tasks = [
        ...
        if isinstance(task, dict) and task.get("ready_for_core_refactor")
    ]
    ...
    return {
        ...
        "implemented_task_count": sum(
            1 for result in component_results if result["implemented"]
        ),
        "missing": missing,
...
def _verify_task(root: Path, task: dict[str, Any]) -> dict[str, Any]:
    ...
    empty_tests = [
        rel
        for rel in tests
        if (root / rel).is_file() and _test_function_count(root / rel) == 0
    ]
    single_module_boundary = (
        component in EXTRACTED_BOUNDARY_MODULES
        and len([rel for rel in boundary_modules if (root / rel).is_file()]) < 2
    )
    missing: list[str] = []
    ...
    return {
        ...
        "test_function_count": sum(
            _test_function_count(root / rel) for rel in tests if (root / rel).is_file()
        ),
        "missing": missing,
    }
