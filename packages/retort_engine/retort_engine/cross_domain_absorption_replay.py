...
def build_cross_domain_absorption_replay(
    ...
    *,
    ...
    source_projects = sorted(
        {str(case["source_project"]) for case in cases if case.get("source_project")}
    )
    direct_modules = sorted(
        {str(case["direct_module"]) for case in cases if case.get("direct_module")}
    )
    ...
    summary = {
        ...
        "pre_absorption_failure_count": sum(
            1 for case in cases if case["pre_absorption"]["failed_expected_behavior"]
        ),
        "post_absorption_pass_count": sum(
            1 for case in cases if case["post_absorption"]["passed_expected_behavior"]
        ),
        "all_before_failed_after_passed": bool(cases)
        and all(case["before_failed_after_passed"] for case in cases),
        "all_direct_modules_executed": bool(cases)
        and all(case["post_absorption"]["direct_module_executed"] for case in cases),
        "all_output_assertions_passed": bool(cases)
...
def _benchmark_oracle_case(lab: Path) -> dict[str, Any]:
    ...
    return _case(
        ...
        artifacts=[
            case_lab / "pre_input.json",
            case_lab / "post_input.json",
            case_lab / "post_output.json",
        ],
...
def _architecture_contract_case(lab: Path) -> dict[str, Any]:
    ...
    (project / "retort_engine" / "codebase_graph.py").write_text(
    ...
    post = evaluate_architecture_contracts(
        project, contracts=[contract], include_tests=True
    )
    ...
    assertions = {
        ...
        "contract_name_returned": any(
            item.get("contract") == "graph_cannot_import_core"
            for item in post.get("violations") or []
        ),
    }
    return _case(
        ...
        pre_summary={
            "status": "pre_absorption_no_architecture_contract_runtime",
...
def _code_graph_case(lab: Path) -> dict[str, Any]:
    ...
    (project / "pkg" / "policy.py").write_text(
        "def allowed():\n    return True\n", encoding="utf-8"
    )
    ...
    assertions = {
        ...
        "hotspot_or_dependency_seen": int(post["summary"]["hotspot_count"]) >= 1
        or int(post["summary"]["local_dependency_edge_count"]) >= 1,
    }
    return _case(
        case_id="code_graph",
...
def _context_pack_case(lab: Path) -> dict[str, Any]:
    ...
    post = build_context_pack(
        project,
        focus_terms=["absorption", "benchmark", "review", "graph"],
        max_files=3,
        max_chars=2000,
    )
    ...
    assertions = {
        ...
        "focused_runtime_selected": "retort_engine/absorption_runtime.py"
        in selected_paths,
        "bounded_output": int(post["summary"]["used_chars"])
        <= int(post["summary"]["max_chars"]),
    }
    return _case(
        ...
        artifacts=[case_lab / "post_output.json"],
        pre_summary={
            "status": "pre_absorption_no_context_pack_runtime",
...
def _license_gate_case(lab: Path) -> dict[str, Any]:
    ...
    assertions = {
        ...
        "detected_agpl": "Affero" in result["detected_license"]
        or "AGPL" in result["detected_license"],
    ...
    return _case(
        ...
        artifacts=[case_lab / "post_output.json"],
        pre_summary={
            "status": "pre_absorption_no_license_runtime_gate",
            "blocked": False,
        },
        post_summary=result,
    )
...
def _static_analysis_case(lab: Path) -> dict[str, Any]:
                ...
                "changes": [
                    {
                        "type": "add",
                        "line": 12,
                        "text": "subprocess.run(command, shell=True, check=False)",
                    },
    ...
    assertions = {
        ...
        "rule_ids_returned": {"subprocess-shell-true", "unsafe-yaml-load"}.issubset(
            {str(item.get("rule_id") or "") for item in result.get("findings") or []}
        ),
    }
    return _case(
        ...
        pre_summary={
            "status": "pre_absorption_no_static_analysis_runtime",
            "finding_count": 0,
        },
...
def _intent_alignment_case(lab: Path) -> dict[str, Any]:
                    ...
                    "changes": [
                        {
                            "type": "add",
                            "line": 10,
                            "text": "def retry_invoice_payment(invoice_id):",
                        },
                        {
                            "type": "add",
                            "line": 11,
                            "text": "    return schedule_billing_retry(invoice_id)",
                        },
                    ]
                }
            ],
    ...
    result = assess_change_intent_alignment(
        files,
        issue_context="billing invoice retry must schedule payment recovery",
        pr_body="Implements invoice retry flow",
    )
    _write_json(
        case_lab / "input.json",
        {
    ...
    assertions = {
        ...
        "source_boundary": result["evidence"]["source"]
        == "ReviewScope issue-mismatch rule",
    }
    return _case(
        ...
        artifacts=[case_lab / "input.json", case_lab / "post_output.json"],
        pre_summary={
            "status": "pre_absorption_no_intent_alignment_gate",
...
def _task_prioritization_case(lab: Path) -> dict[str, Any]:
            ...
            "results": [
                {
                    "status": "completed",
                    "task": {"dimension": "api_contract_quality"},
                    "notes": "missing concurrency blocker fixed",
                },
                {
                    "status": "completed",
                    "task": {"dimension": "product_operability"},
                    "notes": "frontend replay risk closed",
                },
            ]
        },
    )
    ...
    return _case(
        ...
        output_assertions=assertions,
        artifacts=[
            queue_dir / "employee_queue.jsonl",
            result_dir / "zz-results.json",
            case_lab / "post_output.json",
        ],
        pre_summary={
            "status": "pre_absorption_no_feedback_task_graph",
            "prioritized_dimension_count": 0,
        },
        post_summary=result["summary"],
    )
...
def _task_dispatch_case(lab: Path) -> dict[str, Any]:
    ...
    assertions = {
        ...
        "evidence_required": result["summary"]["all_tasks_have_evidence_required"]
        is True,
    }
    return _case(
        ...
        artifacts=[
            retort_dir / "llm_reviews.jsonl",
            retort_dir / "employee_queue.jsonl",
            case_lab / "post_output.json",
        ],
        pre_summary={
            "status": "pre_absorption_no_llm_task_dispatch",
...
def _core_refactor_plan_case(lab: Path) -> dict[str, Any]:
    ...
    assertions = {
        ...
        "contract_present": bool(result["tasks"][0]["contract"])
        if result.get("tasks")
        else False,
    }
    return _case(
        ...
        pre_summary={
            "status": "pre_absorption_no_core_refactor_oracle",
            "ready_task_count": 0,
        },
        post_summary=result["summary"],
    )
...
def _adjudicate_cases(cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for case in cases:
        ...
        checks = {
            "non_pr_domain": str(case.get("domain") or "")
            not in {"", "pr_review", "pull_request"},
            "direct_module_present": bool(case.get("direct_module")),
            "pre_absorption_failed": bool(
                (case.get("pre_absorption") or {}).get("failed_expected_behavior")
            ),
            "post_absorption_passed": bool(
                (case.get("post_absorption") or {}).get("passed_expected_behavior")
            ),
            "output_assertions_passed": bool(assertions)
            and all(bool(value) for value in assertions.values()),
            "artifacts_materialized": bool(artifacts)
