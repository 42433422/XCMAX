...
def build_cross_domain_ci_regression(
    ...
    *,
    ...
    for index in range(1, normalized_rounds + 1):
        run = build_cross_domain_end_to_end(
            root, min_domains=min_domains, run_id=f"{regression_id}-round-{index:02d}"
        )
        ...
        runs.append(
            {
                ...
                "all_stage_outputs_consumed": summary.get(
                    "all_stage_outputs_consumed", False
                ),
                "output_assertions_passed": summary.get(
                    "output_assertions_passed", False
                ),
                "integrated_review_status": summary.get("integrated_review_status", ""),
                "integrated_review_comment_count": summary.get(
                    "integrated_review_comment_count", 0
                ),
    ...
    summary = {
        ...
        "total_domain_replay_count": sum(
            int(item["linked_domain_count"] or 0) for item in runs
        ),
        "all_rounds_ready": bool(runs) and len(ready_runs) == len(runs),
        "all_rounds_chained": bool(runs)
        and all(item["all_stages_chained"] for item in runs),
        "all_outputs_consumed": bool(runs)
        and all(item["all_stage_outputs_consumed"] for item in runs),
        "all_output_assertions_passed": bool(runs)
        and all(item["output_assertions_passed"] for item in runs),
        "all_integrated_reviews_executed": bool(runs)
        and all(item["integrated_review_status"] == "reviewed" for item in runs),
