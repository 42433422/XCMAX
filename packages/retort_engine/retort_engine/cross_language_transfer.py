...
def build_cross_language_transfer(files: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    families = sorted(
        {
            _language_family(str(item.get("path") or ""))
            for item in files
            if _language_family(str(item.get("path") or ""))
        }
    )
        ...
        for hunk in file_review.get("hunks") or []:
            for change in hunk.get("changes") or []:
                ...
                finding = _transfer_finding(
                    path, family, int(change.get("line") or 0), text
                )
                if finding:
    ...
    return {
        ...
        "summary": {
            ...
            "cross_language_core_mapping": bool(
                findings
                and any(
                    family in {"typescript", "javascript", "config"}
...
def _transfer_finding(
    path: str, family: str, line: int, text: str
) -> dict[str, Any] | None:
    lowered = text.lower()
...
def _language_family(path: str) -> str:
    ...
    if name in {"go.mod", "go.sum"}:
        return "go"
    if name in {"action.yml", "action.yaml"} or normalized.startswith(
        ".github/workflows/"
    ):
        return "config"
...
def _is_ci_file(path: str) -> bool:
    normalized = path.replace("\\", "/").lower()
    return normalized.startswith(".github/workflows/") or normalized.endswith(
        ("action.yml", "action.yaml", "workflow.yml", "workflow.yaml")
    )
