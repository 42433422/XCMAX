...
def run_employee_scheduler_stress(
    project: str | Path,
    *,
    round_count: int = 10,
    ...
    run_id = (
        datetime.now(timezone.utc).strftime("scheduler-stress-%Y%m%dT%H%M%SZ")
        + "-"
        + uuid.uuid4().hex[:8]
    )
    ...
    for round_index in range(1, max(1, round_count) + 1):
        tasks = [
            _stress_task(run_id, round_index, task_index)
            for task_index in range(1, max(1, tasks_per_round) + 1)
        ]
        for task in tasks:
        ...
        for worker_index, task_batch in enumerate(
            ...
            payload_path = (
                request_dir
                / f"{run_id}-round-{round_index:02d}-worker-{worker_index:02d}.json"
            )
            result_path = (
                result_dir
                / f"{run_id}-round-{round_index:02d}-worker-{worker_index:02d}.json"
            ...
            payload = {
                ...
                "changed_files": [
                    ...
                    "retort_engine/task_prioritization.py",
                    "retort_engine/employee_scheduler_stress.py",
                ],
                "review_report_path": str(
                    root / "docs" / "retort_task_prioritization_report.json"
                ),
        ...
        failed_process_count += sum(
            ...
            if item["process"]["returncode"] != 0 or not item["result_exists"]
        )
        rounds.append(
            {
                "round_index": round_index,
                ...
                "round_completed": all(
                    item["result_exists"] for item in payload_results
                ),
            }
        )
    ...
    summary: dict[str, Any] = {
        ...
        "history_task_result_count": len(
            set(history_task_ids) & set(expected_task_ids)
        ),
        ...
        "independent_process_verified": process_invocation_count == len(worker_runs)
        and all("-c" in (item["process"].get("command") or []) for item in worker_runs),
        ...
        "concurrent_workers_verified": worker_count > 1
        and process_invocation_count == len(worker_runs)
        and all(item["result_exists"] for item in worker_runs),
        "queue_result_history_consistent": not missing_result_ids
        and not missing_history_ids,
    }
    status = (
        "ready"
...
def _stress_task(run_id: str, round_index: int, task_index: int) -> dict[str, Any]:
    dimension = DEFAULT_DIMENSIONS[
        (round_index + task_index - 2) % len(DEFAULT_DIMENSIONS)
    ]
...
def _employee_task_record(run_id: str, task: dict[str, Any]) -> EmployeeTaskRecord:
    ...
    return EmployeeTaskRecord(
        queue_id=str(uuid.uuid4()), task=improvement, source=run_id, status="executing"
    )
