...
def audit_feedback_closure(
    *,
    ...
    queued_ids = {
        str(row.get("task", {}).get("task_id") or row.get("task_id") or "")
        for row in queued
    }
    ...
    return {
        ...
        "result_tasks_have_queue_records": not result_ids
        or result_ids.issubset(queued_ids),
        "history_matches_employee_results": not result_ids
        or bool(result_ids & history_ids),
