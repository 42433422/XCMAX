...
def build_heterogeneous_absorption_replay(
    ...
    *,
    ...
    language_families = sorted(
        {str(row["language_family"]) for row in rows if row.get("language_family")}
    )
    source_families = sorted(
        {str(row["source_family"]) for row in rows if row.get("source_family")}
    )
    ...
    summary = {
        ...
        "pre_absorption_failure_count": sum(
            1 for row in rows if row["pre_absorption"]["failed_expected_behavior"]
        ),
        "post_absorption_pass_count": sum(
            1 for row in rows if row["post_absorption"]["passed_expected_behavior"]
        ),
        ...
        "cross_language_absorption_verified": len(language_families) >= 4
        and len(source_families) >= 4,
    }
    ready = (
...
def _evaluate_case(root: Path, case: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "before_failed_after_passed": pre["failed_expected_behavior"]
        and post["passed_expected_behavior"],
        "ready": pre["failed_expected_behavior"]
        and post["passed_expected_behavior"]
        and delta >= 35,
    }
...
def _pre_absorption_review(case: dict[str, Any]) -> dict[str, Any]:
    ...
    if any(marker in diff for marker in ("secret", "token", "apikey", "api_key")):
        ...
        marker in diff for marker in ("todo", "fixme", "stub", "bypass", "ignore")
    ):
...
def _post_absorption_review(case: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "passed_expected_behavior": severity_matched
        and context_matched
        and publishable_count > 0,
    }
...
def _adjudicate_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        checks = {
            ...
            "pre_absorption_failed": bool(
                row.get("pre_absorption", {}).get("failed_expected_behavior")
            ),
            "post_absorption_passed": bool(
                row.get("post_absorption", {}).get("passed_expected_behavior")
            ),
        ...
        adjudications.append(
            {
                ...
                "recomputed_delta": int(
                    row.get("post_absorption", {}).get("score") or 0
                )
    ...
    return {
        ...
        "summary": {
            ...
            "language_family_count": len(
                {
                    item["language_family"]
                    for item in adjudications
                    if item["language_family"]
                }
            ),
        },
