...
class RetortHistoryStore:
    ...
    def record_absorption_run(self, result: AbsorptionResult) -> int:
        return self._insert(
            "absorption_runs",
            {
                ...
                "source": result.external_ref.source,
    ...
    def record_employee_task(self, record: EmployeeTaskRecord) -> int:
        return self._insert(
            "employee_tasks",
            {
                ...
                "owner_hint": record.task.owner_hint,
    ...
    def record_task_result(self, task_result: EmployeeTaskResult) -> int:
        return self._insert(
            "task_results",
            {
                ...
                "payload_json": task_result.to_dict(),
            },
        )
    ...
    def latest_task_results(self, limit: int = 20) -> tuple[dict[str, Any], ...]:
        with sqlite3.connect(self.path) as conn:
            rows = conn.execute(
                "SELECT payload_json FROM task_results ORDER BY id DESC LIMIT ?",
                (limit,),
            ).fetchall()
