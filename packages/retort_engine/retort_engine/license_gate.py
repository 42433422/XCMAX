...
def license_gate(
    ...
    *,
    enforce: bool = False,
    allowed: tuple[str, ...] = DEFAULT_ALLOWED_LICENSES,
    blocked: tuple[str, ...] = DEFAULT_BLOCKED_LICENSES,
) -> LicenseGateResult:
    detected = detect_license(project_path)
    if not detected:
        return LicenseGateResult(
            "blocked" if enforce else "warning",
            "",
            "license-incompatible risk: no LICENSE file detected",
        )
    if any(name.lower() in detected.lower() for name in blocked):
        return LicenseGateResult(
            "blocked",
            detected,
            f"license-incompatible risk: blocked license detected: {detected}",
        )
    if any(name.lower() in detected.lower() for name in allowed):
        return LicenseGateResult(
            "passed", detected, f"Allowed license detected: {detected}"
        )
    return LicenseGateResult(
        "blocked" if enforce else "warning",
        detected,
        f"license-incompatible risk: {detected}",
    )
...
def detect_license(project_path: str | Path) -> str:
    ...
    for name in ("LICENSE", "LICENSE.md", "COPYING", "NOTICE"):
        ...
        if path.is_file():
            ...
            return next(
                (line.strip() for line in text.splitlines() if line.strip()), name
            )
    package_json = root / "package.json"
    if package_json.is_file():
        try:
            license_value = json.loads(package_json.read_text(encoding="utf-8")).get(
                "license"
            )
