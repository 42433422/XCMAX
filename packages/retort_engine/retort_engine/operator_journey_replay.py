...
def build_operator_journey_replay(
    ...
    run_id = (
        datetime.now(timezone.utc).strftime("operator-journey-%Y%m%dT%H%M%SZ")
        + "-"
        + uuid.uuid4().hex[:8]
    )
    ...
    summary: dict[str, Any] = {
        ...
        "hashed_artifact_count": sum(
            1 for item in artifacts if item["exists"] and item["sha256"]
        ),
        "source_report_count": sum(
            1
        ...
        "frontend_structure_ready": bool(
            (live_probes.get("blackhole_ui") or {}).get("ready")
        ),
        "frontend_operation_replay_ready": bool(
            (live_probes.get("blackhole_ui_operation_replay") or {}).get("ready")
        ),
        "architecture_contract_ready": bool(
            (live_probes.get("architecture_contracts") or {}).get("ready")
        ),
        "codebase_graph_ready": bool(
            (live_probes.get("codebase_graph") or {}).get("ready")
        ),
        "external_advantage_ci_ready": _report_ready(
            root, "retort_external_advantage_ci_regression.json"
        ),
        "external_process_adjudication_ready": _report_ready(
            root, "retort_external_process_adjudication.json"
        ),
        "upstream_pr_ci_ready": _report_ready(root, "retort_upstream_pr_ci_probe.json"),
        "competitor_runtime_ready": _report_ready(
            root, "retort_competitor_runtime_comparison.json"
        ),
        "competitor_blind_adjudication_ready": _report_ready(
            root, "retort_competitor_blind_adjudication.json"
        ),
        "competitor_behavior_regression_ready": _report_ready(
            root, "retort_competitor_behavior_regression.json"
        ),
        "paibi_cli_cross_adjudication_ready": _report_ready(
            root, "retort_paibi_cli_cross_adjudication.json"
        ),
        "employee_patch_stress_ready": _report_ready(
            root, "retort_employee_patch_stress.json"
        ),
        "contract_stability_ready": _report_ready(
            root, "retort_contract_stability_stress.json"
        ),
        "cross_domain_end_to_end_ready": _report_ready(
            root, "retort_cross_domain_end_to_end.json"
        ),
        "cross_domain_ci_regression_ready": _report_ready(
            root, "retort_cross_domain_ci_regression.json"
        ),
    ...
    status = (
        "ready"
        if not failed_stages and summary["hashed_artifact_count"] >= 8
        else "blocked"
    )
    result = {
        "status": status,
        ...
        "replay": {
            "command": [
                "retort",
                "operator-journey-replay",
                "--project",
                str(root),
                "--output",
                "docs/retort_operator_journey_replay.json",
            ],
            "manifest_path": str(manifest_path),
...
def _journey_stages(
    ...
    paibi_cli_cross = artifact_by_name.get("paibi_cli_cross_adjudication", {})
    ...
    return [
        ...
        _stage(
            ...
            "吸收前定位",
            external_report.get("exists") is True
            and location["ready"]
            and _code_graph_ready(latest_run),
            [
        ...
        _stage(
            "absorb_and_gate",
            "真实吸收和门禁",
            latest_run.get("gates_passed") is True
            and quality.get("exists") is True
            and mainline_proof.get("exists") is True
            and bool(latest_run.get("changed_files")),
            [
        ...
        _stage(
            ...
            "百级补丁回滚压测",
            employee_patch_stress.get("exists") is True
            and _report_ready(root, "retort_employee_patch_stress.json"),
            [
        ...
        _stage(
            "publish_or_degrade",
            "发布或权限降级",
            publish_dry_run.get("exists") is True
            and (
                readonly_probe.get("exists") is True
                or low_permission_probe.get("exists") is True
            ),
            [
        ...
        _stage(
            ...
            "外部独立性证明",
            external_process.get("exists") is True
            and upstream_ci.get("exists") is True
            and competitor_runtime.get("exists") is True
            and competitor_blind.get("exists") is True
            and competitor_behavior.get("exists") is True
            and paibi_cli_cross.get("exists") is True,
            [
                ...
                f"paibi_cli_cross_sha={paibi_cli_cross.get('sha256', '')}",
            ],
        ),
        _stage(
            "sustained_depth_gates",
            "持续深度门禁",
            external_ci.get("exists") is True
            and contract_stability.get("exists") is True
            and cross_domain_e2e.get("exists") is True
            and cross_domain_ci.get("exists") is True,
            [
...
def _artifact_manifest(root: Path, latest_run: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    specs: list[tuple[str, Path, str, tuple[str, ...]]] = [
        (
            ...
            "source_report",
            ("source", "external_snapshot", "review_pipeline"),
        ),
        (
            "quality_gate_bundle",
            docs / "retort_quality_gate_bundle.json",
            "source_report",
            ("status", "summary", "gates"),
        ),
        (
            "product_mainline_absorption_proof",
            docs / "retort_product_mainline_absorption_proof.json",
            "source_report",
            ("status", "summary", "changed_files"),
        ),
        (
            "absorption_continuity_probe",
            docs / "retort_absorption_continuity_probe.json",
            "source_report",
            ("status", "summary", "runs"),
        ),
        (
            "multi_project_absorption_replay",
            docs / "retort_multi_project_absorption_replay.json",
            "source_report",
            ("status", "summary", "projects"),
        ),
        (
            "pr_holdout_blind_eval",
            docs / "retort_pr_holdout_blind_eval.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
        (
            "pr_failure_rollback_replay",
            docs / "retort_pr_failure_rollback_replay.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
        (
            "employee_patch_closure",
            docs / "retort_employee_patch_closure.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
        (
            "employee_patch_stress",
            docs / "retort_employee_patch_stress.json",
            "source_report",
            ("status", "summary", "workers"),
        ),
        (
            "employee_scheduler_stress",
            docs / "retort_employee_scheduler_stress.json",
            "source_report",
            ("status", "summary", "rounds"),
        ),
        (
            "pr_publish_dry_run",
            docs / "retort_pr_publish_dry_run.json",
            "source_report",
            ("status", "summary", "comments"),
        ),
        (
            "pr_readonly_degradation_probe",
            docs / "retort_pr_readonly_degradation_probe.json",
            "source_report",
            ("status", "summary", "evidence"),
        ),
        (
            "pr_low_permission_probe",
            docs / "retort_pr_low_permission_probe.json",
            "source_report",
            ("status", "summary", "evidence"),
        ),
        (
            "production_recovery_drill",
            docs / "retort_production_recovery_drill.json",
            "source_report",
            ("status", "summary", "scenarios"),
        ),
        (
            "review_quality_benchmark",
            docs / "retort_review_quality_benchmark.json",
            "source_report",
            ("status", "summary", "samples"),
        ),
        (
            "external_advantage_matrix",
            docs / "retort_external_advantage_matrix.json",
            "source_report",
            ("status", "summary", "matrix"),
        ),
        (
            "external_advantage_ci_regression",
            docs / "retort_external_advantage_ci_regression.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
        (
            "external_process_adjudication",
            docs / "retort_external_process_adjudication.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
        (
            "external_advantage_repeat",
            docs / "retort_external_advantage_repeat.json",
            "source_report",
            ("status", "summary", "runs"),
        ),
        (
            "upstream_pr_ci_probe",
            docs / "retort_upstream_pr_ci_probe.json",
            "source_report",
            ("status", "summary", "check_runs"),
        ),
        (
            "competitor_runtime_comparison",
            docs / "retort_competitor_runtime_comparison.json",
            "source_report",
            ("status", "summary", "competitor_output"),
        ),
        (
            "competitor_blind_adjudication",
            docs / "retort_competitor_blind_adjudication.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
        (
            "competitor_behavior_regression",
            docs / "retort_competitor_behavior_regression.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
        (
            "paibi_cli_cross_adjudication",
            docs / "retort_paibi_cli_cross_adjudication.json",
            "source_report",
            ("status", "summary", "tool_results"),
        ),
        (
            "heterogeneous_absorption_replay",
            docs / "retort_heterogeneous_absorption_replay.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
        (
            "cross_domain_absorption_replay",
            docs / "retort_cross_domain_absorption_replay.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
        (
            "cross_domain_end_to_end",
            docs / "retort_cross_domain_end_to_end.json",
            "source_report",
            ("status", "summary", "stages"),
        ),
        (
            "cross_domain_ci_regression",
            docs / "retort_cross_domain_ci_regression.json",
            "source_report",
            ("status", "summary", "runs"),
        ),
        (
            "contract_runtime_rehearsal",
            docs / "retort_contract_runtime_rehearsal.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
        (
            "contract_stability_stress",
            docs / "retort_contract_stability_stress.json",
            "source_report",
            ("status", "summary", "runs"),
        ),
        (
            "review_family_behavior_replay",
            docs / "retort_review_family_behavior_replay.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
        (
            "external_merge_landing",
            docs / "retort_external_merge_landing.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
        (
            "review_adjudication_calibration",
            docs / "retort_review_adjudication_calibration.json",
            "source_report",
            ("status", "summary", "cases"),
        ),
    ]
    ...
    if not run_path.is_file() and latest_run:
        run_path = (
            ...
            / f"{latest_run.get('run_id')}.json"
        )
    if latest_run:
        specs.append(
            (
                "latest_real_absorption_run",
                run_path,
                "real_absorption_run",
                ("status", "summary", "changed_files", "code_graph_proof"),
            )
        )
    return [
        _artifact(root, name, path, kind, required)
        for name, path, kind, required in specs
    ]
...
def _live_cross_domain_probes(root: Path) -> dict[str, Any]:
    graph = _safe_call(
        lambda: build_codebase_graph(root, include_tests=True, max_files=400)
    )
    contracts = _safe_call(
        lambda: evaluate_architecture_contracts(root, include_tests=True, max_files=400)
    )
    ui = _safe_call(lambda: blackhole_ui_structure(root))
    ui_replay = _safe_call(lambda: blackhole_ui_operation_replay(root))
    ui_missing = list(ui.get("missing_ids") or []) + list(
        ui.get("missing_functions") or []
    )
    return {
        "codebase_graph": {
        ...
        "architecture_contracts": {
            ...
            "violation_count": (contracts.get("summary") or {}).get(
                "violation_count", 0
            ),
        },
        "blackhole_ui": {
            "ready": bool(ui.get("index_exists"))
            and bool(ui.get("app_exists"))
            and not ui_missing
            and bool(ui.get("has_animation_loop")),
        ...
        "blackhole_ui_operation_replay": {
            ...
            "ready_action_count": (ui_replay.get("summary") or {}).get(
                "ready_action_count", 0
            ),
            "all_actions_bound": (ui_replay.get("summary") or {}).get(
                "all_actions_bound", False
            ),
            "absorbed_project_click_bound": (ui_replay.get("summary") or {}).get(
                "absorbed_project_click_bound", False
            ),
        },
    }
...
def _cross_domain_ready(probes: dict[str, Any]) -> bool:
    return all(
        bool((probes.get(name) or {}).get("ready"))
        for name in (
            "codebase_graph",
            "architecture_contracts",
            "blackhole_ui",
...
def _location_evidence(run: dict[str, Any]) -> dict[str, Any]:
    ...
    has_graph_focus = bool(
        proof.get("changed_focus_files")
        or proof.get("changed_hotspots")
        or proof.get("dependency_impact")
    )
    kind = (
...
def _release_inputs_ready(root: Path) -> bool:
    ...
    paibi_cli_cross = _read_json(docs / "retort_paibi_cli_cross_adjudication.json")
    ...
    return (
        ...
        and int(benchmark.get("summary", {}).get("post_absorption_score_delta") or 0)
        > 0
        ...
        is True
        and int(
            external_matrix.get("summary", {}).get("blind_third_party_minimum_delta")
            or 0
        )
        >= 65
        ...
        and int(
            external_ci.get("summary", {}).get("blind_third_party_minimum_delta") or 0
        )
        >= 80
        and external_process.get("status") == "ready"
        and external_process.get("summary", {}).get("external_all_cases_accepted")
        is True
        and external_process.get("summary", {}).get("script_imports_retort_engine")
        is False
        ...
        and upstream_ci.get("summary", {}).get("all_target_check_runs_successful")
        is True
        ...
        is True
        and competitor_runtime.get("summary", {}).get("multi_competitor_side_by_side")
        is True
        and int(
            competitor_runtime.get("summary", {}).get("ready_competitor_project_count")
            or 0
        )
        >= 3
        and competitor_runtime.get("summary", {}).get(
        ...
        and competitor_blind.get("summary", {}).get("all_competitors_blind_accepted")
        is True
        ...
        and paibi_cli_cross.get("summary", {}).get("oracle_calibrated_cli_consensus")
        is True
        ...
        and cross_domain_replay.get("summary", {}).get("all_before_failed_after_passed")
        is True
        and cross_domain_replay.get("summary", {}).get("all_output_assertions_passed")
        is True
        and int(cross_domain_replay.get("summary", {}).get("non_pr_domain_count") or 0)
        >= 10
        and cross_domain_e2e.get("status") == "ready"
        and cross_domain_e2e.get("summary", {}).get("all_stages_chained") is True
        and cross_domain_e2e.get("summary", {}).get("all_stage_outputs_consumed")
        is True
        ...
        is True
        and contract_runtime.get("summary", {}).get("all_concurrent_rollbacks_verified")
        is True
        ...
        and review_family.get("summary", {}).get("all_direct_review_outputs_verified")
        is True
