...
from retort_engine.paibi_status import _extract_last_json_object  # noqa: F401
...
PAIBI_IDLE_TOOL_STATUSES = {"", "idle", "ready", "available"}
...
class PaibiLLMClient:
    ...
    def _device_tool_candidates(self, device: dict[str, Any]) -> list[str]:
        ...
        for tool in tools:
            ...
            if current_task or status not in PAIBI_IDLE_TOOL_STATUSES:
                continue
        ...
        if dev_tool in PAIBI_SUPPORTED_TOOLS:
            return [dev_tool]
