...
- 重点评估深度，不用功能数量堆高分。
- Retort 的运行前提是 no_human_operating_model=True：没有真人标注员或真人终审可用。不得把“缺真人/缺人工/human_reviewed=False”本身作为扣分项，也不得输出要求真人终审、人工抽检或 human_reviewed=True 的任务；必须改用多 CLI 交叉、oracle 标定、盲测、反事实样本、上游 CI 和独立子进程证据。
...
- evidence_loop_score 用于评价五证闭环完整度；capability_absorption_score 用于评价最近吸收是否真的改强核心能力。calibrated_overall 不得只因 evidence_loop_score 高而升分，但 capability_absorption_score 也不得被当作硬上限；若存在仓库外独立裁决进程、真实上游 PR/CI、竞品运行时并排输出、>100 并发回滚、跨域端到端串联和行为测试证据，可以在说明残余风险后超过 capability_absorption_score。
- 若 external_process_adjudication_imports_retort=False、upstream_pr_ci_probe_multi_repo_generalization=True、competitor_runtime_comparison_multi_side_by_side=True、competitor_runtime_comparison_ready_projects>=3、competitor_blind_adjudication_status=ready 且 competitor_blind_adjudication_imports_retort=False，同时成立，不得再把 comparative_analysis_depth 主要扣为“单竞品/同仓自证”。
- 若 competitor_behavior_regression_status=ready、competitor_behavior_regression_direct_review=True 且 competitor_behavior_regression_all_signals=True，同时成立，不得再把竞品吸收主要扣为“只停留在报告/快照”。
- 若 paibi_cli_cross_adjudication_status=ready、paibi_cli_cross_adjudication_tools=4/4、paibi_cli_cross_adjudication_all_accepted=True、paibi_cli_cross_adjudication_consensus=True、paibi_cli_cross_adjudication_input_score_fields=False、paibi_cli_cross_adjudication_imports_retort=False、paibi_cli_cross_adjudication_no_human_model=True、paibi_cli_cross_adjudication_human_required=False、paibi_cli_cross_adjudication_oracle_calibrated=True、paibi_cli_cross_adjudication_calibration_labels>=50 且 paibi_cli_cross_adjudication_calibration_pass_rate>=0.95，同时成立，应把它视为 Retort 无真人环境下的主可信评审机制；不得再以“缺真人终审 / human_reviewed=False”为主要扣分项或任务项。
- 若 absorption_state_closed_loop_completed_by_design=True、closed_loop_five_proofs_verified=True、absorption_continuity_ready_runs 至少为 5/5 且 absorption_continuity_latest_closed_loop=True，同时成立，不得把 absorption_active=False 扣为“无在线吸收”；这是单轮闭环完成后关闭状态。
- 若 employee_patch_stress_concurrent_workers>100、employee_patch_stress_all_rollbacks=True、employee_patch_stress_state_leaks=0，同时成立，不得再把 failure recovery 主要扣为“无百级回滚”。
...
- 不允许因为 evidence_loop_score 高就自动给 calibrated_overall 90+；也不允许只因 capability_absorption_score 较低就忽略独立外部证据。
...
def prioritized_evidence(evidence: list[str]) -> list[str]:
    critical_keep_prefixes = (
        ...
        "operator_journey_replay_product_mainline_",
        "absorption_release_decision_product_mainline_",
        ...
        "absorption_release_decision_competitor_behavior_",
        "paibi_cli_cross_adjudication_",
        "operator_journey_replay_paibi_cli_cross_",
        "absorption_release_decision_paibi_cli_cross_",
        "review_adjudication_no_human_",
        "review_adjudication_calibration_label_",
        "cross_domain_ci_regression_",
        "absorption_release_decision_cross_domain_ci_",
    ...
    must_keep_prefixes = (
        ...
        "external_advantage_ci_regression_",
        "external_process_adjudication_",
        "upstream_pr_ci_probe_",
        "competitor_runtime_comparison_",
        "competitor_blind_adjudication_",
        ...
        "cross_domain_end_to_end_",
        "cross_domain_ci_regression_",
        "contract_runtime_rehearsal_",
        "contract_stability_stress_",
        "review_family_behavior_replay_",
...
def _evidence_priority(item: str, prefixes: tuple[str, ...]) -> int:
