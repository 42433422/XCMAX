...
def run_live_pr_comment_probe(
    pr_url: str, *, body: str = "", token: str = "", transport: Transport | None = None
) -> dict[str, Any]:
    ...
    repo_status, repo_payload = call(
        "GET", f"https://api.github.com/repos/{owner}/{repo}", None, resolved_token
    )
    pull_status, pull_payload = call(
        "GET",
    ...
    comment_body = (
        body.strip()
        or f"Retort controlled live publish probe. marker={marker}. This comment will be deleted immediately."
    )
    ...
    if repo_status < 400 and pull_status < 400:
        ...
        if create_status < 400:
            ...
            rollback_receipts.append(
                {"created": False, "status_code": create_status, "response": created}
            )
    rollback_verified = bool(created_receipts) and len(created_receipts) == sum(
        1 for item in rollback_receipts if item.get("deleted")
    )
    can_write = any(
        bool(permission.get(key)) for key in ("admin", "maintain", "push", "triage")
    )
...
def run_readonly_pr_degradation_probe(
    pr_url: str, *, transport: Transport | None = None
) -> dict[str, Any]:
    owner, repo, number = _parse_pr_url(pr_url)
    call = transport or _github_public_request
    repo_status, _repo_payload = call(
        "GET", f"https://api.github.com/repos/{owner}/{repo}", None, ""
    )
    pull_status, pull_payload = call(
        "GET", f"https://api.github.com/repos/{owner}/{repo}/pulls/{number}", None, ""
    )
    target_found = (
        repo_status < 400 and pull_status < 400 and bool(pull_payload.get("number"))
    )
    return {
...
def run_low_permission_pr_degradation_probe(
    pr_url: str, *, transport: Transport | None = None
) -> dict[str, Any]:
    owner, repo, number = _parse_pr_url(pr_url)
    if transport is None:
        repo_status, _repo_payload = _github_public_request(
            "GET", f"https://api.github.com/repos/{owner}/{repo}", None, ""
        )
        pull_status, pull_payload = _github_public_request(
            "GET",
            f"https://api.github.com/repos/{owner}/{repo}/pulls/{number}",
            None,
            "",
        )
        create_status, created = _github_request(
            "POST",
            f"https://api.github.com/repos/{owner}/{repo}/issues/{number}/comments",
            {"body": "Retort low-permission probe should not be created."},
            "",
        )
        transport_name = "github_rest_public_read_then_unauthorized_write"
    else:
        repo_status, _repo_payload = transport(
            "GET", f"https://api.github.com/repos/{owner}/{repo}", None, ""
        )
        pull_status, pull_payload = transport(
            "GET",
            f"https://api.github.com/repos/{owner}/{repo}/pulls/{number}",
            None,
            "",
        )
        create_status, created = transport(
            "POST",
            f"https://api.github.com/repos/{owner}/{repo}/issues/{number}/comments",
            {"body": "Retort low-permission probe should not be created."},
            "",
...
def write_live_pr_comment_probe(
    pr_url: str, output: str | Path, *, body: str = ""
) -> dict[str, Any]:
...
def write_readonly_pr_degradation_probe(
    pr_url: str, output: str | Path
) -> dict[str, Any]:
    started = time.monotonic()
...
def write_low_permission_pr_degradation_probe(
    pr_url: str, output: str | Path
) -> dict[str, Any]:
    started = time.monotonic()
    result = run_low_permission_pr_degradation_probe(pr_url)
    result.setdefault("summary", {})["duration_sec"] = round(
