...
def build_product_mainline_absorption_proof(
    project: str | Path, *, output: str | Path = "", commit: str = "HEAD"
) -> dict[str, Any]:
    ...
    parents = (
        _git(["show", "-s", "--format=%P", commit_sha], repo)["stdout"].strip().split()
    )
    parent = parents[0] if parents else f"{commit_sha}^"
    changed_files = _git(
        ["diff", "--name-only", parent, commit_sha, "--", "packages/retort_engine"],
        repo,
    )["stdout"].splitlines()
    source_files = [
        item
        for item in changed_files
        if item.startswith("packages/retort_engine/retort_engine/")
        and item.endswith(".py")
    ]
    test_files = [
        item
        for item in changed_files
        if item.startswith("packages/retort_engine/tests/test_")
        and item.endswith(".py")
    ]
    docs_files = [
        item
    ...
    summary = {
        ...
        "post_merge_quality_gate_passed": quality.get("status") == "ready"
        and quality_summary.get("all_gates_passed") is True,
