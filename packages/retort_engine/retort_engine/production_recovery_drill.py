...
def build_production_recovery_drill(
    ...
    scenarios = [
        _scenario(
            "live_write_rollback",
            root / "docs" / "retort_pr_live_publish_probe.json",
            _live_write_recovered,
        ),
        _scenario(
            "readonly_degradation",
            root / "docs" / "retort_pr_readonly_degradation_probe.json",
            _readonly_recovered,
        ),
        _scenario(
            "low_permission_write_denied",
            root / "docs" / "retort_pr_low_permission_probe.json",
            _low_permission_recovered,
        ),
        _scenario(
            "sandbox_write_rollback",
            root / "docs" / "retort_pr_publish_sandbox.json",
            _sandbox_recovered,
        ),
        _scenario(
            "employee_gate_failure_rollback",
            root / "docs" / "retort_employee_patch_closure.json",
            _patch_recovered,
        ),
        _scenario(
            "quality_gate_recovery",
            root / "docs" / "retort_quality_gate_bundle.json",
            _quality_recovered,
        ),
    ]
    ...
    summary = {
        ...
        "real_network_denial_verified": any(
            item["name"] == "low_permission_write_denied"
            and item["real_network"]
            and item["recovered"]
            for item in scenarios
        ),
        "live_write_rollback_verified": any(
            item["name"] == "live_write_rollback"
            and item["live_github_write"]
            and item["rollback_verified"]
            for item in scenarios
        ),
        "rollback_scenario_count": sum(
            1 for item in scenarios if item["rollback_verified"]
        ),
        "degradation_scenario_count": sum(
            1 for item in scenarios if item["degraded_without_write"]
        ),
    }
    result = {
        "status": "ready"
        if summary["all_recovered"]
        and summary["real_network_denial_verified"]
        and summary["live_write_rollback_verified"]
...
def _scenario(name: str, path: Path, checker: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "rollback_verified": bool(
            ...
            or summary.get("failure_case_rolled_back")
            or summary.get("all_expected_outcomes_verified")
        ),
...
def _live_write_recovered(payload: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and bool(summary.get("rollback_verified"))
    )
...
def _low_permission_recovered(payload: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and bool(summary.get("permission_denied"))
        and bool(evidence.get("real_network"))
    )
