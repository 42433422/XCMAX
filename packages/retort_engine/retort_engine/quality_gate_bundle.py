...
def run_quality_gate_bundle(
    ...
    *,
    ...
    gates = [
        _command_gate(
            "lint", [python, "-m", "ruff", "check", "."], root, command_runner
        ),
        _command_gate(
            "pytest", [python, "-m", "pytest", "tests", "-q"], root, command_runner
        ),
    ...
    summary = {
        ...
        "test_density_missing_lines_to_target": density_gate.get(
            "missing_test_lines_to_target", 0
        ),
...
def _command_gate(
    name: str, command: list[str], root: Path, runner: GateRunner
) -> dict[str, Any]:
...
def _test_density_gate(root: Path) -> dict[str, Any]:
    ...
    target_lines = (
        int(source_lines * TEST_DENSITY_TARGET + 0.999) if source_lines else 0
    )
...
def _gate_ok(gates: list[dict[str, Any]], name: str) -> bool:
    return any(
        str(gate.get("name") or "") == name and bool(gate.get("ok")) for gate in gates
    )
...
def _contract_fixtures() -> dict[str, dict[str, Any]]:
    return {
        ...
        "heterogeneous_absorption_replay_result": {
            "status": "ready",
            "project": "p",
            "summary": {},
            "cases": [],
