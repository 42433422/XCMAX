...
def build_review_calibration_policy(
    ...
    adjudication_ready = (
        adjudication.get("status") == "ready"
        and float(adjudication_summary.get("pass_rate") or 0.0) >= 0.95
    )
    rollback_ready = rollback.get("status") == "ready" and bool(
        rollback_summary.get("all_failures_rolled_back")
    )
    enabled = holdout_ready and adjudication_ready and rollback_ready
    weights = (
        dict(BASE_CONTEXT_WEIGHTS)
        if enabled
        else {context: 0 for context in BASE_CONTEXT_WEIGHTS}
    )
    ...
    return {
        ...
        "summary": {
            ...
            "holdout_reviewed_pr_count": int(
                holdout_summary.get("reviewed_pr_count") or 0
            ),
            "holdout_distinct_repo_count": int(
                holdout_summary.get("distinct_repo_count") or 0
            ),
            "adjudication_pass_rate": float(
                adjudication_summary.get("pass_rate") or 0.0
            ),
            "rollback_verified_count": int(
                rollback_summary.get("rollback_verified_count") or 0
            ),
        },
        "evidence": {
            "source_reports": [
...
def calibration_summary() -> dict[str, Any]:
    ...
    return {
        ...
        "weighted_context_count": sum(
            1 for value in weights.values() if int(value or 0) > 0
        ),
