...
def file_grouping_enabled() -> bool:
    ...
    return bool(REVIEW_CONTEXT_BIAS.get("enabled")) and bool(
        signals
        & {
            "file_grouping",
            "review_pipeline",
...
def context_rank_weight(review_context: str) -> int:
    ...
    if context == "security" and signals & {"safety_policy", "static_analysis"}:
        weight += 30
    if context in {"runtime", "tests", "ci_config"} and signals & {
        "review_pipeline",
        "file_grouping",
        "diff_hunk_review",
    }:
...
def context_rank_weights() -> dict[str, int]:
    ...
    return {
        context: context_rank_weight(context)
        for context in (
            "security",
            "runtime",
            "tests",
