...
def build_absorption_review_report(
    own_project: str | Path, external_project: str | Path, tasks: list[dict[str, Any]]
) -> dict[str, Any]:
...
def build_diff_pipeline_replay(
    ...
    *,
    ...
    comments = [
        ...
        for item in review.get("comments") or []
        if isinstance(item, dict)
    ]
    task_groups = _merge_task_groups(reviews)
    publishable_comments = [
        item for item in comments if item.get("publishable") is not False
    ]
    ...
    replay_summary = {
        ...
        "absorbed_context_signal_strength": int(
            summary.get("absorbed_context_signal_strength") or 0
        ),
        "diff_grouping_depth_score": _diff_grouping_depth_score(
            summary, context_groups, selected_comments, task_groups, chunks
        ),
        ...
        "max_files_per_chunk": max_files_per_chunk,
        "max_chars_per_chunk": max_chars_per_chunk,
        "largest_chunk_file_count": max(
            [int(chunk.get("file_count") or 0) for chunk in chunks] or [0]
        ),
    }
    return {
        "status": (
        ...
        "chunks": [
            {key: value for key, value in chunk.items() if key != "diff_text"}
            for chunk in chunks
        ],
...
def chunk_unified_diff_by_review_context(
    diff_text: str, *, max_files_per_chunk: int = 8, max_chars_per_chunk: int = 30000
) -> list[dict[str, Any]]:
    ...
    ordered = sorted(
        sections,
        key=lambda item: (
            _context_priority(review_context_for_file(str(item["path"]))),
            int(item["index"]),
        ),
    )
...
def group_review_files(root: str | Path) -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        if value["files"] or value["marker_hits"]
    }


def _diff_grouping_depth_score(
    summary: dict[str, Any],
    context_groups: list[dict[str, Any]],
    publishable_comments: list[dict[str, Any]],
        ...
        if line.startswith("diff --git "):
            if current:
                sections.append(
                    {
                        "index": len(sections),
                        "path": current_path or _path_from_section(current),
                        "diff_text": "\n".join(current) + "\n",
                    }
                )
            current = [line]
...
def _diff_chunk(
    index: int, context: str, sections: list[dict[str, Any]]
) -> dict[str, Any]:
...
def _merge_review_summaries(reviews: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "absorbed_context_signal_strength": max(
            [
                int(item.get("absorbed_context_signal_strength") or 0)
                for item in summaries
            ]
            or [0]
        ),
        "ready_for_employee_tasking": any(
            bool(item.get("ready_for_employee_tasking")) for item in summaries
        ),
    }
...
def _merge_context_groups(reviews: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for review in reviews:
        for group in review.get("context_groups") or []:
            ...
            row["file_count"] = int(row["file_count"]) + int(
                group.get("file_count") or 0
            )
            row["hunk_count"] = int(row["hunk_count"]) + int(
                group.get("hunk_count") or 0
            )
            row["added_change_count"] = int(row["added_change_count"]) + int(
                group.get("added_change_count") or 0
            )
    return sorted(
        merged.values(),
        key=lambda group: (
            _context_priority(str(group["context"])),
            str(group["context"]),
        ),
    )
...
def _merge_task_groups(reviews: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for review in reviews:
        for group in review.get("task_groups") or []:
            ...
            row["comment_count"] = int(row.get("comment_count") or 0) + int(
                group.get("comment_count") or 0
            )
    return sorted(
        rows.values(),
        key=lambda group: (
            _context_priority(str(group.get("review_context") or "other")),
...
def compare_component_gaps(
    own_groups: dict[str, dict[str, Any]], external_groups: dict[str, dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    for component, external in external_groups.items():
        ...
        marker_gap = int(external.get("marker_hits") or 0) - int(
            own.get("marker_hits") or 0
        )
        ...
        gaps.append(
            {
                ...
                "representative_external_files": list(
                    (external.get("files") or [])[:5]
                ),
    ...
    return sorted(
        gaps,
        key=lambda item: (int(item["marker_gap"]), int(item["file_gap"])),
        reverse=True,
    )


def build_depth_absorption_workflow(
    own_groups: dict[str, dict[str, Any]],
    ...
    for component, external in external_groups.items():
        ...
        if (
            component in BREADTH_ONLY_COMPONENTS
            and component not in requested_components
        ):
            rejected.append(
                _rejected_component(
                    component, external, "breadth_only_for_current_phase"
                )
            )
            continue
        if component not in DEPTH_FOCUS_COMPONENTS:
            rejected.append(
                _rejected_component(component, external, "not_same_direction_depth")
            )
            continue
        if not (
            own_files
            or own_hits
            or component in requested_components
            or component in task_text
        ):
            rejected.append(
                _rejected_component(component, external, "no_internal_overlap_yet")
            )
            continue
    ...
    breadth_rejections = [
        item for item in rejected if item["reason"] == "breadth_only_for_current_phase"
    ]
    marketplace_candidates = (
        [_marketplace_candidate(item) for item in breadth_rejections]
        if MARKETPLACE_CANDIDATES_ENABLED
        else []
    )
    deferred_breadth = [
        _deferred_breadth_component(item) for item in breadth_rejections
    ]
    quality_gate = {
        ...
        "kept_breadth_component_count": len(
            [item for item in focused if item["component"] in BREADTH_ONLY_COMPONENTS]
        ),
        ...
        "all_employee_tasks_have_acceptance": all(
            bool(task.get("acceptance")) and bool(task.get("evidence_required"))
            for task in employee_tasks
        ),
    }
    quality_gate["passed"] = bool(
        quality_gate["focused_component_count"]
        >= quality_gate["minimum_focused_component_count"]
...
def _prioritize_absorptions(
    component_gaps: list[dict[str, Any]], tasks: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    task_text = " ".join(
        str(task.get("title", "")) + " " + str(task.get("why", "")) for task in tasks
    ).lower()
    prioritized = []
    for gap in component_gaps[:8]:
        component = str(gap["component"])
        priority = (
            "P0" if component in task_text or int(gap["marker_gap"]) >= 20 else "P1"
        )
        prioritized.append(
            {
                "component": component,
...
def _rejected_component(
    component: str, external: dict[str, Any], reason: str
) -> dict[str, Any]:
    return {
        "component": component,
        "reason": reason,
...
def _similarity_score(
    ...
    if external_hits:
        score += min(
            15, int((min(own_hits, external_hits) / max(external_hits, 1)) * 15)
        )
    return min(100, score)
...
def _marketplace_candidate(rejected_component: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence_required": [
            ...
            "sandboxed employee task",
            "market listing acceptance test",
        ],
...
def _benchmark_metrics(
    ...
    return {
        ...
        "minimum_expected_behavior_tests": max(
            3, min(12, len(covered) + len(task_dimensions))
        ),
    }
