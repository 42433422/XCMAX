...
def build_review_quality_benchmark(
    project: str | Path, *, sample_count: int = 80, negative_sample_count: int = 0
) -> dict[str, Any]:
    root = Path(project).expanduser().resolve()
    samples = _golden_samples(max(1, sample_count)) + _negative_samples(
        max(0, negative_sample_count)
    )
    ...
    for sample in samples:
        review = review_diff(
            str(sample["diff"]),
            max_comments=8,
            previous_diff_text=str(sample.get("previous_diff") or ""),
        )
        comments = [
            item for item in review.get("comments") or [] if isinstance(item, dict)
        ]
        baseline = _baseline_review_sample(sample)
        baseline_results.append(baseline)
        expected = [
            item
            for item in sample.get("expected_findings") or []
        ...
        if (
            ...
            and int(incremental.get("skipped_existing_change_count") or 0) > 0
        ):
            incremental_verified += 1
        if sample.get("expected_severity") == "info":
            sample_false_positives = sum(
                1
        ...
        sample_results.append(
            {
                ...
                "observed_severities": [
                    str(item.get("severity") or "") for item in comments
                ],
                ...
                "max_rank_score": max(
                    [int(item.get("rank_score") or 0) for item in comments] or [0]
                ),
    ...
    macro_pass_rate = (
        sum(float(item["pass_rate"]) for item in category_summary.values())
    ...
    return {
        ...
        "summary": {
            ...
            "incremental_sample_count": sum(
                1 for item in samples if item.get("requires_incremental_skip")
            ),
            ...
            "publishable_comment_count": sum(
                int(item.get("publishable_comment_count") or 0)
            ...
            "cross_project_publishable_comment_count": cross_summary[
...
def _run_cross_project_regression_suite() -> dict[str, Any]:
    ...
    summary = {
        ...
        "publishable_comment_count": sum(
            int(item.get("publishable_comment_count") or 0) for item in results
        ),
    }
    return {
        "status": "ready"
...
def _cross_project_family_summary(
    ...
    for result in results:
        ...
        row["publishable_comment_count"] += int(
            result.get("publishable_comment_count") or 0
        )
    ...
    for row in rows.values():
        ...
        row["pass_rate"] = (
            round(int(row["passed_count"]) / int(row["case_count"]), 4)
            if row["case_count"]
            else 0.0
        )
    ...
    return [
        {
            ...
            "expected_findings": [
                {"severity": "high", "message_keywords": ["凭证", "密钥"]}
            ],
            "issue_context": "review pull request secret safety",
            "diff": _single_add_diff(
                "pr_agent/settings.py", 'OPENAI_API_TOKEN = "live-token-value"'
            ),
        },
        {
            ...
            "diff": _single_add_diff(
                ".github/workflows/review.yml", 'REVIEWDOG_TOKEN: "review-token-value"'
            ),
        },
        {
            ...
            "expected_findings": [
                {"severity": "medium", "message_keywords": ["todo", "占位"]}
            ],
        ...
        },
        {
            ...
            "expected_findings": [
                {"severity": "low", "message_keywords": ["print", "调试"]}
            ],
            "issue_context": "coverage runner should keep output structured",
            "diff": _single_add_diff(
                "coverage/collector.py", 'print("debug collected arcs")'
            ),
        },
        {
            ...
            "diff": _single_add_diff(
                "importlinter/contracts/layers.py",
        ...
        },
        {
            ...
            "expected_findings": [
                {"severity": "info", "message_keywords": ["未发现阻断"]}
            ],
            "issue_context": "package repository context without leaking secrets",
            "diff": _single_add_diff(
                "src/context_pack.ts", "export const contextPackVersion = 2"
            ),
        },
    ]
...
def _category_summary(
    ...
    for row in rows.values():
        row["failed_sample_count"] = int(row["sample_count"]) - int(
            row["passed_sample_count"]
        )
        row["pass_rate"] = (
            round(int(row["passed_sample_count"]) / int(row["sample_count"]), 4)
            if row["sample_count"]
            else 0.0
        )
        row["recall"] = (
            round(
                int(row["matched_finding_count"]) / int(row["expected_finding_count"]),
                4,
            )
            if row["expected_finding_count"]
            else 1.0
        )
    return dict(sorted(rows.items()))
...
def _aggregate_score(
    *,
    pass_rate: float,
    macro_pass_rate: float,
    false_positive_count: int,
    incremental_verified: int,
) -> int:
    score = round(
        (
            (pass_rate * 0.55)
            + (macro_pass_rate * 0.35)
            + (min(incremental_verified, 5) / 5 * 0.10)
        )
        * 100
    )
...
def _baseline_review_sample(sample: dict[str, Any]) -> dict[str, Any]:
    ...
    expected = [
        item for item in sample.get("expected_findings") or [] if isinstance(item, dict)
    ]
    matched = False
    false_positive_count = 0
...
def _baseline_summary(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    false_positives = sum(
        int(item.get("false_positive_count") or 0) for item in results
    )
    pass_rate = passed / len(results) if results else 0.0
    aggregate_score = max(
        0, min(100, round(pass_rate * 90) - min(false_positives * 5, 50))
    )
    return {
...
def _long_line_sample(index: int) -> dict[str, Any]:
    ...
    line = "review_payload = " + json.dumps(
        {"payload": "x" * 160, "index": index}, sort_keys=True
    )
    return {
...
def _incremental_sample(index: int) -> dict[str, Any]:
    ...
    current = _multi_add_diff(
        path, [f"# TODO: old issue {index}", f'SERVICE_TOKEN_{index} = "token-{index}"']
    )
    return {
...
def _documented_key_sample(index: int) -> dict[str, Any]:
    ...
    return {
        ...
        "expected_findings": [{"severity": "info", "message_keywords": ["未发现阻断"]}],
        "diff": _single_add_diff(
            path, f"# resolve_api_key documents redacted SERVICE_TOKEN fallback {index}"
        ),
    }
...
def _fake_fixture_key_sample(index: int) -> dict[str, Any]:
    ...
    return {
        ...
        "diff": _single_add_diff(
            path,
            f'monkeypatch.setattr(resolver, "resolve_api_key", lambda *a: ("platform-key-{index}", "platform"))',
        ),
    }
