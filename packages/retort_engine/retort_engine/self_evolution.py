...
class RetortSelfEvolutionRunner:
    def __init__(
        self,
        evaluator: RetortEvaluator,
        improver: RetortImprover | None = None,
        *,
        threshold: float = 90.0,
        max_rounds: int | None = 8,
        stop_on_repeated_scores: bool = True,
    ) -> None:
    ...
    def run(self, initial_state: dict[str, Any] | None = None) -> SelfEvolutionResult:
        ...
        while True:
            ...
            tasks = build_improvement_tasks(
                assessment, questions, self.threshold, round_index
            )
            rounds.append(
                EvolutionRound(round_index, assessment, questions, tasks, passed)
            )
            if passed:
                return SelfEvolutionResult(
                    ...
                    self.max_rounds,
                    tuple(rounds),
                    "all_scores_strictly_above_threshold",
                )
            if self.max_rounds is not None and round_index >= self.max_rounds:
                return SelfEvolutionResult(
                    ...
                    self.max_rounds,
                    tuple(rounds),
                    "max_rounds_reached_before_all_scores_passed",
                )
            ...
            if self.stop_on_repeated_scores and signature in seen_signatures:
                return SelfEvolutionResult(
                    ...
                    self.max_rounds,
                    tuple(rounds),
                    "scores_repeated_without_convergence",
                )
...
def build_retort_questions(
    assessment: ProjectAssessment, threshold: float = 90.0
) -> tuple[RetortQuestion, ...]:
    return tuple(
        RetortQuestion(
            score.dimension,
            f"{score.dimension} is {score.value:.1f}. What exact evidence keeps it from being greater than {threshold:.0f}, and what is the smallest verifiable change that would raise it?",
            score.reason or "This score has not passed the Retort stop gate.",
        )
        for score in assessment.weak_scores(threshold)
    )
...
def build_improvement_tasks(
    ...
    for index, question in enumerate(questions, start=1):
        ...
        tasks.append(
            ImprovementTask(
                f"retort-r{round_index:02d}-{index:02d}-{_slug(question.dimension)}",
                f"Raise {question.dimension} above {threshold:.0f}",
                question.dimension,
                question.rationale,
                "Implement the smallest safe improvement and attach validation output.",
                f"Reassessment shows {question.dimension} > {threshold:.0f}; current score is {score.value:.1f}.",
                owner_hint_for_dimension(question.dimension),
                _priority(score.value, threshold),
            )
        )
    return tuple(tasks)
...
def state_fingerprint(state: dict[str, Any]) -> str:
    return hashlib.sha256(
        json.dumps(state, sort_keys=True, ensure_ascii=True, default=str).encode(
            "utf-8"
        )
    ).hexdigest()
