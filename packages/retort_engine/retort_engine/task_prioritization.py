...
def build_task_prioritization_report(project: str | Path) -> dict[str, Any]:
    ...
    dimension_counts = Counter(
        str(
            (
                (item.get("task") or {}) if isinstance(item.get("task"), dict) else {}
            ).get("dimension")
            or "unknown"
        )
    ...
    feedback_counts = _dimension_feedback_counts(
        employee_results, set(dimension_counts)
    )
    priorities = []
    dimensions = sorted(
        dimension_counts,
        key=lambda item: (feedback_counts.get(item, 0), dimension_counts[item]),
        reverse=True,
    )
    for dimension in dimensions:
        count = dimension_counts[dimension]
        completed_count = sum(
            1 for item in completed if dimension in json.dumps(item, ensure_ascii=False)
        )
        ...
        priorities.append(
            {
                ...
                "priority": "P0"
                if feedback_count > 0
                else ("P1" if count >= 3 else "P2"),
                "next_action": f"feedback_driven_replay_and_verify_{dimension}"
                if feedback_count > 0
                else f"replay_and_verify_{dimension}",
                "acceptance": f"Next absorption run improves or preserves {dimension} with post-tests passing.",
                "evidence_required": [
                    "queued_task_id",
                    "employee_result",
                    "post_absorption_test",
                    "replay_report",
                ],
    ...
    ready_count = sum(
        ...
        if item.get("ready_for_employee")
        and item.get("acceptance")
        and item.get("evidence_required")
    )
    feedback_priority_count = sum(
        1 for item in priorities if item.get("feedback_driven")
    )
    return {
        ...
        "summary": {
            ...
            "all_tasks_have_acceptance": bool(priorities)
            and ready_count == len(priorities),
        },
...
def _dimension_feedback_counts(
    employee_results: list[dict[str, Any]], queued_dimensions: set[str]
) -> Counter[str]:
