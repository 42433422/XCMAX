...
def test_record_post_absorption_hardening_run_materializes_latest_behavior_diff(
    ...
    (tmp_path / "docs").mkdir()
    (tmp_path / "retort_engine" / "pr_review.py").write_text(
        "def review():\n    return 'base'\n", encoding="utf-8"
    )
    (tmp_path / "tests" / "test_pr_review.py").write_text(
        "def test_review():\n    assert True\n", encoding="utf-8"
    )
    ...
    (tmp_path / "retort_engine" / "external_advantage_matrix.py").write_text(
        "def matrix():\n    return {'ready': True}\n", encoding="utf-8"
    )
    (tmp_path / "tests" / "test_external_advantage_matrix.py").write_text(
        "def test_matrix():\n    assert True\n", encoding="utf-8"
    )
    (tmp_path / "docs" / "retort_external_advantage_matrix.json").write_text(
    ...
    result = record_post_absorption_hardening_run(
        tmp_path, python_executable=sys.executable, worker_count=2
    )
    ...
    assert all(
        item["pid_cross_checked"] is True for item in process_isolation["traces"]
    )
    assert all(
        item["runtime_boundary_verified"] is True
        for item in process_isolation["traces"]
    )
    assert all(
        item["crash_isolation_verified"] is True for item in process_isolation["traces"]
    )
    assert all(
        item["crash_probe_returncode"] == 73 for item in process_isolation["traces"]
    )
