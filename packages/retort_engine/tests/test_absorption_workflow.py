...
from retort_engine.absorption_workflow import (
    ...
    is_complete_absorption_stdout_json,
    truthy,
)
...
def test_extract_json_from_stdout_prefers_last_complete_absorption_payload() -> None:
    ...
    parsed = extract_json_from_stdout(
        f"log {json.dumps(first)}\n{json.dumps(incomplete)}\nfinal {json.dumps(second)}"
    )
...
def test_extract_json_from_stdout_rejects_incomplete_absorption_payload() -> None:
    assert (
        extract_json_from_stdout(json.dumps({"status": "applied", "changed_files": []}))
        == {}
    )
...
def test_absorption_status_and_summary_are_fail_closed() -> None:
    ...
    assert (
        absorption_status(tasks, {"status": "applied"})
        == "absorption_execution_applied"
    )
    assert (
        absorption_status(tasks, {"status": "failed"}) == "absorption_execution_failed"
    )
    assert (
        absorption_status([], {"status": "disabled"}) == "no_external_advantage_found"
    )
    assert "failed" in absorption_summary(
        tasks, {"status": "failed", "summary": "boom"}
    )
