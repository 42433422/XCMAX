...
def test_architecture_memory_accumulates_sources_and_repeated_components(
    tmp_path: Path,
) -> None:
    ...
    first = build_architecture_record(
        ...
        profile={
            ...
            "signal_evidence": {
                ...
                "file_grouping": ["aider/repomap.py"],
        ...
        review_report={
            "review_pipeline": {
                "depth_absorption_workflow": {
                    "focused_components": [
                        {"component": "review_pipeline", "source_files": ["README.md"]}
                    ]
                }
            }
        },
    ...
    second = build_architecture_record(
        ...
        profile={
            ...
            "signal_evidence": {
                "review_pipeline": ["README.md"],
                "benchmarking": ["eval.py"],
                "multi_provider": ["provider.py"],
            },
        },
        review_report={
            "review_pipeline": {
        ...
        changed_files=[
            "retort_engine/architecture_memory.py",
            "tests/test_architecture_memory.py",
        ],
        gates=[{"ok": True}],
    )
...
def test_architecture_memory_marks_three_source_components_ready(
    tmp_path: Path,
) -> None:
    path = tmp_path / "retort_architecture_memory.json"
    ...
    assert (
        memory["component_index"]["review_pipeline"]["ready_for_deep_refactor"] is True
    )
...
def test_architecture_memory_counts_code_graph_proof(tmp_path: Path) -> None:
    ...
    record = build_architecture_record(
        ...
        profile={
            ...
            "signal_evidence": {"codebase_graph": ["src/graph.py"]},
        },
        review_report={
            "review_pipeline": {
                "depth_absorption_workflow": {
