...
from retort_engine.capability_audit import (
    absorption_external_projects,
    capability_absorption_audit,
    code_health,
    post_absorption_hardening_files,
    pr_review_runtime_evidence,
)


def test_capability_audit_accepts_behavior_code_and_test_absorption(
    tmp_path: Path,
) -> None:
        ...
        json.dumps(
            {
                "source": "https://github.com/example/repo",
                "changed_files": [str(source), str(test)],
                "employee_results_path": str(aggregate_result),
            }
        ),
        encoding="utf-8",
    )
    ...
    (docs / "retort_architecture_memory.json").write_text(
        json.dumps({"summary": {"source_count": 3}, "component_index": {}}),
        encoding="utf-8",
    )
...
def test_absorption_external_projects_combines_run_sources_and_architecture_memory(
    tmp_path: Path,
) -> None:
    run_dir = tmp_path / ".retort" / "real_absorption_runs"
    run_dir.mkdir(parents=True)
    (run_dir / "one.json").write_text(
        json.dumps({"source": "https://github.com/a/one"}), encoding="utf-8"
    )
    (run_dir / "duplicate.json").write_text(
        json.dumps({"source": "https://github.com/a/one"}), encoding="utf-8"
    )
    docs = tmp_path / "docs"
    docs.mkdir()
...
def test_capability_audit_counts_post_absorption_core_hardening(tmp_path: Path) -> None:
    ...
    subprocess.run(
        ["git", "config", "user.email", "retort@example.test"], cwd=tmp_path, check=True
    )
    subprocess.run(
        ["git", "config", "user.name", "Retort Test"], cwd=tmp_path, check=True
    )
    run_dir = tmp_path / ".retort" / "real_absorption_runs"
    run_dir.mkdir(parents=True)
    ...
    state.write_text(
        json.dumps(
            {"closed_loop_proof": {"evidence": [f"merge_commit={merge_commit}"]}}
        ),
        encoding="utf-8",
    )
    subprocess.run(["git", "add", "."], cwd=tmp_path, check=True)
    subprocess.run(
        ["git", "commit", "-m", "record absorption state"],
        cwd=tmp_path,
        check=True,
        stdout=subprocess.PIPE,
    )
    for rel in (
        ...
        "retort_engine/llm_absorption_evidence.py",
        ...
        path.write_text(
            "def testable():\n    return True\n"
            if rel.startswith("retort_engine/")
            else "def test_testable():\n    assert True\n",
            encoding="utf-8",
        )
    subprocess.run(["git", "add", "."], cwd=tmp_path, check=True)
    subprocess.run(
        ["git", "commit", "-m", "harden after absorption"],
        cwd=tmp_path,
        check=True,
        stdout=subprocess.PIPE,
    )
    ...
    assert (
        "retort_engine/pr_review.py"
        in audit["post_absorption_hardening"]["behavior_source_files"]
    )
...
def test_test_code_health_ignores_generated_absorption_files(tmp_path: Path) -> None:
    ...
    (tmp_path / "retort_engine" / "core.py").write_text(
        "def core():\n    return True\n", encoding="utf-8"
    )
    (tmp_path / "retort_engine" / "absorbed_capabilities.py").write_text(
        "generated = True\n", encoding="utf-8"
    )
...
def test_pr_review_runtime_evidence_reports_extension_policy_depth() -> None:
    ...
    assert {"runtime", "frontend", "ci_config", "docs", "config"}.issubset(
        set(evidence["extension_policy_review_contexts"])
    )
    assert "retort_engine/diff_extension_policy.py" in evidence["behavior_source_files"]
    assert (
        "retort_engine/cross_language_transfer.py" in evidence["behavior_source_files"]
    )
