...
from retort_engine.codebase_graph import (
    build_absorption_focus_map,
    build_codebase_graph,
    code_graph_absorption_proof,
)
...
def test_codebase_graph_tracks_imports_definitions_and_calls(tmp_path: Path) -> None:
    ...
    assert any(
        edge
        == {
            "from": "retort_engine/worker.py:run",
            "to": "retort_engine/worker.py:helper",
            "kind": "calls",
        }
    ...
    assert any(
        edge
        == {"from": "retort_engine/worker.py:run", "to": "json.dumps", "kind": "calls"}
        for edge in graph["edges"]
    )
...
def test_codebase_graph_resolves_local_dependencies_and_cycles(tmp_path: Path) -> None:
    ...
    assert graph["dependency_cycles"] == [
        ["pkg/a.py", "pkg/b.py", "pkg/c.py", "pkg/a.py"]
    ]
...
def test_absorption_focus_map_ranks_matching_graph_files(tmp_path: Path) -> None:
    ...
    (own / "retort_engine" / "other.py").write_text(
        "def other():\n    return 1\n", encoding="utf-8"
    )
    ...
    focus = build_absorption_focus_map(
        ...
        external,
        tasks=[
            {"task_id": "retort-depth-codebase-graph", "title": "Deepen codebase graph"}
        ],
...
def test_code_graph_absorption_proof_requires_focus_or_hotspot_hit(
    tmp_path: Path,
) -> None:
    ...
    missed = code_graph_absorption_proof(
        project,
        [str(project / "tests" / "test_codebase_graph.py")],
        {"own_focus_files": ["retort_engine/codebase_graph.py"]},
    )
...
def test_code_graph_absorption_proof_reports_dependency_impact(tmp_path: Path) -> None:
    ...
    assert proof["dependency_impact"]["direct_dependencies"] == {
        "pkg/b.py": ["pkg/c.py"]
    }
    assert proof["dependency_impact"]["direct_dependents"] == {"pkg/b.py": ["pkg/a.py"]}
    assert proof["dependency_impact"]["touched_cycles"] == [
        ["pkg/a.py", "pkg/b.py", "pkg/c.py", "pkg/a.py"]
    ]
