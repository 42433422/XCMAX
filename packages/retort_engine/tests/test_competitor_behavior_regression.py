...
def test_competitor_behavior_regression_turns_competitor_signals_into_review_diff_assertions(
    tmp_path: Path,
) -> None:
    ...
    assert (
        validate_contract("competitor_behavior_regression_result", result)["valid"]
        is True
    )
...
def test_competitor_behavior_regression_cli_outputs_contract(tmp_path: Path) -> None:
    completed = subprocess.run(
        [
            ...
            "-m",
            "retort_engine.cli",
            "competitor-behavior-regression",
            "--project",
            str(tmp_path),
            "--json",
        ],
    ...
    assert (
        validate_contract("competitor_behavior_regression_result", payload)["valid"]
        is True
    )
