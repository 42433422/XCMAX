...
def test_competitor_blind_adjudication_labels_retort_win_from_artifacts(
    tmp_path: Path,
) -> None:
    _write_competitor_runtime_report(tmp_path)

    result = build_competitor_blind_adjudication(
        tmp_path, run_id="unit-competitor-blind"
    )
    ...
    assert (
        validate_contract("competitor_blind_adjudication_result", result)["valid"]
        is True
    )
...
def test_competitor_blind_adjudication_cli_outputs_contract(tmp_path: Path) -> None:
    ...
    assert (
        validate_contract("competitor_blind_adjudication_result", payload)["valid"]
        is True
    )
...
def _write_competitor_runtime_report(project: Path) -> None:
    ...
    outputs = []
    for name, payload in {
        "mopemope__pr-ai-review-bot": {
            "status": "parsed",
            "hunk_count": 2,
            "hunks": [{"to": {"content": ["+debug"]}}, {"to": {"content": ["+info"]}}],
        },
        "qodo-ai__pr-agent": {
            "status": "review_signal_counted",
            "finding_count": 2,
            "findings": [{"rule": "debug"}, {"rule": "error_path"}],
        },
        "reviewdog__reviewdog": {
            "status": "diagnostics_mapped",
            "finding_count": 3,
            "diagnostics": [{"line": 5}, {"line": 84}, {"line": 86}],
        },
    }.items():
