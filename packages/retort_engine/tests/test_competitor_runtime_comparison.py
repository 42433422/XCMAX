...
def test_competitor_runtime_comparison_materializes_side_by_side_outputs(
    ...
    result = build_competitor_runtime_comparison(
        tmp_path, competitor_root=competitor, run_id="unit-competitor-runtime"
    )
    ...
    assert (
        result["retort_output"]["summary"]["external_diagnostic_ingestion"][
            "accepted_count"
        ]
        >= 1
    )
    ...
    assert (
        validate_contract("competitor_runtime_comparison_result", result)["valid"]
        is True
    )


def test_competitor_runtime_comparison_can_force_live_materialized_sources(
    ...
    monkeypatch.setattr(
        "retort_engine.competitor_runtime_comparison._gh_api", fake_gh_api
    )
    ...
    assert (
        result["summary"]["live_refresh_used_count"]
        == result["summary"]["competitor_project_count"]
    )
    assert result["summary"]["all_live_upstream_sources_materialized"] is True
    assert result["summary"]["external_diagnostic_count"] >= 1
    assert (
        result["retort_output"]["summary"]["core_review_score"][
            "external_diagnostic_core_behavior_active"
        ]
        is True
    )
    assert {
        item["source_mode"] for item in result["competitor_output"]["runtimes"]
    } == {"live_materialized"}
    assert (
        validate_contract("competitor_runtime_comparison_result", result)["valid"]
        is True
...
def test_service_exposes_competitor_runtime_comparison(tmp_path: Path) -> None:
    ...
    result = RetortService().competitor_runtime_comparison(
        {"project": str(tmp_path), "competitor_root": str(competitor)}
    )

    assert result["status"] == "ready"
    assert result["summary"]["retort_exceeds_patch_parser_by_semantic_comments"] is True
    assert (
        result["summary"]["retort_exceeds_all_competitors_by_semantic_comments"] is True
    )
...
def test_competitor_runtime_comparison_cli_outputs_contract(tmp_path: Path) -> None:
    ...
    assert (
        validate_contract("competitor_runtime_comparison_result", payload)["valid"]
        is True
    )
