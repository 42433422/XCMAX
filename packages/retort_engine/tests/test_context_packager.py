...
def test_context_pack_ranks_focus_terms_and_skips_generated_dirs(
    tmp_path: Path,
) -> None:
    write(
    ...
    write(
        ...
        "retort absorb context review security\n",
    )
...
def test_context_pack_respects_character_budget(tmp_path: Path) -> None:
    ...
    pack = build_context_pack(
        tmp_path, focus_terms=["review"], max_files=2, max_chars=80
    )
...
def test_context_pack_falls_back_to_deterministic_files_without_term_hits(
    tmp_path: Path,
) -> None:
