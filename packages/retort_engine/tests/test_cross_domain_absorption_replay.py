...
def test_cross_domain_absorption_replay_executes_non_pr_core_modules(
    tmp_path: Path,
) -> None:
    result = build_cross_domain_absorption_replay(
        tmp_path, min_domains=10, run_id="unit-cross-domain"
    )
    ...
    assert (
        ...
        in result["summary"]["direct_modules"]
    )
    assert (
    ...
    assert (
        "retort_engine.license_gate.license_gate" in result["summary"]["direct_modules"]
    )
    assert (
    ...
    assert (
        result["evidence"]["claim_boundary"]
        == "direct_core_modules_not_pr_review_manifest"
    )
    assert (
        validate_contract("cross_domain_absorption_replay_result", result)["valid"]
        is True
    )
...
def test_cross_domain_absorption_replay_writes_report(tmp_path: Path) -> None:
    ...
    result = build_cross_domain_absorption_replay(
        tmp_path, output=output, run_id="write-cross-domain"
    )
...
def test_service_exposes_cross_domain_absorption_replay(tmp_path: Path) -> None:
    result = RetortService().cross_domain_absorption_replay(
...
def test_cross_domain_absorption_replay_cli_outputs_contract(tmp_path: Path) -> None:
    ...
    assert (
        validate_contract("cross_domain_absorption_replay_result", payload)["valid"]
        is True
    )
