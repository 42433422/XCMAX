...
def test_cross_domain_absorption_replay_executes_non_pr_core_modules(tmp_path: Path) -> None:
    result = build_cross_domain_absorption_replay(tmp_path, min_domains=4, run_id="unit-cross-domain")

    assert result["status"] == "ready"
    assert result["summary"]["ready_case_count"] == 4
    assert result["summary"]["non_pr_domain_count"] == 4
...
def test_service_exposes_cross_domain_absorption_replay(tmp_path: Path) -> None:
    result = RetortService().cross_domain_absorption_replay({"project": str(tmp_path), "min_domains": 4})
