...
from retort_engine.devour_session import (
    assessment_file_count,
    assessment_score,
    build_devour_session,
)
...
def test_devour_session_keeps_flow_panels_and_improvement_proof() -> None:
    ...
    external = {
        ...
        "metadata": {"score_source": "external_evidence_only"},
    }
    own_after = {
        "project": "own",
        ...
        "metadata": {
            ...
            "capability_absorption_audit": {
                "behavior_source_files": ["retort_engine/a.py"],
                "behavior_test_files": ["tests/test_a.py"],
                "blockers": [],
            },
        },
    }
    ...
    session = build_devour_session(
        ...
        execution={
            "status": "applied",
            "summary": "ok",
            "changed_files": ["retort_engine/a.py"],
            "gates": [{"ok": True}],
            "gates_passed": True,
        },
        ...
        external_project_profile=lambda _path: {
            "benchmarking": True,
    ...
    assert (
        session["pre_dual_review"]["panels"][1]["score_status"]
        == "external_evidence_collected_needs_llm"
    )
...
def test_assessment_score_prefers_calibrated_overall_and_file_count_evidence() -> None:
    assessment = {
        "scores": [
            {"dimension": "product_level", "value": 80},
            {"dimension": "calibrated_overall", "value": 77.4},
        ],
