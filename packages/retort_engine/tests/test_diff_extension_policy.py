...
def test_extension_policy_for_holdout_project_extensions(
    path: str, family: str, context: str, risk_tag: str
) -> None:
...
def test_extension_policy_summary_exposes_cross_language_review_depth() -> None:
    ...
    assert {"runtime", "frontend", "ci_config", "docs"}.issubset(
        set(summary["review_contexts"])
    )
    assert {"ownership", "concurrency", "memory_safety", "operator_contract"}.issubset(
