...
def test_employee_scheduler_stress_verifies_queue_result_history(
    tmp_path: Path,
) -> None:
    ...
    assert (
        result["summary"]["unique_process_id_count"]
        == result["summary"]["process_invocation_count"]
    )
    assert (
        validate_contract("employee_scheduler_stress_result", result)["valid"] is True
    )


def test_employee_scheduler_stress_verifies_concurrent_workers(tmp_path: Path) -> None:
    result = run_employee_scheduler_stress(
        tmp_path, round_count=10, tasks_per_round=4, workers_per_round=2
    )
...
def test_employee_scheduler_stress_service_passes_concurrent_worker_count(
    tmp_path: Path,
) -> None:
