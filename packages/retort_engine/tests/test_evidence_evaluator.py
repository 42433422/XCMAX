...
def create_focused_tool_package(root: Path) -> None:
    ...
    write_file(
        root / "demo_tool" / "models.py",
        "class Result:\n    def to_dict(self):\n        return {}\n",
    )
    write_file(
        root / "demo_tool" / "self_evolution.py", "class TaskBacklogImprover: pass\n"
    )
    write_file(
        root / "demo_tool" / "prompts.py",
        "PROMPT = 'scores_repeated_without_convergence'\n",
    )
...
def test_focused_tool_package_returns_evidence_without_scores(tmp_path: Path) -> None:
    ...
    assessment = EvidenceProjectEvaluator().evaluate(
        {
