...
def test_evolution_map_combines_graph_proof_and_refactor_priority(
    tmp_path: Path,
) -> None:
    ...
    (project / "tests").mkdir()
    (project / "tests" / "test_codebase_graph.py").write_text(
        "def test_ok():\n    assert True\n", encoding="utf-8"
    )
        ...
        json.dumps(
            {
                ...
                "pre_absorption_focus": {
                    ...
                    "external_focus_files": ["src/graph.py"],
                },
                "code_graph_proof": {
                    "passed": True,
                    "changed_focus_files": ["retort_engine/codebase_graph.py"],
                    "summary": {"changed_focus_file_count": 1},
                },
            }
        ),
        encoding="utf-8",
    ...
    assert result["latest_absorption"]["pre_absorption_focus"]["own_focus_files"] == [
...
def test_evolution_map_does_not_treat_closed_loop_smoke_as_per_run_proof(
    tmp_path: Path,
) -> None:
    project = tmp_path / "project"
    package = project / "retort_engine"
    ...
    (project / ".retort" / "absorption_state.json").write_text(
        json.dumps(
            {
                "closed_loop_proof": {
                    "evidence": ["codebase_graph_smoke=ready_1_files_1_nodes_0_edges"]
                }
            }
        ),
        encoding="utf-8",
    ...
    (runs / "run.json").write_text(
        json.dumps({"run_id": "run-1", "status": "applied", "source": "source"}),
        encoding="utf-8",
    )
    ...
    assert (
        proof["summary"]["graph_smoke"]
        == "codebase_graph_smoke=ready_1_files_1_nodes_0_edges"
    )
