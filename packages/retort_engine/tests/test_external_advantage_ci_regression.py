...
def test_external_advantage_ci_regression_replays_all_cases_with_blind_delta(
    tmp_path: Path,
) -> None:
    ...
    assert (
        validate_contract("external_advantage_ci_regression_result", result)["valid"]
        is True
    )


def test_service_exposes_external_advantage_ci_regression(tmp_path: Path) -> None:
    result = RetortService().external_advantage_ci_regression(
...
def test_external_advantage_ci_regression_cli_outputs_contract(tmp_path: Path) -> None:
    completed = subprocess.run(
        [
            ...
            "-m",
            "retort_engine.cli",
            "external-advantage-ci-regression",
            "--project",
            str(tmp_path),
            "--json",
        ],
    ...
    assert (
        validate_contract("external_advantage_ci_regression_result", payload)["valid"]
        is True
    )
