...
def test_external_advantage_deltas_are_executable_behavior_regressions(tmp_path):
    ...
    assert (
        regression["summary"]["regression_case_count"]
        == matrix["summary"]["case_count"]
    )
    assert (
        regression["summary"]["passed_regression_case_count"]
        == matrix["summary"]["case_count"]
    )
    assert (
        regression["summary"]["direct_execution_case_count"]
        == matrix["summary"]["case_count"]
    )
...
def test_external_advantage_matrix_exposes_regression_backing(tmp_path):
    ...
    assert (
        result["summary"]["direct_regression_case_count"]
    ...
    assert (
        result["evidence"]["regression_verifier"]
        == "retort_engine.external_advantage_regression.verify_external_advantage_rows"
    )
    assert (
        result["evidence"]["regression_runtime"]
        == "retort_engine.pr_review.review_diff"
    )
    assert (
        result["evidence"]["regression_model"] == "executable_input_output_diff_replay"
    )
    assert (
        result["evidence"]["regression_test_module"]
        == "tests/test_external_advantage_regression.py"
    )
