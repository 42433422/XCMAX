...
from retort_engine.generalization_proof import (
    build_generalization_proof,
    write_generalization_proof,
)
...
def test_generalization_proof_requires_cross_language_ci_and_pr_replay(
    tmp_path: Path,
) -> None:
    ...
    _write(
        root / "docs" / "retort_complex_pr_replay.json", json.dumps({"status": "ready"})
    )
    ...
    assert {check["id"]: check["passed"] for check in proof["checks"]}[
        "pr_runtime_replay_available"
    ] is True
