...
def test_maybe_request_llm_review_returns_disabled_contract_when_not_enabled(
    tmp_path: Path,
) -> None:
...
def test_attach_llm_scoring_records_completed_scores(tmp_path: Path) -> None:
    ...
    def request_review(**kwargs):
        assert kwargs["evidence"] == [
            "source_files=1",
            "closed_loop_five_proofs_verified=True",
        ]
        return {
            "provider": "paibi",
...
def test_attach_llm_scoring_fails_closed_when_required_scores_do_not_finish(
    tmp_path: Path,
) -> None:
