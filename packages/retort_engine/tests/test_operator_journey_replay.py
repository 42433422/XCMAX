...
def test_operator_journey_replay_builds_hash_bound_end_to_end_pack(
    tmp_path: Path,
) -> None:
    _write_project_fixture(tmp_path)

    result = build_operator_journey_replay(
        tmp_path, output=tmp_path / "docs" / "retort_operator_journey_replay.json"
    )
...
def test_operator_journey_replay_blocks_without_latest_absorption_run(
    tmp_path: Path,
) -> None:
    _write_project_fixture(tmp_path)
    ...
    assert any(
        stage["name"] == "select_external_source" and not stage["ready"]
        for stage in result["stages"]
    )


def test_operator_journey_replay_accepts_post_absorption_graph_focus(
    tmp_path: Path,
) -> None:
    _write_project_fixture(tmp_path)
    ...
    payload["code_graph_proof"]["changed_focus_files"] = [
    ...
    stage = next(
        item for item in result["stages"] if item["name"] == "pre_absorption_locate"
    )
    assert result["status"] == "ready"
...
def _write_frontend(root: Path) -> None:
    ...
    body = "\n".join(
        f'<div id="{item}"></div>' for item in ids if item != "blackholeCanvas"
    )
...
def _write_docs(root: Path) -> None:
    ...
    fixtures = {
        ...
        "retort_multi_project_absorption_replay.json": {
            "status": "ready",
            "summary": {},
            "projects": [],
        },
        ...
        "retort_employee_scheduler_stress.json": {
            "status": "ready",
            "summary": {"unique_successful_process_id_count": 30},
            "rounds": [],
        },
        "retort_pr_publish_dry_run.json": {
            "status": "dry_run_ready",
            "summary": {},
            "comments": [],
        },
        "retort_pr_readonly_degradation_probe.json": {
            "status": "read_only_degraded",
            "summary": {},
            "evidence": {},
        },
        "retort_pr_low_permission_probe.json": {
        ...
        "retort_external_advantage_repeat.json": {
            "status": "ready",
            "summary": {"stable_case_set": True, "stable_score_delta": True},
            "runs": [],
        },
        ...
        "retort_absorption_release_decision.json": {
            "status": "ready",
            "summary": {"all_core_decisions_ready": True},
            "decisions": [],
        },
    }
...
def _write_latest_absorption_run(root: Path) -> None:
    ...
    payload = {
        ...
        "changed_files": [
            "retort_engine/codebase_graph.py",
            "tests/test_operator_journey_replay.py",
        ],
        "employee_results_path": str(
            root / ".retort" / "employee_results" / "fixture.json"
        ),
    ...
    (run_dir / "fixture-run.json").write_text(
        json.dumps(payload, ensure_ascii=False), encoding="utf-8"
    )
