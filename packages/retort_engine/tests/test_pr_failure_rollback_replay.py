...
def test_failure_rollback_replay_reverts_failed_candidate_for_real_prs(
    tmp_path: Path,
) -> None:
    urls = [
    ...
    result = build_pr_failure_rollback_replay(
        tmp_path, pr_urls=urls, min_cases=3, reviewer=_reviewer
    )
    ...
    assert all(
        case["candidate_commit"] != case["revert_commit"] for case in result["cases"]
    )
    assert (
        validate_contract("pr_failure_rollback_replay_result", result)["valid"] is True
    )
...
def test_failure_rollback_replay_blocks_when_pr_review_fails(tmp_path: Path) -> None:
    ...
    result = build_pr_failure_rollback_replay(
        tmp_path, pr_urls=urls, min_cases=3, reviewer=reviewer
    )
...
def test_failure_rollback_replay_writes_report(tmp_path: Path) -> None:
    ...
    result = build_pr_failure_rollback_replay(
        tmp_path, pr_urls=urls, min_cases=3, output=output, reviewer=_reviewer
    )
