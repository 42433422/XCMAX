...
def test_holdout_blind_eval_is_ready_with_distinct_unseen_prs(tmp_path: Path) -> None:
    ...
    result = build_pr_holdout_blind_eval(
        tmp_path, pr_urls=urls, target_prs=3, reviewer=_reviewer
    )
...
def test_holdout_blind_eval_records_single_pr_failure_without_aborting(
    tmp_path: Path,
) -> None:
    urls = [
    ...
    result = build_pr_holdout_blind_eval(
        tmp_path, pr_urls=urls, target_prs=3, reviewer=reviewer
    )
...
def test_holdout_blind_eval_blocks_when_too_many_prs_are_truncated(
    tmp_path: Path,
) -> None:
    ...
    result = build_pr_holdout_blind_eval(
        tmp_path, pr_urls=urls, target_prs=4, reviewer=reviewer
    )
...
def test_holdout_blind_eval_writes_output_report(tmp_path: Path) -> None:
    ...
    result = build_pr_holdout_blind_eval(
        tmp_path, pr_urls=urls, target_prs=3, output=output, reviewer=_reviewer
    )
