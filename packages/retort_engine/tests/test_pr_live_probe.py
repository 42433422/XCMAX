...
def test_low_permission_pr_degradation_probe_attempts_write_and_degrades() -> None:
    ...
    result = run_low_permission_pr_degradation_probe("https://github.com/owner/repo/pull/7", transport=transport)
    ...
    assert result["summary"]["permission_denied"] is True
    assert result["summary"]["degraded_without_write"] is True
    assert result["evidence"]["degradation"] == "write_attempt_denied_no_comment_created"
    assert ("POST", "https://api.github.com/repos/owner/repo/issues/7/comments") in calls
    assert validate_contract("pr_live_publish_probe_result", result)["valid"] is True
