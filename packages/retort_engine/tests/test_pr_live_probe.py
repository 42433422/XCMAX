...
def test_live_pr_comment_probe_creates_and_rolls_back_with_transport() -> None:
    ...
    result = run_live_pr_comment_probe(
        "https://github.com/owner/repo/pull/7", token="token", transport=transport
    )
...
def test_readonly_pr_degradation_probe_uses_no_write_path() -> None:
    ...
    result = run_readonly_pr_degradation_probe(
        "https://github.com/owner/repo/pull/7", transport=transport
    )
    ...
    assert (
        validate_contract("pr_readonly_degradation_probe_result", result)["valid"]
        is True
    )


def test_low_permission_pr_degradation_probe_attempts_write_and_degrades() -> None:
    calls: list[tuple[str, str]] = []
    ...
    result = run_low_permission_pr_degradation_probe(
        "https://github.com/owner/repo/pull/7", transport=transport
    )

    assert result["status"] == "permission_denied_degraded"
    assert result["summary"]["write_attempt_status"] == 401
    assert result["summary"]["live_github_write"] is False
    assert result["summary"]["permission_denied"] is True
    assert result["summary"]["degraded_without_write"] is True
    assert (
        result["evidence"]["degradation"] == "write_attempt_denied_no_comment_created"
    )
    assert (
        "POST",
        "https://api.github.com/repos/owner/repo/issues/7/comments",
