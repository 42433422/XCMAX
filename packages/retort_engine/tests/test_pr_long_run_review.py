...
def test_pr_long_run_review_promotes_complex_replay_to_production_run(
    tmp_path: Path,
) -> None:
...
def test_service_exposes_pr_long_run_review(tmp_path: Path) -> None:
    ...
    result = RetortService().pr_long_run_review(
        {"project": str(tmp_path), "min_prs": 10}
    )
...
def _write_long_run_inputs(root: Path) -> None:
        ...
        json.dumps(
            {
                ...
                "summary": {"permission_denied": True, "degraded_without_write": True},
    ...
    (docs / "retort_pr_publish_sandbox.json").write_text(
        json.dumps(
            {"status": "sandbox_rolled_back", "summary": {"rollback_verified": True}},
            ensure_ascii=False,
        ),
        encoding="utf-8",
    )
