...
def test_product_mainline_absorption_proof_requires_merge_with_behavior_and_tests(
    tmp_path: Path,
) -> None:
    ...
    assert (
        validate_contract("product_mainline_absorption_proof_result", result)["valid"]
        is True
    )
...
def test_service_exposes_product_mainline_absorption_proof(tmp_path: Path) -> None:
    ...
    result = RetortService().product_mainline_absorption_proof(
        {"project": str(project)}
    )
...
def test_product_mainline_absorption_proof_cli_outputs_contract(tmp_path: Path) -> None:
    ...
    assert (
        validate_contract("product_mainline_absorption_proof_result", payload)["valid"]
        is True
    )
...
def _write_git_merge_fixture(root: Path) -> Path:
    ...
    source.write_text("def value():\n    return 2\n", encoding="utf-8")
    test.write_text(
        "from retort_engine.mainline_behavior import value\n\n\ndef test_value():\n    assert value() == 2\n",
        encoding="utf-8",
    )
    _write_quality(project)
    _git(root, "add", ".")
