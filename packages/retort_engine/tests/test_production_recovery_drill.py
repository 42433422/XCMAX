...
def test_production_recovery_drill_requires_all_faults_recovered(
    tmp_path: Path,
) -> None:
    ...
    assert (
        validate_contract("production_recovery_drill_result", result)["valid"] is True
    )
...
def _write_recovery_inputs(root: Path) -> None:
        ...
        json.dumps(
            {
                ...
                "summary": {"live_github_write": True, "rollback_verified": True},
                "evidence": {"real_network": True},
            },
            ensure_ascii=False,
        ),
        encoding="utf-8",
        ...
        json.dumps(
            {
                ...
                "summary": {"degraded_without_write": True, "rollback_verified": True},
                "evidence": {"real_network": True},
            },
            ensure_ascii=False,
        ),
        ...
        json.dumps(
            {
                ...
                "summary": {"created_comment_count": 1, "rollback_verified": True},
            },
            ensure_ascii=False,
        ),
        encoding="utf-8",
        ...
        json.dumps(
            {
                "status": "ready",
                "summary": {
                    "all_expected_outcomes_verified": True,
                    "unexpected_gate_failure_count": 0,
    ...
    (docs / "retort_quality_gate_bundle.json").write_text(
        json.dumps(
            {"status": "ready", "summary": {"all_gates_passed": True}},
            ensure_ascii=False,
        ),
        encoding="utf-8",
    )
