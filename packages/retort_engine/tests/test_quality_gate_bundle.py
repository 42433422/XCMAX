...
def test_quality_gate_bundle_runs_lint_pytest_and_contracts(tmp_path: Path) -> None:
    ...
    result = run_quality_gate_bundle(
        tmp_path, python_executable="python", runner=runner
    )
...
def test_quality_gate_bundle_reports_density_target_gap(tmp_path: Path) -> None:
    ...
    density_gate = next(
        gate for gate in result["gates"] if gate["name"] == "test_density"
    )
    assert result["status"] == "failed"
...
def test_quality_gate_bundle_writes_report_and_service_exposes_it(
    tmp_path: Path,
) -> None:
    _write_density_fixture(tmp_path, source_lines=5, test_lines=4)
    ...
    result = run_quality_gate_bundle(
        tmp_path, output=output, python_executable="python", runner=runner
    )
...
def _write_density_fixture(root: Path, *, source_lines: int, test_lines: int) -> None:
    ...
    (package / "feature.py").write_text(
        "\n".join(f"VALUE_{index} = {index}" for index in range(source_lines)),
        encoding="utf-8",
    )
    (tests / "test_feature.py").write_text(
        "\n".join(f"assert {index} == {index}" for index in range(test_lines)),
        encoding="utf-8",
    )
