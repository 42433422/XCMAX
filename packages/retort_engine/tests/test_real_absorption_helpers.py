...
def test_external_profile_detects_signals_suffix_counts_and_git_revision(
    tmp_path: Path,
) -> None:
    external = tmp_path / "external"
    init_repo(external)
    write(
        external / "review" / "pipeline.py",
        "code review pipeline reviewer reflection localization\n",
    )
    write(
        external / "review" / "grouping.ts",
        "file group group files changed files diff hunk patch set\n",
    )
    write(
        external / "eval" / "bench.md", "benchmark precision recall evaluation eval\n"
    )
    write(
        external / "graph" / "codebase.py",
        "codebase graph dependency graph call graph symbol graph imports hotspot\n",
    )
    write(external / "plugins" / "action.yml", "plugin cli github action codex\n")
    write(
        external / "providers" / "models.py", "provider model openai anthropic ollama\n"
    )
    write(
        external / "security" / "rules.py",
        "static analysis security scan scanner taint rule engine vulnerability secret\n",
    )
    write(
        external / "context" / "pack.md",
        "repo map repository context codebase context context pack prompt context code digest\n",
    )
    write(
        external / "index" / "symbols.py",
        "semantic index symbol index language server definition reference xref scip lsif\n",
    )
    git(external, "add", ".")
...
def test_external_profile_ignores_runtime_and_dependency_directories(
    tmp_path: Path,
) -> None:
    external = tmp_path / "external"
...
def test_external_profile_detects_planet_frontend_visual_signals(
    tmp_path: Path,
) -> None:
    external = tmp_path / "external"
    write(
...
def test_external_profile_detects_high_fidelity_earth_visual_signals(
    tmp_path: Path,
) -> None:
    external = tmp_path / "external"
    write(
    ...
    write(
        external / "README.md",
        "day + night textures, bump elevation, clouds, and Fresnel atmosphere glow\n",
    )
...
def test_review_context_bias_generation_is_executable(tmp_path: Path) -> None:
    ...
    assert (
        module.review_context_bias()["source"] == "https://github.com/example/reviewer"
    )
...
def test_review_context_bias_test_template_uses_only_referenced_symbols() -> None:
    content = real._review_context_bias_test_content(
        "retort_engine.review_context_bias", "https://github.com/example/reviewer"
    )
...
def test_review_context_bias_generation_merges_previous_absorptions(
    tmp_path: Path,
) -> None:
...
def test_review_context_bias_is_only_written_for_context_signals() -> None:
    assert (
        real._should_absorb_review_context_bias({"signals": ["benchmarking"]}) is False
    )
    assert (
        real._should_absorb_review_context_bias({"signals": ["plugin_surface"]})
        is False
    )
    assert (
        real._should_absorb_review_context_bias({"signals": ["review_pipeline"]})
        is True
    )
    assert (
        real._should_absorb_review_context_bias({"signals": ["file_grouping"]}) is True
    )
    assert (
        real._should_absorb_review_context_bias({"signals": ["diff_hunk_review"]})
        is True
    )
    assert (
        real._should_absorb_review_context_bias({"signals": ["static_analysis"]})
        is True
    )
    assert (
        real._should_absorb_review_context_bias({"signals": ["context_packaging"]})
        is True
    )
    assert (
        real._should_absorb_review_context_bias({"signals": ["semantic_index"]}) is True
    )


def test_capability_model_is_only_written_for_review_depth_signals() -> None:
    assert (
        real._should_absorb_capability_model(
            {
                "signals": [
                    "planet_frontend",
                    "atmosphere_shader",
    ...
    assert (
        real._should_absorb_capability_model(
            {"signals": ["multi_provider", "planet_frontend"]}
        )
        is False
    )
    assert (
        real._should_absorb_capability_model({"signals": ["review_pipeline"]}) is True
    )
    ...
    assert (
        real._should_absorb_capability_model({"signals": ["static_analysis"]}) is True
    )
    assert (
        real._should_absorb_capability_model({"signals": ["context_packaging"]}) is True
    )
...
def test_frontend_visual_profile_rewrite_and_generated_test_are_executable(
    tmp_path: Path,
) -> None:
...
""",
    ...
    real._write_frontend_visual_profile(
        app,
        "run-planet",
        "https://github.com/example/planet",
        tmp_path / "external",
        profile,
    )
    test_path = tests / "test_frontend_visual_absorption.py"
    test_path.write_text(
        real._frontend_visual_test_content("https://github.com/example/planet"),
        encoding="utf-8",
    )

    text = app.read_text(encoding="utf-8")
    payload = json.loads(
        text.split("const ABSORBED_PLANET_VISUAL_PROFILE = ", 1)[1].split(
            ";\nfunction", 1
        )[0]
    )
    result = real._run_command(
        [sys.executable, "-m", "pytest", str(test_path), "-q"], tmp_path, timeout=60
    )

    assert payload["enabled"] is True
    assert payload["source"] == "https://github.com/example/planet"
    assert payload["visual_family"] == "absorbed-procedural-planet"
    assert set(payload["absorbed_signals"]) == {
        "planet_frontend",
        "atmosphere_shader",
        "procedural_surface",
        "webgl_scene",
    ...
    assert (
        payload["license_boundary"]
        == "visual principles only; no external source or texture copied"
    )
    assert result["ok"] is True, result["stderr_tail"] + result["stdout_tail"]


def test_frontend_visual_profile_enables_high_fidelity_earth_layers(
    tmp_path: Path,
) -> None:
    app = tmp_path / "retort_engine" / "frontend" / "app.js"
    write(
...
""",
    ...
    real._write_frontend_visual_profile(
        app,
        "run-earth",
        "https://github.com/example/earth",
        tmp_path / "external",
        profile,
    )
    ...
    payload = json.loads(
        ...
        .split("const ABSORBED_PLANET_VISUAL_PROFILE = ", 1)[1]
        .split(";\nfunction", 1)[0]
    )
    assert set(payload["absorbed_signals"]) >= {
        "day_night_textures",
...
""",
    ...
    write(
        external / "scripts" / "planet.js",
        "three.js WebGL scene procedural planet atmosphere shader cloud terrain noise\n",
    )
    ...
    changed = {
        Path(path).relative_to(project).as_posix() for path in result["changed_files"]
    }
...
def test_capability_module_content_exposes_depth_gate_and_mapping(
    tmp_path: Path,
) -> None:
    review_report = {
        "review_pipeline": {
            ...
            "prioritized_absorptions": [
                {"component": "review_pipeline", "priority": "P0"}
            ],
    ...
    assert [item["component"] for item in depth["ranked_focus_components"]][:1] == [
        "review_pipeline"
    ]
    assert module.marketplace_candidate_queue() == []
    assert module.deferred_breadth_queue()[0]["component"] == "plugin_surface"
    assert (
        module.depth_first_task_queue()[0]["task_id"] == "retort-depth-review-pipeline"
    )
    assert module.ranked_capabilities()[0]["signal"] in {
        "review_pipeline",
        "diff_hunk_review",
        "file_grouping",
    }
    assert (
        module.review_strategy_for_file("src/review.py")["strategy"]
        == "diff_hunk_review"
    )
    assert (
        module.multi_project_reproduction_index(["a", "b", "a"])[
            "ready_for_product_score"
        ]
        is False
    )
...
def test_capability_test_content_targets_generated_module_source() -> None:
    content = real._capability_test_content(
        "retort_engine.absorbed_capabilities", "https://github.com/example/reviewer"
    )

    assert (
        "EXPECTED_ABSORPTION_SOURCE = 'https://github.com/example/reviewer'" in content
    )
...
def test_capability_import_name_handles_package_and_root_modules(
    tmp_path: Path,
) -> None:
    ...
    assert (
        real._capability_import_name(tmp_path, package / "absorbed_capabilities.py")
        == "retort_engine.absorbed_capabilities"
    )
    assert (
        real._capability_import_name(
            tmp_path, tmp_path / "tests" / "test_absorbed_capabilities.py"
        )
        == "test_absorbed_capabilities"
    )
    assert (
        real._capability_import_name(tmp_path, tmp_path / "absorbed_capabilities.py")
...
def test_execution_result_and_run_id_are_stable_shape(tmp_path: Path) -> None:
    ...
    assert run_id.endswith(
        "-" + real.hashlib.sha1(b"https://github.com/example/reviewer").hexdigest()[:10]
    )


def test_run_command_captures_success_failure_and_tail(tmp_path: Path) -> None:
    success = real._run_command(
        [sys.executable, "-c", "print('ok')"], tmp_path, timeout=30
    )
    failure = real._run_command(
...
def test_git_diff_summary_falls_back_to_line_counts_for_unstaged_new_file(
    tmp_path: Path,
) -> None:
    init_repo(tmp_path)
