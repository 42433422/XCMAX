...
def test_real_absorption_records_per_run_code_graph_proof(tmp_path: Path) -> None:
    ...
    (external / "src" / "review.py").write_text(
        "review pipeline changed files diff hunk benchmark provider plugin\n",
        encoding="utf-8",
    )
    ...
    assert any(
        gate["command"][:2] == ["retort", "verify-per-run-code-graph-proof"]
        and gate["ok"]
        for gate in result["gates"]
    )


def test_real_absorption_run_record_rejects_missing_per_run_proof(
    tmp_path: Path,
) -> None:
    with pytest.raises(RuntimeError, match="missing per-run code graph proof"):
        record_real_absorption_run(
            tmp_path, {"run_id": "run-1", "code_graph_proof": {"run_id": "other"}}
        )
