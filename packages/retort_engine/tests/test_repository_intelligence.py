...
def test_compare_repository_gaps_prefers_graph_decision(tmp_path: Path) -> None:
    ...
    (external / "a" / "mod.py").write_text("def absorb():\n    return 1\n\ndef agent_loop():\n    pass\n", encoding="utf-8")
...
def test_tasks_from_repository_gaps_include_missing_symbols_and_targets(tmp_path: Path) -> None:
    own = tmp_path / "own"
    external = tmp_path / "ext"
    ...
    own_map = build_ranked_repository_map(own, focus_terms=("absorb", "agent"), max_files=8)
    gap = compare_repository_gaps(own, external, focus_terms=("absorb", "agent"))
