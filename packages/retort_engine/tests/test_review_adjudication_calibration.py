...
def test_review_adjudication_calibration_runs_no_human_oracle_cases(
    tmp_path: Path,
) -> None:
    ...
    assert (
        result["summary"]["pre_calibration_pass_rate"]
        < result["summary"]["post_calibration_pass_rate"]
    )
    ...
    assert (
        result["summary"]["pre_calibration_false_negative_count"]
        > result["summary"]["false_negative_count"]
    )
    assert (
        result["summary"]["pre_calibration_false_positive_count"]
        > result["summary"]["false_positive_count"]
    )
    ...
    assert any(
        case["pre_calibration"]["passed"] is False
        and case["post_calibration"]["passed"] is True
        for case in result["cases"]
    )
    assert {"security", "tests", "ci_config", "runtime"} <= set(
        result["summary"]["contexts"]
    )
    assert output.is_file()
    assert (
        validate_contract("review_adjudication_calibration_result", result)["valid"]
        is True
    )
