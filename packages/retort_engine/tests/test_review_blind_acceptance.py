from retort_engine.review_blind_acceptance import PASS_RATE_FLOOR, build_review_blind_acceptance
...
def test_review_blind_manifest_is_sealed() -> None:
    ...
    assert "competitor_blind" not in str(result["evidence"]).lower() or result["summary"]["competitor_blind_excluded"]
