...
from retort_engine.review_pipeline import (
    build_absorption_review_report,
    build_depth_absorption_workflow,
    build_diff_pipeline_replay,
    chunk_unified_diff_by_review_context,
...
def test_review_pipeline_groups_external_advantages(tmp_path: Path) -> None:
    ...
    write(
        external / "internal" / "review.ts",
        "review pipeline reflection localization changed files patch set\n",
    )
    write(
        external / "internal" / "providers.ts",
        "provider model openai anthropic ollama plugin extension\n",
    )
    write(external / "bench" / "eval.md", "benchmark precision recall evaluation\n")

    groups = group_review_files(external)
    report = build_absorption_review_report(
        own,
        external,
        [
            {
    ...
    assert all(
        item["component"] not in {"provider_surface", "plugin_surface"}
        for item in workflow["focused_components"]
    )
    assert {item["component"] for item in workflow["rejected_breadth_components"]} & {
        "provider_surface",
        "plugin_surface",
    }
    assert all(
        task["acceptance"] and task["evidence_required"]
        for task in workflow["employee_tasks"]
    )
    assert workflow["quality_gate"]["marketplace_candidate_count"] == len(
        workflow["marketplace_candidates"]
    )
    assert workflow["marketplace_candidates_enabled"] is False
    assert workflow["marketplace_candidates"] == []
    assert {item["component"] for item in workflow["deferred_breadth_components"]} & {
        "provider_surface",
        "plugin_surface",
    }
    assert all(
        item["status"] == "closed_until_similarity_saturation"
        for item in workflow["deferred_breadth_components"]
    )


def test_group_review_files_skips_runtime_and_dependency_directories(
    tmp_path: Path,
) -> None:
...
def test_compare_component_gaps_sorts_by_marker_then_file_gap() -> None:
    ...
    external = {
        "review_pipeline": {
            "files": ["external/review.py", "external/pipeline.py"],
            "marker_hits": 20,
        },
        "benchmark_eval": {"files": ["external/bench.py"], "marker_hits": 8},
        "workflow_ci": {
            "files": ["external/ci.yml", "external/gate.py"],
            "marker_hits": 8,
        },
    ...
    assert [gap["component"] for gap in gaps] == [
        "review_pipeline",
        "workflow_ci",
        "benchmark_eval",
    ]
    ...
    assert gaps[1]["representative_external_files"] == [
        "external/ci.yml",
        "external/gate.py",
    ]
...
def test_depth_workflow_focuses_overlap_and_rejects_breadth() -> None:
    ...
    rejected = {
        item["component"]: item for item in workflow["rejected_breadth_components"]
    }
    assert workflow["focus_mode"] == "similar_function_depth_only"
    assert set(focused) >= {
        "review_pipeline",
    ...
    assert (
        focused["review_pipeline"]["similarity_score"]
        >= focused["workflow_ci"]["similarity_score"]
    )
...
def test_depth_workflow_rejects_non_same_direction_components_even_when_requested() -> (
    None
):
    workflow = build_depth_absorption_workflow(
        {},
...
def test_build_absorption_review_report_preserves_pipeline_contract(
    tmp_path: Path,
) -> None:
    own = tmp_path / "own"
    external = tmp_path / "external"
    write(
        own / "retort_engine" / "pr_review.py", "code review diff hunk workflow gate\n"
    )
    write(own / "tests" / "test_pr_review.py", "def test_review(): pass\n")
    write(
        external / "review" / "pipeline.py",
        "code review reflection localization diff hunk changed lines\n",
    )
    write(
        external / "review" / "grouping.py",
        "file group group files related files changed files pathspec\n",
    )
    write(
        external / "eval" / "benchmark.py",
        "benchmark precision recall evaluation eval\n",
    )
    write(external / "ops" / "ci.yml", "workflow pipeline ci gate test\n")
    write(
        external / "security" / "license.py",
        "license security policy permission sandbox\n",
    )
    write(external / "plugins" / "action.yml", "plugin extension github action codex\n")
    tasks = [
        {
            "task_id": "depth",
            "title": "deep review pipeline",
            "dimension": "comparative_analysis_depth",
            "why": "same direction",
        },
        {
    ...
    assert all(
        item["acceptance"].endswith("not only a recorded signal.")
        for item in report["prioritized_absorptions"]
    )
    ...
    workflow = report["depth_absorption_workflow"]
    assert workflow["quality_gate"]["passed"] is True
    assert {item["component"] for item in workflow["focused_components"]} >= {
        "review_pipeline",
        "diff_hunk_review",
        "file_grouping",
    }
...
"""
    ...
    replay = build_diff_pipeline_replay(
        diff, issue_context="Fix login token handling", max_comments=10
    )
    ...
    assert {item["context"] for item in replay["context_groups"]} >= {
        "security",
        "tests",
        "ci_config",
    }
    ...
    assert (
        replay["evidence"]["core_behavior"]
        == "diff_grouping_to_publishable_review_and_employee_tasking"
    )
    assert (
        validate_contract("review_pipeline_diff_replay_result", replay)["valid"] is True
    )
...
def test_diff_pipeline_replay_chunks_large_diff_by_context() -> None:
    ...
    for index in range(4):
        sections.append(
            _diff_section(
                f"app/auth_{index}.py", f"def login_{index}():", '+    token = "secret"'
            )
        )
    for index in range(4):
        sections.append(
            _diff_section(
                f"tests/test_auth_{index}.py",
                f"def test_login_{index}():",
                "+    assert True",
            )
        )
    for index in range(4):
        sections.append(
            _diff_section(f"docs/change_{index}.md", "# Change", "+Documented rollout")
        )
    diff = "".join(sections)
    ...
    assert {chunk["context"] for chunk in replay["chunks"]} >= {
        "security",
        "tests",
        "docs",
    }
