...
def test_real_package_self_depth_can_unlock_other_modules() -> None:
    ...
    if report["external_improvement_allowed"]:
        assert external_improvement_gate(root, root / "other-module-placeholder")["status"] == "allowed"
...
def test_apply_and_core_absorb_share_self_depth_gate(tmp_path: Path) -> None:
    ...
    with patch("retort_engine.core.external_improvement_gate", return_value=blocked):
        core_result = absorb({"own_project": str(other), "github_url": "https://github.com/example/x"})
    assert core_result["status"] == "blocked_by_self_depth_gate"

    with patch("retort_engine.absorption.external_improvement_gate", return_value=blocked):
        runner = RetortAbsorptionRunner()
        result = runner.run(
            own_project=str(other),
            external_ref=ExternalProjectRef(source="https://github.com/example/x", source_type="path", local_path=str(other)),
        )
