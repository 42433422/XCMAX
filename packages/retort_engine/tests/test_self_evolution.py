...
class ScriptedEvaluator:
    ...
    def evaluate(self, state: dict[str, Any]) -> ProjectAssessment:
        ...
        self.calls += 1
        scores = tuple(
            Score(dim, value, f"{dim} reason")
            for dim, value in self.score_sets[index].items()
        )
        return ProjectAssessment("demo", scores, "scripted")


def test_score_90_does_not_converge_but_91_does() -> None:
    result = RetortSelfEvolutionRunner(
        ScriptedEvaluator([{"project_level": 90.0}, {"project_level": 91.0}]),
        threshold=90,
        max_rounds=3,
    ).run({})
...
def test_blocks_when_scores_repeat_without_convergence() -> None:
    result = RetortSelfEvolutionRunner(
        ScriptedEvaluator([{"project_level": 60.0}, {"project_level": 60.0}]),
        threshold=90,
        max_rounds=None,
    ).run({})
...
def test_generates_tasks_for_weak_scores() -> None:
    result = RetortSelfEvolutionRunner(
        ScriptedEvaluator([{"architecture": 72.0, "docs": 96.0}]),
        threshold=90,
        max_rounds=1,
    ).run({})
