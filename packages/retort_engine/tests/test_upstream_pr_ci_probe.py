...
from retort_engine.upstream_pr_ci_probe import (
    DEFAULT_UPSTREAM_PR_TARGETS,
    build_upstream_pr_ci_probe,
)


def test_upstream_pr_ci_probe_accepts_multi_repo_merged_prs_with_successful_checks(
    tmp_path: Path,
) -> None:
    ...
    assert set(result["summary"]["target_language_families"]) >= {
        "python",
        "go",
        "typescript",
        "rust",
...
def test_upstream_pr_ci_probe_keeps_single_repo_compatibility(tmp_path: Path) -> None:
    result = build_upstream_pr_ci_probe(
        tmp_path, repo="psf/requests", pr_number=7536, fetcher=_fetcher
    )
...
def test_service_exposes_upstream_pr_ci_probe_with_real_defaults_shape(
...
def _fetcher(path: str) -> dict:
    target_by_path = {
        f"repos/{item['repo']}/pulls/{item['pr_number']}": item
        for item in DEFAULT_UPSTREAM_PR_TARGETS
    }
