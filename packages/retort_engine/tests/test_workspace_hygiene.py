...
from retort_engine.workspace_hygiene import clean_workspace, close_run_workspace, workspace_residue_report


def test_clean_workspace_removes_ephemeral_residue_keeps_durable_state(tmp_path: Path) -> None:
    runtime = tmp_path / ".retort"
    (runtime / "cache" / "github" / "o" / "r").mkdir(parents=True)
    (runtime / "cache" / "github" / "o" / "r" / "README.md").write_text("x", encoding="utf-8")
    ...
    (runtime / "absorption_state.json").write_text('{"active": false}', encoding="utf-8")
...
def test_apply_real_absorption_closes_workspace_by_default(tmp_path: Path) -> None:
    ...
    external.mkdir()
    (external / "README.md").write_text("review pipeline changed files benchmark\n", encoding="utf-8")
    ...
    result = apply_real_absorption(
        {
            ...
            "tasks": [{"task_id": "t1", "title": "Absorb", "dimension": "comparative_analysis_depth", "priority": "P1"}],
...
def test_close_run_workspace_reports_closed_status(tmp_path: Path) -> None:
    ...
    (tmp_path / ".retort" / "trajectories" / "x.json").write_text("{}", encoding="utf-8")
