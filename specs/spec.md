...
| 移动 AI 协同 App | P2 配套线 | `FHD/mobile-flutter/`（Flutter 主线，Android + iOS 统一）只做桌面端协同，不复刻完整 ERP |
...
|------|---------------------|----------|-------------|-------------|
| Python 测试覆盖率 | 全量 `app` **HEAD 85.07% 行**；WIP **74.56%**（2026-06-17 快照，红灯已收口） | 全量 **≥90% 行 / ≥85% 分支** | 全局 floor **42%** | 全局 **≥55%** |
...
**现状**（2026-06-20）：**全量** `source=[app]` + branch；HEAD **85.07% 行 / 74.22% 分支**（`bb5e15a7` 全绿 bump）；WIP **74.56% 行**（2026-06-17 快照，196 红灯已于 2026-06-20 复核收口）。棘轮 floor：行 **84%**、分支 **73%**。前端 HEAD **90.03% 行**；vitest **1,143+** 测例。详见 `FHD/docs/reports/COVERAGE_RAMP.md`。
...
- [x] HEAD 行覆盖 ≥50%（2026-06-20 已达 85.07%）
