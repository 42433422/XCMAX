...
该脚本会为 `modstore` 写入 drop-in（含 `MODSTORE_RUN_BACKGROUND_JOBS=0`），并安装、enable、`restart` `**modstore-scheduler**`。3. 验证：`systemctl is-active modstore-scheduler`；`journalctl -u modstore-scheduler -f`。
