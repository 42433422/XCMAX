...
FHD 约定：**Mod 业务代码只允许** `from app.mod_sdk.<子模块> import ...`（见 [`FHD/app/mod_sdk/__init__.py`](e:\FHD\app\mod_sdk__init__.py)）。常见子模块：
...
_文档版本：与仓库 `WorkbenchHomeView.vue` / `ModAuthoringView.vue` / `app.mod_sdk` 对接策略对齐；若后端编排契约变更，请同步更新「编排与进度」与 API 小节。_
