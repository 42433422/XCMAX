...
describe('auth api', () => {
  ...
  it('submitLandingContact fills defaults for optional fields', async () => {
    ...
    expect(body.privacy_agreed).toBe(false)
    expect(body.privacy_version).toBe('2026-06-20')
    expect(body.privacy_url).toBe('/privacy.html')
  })

  it('submitLandingContact uses provided optional fields', async () => {
    vi.mocked(req).mockResolvedValue({})
    await auth.submitLandingContact({
      name: 'n',
      email: 'e@e.com',
      phone: '1',
      company: 'c',
    ...
    expect(body.privacy_agreed).toBe(true)
  })
