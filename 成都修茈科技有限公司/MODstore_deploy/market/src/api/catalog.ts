...
import type { CatalogDetail, CatalogItem } from '../domain/catalog/types'

export interface MarketCatalogResponse extends Record<string, unknown> {
  items: CatalogItem[]
  total?: number
}

export interface CatalogFacetsResponse extends Record<string, unknown> {
  industries?: string[]
  artifacts?: string[]
...
export interface CatalogActionResponse extends Record<string, unknown> {
  ok?: boolean
  message?: string
  favorited?: boolean
}

export interface MyStoreResponse extends Record<string, unknown> {
  items: Array<{
...
export const catalog = {
  catalog: (
    ...
    return req<MarketCatalogResponse>(`/api/market/catalog?${p}`)
  },
  ...
  catalogFacets: () => req<CatalogFacetsResponse>('/api/market/facets'),
  catalogDetail: (id: string | number) => req<CatalogDetail>(`/api/market/catalog/${encodeURIComponent(String(id))}`),
