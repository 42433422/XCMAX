...
export const workbench = {
  ...
  workbenchStartSession: (body: unknown) =>
    req<WorkbenchSessionResponse>('/api/workbench/sessions', {
      method: 'POST',
  ...
  workbenchStartScriptSession: (metadata: unknown, files: File[]) => {
    ...
    for (const f of files || []) fd.append('files', f)
    return req<WorkbenchSessionResponse>('/api/workbench/script-sessions', {
      method: 'POST',
      body: fd,
    })
  },
  ...
  refineSystemPrompt: (body: { current_prompt: string; instruction: string; role_context?: string; provider?: string; model?: string }) =>
    req<{ improved_prompt?: string; diff_explanation?: string }>('/api/workbench/employee-ai/refine-prompt', {
      method: 'POST',
      body: JSON.stringify(body),
    }),
...
export const knowledge = {
  ...
  knowledgeUploadDocument: (file: File, opts?: { embeddingProvider?: string; embeddingModel?: string }) => {
    ...
    return req<KnowledgeDocumentResponse>('/api/knowledge/documents', {
      method: 'POST',
      body: form,
    })
  },
  ...
  knowledgeSearch: (query: string, limit = 6, opts?: { embeddingProvider?: string; embeddingModel?: string }) =>
    req<KnowledgeRetrieveResponse>('/api/knowledge/search', {
      method: 'POST',
      body: JSON.stringify({
        query,
        limit,
  ...
  knowledgeV2UploadDocument: (id: number, file: File, opts?: { embeddingProvider?: string; embeddingModel?: string }) => {
    ...
    if (opts?.embeddingProvider) form.append('embedding_provider', opts.embeddingProvider)
    if (opts?.embeddingModel) form.append('embedding_model', opts.embeddingModel)
    return req(`/api/knowledge/v2/collections/${encodeURIComponent(String(id))}/documents`, {
      method: 'POST',
      body: form,
    })
  },
  ...
  knowledgeV2Unshare: (id: number, membershipId: number) =>
    req(`/api/knowledge/v2/collections/${encodeURIComponent(String(id))}/share/${encodeURIComponent(String(membershipId))}`, {
      method: 'DELETE',
    }),
  knowledgeV2Retrieve: (body: {
    ...
    req<KnowledgeRetrieveResponse>('/api/knowledge/v2/retrieve', {
      method: 'POST',
      body: JSON.stringify(body),
    }),
}
...
export const openApiConnectors = {
  ...
  openApiToggleOperation: (id: number | string, operationId: string, enabled: boolean) =>
    req(`/api/openapi-connectors/${encodeURIComponent(String(id))}/operations/${encodeURIComponent(operationId)}`, {
      method: 'PATCH',
      body: JSON.stringify({ enabled }),
    }),
  openApiTestOperation: (id: number | string, operationId: string, payload: unknown) =>
    req(`/api/openapi-connectors/${encodeURIComponent(String(id))}/operations/${encodeURIComponent(operationId)}/test`, {
      method: 'POST',
...
export const customerService = {
  ...
  customerServiceCreateIntegration: (payload: unknown) =>
    req('/api/customer-service/integrations', {
      method: 'POST',
      body: JSON.stringify(payload || {}),
    }),
...
export const butler = {
  ...
  agentCorpIntakeFill: (payload: { message: string; current_draft?: Record<string, unknown>; page_summary?: string }) =>
    req('/api/agent/butler/corp-intake-fill', { method: 'POST', body: JSON.stringify(payload) }),
  ...
  agentButlerChatStream: (payload: { messages: unknown[]; conversation_id?: number | null; page_context?: string }, signal?: AbortSignal) =>
    requestStreamResponse('/api/agent/butler/chat/stream', {
      method: 'POST',
      signal,
      headers: {
  ...
  csSsotRetrieve: (payload: { query: string; top_k?: number }) =>
    req<KnowledgeRetrieveResponse>('/api/agent/butler/cs-ssot/retrieve', {
      method: 'POST',
      body: JSON.stringify(payload),
    }),
