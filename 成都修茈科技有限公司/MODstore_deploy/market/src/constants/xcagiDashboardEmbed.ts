...
export function xcagiAibizDashboardUrl(): string {
  const base = (import.meta.env.VITE_XCAGI_DASHBOARD_ORIGIN as string | undefined)?.trim().replace(/\/$/, '')
  const origin = base || (typeof window !== 'undefined' && window.location?.origin ? window.location.origin.replace(/\/$/, '') : '')
  if (origin) {
