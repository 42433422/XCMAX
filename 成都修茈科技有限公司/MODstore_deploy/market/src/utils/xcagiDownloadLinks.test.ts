...
describe('xcagiDownloadLinks', () => {
  ...
  it('defaults to the v10 (10.0.0) dual-SKU release root', () => {
    expect(normalizeXcagiDownloadBase(undefined)).toBe('https://xiu-ci.com/xcagi-v10.0.0')
  })

  it('exposes the official manifest URL pinned to v10', () => {
    expect(OFFICIAL_MANIFEST_URL).toBe('https://xiu-ci.com/xcagi-v10.0.0/manifest.json')
  })
  ...
  it('builds personal and enterprise Windows URLs without offline SKU', () => {
    ...
    expect(xcagiDownloadUrl('personal', 'win', base)).toBe(
      'https://xiu-ci.com/releases/stable/personal/XCAGI-Personal-Setup-10.0.0-x64.exe',
    )
    expect(xcagiDownloadUrl('enterprise', 'win', base)).toBe(
      'https://xiu-ci.com/releases/stable/enterprise/XCAGI-Enterprise-Setup-10.0.0-x64.exe',
    )
  ...
  it('builds personal and enterprise macOS dmg URLs for x64 and arm64', () => {
    ...
    expect(xcagiDownloadUrl('personal', 'mac', base, '8.1.0', '1.5.0', 'arm64')).toBe(
      'https://xiu-ci.com/xcagi-v8.1.0/personal/XCAGI-8.1.0-mac-arm64.dmg',
    )
    expect(xcagiDownloadUrl('enterprise', 'mac', base, '8.1.0', '1.5.0', 'x64')).toBe(
      'https://xiu-ci.com/xcagi-v8.1.0/enterprise/XCAGI-8.1.0-mac-x64.dmg',
    )
  })
...
const mockManifest: XcagiDownloadManifest = {
  ...
  channels: {
    auto_update: {
      ...
      personal: {
        win: {
          url: 'https://xiu-ci.com/releases/stable/personal/XCAGI-Personal-Setup-10.0.0-x64.exe',
      ...
      enterprise: {
        win: {
          url: 'https://xiu-ci.com/releases/stable/enterprise/XCAGI-Enterprise-Setup-10.0.0-x64.exe',
    ...
    official_download: {
      ...
      personal: {
        win: {
          url: 'https://xiu-ci.com/xcagi-v10.0.0/personal/XCAGI-Personal-Setup-10.0.0-x64.exe',
          filename: 'XCAGI-Personal-Setup-10.0.0-x64.exe',
          sha256: 'aaaa',
          size: 1000,
          platform_label: 'Windows x64',
        ...
        mac: [
          {
            url: 'https://xiu-ci.com/xcagi-v10.0.0/personal/XCAGI-10.0.0-mac-arm64.dmg',
...
describe('fetchDownloadManifest', () => {
  ...
  it('returns null on schema mismatch', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => ({
      ok: true,
      status: 200,
      json: async () => ({ schema: 'wrong', version: '10.0.0' }),
    })))
    const m = await fetchDownloadManifest('https://example.com/manifest.json', { force: true })
    expect(m).toBeNull()
  })
...
describe('findManifestEntry', () => {
  it('finds Windows entry for personal SKU', () => {
    ...
    expect(entry?.filename).toBe('XCAGI-Personal-Setup-10.0.0-x64.exe')
...
describe('resolveDownloadEntry', () => {
  it('uses manifest entry when available', async () => {
    ...
    expect(entry.url).toBe(
      'https://xiu-ci.com/xcagi-v10.0.0/enterprise/XCAGI-Enterprise-Setup-10.0.0-x64.exe',
    )
  ...
  it('falls back to static URL when manifest fetch fails', async () => {
    ...
    expect(entry.url).toBe(
      'https://xiu-ci.com/xcagi-v10.0.0/personal/XCAGI-Personal-Setup-10.0.0-x64.exe',
    )
