...
def cmd_export_fhd_shell(ns: argparse.Namespace) -> int:
    lib = _library_path(ns)
    if ns.dry_run:
        rows = build_fhd_shell_mod_rows(lib)
        print(json.dumps(rows, ensure_ascii=False, indent=2))
        return 0
    if ns.output:
        out = Path(ns.output).expanduser().resolve()
    else:
        out = project_root().parent / "backend" / "shell" / "fhd_shell_mods.json"
    n = write_fhd_shell_mods_json(lib, out)
    print(f"已写入 {n} 条 -> {out}")
    return 0


def cmd_migrate_db_into_mod(ns: argparse.Namespace) -> int:
    lib = _library_path(ns)
    mod_dir = (lib / ns.mod_id).resolve()
    sources: list[Path] = []
    for f in ns.from_file or []:
        sources.append(Path(f))
    if ns.from_dir:
        sources.extend(collect_sources_from_dir(Path(ns.from_dir), ns.glob_pat))
    if not sources:
        print("请至少指定 --from（可多次）或 --from-dir", file=sys.stderr)
        return 1
    try:
        out = copy_databases_into_mod(
            mod_dir,
            sources,
            dest_subdir=ns.dest_subdir,
            dry_run=ns.dry_run,
        )
...
def cmd_publish(ns: argparse.Namespace) -> int:
    return main_publish(ns.zip, ns.catalog_url, ns.token)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp = sub.add_parser(
        "validate", parents=[lib_arg], help="校验 manifest（可指定路径或扫描整个库）"
    )
    ...
    sp = sub.add_parser(
        "export-fhd-shell",
        parents=[lib_arg],
        help="从库生成 FHD backend/shell/fhd_shell_mods.json（供 GET /api/mods 读取）",
    )
    sp.set_defaults(func=cmd_export_fhd_shell)
    sp.add_argument(
        "-o",
        "--output",
        help="输出 JSON 路径（默认: <FHD>/backend/shell/fhd_shell_mods.json，即 MODstore 上级仓库）",
    )
    sp.add_argument(
        "--dry-run",
        action="store_true",
    ...
    sp = sub.add_parser(
        "migrate-db-into-mod",
        parents=[lib_arg],
        help="把 SQLite 等数据文件拷入库中某 Mod 的 data/（便于随 Mod 分发；见 data/MODMAN_DATABASE_README.txt）",
    )
    sp.set_defaults(func=cmd_migrate_db_into_mod)
    sp.add_argument("mod_id", help="库中 Mod id（与目录名一致）")
    sp.add_argument(
        "--from",
        dest="from_file",
        action="append",
        default=[],
        metavar="FILE",
        help="源数据库文件路径（可重复传入多个）",
    )
    sp.add_argument(
        "--from-dir",
        metavar="DIR",
        help="从目录中按通配收集文件（默认 *.db）",
    )
    sp.add_argument(
        "--glob",
        dest="glob_pat",
        default="*.db",
        help="与 --from-dir 连用，默认 *.db",
    )
    sp.add_argument(
        "--dest-subdir",
        default="data",
        help="Mod 内目标子目录，默认 data",
    )
    sp.add_argument("--dry-run", action="store_true", help="只列出将写入的相对路径，不拷贝")

    sp = sub.add_parser(
        "publish",
        help="将 .xcmod/.xcemp 发布到 Catalog（需 Catalog 已配置 MODSTORE_CATALOG_UPLOAD_TOKEN）",
    )
    sp.set_defaults(func=cmd_publish)
    sp.add_argument("zip", help="包文件路径")
    sp.add_argument(
        "--catalog-url",
        required=True,
        help="Catalog 根 URL，如 https://mods.example.com",
    )
    sp.add_argument(
        "--token",
        required=True,
        help="与服务器环境变量 MODSTORE_CATALOG_UPLOAD_TOKEN 一致的 Bearer 密钥",
    )
