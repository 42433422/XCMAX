...
def _display_name_for_user(
    user: _facade().Optional[_facade().User], *, db: _facade().Optional[_facade().Session] = None
) -> tuple[str, str]:
    address = ""
    member_hint = ""
    try:
...
def _peek_prior_user_issue(
    db: _facade().Session, *, session: _facade().CustomerServiceSession, exclude_text: str = ""
) -> str:
    rows = (
...
def session_payload(row: _facade().CustomerServiceSession) -> _facade().Dict[str, _facade().Any]:
    return {
        "id": row.id,
        "user_id": row.user_id,
        "channel": row.channel,
