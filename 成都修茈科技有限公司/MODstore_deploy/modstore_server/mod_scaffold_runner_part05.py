    ...
    *,
    ...
    for idx, entry in enumerate(rows):
        ...
        if summary and summary.get("pass") is False:
            errors.append(
                audit_failure_error_payload(
                    pack_id=pack_id, workflow_index=idx, audit=audit, classification=classification
                )
            )
            continue
        if status_hook:
        ...
        meta_diff = registration_metadata_mismatches(
            wf_entry=entry, mod_manifest=data, audit_manifest=audit_manifest, catalog_rec=rec
        )
        if meta_diff:
            errors.append(
                {
    ...
    for idx, entry in enumerate(rows):
        ...
        if emp_rows:
            ...
            try:
                ...
                db.rollback()
                patches.append(
                    {"workflow_index": idx, "workflow_id": wf_id, "error": f"update failed: {e}"}
                )
            continue
        try:
            start = (
            ...
            db.rollback()
            patches.append(
                {"workflow_index": idx, "workflow_id": wf_id, "error": f"insert failed: {e}"}
            )
