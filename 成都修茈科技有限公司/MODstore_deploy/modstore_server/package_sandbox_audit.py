...
def _dim_manifest_compliance(
    manifest: Dict[str, Any], expected_artifact: Optional[str]
) -> Tuple[int, List[str]]:
    reasons: List[str] = []
    ...
    if art == ARTIFACT_EMPLOYEE_PACK:
        ...
        if not isinstance(wes, list) or not wes:
            ...
            for i, w in enumerate(wes):
                ...
                if (
                    not str(w.get("label") or "").strip()
                    and not str(w.get("panel_title") or "").strip()
                ):
...
def _functional_checks(
    ...
    if base:
        ...
        out.append(
            {
                "name": "phone_agent 路由静态检查",
                "ok": True,
                "detail": "未声明 phone_agent_base_path，跳过",
            }
        )
    return out
    ...
    *,
    ...
    if not base:
        return {
            ...
            "detail": "未配置 MODSTORE_SANDBOX_PROBE_BASE_URL",
        }
    try:
        u = urlparse(base)
...
async def run_package_audit_async(
    ...
    metadata_v2 = (
        meta.get("employee_config_v2") if isinstance(meta.get("employee_config_v2"), dict) else None
    )
    ...
    try:
        ...
        if metadata_v2:
            v2errs = validate_v2_config(
                metadata_v2, db=None, user_id=None, require_workflow_heart=True
            )
            if v2errs:
