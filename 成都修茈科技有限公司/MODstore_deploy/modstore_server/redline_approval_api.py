...
@router.get("/pending")
async def api_pending_redline_requests(_admin_user: User = Depends(require_admin)):
    _ = _admin_user
...
async def api_approve_redline(
    ...
    result = approve_redline_request(cr_id, int(_admin_user.id), comment=body.comment)
...
async def api_reject_redline(
    ...
    result = reject_redline_request(cr_id, int(_admin_user.id), reason=body.reason)
    return {"ok": result.get("ok", False), "data": result}


@router.get("/domains")
async def api_redline_domains(_admin_user: User = Depends(require_admin)):
    _ = _admin_user
...
async def api_check_redline_timeout(_admin_user: User = Depends(require_admin)):
    _ = _admin_user
