...
def mark_clarification_resolved(session_id: str) -> dict[str, _facade().Any]:
    ...
    with _facade()._LOCK:
        ...
        if not isinstance(row, dict):
            return {"ok": False, "error": "session_not_found"}
        if row.get("status") not in {_facade()._STATUS_ANSWERED, _facade()._STATUS_RESOLVED}:
...
def _latest_session_for_subject(subject: str) -> _facade().Optional[dict[str, _facade().Any]]:
    with _facade()._LOCK:
...
def clarification_blocks_auto_approve(change_request_id: int) -> dict[str, _facade().Any]:
