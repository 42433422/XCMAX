...
def _verify_branch_on_remote(repo_url: str, branch: Optional[str]) -> bool:
    """Verify that ``branch`` actually exists on ``repo_url`` remote.

    Uses `git ls-remote --heads --exit-code` so this works for both file:// and
    https:// remotes without cloning. Returns True only when ls-remote succeeds
    with a non-empty match. Returns False on any failure (missing branch,
    missing remote, command error). Never raises.
    """
    ...
    try:
        ...
        logger.exception(
            "verify_branch_on_remote crashed for repo=%s branch=%s",
            repo_url,
            branch,
        )
        return False
...
def _loop_startup_preflight() -> Dict[str, Any]:
    """Fail-fast configuration check at loop entry.

    The loop historically wasted full code+review+qa cycles before landing in
    `completed_waiting_human_strategy` with reasons like
    `repo_url_not_file_url` or `branch_not_on_remote_or_empty`. This preflight
    catches misconfiguration in <1ms so we surface the problem immediately as
    a `failed` ledger record instead of after ~20 minutes of employee work.

    Returns ``{"ok": True}`` when configuration is healthy, otherwise
    ``{"ok": False, "reason": "misconfigured_*", ...}``.
    """
    ...
    bare_repo = os.environ.get("MODSTORE_PARA_BARE_REPO", "").strip()
    allow_remote = _env_bool("MODSTORE_AUTO_MERGE_ALLOW_REMOTE", False)
    allow_desktop = _env_bool("MODSTORE_SELF_MAINTENANCE_ALLOW_DESKTOP_REPO", False)

    if not repo_url:
        return {"ok": False, "reason": "misconfigured_repo_url_missing"}
    if not branch:
        return {"ok": False, "reason": "misconfigured_branch_missing"}
    if not api_base:
        return {"ok": False, "reason": "misconfigured_api_base_missing"}
    is_file_url = repo_url.startswith("file://")
    is_remote_url = repo_url.startswith(("http://", "https://"))
    if not is_file_url and not is_remote_url:
        return {
            "ok": False,
            "reason": "misconfigured_repo_url_scheme",
            "repo_url": repo_url,
        }
    if is_remote_url and not (allow_remote or allow_desktop):
        return {
            "ok": False,
            "reason": "misconfigured_remote_url_without_allow_remote",
            "repo_url": repo_url,
        }
    if is_file_url:
        path = _file_url_to_path(repo_url)
        if path is None or not path.exists():
            return {
                "ok": False,
                "reason": "misconfigured_file_url_path_missing",
                "repo_url": repo_url,
                "expected_path": str(path) if path else "",
            }
    warnings = []
    if is_file_url and not bare_repo:
        warnings.append("bare_repo_missing_push_mirror_unverified")
    elif is_file_url and bare_repo and not Path(bare_repo).exists():
        warnings.append("bare_repo_path_missing_push_mirror_unverified")
...
def _last_in_progress_start() -> Optional[Dict[str, Any]]:
    ...
    in_progress.sort(
        key=lambda r: (
            _parse_iso(r.get("started_at") or r.get("created_at"))
...
def should_run_self_maintenance_loop(
    ...
    if not force and cooldown_minutes > 0:
        if mode == "legacy":
            ...
            anchor = (
                _parse_iso(
                    last_terminal.get("completed_at") or last_terminal.get("started_at")
                )
                if last_terminal
                else None
            )
...
def _focused_test_command() -> str:
    ...
    test_path = (
        "成都修茈科技有限公司/MODstore_deploy/tests/"
        "test_self_maintenance_loop_runner_policy.py"
    )
...
def _code_task_text(
    ...
    return (
        ...
        # Push-to-mirror 客观验证：e2e-agent 文字声称 push 但历史多次未真正推送，
        # 服务端用 git ls-remote 双 remote 验证，未推送则直接 fail。
        "MANDATORY PUSH-TO-MIRROR: After committing your change, you MUST push the "
        "branch to BOTH remotes listed below. The loop server will verify via "
        "`git ls-remote --heads --exit-code` that the branch actually exists on at "
        "least one of them — text reports of \"pushed\" are NOT trusted. If neither "
        "remote has the branch after your step reports ok, the loop will fail this "
        "run with reason=branch_not_pushed_to_any_remote. "
        f"  primary remote (MODSTORE_PARA_REPO_URL): {os.environ.get('MODSTORE_PARA_REPO_URL', '').strip() or '(unset)'}\n"
        f"  mirror remote  (MODSTORE_PARA_BARE_REPO): {os.environ.get('MODSTORE_PARA_BARE_REPO', '').strip() or '(unset)'}\n"
        "Push commands (run both):\n"
        "  git push <primary_remote> HEAD:<your_branch>\n"
        "  git push <mirror_remote> HEAD:<your_branch>\n"
        "If the primary remote rejects (e.g., protected branch), the mirror remote "
        "is the canonical backup — at least one MUST accept the push. "
...
def _changed_files_for_branch(
    *, repo_url: str, base_branch: str, branch: str, workspace: Path
    ...
    """Return changed file list for ``branch`` vs ``base_branch``.

    Returns ``None`` when the branch is NOT present on any remote (origin or
    Para bareRepo) — this is a real push failure that should NOT be confused
    with an empty diff. Returns ``[]`` when the branch IS on a remote but the
    diff against base is empty (a legitimate NO_ACTION success state).
    """
...
def _auto_merge_safety_score_v2(
    ...
    *,
    ...
    file_penalty = min(
        25, int((risk_v1.get("components") or {}).get("file_score") or 0) // 2
    )
    line_penalty = min(
        18, int((risk_v1.get("components") or {}).get("line_score") or 0)
    )
    keyword_penalty = min(
                ...
                return _decision(
                    {
                        ...
                        "diff_stats_consistency": consistency,
                        "line_changes": int(
                            (diff_stats or {}).get("line_changes") or 0
                        ),
                        "ok": True,
...
def _auto_merge_low_risk_branch(
    *,
    ...
    if files is None:
        # 分支不在任何 remote (origin + bareRepo) → 真实 push 失败，不能 auto_merge。
        return {
            "ok": False,
            "reason": "branch_not_on_remote",
            "branch": branch,
            "changed_files": [],
        }
    if not files:
        # 分支在 remote 上但 diff 为空 → 合法的 NO_ACTION 成功终态，
        # 不应该和 push 失败混淆降级到 await_human。
        return {
            "ok": True,
            "reason": "no_safe_actionable_source_change",
            "branch": branch,
            "changed_files": [],
...
def _decide_post_loop_policy(
    *,
    ...
    if merge_result.get("ok"):
        ...
        no_action = bool(merge_result.get("no_action"))
        if no_action:
            # Empty diff 是合法成功终态（branch 在 remote 但无变更）
            return {
                "action": "auto_continue_no_action",
                "active_gates": active_gates,
                "evolution_gate": evolution_gate,
                "gate": gate,
                "governance_gate": governance_gate,
...
def run_self_maintenance_loop(
    *, triggered_by: str = "manual", force: bool = False, reason: Optional[str] = None
    ...
    # Fail-fast configuration preflight: catch misconfiguration in <1ms instead
    # of wasting ~20 minutes of employee work before landing in
    # completed_waiting_human_strategy with reasons like repo_url_not_file_url
    # or branch_not_on_remote_or_empty.
    preflight = _loop_startup_preflight()
    if not preflight.get("ok"):
        final = {
            "completed_at": _iso(_utc_now()),
            "error": preflight.get("reason") or "preflight_failed",
            "phase": "complete",
            "policy_decision": {
                "action": "stop",
                "reason": preflight.get("reason") or "preflight_failed",
                "preflight": preflight,
            },
            "preflight": preflight,
            "run_id": run_id,
    ...
    try:
        for employee_id, step_name, task_text, extra in plan:
            ...
            # Push-to-mirror verification: e2e-agent 文字声称 commit+push
            # 但可能并未真正推送（历史根因）。在 code step ok 之后、进入
            # review/qa 之前用 git ls-remote 客观验证 branch 在两个 remote
            # (MODSTORE_PARA_REPO_URL + MODSTORE_PARA_BARE_REPO) 至少有一个
            # 存在，否则直接 fail 不浪费后续步骤。
            if step_name == "code" and ok and code_branch:
                repo_url_for_check = os.environ.get("MODSTORE_PARA_REPO_URL", "").strip()
                bare_repo_for_check = os.environ.get("MODSTORE_PARA_BARE_REPO", "").strip()
                pushed_primary = (
                    _verify_branch_on_remote(repo_url_for_check, code_branch)
                    if repo_url_for_check
                    else False
                )
                pushed_mirror = (
                    _verify_branch_on_remote(bare_repo_for_check, code_branch)
                    if bare_repo_for_check
                    else False
                )
                if not pushed_primary and not pushed_mirror:
                    logger.warning(
                        "self_maintenance_loop: code step reported ok but branch %s "
                        "not found on any remote (primary=%s mirror=%s) — "
                        "e2e-agent reported push but no remote has it",
                        code_branch,
                        repo_url_for_check,
                        bare_repo_for_check,
                    )
                    failure_reason = (
                        "branch_not_pushed_to_any_remote: "
                        f"primary={repo_url_for_check} mirror={bare_repo_for_check} "
                        f"branch={code_branch}"
                    )
                    para_meta = steps[-1].get("para") if steps else None
                    step_retry_record = {
                        "employee_id": steps[-1].get("employee_id") if steps else None,
                        "error": failure_reason,
                        "ok": False,
                        "para": para_meta,
                        "phase": "step_retry",
                        ...
                        "step": step_name,
                        "timestamp": _iso(_utc_now()),
                        "reason": "branch_not_pushed_to_any_remote",
                    }
                    _append_ledger(step_retry_record)
                    final = {
                        "branch": code_branch,
                        "completed_at": _iso(_utc_now()),
                        "error": failure_reason,
        ...
        elif policy_decision.get("action") == "auto_continue_no_action":
...
def _burnin_status_safe() -> Dict[str, Any]:
    """Read burn-in status with fail-open semantics.

    Any error (missing module, file IO crash) is logged and a safe default
    is returned so the runtime status endpoint never breaks because of the
    burn-in subsystem.
    """

    try:
        from modstore_server.self_maintenance_burnin import get_burnin_status

        return get_burnin_status()
    except Exception as exc:
        logger.exception("burnin: failed to read burnin status")
        return {
            "active": False,
            "error": str(exc),
            "message": "burnin status unavailable",
        }
