...
def _close_items_resolved_by_final(
    memory: _facade().Dict[str, _facade().Any], final: _facade().Dict[str, _facade().Any]
...
def _resume_dispatch_context(
    resume_candidate: _facade().Optional[_facade().Dict[str, _facade().Any]], steps_to_run: set[str]
...
def _recent_incident_signals(
    ...
    """
    since = _facade()._utc_now() - _facade().timedelta(hours=lookback_hours)
    event_types = {"ci.failed", "incident.unknown", "on_error", "on_quality_fail", "security.alert"}
    db = _facade().get_session_factory()()
    try:
        query = (
