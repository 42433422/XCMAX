...
def _delivery_validation_gate(result: _facade().Any) -> _facade().Dict[str, _facade().Any]:
...
def _is_transient_employee_dispatch_failure(result: _facade().Dict[str, _facade().Any]) -> bool:
    ...
    *,
    ...
    for attempt in range(1, attempts + 1):
        ...
        result = _facade().execute_employee_task(
            employee_id, task_text, input_data, user_id=user_id, bench_llm_override=bench_override
        )
...
def _run_step_with_inner_retries(
    *,
    ...
    _facade().Dict[str, _facade().Any], bool, str, _facade().Dict[str, _facade().Any], str, int, int
]:
