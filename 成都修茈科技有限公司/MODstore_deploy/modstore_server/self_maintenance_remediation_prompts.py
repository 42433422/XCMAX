...
def external_merge_remediation_prompt(resume_candidate: Any) -> str:
    ...
    if continue_on_branch:
        strategy = (
            ...
            "(post-dispatch required-check polling, merge-worker hold-label infrastructure, or "
            "gh pr checks polling). Continue on the rejected branch as the mutable base: diff it "
            "against main, keep the existing production fix when already present on main, and only "
            "add the smallest delta still missing. Do not restart from the clean baseline or "
            "reimplement an already-merged fix. If every executable gap is already on main, update "
            "loop status with NO_ACTION instead of marker-only edits."
        )
    else:
        strategy = (
