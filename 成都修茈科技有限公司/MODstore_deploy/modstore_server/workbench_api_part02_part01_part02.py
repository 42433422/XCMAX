...
def _employee_handlers_contract_ok(
    ...
    if "direct_python" in handlers and manifest_expects_word_runtime(mf):
        ...
        return (
            False,
            "声明 direct_python 但库内无 runtime 实现（画布保存不能替代 generate）",
        )
    return (True, "")
...
def _refresh_employee_pack_catalog_zip(
    ...
    try:
        ...
        db.commit()
        try:
            from modstore_server.api.catalog_public_routes import (
                _invalidate_catalog_list_caches,
            )
