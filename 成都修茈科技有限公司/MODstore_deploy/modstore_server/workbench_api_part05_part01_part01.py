...
import importlib
from modstore_server.workbench_api_part01_part01_part01 import EmployeeAiDraftBody
from modstore_server.workbench_api_part01_part01_part01 import (
    EmployeeAiRefinePromptBody,
)
...
@_facade().router.post(
    "/employee-bench-test",
    summary="员工上架前基准测试（LLM 生成任务 + 执行 + 五维审核）",
)
...
class EmployeeSyncTestRequest(_facade().BaseModel):
    ...
    fhd_base_url: _facade().Optional[str] = _facade().Field(
        None,
        description="宿主 fhd-sandbox-runtime 的 base URL，如 http://localhost:9999",
    )
    provider: _facade().Optional[str] = _facade().Field(None)
    model: _facade().Optional[str] = _facade().Field(None)
    price: float = _facade().Field(0.0)
