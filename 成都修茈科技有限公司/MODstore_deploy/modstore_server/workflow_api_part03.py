...
async def execute_workflow(
    workflow_id: int,
    body: _facade().WorkflowExecuteBody,
...
async def create_workflow_trigger(
    workflow_id: int,
    body: _facade().WorkflowTriggerBody,
    db: _facade().Session = _facade().Depends(_facade().get_db),
    user: _facade().User = _facade().Depends(_facade()._get_current_user),
):
    workflow = (
...
async def publish_workflow_version(
    workflow_id: int,
    body: _facade().PublishVersionBody,
    db: _facade().Session = _facade().Depends(_facade().get_db),
    user: _facade().User = _facade().Depends(_facade()._get_current_user),
):
    workflow = (
...
@_facade().router.get(
    "/{workflow_id}/versions/{version_id}", summary="工作流版本详情（含 graph_snapshot）"
)
...
async def get_execution_detail(
    ...
    execution = (
        ...
        .filter(
            _facade().WorkflowExecution.id == execution_id, _facade().Workflow.user_id == user.id
        )
        .first()
    )
    if not execution:
...
@_facade().workflow_hooks_router.post(
    "/webhook/{trigger_key}", summary="公开 Webhook 触发工作流（需在触发器 config 配置 secret）"
)
async def public_webhook_run_workflow(
    ...
    if secret:
        ...
        raise _facade().HTTPException(
            400, "请在触发器 config 中配置 secret，或关闭 MODSTORE_REQUIRE_WEBHOOK_SECRET"
        )
    try:
    ...
    try:
        return _facade().run_workflow_for_trigger(
            workflow_id=int(trig.workflow_id), user_id=int(trig.user_id), input_data=body
        )
    except Exception as e:
        raise _facade().HTTPException(500, str(e)) from e
