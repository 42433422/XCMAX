...
def main() -> int:
    ...
    word_res = _simulate_execute_file("word-full-read-employee", minimal_docx_bytes(), "exam-sample.docx")
    if not word_res.get("ok"):
        print(json.dumps({"ok": False, "stage": "word", "error": word_res.get("error")}, ensure_ascii=False, indent=2))
        return 1

    try:
        json_payload = _json_bytes_from_word_result(word_res)
    except Exception as exc:
        print(json.dumps({"ok": False, "stage": "prepare_json", "error": str(exc)}, ensure_ascii=False, indent=2))
        return 1
