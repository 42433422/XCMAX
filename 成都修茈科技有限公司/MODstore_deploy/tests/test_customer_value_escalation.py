...
def _init_db(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "MODSTORE_DB_PATH", str(tmp_path / "customer-value-escalation.sqlite")
    )
    models.init_db()
...
def test_zero_authoritative_customer_value_creates_one_human_gated_action(
    ...
    with sf() as session:
        decision = session.execute(
            select(StrategicDecision).where(
                StrategicDecision.decision_id == first["decision_id"]
            )
        ).scalar_one()
    ...
    assert (
        actions[0].block_reason
        == "customer_value_recovery_requires_human_strategy_approval"
    )
