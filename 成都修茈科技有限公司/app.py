...
# ---------------------------------------------------------------------------
MODSTORE_BACKEND = os.environ.get(
    "MODSTORE_BACKEND_URL", "http://127.0.0.1:8000"
).rstrip("/")
...
def proxy_modstore_api(subpath: str):
    ...
    try:
        ...
        err = jsonify(
            {
                "detail": f"ModStore 后端不可用（请检查 Uvicorn 与 MODSTORE_BACKEND_URL）: {exc}"
            }
        )
        err.status_code = 502
...
def admin():
    ...
    activities_sorted = sorted(
        activities, key=lambda x: x.get("date", ""), reverse=True
    )
