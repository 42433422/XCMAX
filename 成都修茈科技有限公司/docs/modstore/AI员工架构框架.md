...
function upgradeManifest(legacy: LegacyManifest): EmployeeConfig {
  return {
    ...
    commerce:
      legacy.commerce || legacy.industry
        ? {
  ...
  actions: {
    voice_output: {
      ...
      voice_cloning: {
        voice_id: '',
        voice_name: '',
        settings: { speed: 1.0, pitch: 0, emotion: 'professional', language: 'zh-CN' },
      },
    },
  },
  memory: {
    long_term: {
      enabled: true,
      sources: [],
      retrieval: { strategy: 'hybrid', top_k: 5, similarity_threshold: 0.75, rerank_enabled: true },
    },
  },
}
...
const TemplateDataProcessor: Partial<EmployeeConfig> = {
  ...
  perception: {
    data_input: {
      enabled: true,
      api_sources: [],
      file_watch: { enabled: false, watch_paths: [], file_extensions: [] },
    },
  },
  cognition: {
    agent: {
...
const TemplateFullStack: Partial<EmployeeConfig> = {
  ...
  management: {
    ...
    security: {
      data_masking: { enabled: true, fields: ['phone', 'id_card'] },
      audit_log: true,
      rate_limit: { requests_per_minute: 60, requests_per_hour: 1000 },
    },
    performance: { enabled: true, metrics: ['response_time', 'success_rate'], auto_optimize: true },
  },
}
```
