...
def analyze_code(source_code: str, expected_features: list[str]) -> dict:
    ...
    for feat in expected_features:
        ...
        if feat.lower() in source_code.lower() or any(keyword in source_code.lower() for keyword in feat.split()):
...
def run_batch_test():
    ...
    for diff in ["easy", "medium", "hard", "very_hard"]:
        ...
        avg_feat = sum(
            len(r.features_found) / max(len(tc.expected_features), 1) * 100
            for r, tc in zip(group, [t for t in TEST_CASES if t.name == r.name], strict=False)
            if r.success
        ) / max(success_count, 1)

        print(
            f"{diff:<12} {total:<6} {success_count:<8} {rate:<8.0f}% {avg_time:<10.1f}s {avg_lines:<10.0f} {avg_feat:<10.0f}%"
        )
    ...
    for r in results:
        ...
        if r.success:
            ...
            print(
                f"  Type hints: {'Yes' if r.has_type_hints else 'No'}, "
