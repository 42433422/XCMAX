...
def run_long_code_test():
    ...
    for i, tc in enumerate(LONG_CODE_TEST_CASES, 1):
        ...
        try:
            ...
            if result.actual_lines < tc["expected_min_lines"]:
                print("  ⚠️  Code shorter than expected!")
    ...
    if total_success > 0:
        ...
        print(
            f"  Error handling coverage: {sum(1 for r in successful if r.has_error_handling) / len(successful) * 100:.0f}%"
        )
        ...
        print(
            f"  Avg features coverage: {sum(len(r.features_found) / max(len(LONG_CODE_TEST_CASES[i]['features']), 1) * 100 for i, r in enumerate(results) if r.success) / len(successful):.0f}%"
        )
    ...
    print("=" * 70)
    print(
        f"{'Name':<25} {'Status':<7} {'Time':<7} {'Lines':<6} {'Func':<5} {'Class':<5} {'Features':<10} {'Complexity':<10}"
    )
    print("-" * 70)
    ...
    for r in results:
        ...
        print(
            f"{r.name:<25} {status:<7} {time_str:<7} {lines_str:<6} {func_str:<5} {class_str:<5} {feat_str:<10} {complex_str:<10}"
        )
