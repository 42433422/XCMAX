        # Runbook：产物生成员工 (`artifact-generator`)
        ...
        ### handoff: employee-planner → 本岗
- **触发条件**：`employee.task.done:employee-planner`
- **输入**：待补充（参见 `yuangon/**/employee-planner/runbook.md`）
- **门禁**：依赖完成前本岗不得继续
...
- `yuangon/craft-workshop/artifact-generator/**`
        ...
        | 场景 | 处置 |
        |------|------|
        | LLM 调用失败 | retry 2 次 → 上报 `employee.task.failed:artifact-generator` |
