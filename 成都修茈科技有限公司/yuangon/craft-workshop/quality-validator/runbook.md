        # Runbook：质检员工 (`quality-validator`)
        ...
        ### handoff: artifact-generator → 本岗
- **触发条件**：`employee.task.done:artifact-generator`
- **输入**：待补充（参见 `yuangon/**/artifact-generator/runbook.md`）
- **门禁**：依赖完成前本岗不得继续
...
- `yuangon/craft-workshop/quality-validator/**`
        ...
        | 场景 | 处置 |
        |------|------|
        | LLM 调用失败 | retry 2 次 → 上报 `employee.task.failed:quality-validator` |
