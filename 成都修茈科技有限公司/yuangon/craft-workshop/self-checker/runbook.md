        ...
        ### handoff: code-validator → 本岗
- **触发条件**：`employee.task.done:code-validator`
- **输入**：待补充（参见 `yuangon/**/code-validator/runbook.md`）
- **门禁**：依赖完成前本岗不得继续
...
- `yuangon/craft-workshop/self-checker/**`
        ...
        | 场景 | 处置 |
        |------|------|
        | LLM 调用失败 | retry 2 次 → 上报 `employee.task.failed:self-checker` |
